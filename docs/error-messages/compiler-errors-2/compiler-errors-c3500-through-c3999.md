---
description: 了解有关以下内容的详细信息：编译器错误 C3500 到 C3999
...
|编译器错误 C3514|"*character*" (*值*) ：不支持的原始字符串文本分隔符字符|
...
|[编译器错误 C3551](compiler-error-c3551.md)|如果使用尾随返回类型，则前导返回类型应该是单个类型说明符 "auto" (不是 "*type*" ) |
|[编译器错误 C3552](compiler-error-c3552.md)|应为 "decltype (auto) "|
...
|编译器错误 C3565|为 concurrency：:p 编译调用关系图时，tile_static 的内存总量 (*数字字节数*) 超过了 *数字* 字节的限制 arallel_for_each|
...
|编译器错误 C3574|"*type*"：非法磁贴区：必须为正数， (1) Z <= *number*; (2) Z *Y* X <= *number*|
|编译器错误 C3575|"*type*"：非法的计算域参数;缺少公共成员： "concurrency：： index<*number*> _map_index (const concurrency：： index<*number*>&)  (amp) "|
|编译器错误 C3576|"*type*"： concurrency：:d etails：： _Parallel_for_each 参数 #*number* 的类型不受支持|
|编译器错误 C3577|concurrency：:p arallel_for_each 内核参数是非法的：无法解析对成员 "void operator 的调用 ( # A1 # B2 *类型*) 限制 (amp) "|
...
|编译器错误 C3582|已过时。|
|编译器错误 C3583|"*identifier*"：变量的大小 (数字字节 *数*) 大于 amp 限制代码中支持的最大字节 *数 () 字节数*|
...
|编译器错误 C3594|amp 限制代码中不支持 (try、catch、throw 等 ) 的异常处理|
...
|编译器错误 C3600|"*function*"： \_ 编译非平铺并发的调用关系图时，检测到磁贴静态内存：:p arallel \_ for_each： "*function*"|
...
|编译器错误 C3605|编译并发的调用关系图时，已捕获的采样器 (*编号**和预定义) 数的* 总数超出了 *数量*：:p arallel_for_each： "*function*"|
|编译器错误 C3606|已过时。|
|编译器错误 C3607|已过时。|
|编译器错误 C3608|已过时。|
...
|编译器错误 C3621|"*type*"： WinRT 类型仅允许默认包装值 (*数字*) |
...
|编译器错误 C3635|"*identifier*"：不能应用于托管的/WinRT 类 (请改用 "*identifier*") |
...
|[编译器错误 C3739](compiler-error-c3739.md)|"*class*"：仅当 event_receiver 的 "布局 \_ 相关" 参数为 true 时才支持语法|
|[编译器错误 C3740](compiler-error-c3740.md)|"*template*"：模板不能源或接收事件|
|[编译器错误 C3741](compiler-error-c3741.md)|"*class*"：当 event_receiver 的 "布局 \_ 相关" 参数为 true 时，必须是 coclass|
...
|[编译器错误 C3767](compiler-error-c3767.md)|"*function*"：候选函数 (s) 无法访问|
...
|编译器错误 C3795|已过时。|
|编译器错误 C3796|已过时。|
|[编译器错误 C3797](compiler-error-c3797.md)|"*关键字*"：事件声明不能具有重写说明符 (应将该说明符放在事件的 add/remove/raise 方法上) |
|[编译器错误 C3798](compiler-error-c3798.md)|"*关键字*"：属性声明不能具有重写说明符 (应改为将其放在属性的 get/set 方法上) |
...
|[编译器错误 C3854](compiler-error-c3854.md)|"=" 左边的表达式的计算结果为函数。  (函数不是左值，因此无法分配给函数) |
...
|[编译器错误 C3869](compiler-error-c3869.md)|gcnew 约束缺少空参数列表 " ( # A1"|
|编译器错误 C3870|"*parameter*"： "__declspec (*说明符*) " 只能应用于整型参数|
|编译器错误 C3871|"*parameter*"： "__declspec (guard (溢出) # A3" 仅在函数的前64个参数上受支持|
...
|[编译器错误 C3904](compiler-error-c3904.md)|"*property*"：必须指定 *number* 参数 (s) |
...
|[编译器错误 C3915](compiler-error-c3915.md)|"*identifier*" 没有默认索引属性 (类索引器) |
|编译器错误 C3916|已过时。|
|[编译器错误 C3917](compiler-error-c3917.md)|"*token*"：过时的 *构造* 声明样式 (您是否希望改用 "[' ']"？ ) |
...
|[编译器错误 C3920](compiler-error-c3920.md)|"*operator*"：不能定义后缀增量/减量 Clr/winrt 运算符。调用后缀 clr/winrt 运算符将调用相应的前缀 Clr/winrt 运算符 (op_Increment/op_Decrement) 但使用后缀语义|
|编译器错误 C3921|已过时。|
|编译器错误 C3922|已过时。|
|[编译器错误 C3923](compiler-error-c3923.md)|"*member*"：局部类、结构或联合定义不允许在托管的/WinRT 类的成员函数中使用|
|编译器错误 C3924|参数 # 委托构造函数调用 "*构造函数*" 的 *数目* 错误：|
|编译器错误 C3925|应有循环 (for、while 或 do) 后面的 "*指令*" 指令|
...
|编译器错误 C3934|采用 "main" 格式的函数不能包含限制 (cpu 之外的限制说明符) |
...
|编译器错误 C3940|"*identifier*"：找不到标识符-编译器和库版本之间可能不匹配。 请确保 vccorlib.h/.lib、vccorlib120.dll 和 c1xx.dll 匹配|
...
|编译器错误 C3952|"*type*"： WinRT 不支持 "in/out" 数组。 在 \<T> 公共 api 上使用 "in" 和 "WriteOnlyArray \<T> " 或 "Array \<T> ^ *" 作为 "out" 的 "const Array ^"|
|编译器错误 C3953|在 WinRT 模块中不能使用托管类 "*type*"。|
|编译器错误 C3954|"*type*"：从 WinRT 类型的已发布方法返回的数组必须使用 "array \<T> ^" 形式|
...
|编译器错误 C3968|标记 "*token*" 无效，因为模块名称分隔符;使用句点 ( "。"改为 ) |
...
|编译器错误 C3999|未知错误，请选择 Visual C++ "帮助" 菜单上的 "技术支持" 命令，或打开技术支持帮助文件以获取详细信息|
