---
description: 了解有关以下内容的详细信息：编译器错误 C3400 到 C3499
...
|[编译器错误 C3415](compiler-error-c3415.md)|找到多个 "*section*" 节，其中包含不同的属性 ( "0x *value*" ) |
|编译器错误 C3416|已过时。|
...
|编译器错误 C3427|"*context*"： "*关键字*" 不能与 layout_version (*号* 一起使用) |
...
|编译器错误 C3439|layout_version 的 (*号*) ：无效的版本号|
|编译器错误 C3440|"*声明*"： layout_version (*号*) 与之前的声明不兼容|
|编译器错误 C3441|"*声明*"：定义类后，不能应用 "*关键字*"|
|编译器错误 C3442|正在初始化 union 的多个成员： "*member1*" 和 "*member2*"|
|编译器错误 C3443|"*Class*" 的默认成员初始值设定项为递归|
|编译器错误 C3444|空的聚合类 "*class*" 必须用 "" 进行初始化 {}|
...
|[编译器错误 C3456](compiler-error-c3456.md)|\_托管/WinRT 类声明上不允许使用 [source annotation_attribute]|
...
|[编译器错误 C3459](compiler-error-c3459.md)|"[*attribute*]"：属性只能在类索引器 (默认索引属性上使用) |
...
|编译器错误 C3471|在命令行上设置的新模块名称 *名称*) 与上一个名称 *名称* 冲突 (|
|编译器错误 C3472| (在命令行上设置的新输出文件名 *文件名*) 与以前的文件名 *文件名* 冲突|
