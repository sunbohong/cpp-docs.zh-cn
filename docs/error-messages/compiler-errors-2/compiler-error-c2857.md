...
当你在源文件上使用 **/yc**<em>filename</em> 选项来创建 (PCH) 文件的预编译头时，该源文件必须包含 *filename* 标头文件。 源文件包含的每个文件（包括指定的 *文件名*）都包含在 PCH 文件中。 在使用 **/yu**<em>filename</em> 选项编译的其他源文件中，若要使用 PCH 文件，文件中的第一个非注释行必须包含 *filename* 。 编译器将忽略源文件中包含的任何内容。

如果 `#include "filename"` 未在 PCH 源文件中编译的条件编译块中的语句，则可能会导致此错误。
