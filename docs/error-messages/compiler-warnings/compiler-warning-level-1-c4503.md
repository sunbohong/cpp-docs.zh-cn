---
description: 了解详细信息：编译器警告 (等级 1) C4503
...
当使用较旧版本的 Visual Studio 时，如果您的代码重复包含在模板上专用化的模板，则可以发出此警告。 例如，从 c + + 标准库)  (映射的地图。 在这种情况下，你可以使 typedef 成为 (类型 **`struct`** ，例如包含该映射的) 。
