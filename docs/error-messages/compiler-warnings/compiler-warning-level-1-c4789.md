...
对于从不执行的代码路径，Visual C++ 可能会生成此警告。 可以使用 `#pragma` 临时禁用该警告，如此示例中所示：
...
这种用法使 Visual C++ 为特定代码块生成警告。 `#pragma warning(push)` 会在 `#pragma warning(disable: 4789)` 更改现有状态之前保留该状态。 `#pragma warning(pop)` 会还原压入的状态，并移除 `#pragma warning(disable:4789)` 的效果。 有关 c + + 预处理器指令的详细信息 `#pragma` ，请参阅 [警告](../../preprocessor/warning.md) 和 [杂注指令以及 __Pragma 关键字](../../preprocessor/pragma-directives-and-the-pragma-keyword.md)。
