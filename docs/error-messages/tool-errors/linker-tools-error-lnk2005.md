---
description: 了解详细信息：链接器工具错误 LNK2005
...
- 如果符号是 (通过使用 [/gy](../../build/reference/gy-enable-function-level-linking.md) 编译创建的打包函数) 并且包含在多个文件中，但在编译之间发生了更改，则会出现此错误。 若要解决此问题，请重新编译包含打包函数的所有文件。
...
- 如果将 uuid .lib 与定义 Guid (例如 oledb 和 adsiid) 的其他 .lib 文件结合使用，则会出现此错误。 例如：
