---
description: 了解详细信息：链接器工具错误 LNK2001
...
- 当项目缺少对库 ( 的引用时。LIB) 或对象 (。OBJ) 文件。 若要解决此问题，请在项目中添加对所需库或对象文件的引用。 有关详细信息，请参阅 [Lib 文件作为链接器输入](../../build/reference/dot-lib-files-as-linker-input.md)。

- 当项目具有对库 ( 的引用时。LIB) 或对象 (。OBJ) 文件又需要另一个库中的符号。 即使您不调用导致依赖关系的函数，也可能会发生这种情况。 若要解决此问题，请向项目添加对另一个库的引用。 有关详细信息，请参阅 [了解用于链接的传统模型：获取用于进行操作的符号](https://devblogs.microsoft.com/oldnewthing/20130108-00/?p=5623)。
...
- 如果安装了新的 Windows SDK (可能) 不同的位置。 必须更新库搜索顺序才能指向新位置。 通常情况下，应将新的 SDK 包含的路径和 lib 目录放在默认 Visual C++ 位置之前。 此外，包含嵌入路径的项目仍可指向有效但过期的旧路径。 更新安装到不同位置的新版本添加的新功能的路径。
...
- 源代码或模块定义中的大小写不匹配 ( .def) 文件。 例如，如果您 `var1` 在一个 c + + 源文件中命名一个变量，然后尝试 `VAR1` 在另一个中访问它，则会生成此错误。 若要解决此问题，请使用一致拼写和大小写的名称。
...
- 尝试使用在函数内声明的变量 (该函数作用域外) 的 [局部变量](../../error-messages/tool-errors/automatic-function-scope-variables.md) 。 若要解决此问题，请删除对不在作用域中的变量的引用，或将变量移动到更高的作用域。

- 生成 ATL 项目的发行版时，会生成一条消息，要求 CRT 启动代码是必需的。 若要解决此问题，请执行以下操作之一：

  - `_ATL_MIN_CRT`从预处理器定义的列表中删除，以允许包含 CRT 启动代码。 有关详细信息，请参阅 " [常规属性页 (项目) ](../../build/reference/general-property-page-project.md)"。
...
混合不同模块上的[内联和非内联编译选项](../../error-messages/tool-errors/function-inlining-problems.md)可能会导致 LNK2001。 如果在创建 c + + 库时打开了函数内联 (**/Ob1** 或 **/Ob2**) 但描述函数的相应标头文件已关闭 (不 **`inline`**) 关键字，则会发生此错误。 若要解决此问题，请 **`inline`** 在其他源文件中包含的标头文件中定义函数。
...
为了帮助你找到原因，错误消息会显示两个名称版本。 它同时显示 "友好名称"、"在源代码中使用的名称" 和 "修饰名" (括号) 。 不需要知道如何解释修饰名。 你仍可以搜索并将其与其他修饰名称进行比较。 命令行工具可帮助查找和比较预期符号名称和实际符号名称：
