...
[__cdecl](../../cpp/cdecl.md)将 (、 [__stdcall](../../cpp/stdcall.md) 、或) 的调用约定 [__fastcall](../../cpp/fastcall.md) [__vectorcall](../../cpp/vectorcall.md) 编码为修饰名称的一部分。 请确保调用约定是相同的。

### <a name="a-symbol-is-defined-in-a-c-file-but-declared-without-using-no-locextern-c-in-a-c-file"></a>符号在 C 文件中定义，但未 extern 在 c + + 文件中使用 "C" 进行声明

在编译为 c 的文件中定义的符号具有与 c + + 文件中声明的符号不同的修饰名称，除非使用[ extern "C"](../../cpp/extern-cpp.md)修饰符。 请确保该声明匹配每个符号的编译链接。 同样，如果在 C 程序将使用的 C++ 文件中定义符号，请在定义中使用 `extern "C"` 。

### <a name="a-symbol-is-defined-as-no-locstatic-and-then-later-referenced-outside-the-file"></a>符号定义为 static ，稍后在文件外部引用
...
### <a name="a-no-locstatic-member-of-a-class-isnt-defined"></a>static未定义类的成员
...
自动（函数范围）变量仅可在该函数的范围内使用。 不能 **`extern`** 在其他源文件中声明和使用这些变量。 有关示例，请参见 [Automatic (Function Scope) Variables](../../error-messages/tool-errors/automatic-function-scope-variables.md)。
...
### <a name="you-mix-code-that-uses-native-no-locwchar_t-with-code-that-doesnt"></a>混合使用本机代码 wchar_t 和代码
...
如果尝试在生成过程中配置第三方库时遇到此错误，请考虑使用 [vcpkg](../../build/vcpkg.md)（c + + 程序包管理器）安装和生成库。 vcpkg 支持较大和不断增长 [的第三方库列表](https://github.com/Microsoft/vcpkg/tree/master/ports)。 它将成功生成所需的所有配置属性和依赖项设置为项目的一部分。
...
### <a name="a-no-locstatic-data-member-is-declared-but-not-defined"></a>static已声明但未定义数据成员
...
### <a name="inconsistent-no-locwchar_t-type-definitions"></a>wchar_t类型定义不一致
