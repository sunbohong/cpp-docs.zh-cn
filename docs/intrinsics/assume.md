...
如果 **`__assume`** 语句作为冲突写入 (始终计算为 false) 表达式，则它始终被视为 `__assume(0)` 。 如果代码表现得不如预期，确保你定义的 `expression` 为有效和 true，如前所述。 有关更多预期 `__assume(0)` 行为的信息，请参阅后面的备注。
...
为了与早期版本兼容， **`_assume`** **`__assume`** 除非指定了编译器选项/Za " [ \( 禁用语言) 扩展](../build/reference/za-ze-disable-language-extensions.md) "，否则将是同义词。
