---
description: '了解详细信息：记录视图的数据交换 (MFC 数据访问) '
...
记录视图的 DDX 与 (ODBC) 类的记录集的 [RFX](../data/odbc/record-field-exchange-rfx.md) 一起工作 `CRecordset` 。  RFX 在数据源的当前记录和记录集对象的字段数据成员之间移动数据。 DDX 可将数据从字段数据成员移动到窗体的控件中。 此组合在初始时和用户逐条移动记录时填充窗体控件。 它还可以将更新的数据移回记录集，然后再移到数据源。
