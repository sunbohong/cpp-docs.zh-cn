---
description: 了解详细信息：重写提供程序服务默认值
title: 重写提供程序服务默认值
ms.date: 10/29/2018
helpviewer_keywords:
...
只要存在注册表项，就会聚合提供程序的对象。 用户可以通过在初始化之前设置 DBPROP_INIT_OLEDBSERVICES 属性来覆盖已启用服务的提供程序的默认设置。 若要启用或禁用某个特定的服务，用户可以获取 DBPROP_INIT_OLEDBSERVICES 属性的当前值，设置或清除要启用或禁用的特定属性的位，并重置该属性。 可以直接在 OLE DB 或传递到 ADO 或的连接字符串中设置 DBPROP_INIT_OLEDBSERVICES `IDataInitialize::GetDatasource` 。 下表列出了用于启用/禁用单个服务的相应值。
