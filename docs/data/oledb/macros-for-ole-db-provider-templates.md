...
*flag*<br/>
中指定返回数据的方式。 请参阅 `dwFlags` [DBBINDING 结构](/previous-versions/windows/desktop/ms716845(v=vs.85))中的说明。

*colSize*<br/>
中列大小。

*dbtype*<br/>
中指示值的数据类型。 请参阅 `wType` [DBBINDING 结构](/previous-versions/windows/desktop/ms716845(v=vs.85))中的说明。

*精度*<br/>
中指示当 *dbType* 为 DBTYPE_NUMERIC 或 DBTYPE_DECIMAL 时，获取数据时要使用的精度。 请参阅 `bPrecision` [DBBINDING 结构](/previous-versions/windows/desktop/ms716845(v=vs.85))中的说明。
...
[OLE DB 提供程序模板的宏](../../data/oledb/macros-for-ole-db-provider-templates.md)
