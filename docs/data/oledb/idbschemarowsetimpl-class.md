---
description: 了解详细信息： IDBSchemaRowsetImpl 类
...
`IDBSchemaRowset`对象调用 `SetRestrictions` 来确定在特定架构行集上支持哪些限制 (通过向上转换指针) 来[](#getschemas)调用。 限制允许使用者只提取匹配行（例如，查找表“MyTable”中的所有列）。 限制是可选的，如果不支持任何限制（默认设置），则始终返回所有数据。
