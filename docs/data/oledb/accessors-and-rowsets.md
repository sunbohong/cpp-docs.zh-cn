...
所有访问器均派生自 [CAccessorBase](../../data/oledb/caccessorbase-class.md)。 `CAccessorBase` 同时提供参数绑定和列绑定。
...
- [CDynamicAccessor](../../data/oledb/cdynamicaccessor-class.md) 如果在设计时不知道数据库的结构，请使用此访问器。 `CDynamicAccessor` 调用 `IColumnsInfo::GetColumnInfo` 以获取数据库列信息。 它创建和管理访问器和缓冲区。

- [CDynamicParameterAccessor](../../data/oledb/cdynamicparameteraccessor-class.md) 使用此访问器处理未知的命令类型。 准备命令时， `CDynamicParameterAccessor` `ICommandWithParameters` 如果提供程序支持，则可以从接口获取参数信息 `ICommandWithParameters` 。

- 当你不知道数据库架构时， [CDynamicStringAccessor](../../data/oledb/cdynamicstringaccessor-class.md)、 [CDynamicStringAccessorA](../../data/oledb/cdynamicstringaccessora-class.md)和[CDynamicStringAccessorW](../../data/oledb/cdynamicstringaccessorw-class.md)将使用这些类。 `CDynamicStringAccessorA` 检索作为 ANSI 字符串的数据; `CDynamicStringAccessorW` 以 Unicode 字符串的形式检索数据。
...
OLE DB 模板支持三种类型的行集 (请参阅上图) ：通过 [CRowset](../../data/oledb/crowset-class.md)) 实现的单个行集 (，通过 [CBulkRowset](../../data/oledb/cbulkrowset-class.md) (实现的批量行集) ，以及 [CArrayRowset](../../data/oledb/carrayrowset-class.md) (实现的数组行集。 调用时，单个行集提取单个行句柄 `MoveNext` 。 大容量行集可以提取多个行句柄。 数组行集是可使用数组语法访问的行集。
