---
description: 了解更多相关信息： MFC：使用不含文档和视图的数据库类
...
|选项|查看|文档|
|------------|----------|--------------|
|无|派生自 `CView`。|不提供数据库支持。 这是默认选项。<br /><br /> 如果您在 "[应用程序类型"-"MFC 应用程序向导](../mfc/reference/application-type-mfc-application-wizard.md)" 页上选择 "**文档/视图体系结构支持**" 选项，则可以在 "**文件**" 菜单上获取完整文档支持，包括序列化和 **新** 的、**打开**、**保存** 和 **另存为** 命令。 请参阅 [不包含文档的应用程序](#_core_applications_with_no_document)。|
|**仅头文件**|派生自 `CView`。|为应用程序提供基本的数据库支持级别。<br /><br /> 包括 Afxdb。 添加链接库，但不创建任何特定于数据库的类。 稍后可以创建记录集，并使用它们来检查和更新记录。|
...
MFC 应用程序向导有两个选项可支持基于窗体的数据访问应用程序。 每个选项都创建一个 `CRecordView` 派生视图类和一个文档。 它们在文档中遗留的内容有所不同。
...
- 这是存储对象的便利位置 `CRecordset` 。

   此使用情况类似于普通文档概念：文档将数据存储 (，在本例中为一组记录) ，视图是文档视图。

- 如果你的应用程序提供多个视图 (例如) 多个记录视图，则文档支持协调视图。
...
有时可能需要编写不使用文档或视图的应用程序。 如果没有文档，可以将数据存储 (如 `CRecordset` 框架窗口类或应用程序类中的对象) 。 任何其他要求取决于应用程序是否提供用户界面。

### <a name="database-support-with-a-user-interface"></a><a name="_core_database_support_with_a_user_interface"></a> 具有用户界面的数据库支持

例如，如果您的用户界面 (之外，例如控制台命令行界面) ，则您的应用程序将直接绘制到框架窗口的工作区而不是视图中。 此类应用程序不使用 `CRecordView` 、 `CFormView` 或 `CDialog` 作为其主用户界面，但它通常用于 `CDialog` 普通对话框。

### <a name="writing-applications-without-documents"></a><a name="_core_writing_applications_without_documents"></a> 不带文档编写应用程序

由于应用程序向导不支持创建没有文档的应用程序，因此必须编写您自己 `CWinApp` 的派生类，如果需要，还可以创建一个 `CFrameWnd` 或 `CMDIFrameWnd` 类。 重写 `CWinApp::InitInstance` 应用程序对象并将其声明为：
...
- 一个中间数据访问对象，其他应用程序 (客户端) 调用应用程序和数据源之间的特定数据处理。

- 在无需用户干预的情况下处理数据的应用程序，例如将数据从一个数据库格式移到另一个数据库格式的应用程序，或者用于执行计算和执行批处理更新的应用程序。

因为没有任何文档拥有该 `CRecordset` 对象，所以你可能希望将其作为嵌入数据成员存储在 `CWinApp` 派生的应用程序类中。 替代方案包括：

- 永远不会保留永久 `CRecordset` 对象。 可以将 NULL 传递给记录集类构造函数。 在这种情况下，框架 `CDatabase`  使用记录集成员函数中的信息创建临时对象 `GetDefaultConnect` 。 这是最可能的替代方法。

- 使 `CRecordset` 对象成为全局变量。 此变量应为指向在替代中动态创建的记录集对象的指针 `CWinApp::InitInstance` 。 这避免了在初始化框架之前尝试构造对象。
