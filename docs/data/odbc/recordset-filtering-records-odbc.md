---
description: '了解详细信息：记录集：筛选记录 (ODBC) '
title: 记录集：筛选记录 (ODBC)
ms.date: 11/04/2016
helpviewer_keywords:
- data [MFC], filtering
...
你必须在构造对象之后、调用其成员函数之前建立记录集对象的筛选器 (或之前调用，然后再 `Open` 调用之前 `Requery` `Open` 已) 调用成员函数的现有记录集对象的成员函数。
...
1. 设置所需的任何其他选项，例如，排序顺序、锁定模式或参数。 指定参数特别有用。 有关参数化筛选器的信息，请参阅 [记录集： (ODBC) 参数化记录集 ](../../data/odbc/recordset-parameterizing-a-recordset-odbc.md)。

1. `Open`为新对象 (或) 上 `Requery` 打开的对象。
...
记录集仅选择符合您指定的搜索条件的记录。 例如，若要指定上述课程筛选器 (假设 `strCourseID` 当前已设置变量（例如）为 "MATH101" ) ，请执行以下操作：
...
> [!NOTE]
> 如果通过将自己的 SQL 字符串传递到来选择覆盖记录集的默认 SQL 字符串 `Open` ，则在自定义字符串具有 **WHERE** 子句时，不应设置筛选器。 有关替代默认 SQL 的详细信息，请参阅 [sql：自定义记录集的 Sql 语句 (ODBC) ](../../data/odbc/sql-customizing-your-recordsets-sql-statement-odbc.md)。
