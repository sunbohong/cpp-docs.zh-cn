---
description: '了解详细信息：记录集：添加、更新和删除记录 (ODBC) '
...
> [!NOTE]
> 你现在可以更有效地批量添加记录。 有关详细信息，请参阅 [记录集：批量 (ODBC) 中添加记录 ](../../data/odbc/recordset-adding-records-in-bulk-odbc.md)。

> [!NOTE]
> 本主题适用于从 `CRecordset` 派生的对象，其中尚未实现批量提取行。 如果使用批量取行，请参阅 [记录集：批量提取记录 (ODBC) ](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。

可更新的快照和动态集使你可以添加、编辑 (更新) 和删除记录。 本主题介绍：
...
1. 调用 recordset 对象的 [CanUpdate](../../mfc/reference/crecordset-class.md#canupdate) 成员函数。
   ...
   `CanUpdate` 如果记录集可更新，则返回一个非零值。

默认情况下，记录集是完全可更新的 (可以 `AddNew`) 执行、 `Edit` 和 `Delete` 操作。 但你也可以使用 [appendOnly](../../mfc/reference/crecordset-class.md#open) 选项来打开可更新的记录集。 以这种方式打开的记录集只允许添加新记录 `AddNew` 。 您无法编辑或删除现有记录。 您可以通过调用 [CanAppend](../../mfc/reference/crecordset-class.md#canappend) 成员函数来测试是否只为追加而打开记录集。 `CanAppend` 如果记录集是完全可更新的，则返回一个非零值，或者仅为追加而打开。

下面的代码演示如何将 `CanUpdate` 用于名为的记录集对象 `rsStudentSet` ：
...
> [!CAUTION]
> 当您准备通过调用更新记录集时 `Update` ，请注意您的记录集包括构成表的主键的所有列 (或表) 的任何唯一索引的所有列。 在某些情况下，框架只能使用记录集中所选的列来标识要更新的表中的记录。 如果没有所有必需的列，表中的多个记录可能会更新，这可能会破坏表的引用完整性。 在这种情况下，当调用时，框架会引发异常 `Update` 。
...
1. 调用记录集对象的 [AddNew](../../mfc/reference/crecordset-class.md#addnew) 成员函数。
   ...
   `AddNew` 准备要用作编辑缓冲区的记录集。 所有字段数据成员都设置为特殊值 Null 并标记为 "未更改"，因此，在调用 [Update](../../mfc/reference/crecordset-class.md#update)时，仅将 (脏) 值写入数据源。
...
1. 调用记录集对象的 `Update` 成员函数。
   ...
   `Update` 通过向数据源写入新记录完成添加。 有关未能调用的情况的详细信息 `Update` ，请参阅 [记录集：记录集如何更新记录 (ODBC) ](../../data/odbc/recordset-how-recordsets-update-records-odbc.md)。

有关如何添加记录以及添加记录在记录集中可见的方式的信息，请参阅 [记录集： AddNew、Edit 和 Delete (ODBC) 的工作 ](../../data/odbc/recordset-how-addnew-edit-and-delete-work-odbc.md)方式。
...
> [!TIP]
> 若要取消 `AddNew` 或 `Edit` 调用，只需对或调用 `AddNew` `Edit` `Move` *AFX_MOVE_REFRESH* 参数即可。 数据成员重置为其以前的值，您仍处于 `Edit` 或 `Add` 模式。
...
1. 调用记录集对象的 `Update` 成员函数。
   ...
   `Update` 通过将更改的记录写入数据源来完成编辑。 有关未能调用的情况的详细信息 `Update` ，请参阅 [记录集：记录集如何更新记录 (ODBC) ](../../data/odbc/recordset-how-recordsets-update-records-odbc.md)。

编辑记录后，编辑的记录将保留当前记录。

下面的示例演示了一个 `Edit` 操作。 它假定用户已移动到他（或她）想要编辑的记录。
...
1. 调用 recordset 对象的 [Delete](../../mfc/reference/crecordset-class.md#delete) 成员函数。
   ...
   与 `AddNew` 和不同 `Edit` ， `Delete` 没有相应的 `Update` 调用。
...
下面的示例演示了一个 `Delete` 操作。 它假定用户已移动到用户要删除的记录。 `Delete`调用后，移动到新记录很重要。
...
有关、和成员函数的影响的详细信息 `AddNew` `Edit` `Delete` ，请参阅 [记录集：记录集如何更新记录 (ODBC) ](../../data/odbc/recordset-how-recordsets-update-records-odbc.md)。
