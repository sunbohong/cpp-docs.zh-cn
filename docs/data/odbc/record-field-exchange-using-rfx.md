---
description: 了解详细信息：记录字段交换：使用 RFX
title: 记录字段交换：使用 RFX
ms.date: 11/04/2016
helpviewer_keywords:
...
- [记录字段交换：使用向导代码](../../data/odbc/record-field-exchange-working-with-the-wizard-code.md) 会引入 RFX 的主要组件，并说明 Mfc 应用程序向导和 **添加类** (的代码，如 [添加 MFC ODBC 使用者添加 mfc ODBC 使用) 者](../../mfc/reference/adding-an-mfc-odbc-consumer.md) 中所述，用于支持 RFX，以及如何修改向导代码。

- [记录字段交换：使用 RFX 函数](../../data/odbc/record-field-exchange-using-the-rfx-functions.md) 说明了在重写中对 RFX 函数的编写调用 `DoFieldExchange` 。
...
|你|框架|
|---------|-------------------|
|使用向导声明记录集类。 指定字段数据成员的名称和数据类型。|向导将派生一个 `CRecordset` 类，并为您编写 [DoFieldExchange](../../mfc/reference/crecordset-class.md#dofieldexchange) 替代，包括每个字段数据成员的 RFX 函数调用。|
| (可选) 手动将任何所需的参数数据成员添加到类。 `DoFieldExchange`为每个参数数据成员手动添加 RFX 函数调用，为参数组添加对[CFieldExchange：： SetFieldType](../../mfc/reference/cfieldexchange-class.md#setfieldtype)的调用，并指定[m_nParams](../../mfc/reference/crecordset-class.md#m_nparams)中的参数总数。 请参阅 [记录集： (ODBC) 参数化记录集 ](../../data/odbc/recordset-parameterizing-a-recordset-odbc.md)。||
| (可选) 手动将其他列绑定到字段数据成员。 手动递增 [m_nFields](../../mfc/reference/crecordset-class.md#m_nfields)。 请参阅 [记录集：动态绑定数据列 (ODBC) ](../../data/odbc/recordset-dynamically-binding-data-columns-odbc.md)。||
