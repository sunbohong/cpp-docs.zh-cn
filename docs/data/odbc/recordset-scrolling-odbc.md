---
description: '了解详细信息：记录集：滚动 (ODBC) '
title: 记录集：滚动 (ODBC)
ms.date: 11/04/2016
helpviewer_keywords:
- recordsets [C++], end of
...
类 `CRecordset` 提供 `Move` 成员函数以便在记录集中滚动。 这些函数按行集移动当前记录。 如果已实现批量行提取，则 `Move` 操作会按行集的大小重新定位记录集。 如果尚未实现批量行提取，则调用 `Move` 函数每次都会将记录集重定位一条记录。 有关批量行提取的详细信息，请参阅 [记录集：批量提取记录 (ODBC) ](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。

> [!NOTE]
> 在记录集中时，可能不会跳过已删除的记录。 有关详细信息，请参阅 [IsDeleted](../../mfc/reference/crecordset-class.md#isdeleted) 成员函数。

除了 `Move` 函数外， `CRecordset` 还提供了成员函数，用于检查您是否已滚动到记录集的开头或前面。
...
`IsEOF` 如果记录集位于最后一条记录之后，则返回一个非零值。 `IsBOF` 如果记录集定位在所有记录) 之前 (第一条记录的前面，则返回一个非零值。 在任一情况下，都没有要操作的当前记录。 如果在已为 true `MovePrev` 时调用 `IsBOF` ，或 `MoveNext` 在 `IsEOF` 已为 true 时调用，则该框架将引发 `CDBException` 。 你还可以使用 `IsBOF` 和 `IsEOF` 来检查是否有空记录集。
...
> [!NOTE]
> 与 MFC DAO 类不同，MFC ODBC 类不提供一组 `Find` 函数来定位满足指定条件的下一 (或上一) 记录。
