---
description: 了解更多： MFC ActiveX 控件：方法
...
Microsoft 基础类库 (MFC) 实现了一种机制，该机制允许控件支持常用和自定义方法。 第一部分是 class `COleControl` 。 派生自 `CWnd` ， `COleControl` 成员函数支持所有 ActiveX 控件通用的常用方法。 此机制的第二部分是调度映射。 调度映射类似于消息映射;但是，调度映射将虚拟成员函数映射到 IDispatch ID，而不是将函数映射到 Windows 消息 ID。

为了使控件正确支持各种方法，其类必须声明一个调度映射。 这是通过 ( 控件类标头中的以下代码行来实现的。H) 文件：

[!code-cpp[NVC_MFC_AxUI#13](codesnippet/cpp/mfc-activex-controls-methods_1.h)]

调度映射的主要目的是在外部调用方使用的方法名称之间建立关系 (例如容器) 和实现方法的控件类的成员函数。 声明调度映射后，需要在控件的实现 ( 中定义它。CPP) 文件。 以下代码行将定义调度映射：
