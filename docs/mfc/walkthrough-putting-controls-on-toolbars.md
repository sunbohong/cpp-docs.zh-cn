---
description: 了解详细信息：演练：将控件置于工具栏上
title: 演练：将控件置于工具栏上
ms.date: 04/25/2019
helpviewer_keywords:
...
1. 在处理消息的消息处理程序中 `AFX_WM_RESETTOOLBAR` ，执行以下步骤：
...
"**自定义**" 对话框的 "**命令**" 选项卡包含可在应用程序中使用的命令的列表。 默认情况下，" **自定义** " 对话框处理应用程序菜单，并在每个菜单类别中生成标准工具栏按钮的列表。 若要保持工具栏控件提供的扩展功能，必须将标准工具栏按钮替换 **为自定义对话框中** 的自定义控件。

启用自定义时，可以 `OnViewCustomize` 通过使用[CMFCToolBarsCustomizeDialog 类](../mfc/reference/cmfctoolbarscustomizedialog-class.md)类在自定义处理程序中创建 "自定义" 对话框。 在通过调用 [CMFCToolBarsCustomizeDialog：： Create](../mfc/reference/cmfctoolbarscustomizedialog-class.md#create)显示 "**自定义**" 对话框之前，调用 [CMFCToolBarsCustomizeDialog：： ReplaceButton](../mfc/reference/cmfctoolbarscustomizedialog-class.md#replacebutton)将标准按钮替换为新控件。

## <a name="example-creating-a-find-combo-box"></a>示例：创建一个 Find 组合框

本部分介绍如何创建显示在工具栏上并包含最近使用过的搜索字符串的 " **查找** " 组合框控件。 用户可在控件中键入一个字符串，然后按 Enter 键来搜索文档，或按 Esc 键将焦点返回主框架。 此示例假定文档显示在 [CEditView 类](../mfc/reference/ceditview-class.md)派生视图中。
...
1. 将此按钮及其命令添加到应用程序资源：
   ...
   1. 使用命令 ID 创建新的菜单项 `ID_EDIT_FIND` 。
...
1. 创建一个 `CFindComboBox` 派生自 [CComboBox 类](../mfc/reference/ccombobox-class.md)的新类。
...
1. 在 `ID_EDIT_FIND` 消息处理程序中 `OnFind` ，使用 [CMFCToolBar：： IsLastCommandFromButton](../mfc/reference/cmfctoolbar-class.md#islastcommandfrombutton) 来确定是否已从 " **查找** " 组合框按钮发送 "查找" 命令。 如果是这样，则查找文本并将搜索字符串添加到组合框。
...
1. 在 `AFX_WM_RESETTOOLBAR` 处理程序中，检查工具栏 ID，即 AFX_WM_RESETTOOLBAR 消息的 *WPARAM* 。 如果工具栏 ID 等于包含 " **查找** " 组合框按钮的工具栏的 ID，请调用 [CMFCToolBar：： ReplaceButton](../mfc/reference/cmfctoolbar-class.md#replacebutton) 来替换 " **查找** " 按钮， (即带有包含对象的命令 ID 的按钮 `ID_EDIT_FIND)` `CFindComboButton` 。
...
在自定义处理程序中 `OnViewCustomize` ，调用 [CMFCToolBarsCustomizeDialog：： ReplaceButton](../mfc/reference/cmfctoolbarscustomizedialog-class.md#replacebutton) 来替换 " **查找** " 按钮 (即，命令 ID 为的按钮 `ID_EDIT_FIND` 具有 `CFindComboButton` 对象) 。
