---
description: 了解详细信息：访问 Run-Time 类信息
title: 访问运行时类信息
ms.date: 11/04/2016
helpviewer_keywords:
...
> [!NOTE]
> MFC 在 Visual C++ 4.0 中引入 (RTTI) 支持时，不使用 [运行时类型信息](../cpp/run-time-type-information.md) 。
...
1. 确保类具有运行时类支持。 也就是说，该类必须直接或间接派生 `CObject` ，并使用在 `IMPLEMENT_DYNAMIC` `DECLARE_DYNCREATE` `IMPLEMENT_DYNCREATE` `DECLARE_SERIAL` `IMPLEMENT_SERIAL` [从 CObject 派生类一](deriving-a-class-from-cobject.md)文中介绍的 DECLARE _ DYNAMIC 和、和、或和宏。
