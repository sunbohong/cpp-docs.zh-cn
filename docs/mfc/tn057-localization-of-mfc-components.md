---
description: 了解详细信息： TN057： MFC 组件的本地化
...
本地化应用程序或 DLL 涉及将这些资源替换为与目标语言匹配的资源。 对于您自己的资源，此操作比较简单：在资源编辑器中编辑资源并生成您的应用程序。 如果已正确编写你的代码，则不会有要本地化为 c + + 源代码的硬编码的字符串或文本-只需修改资源即可实现所有本地化。 事实上，你可以以某种方式实现你的组件，使它们都提供甚至不涉及生成原始代码的本地化版本。 这种方法更复杂，但很值得，并且这是为 MFC 本身选择的机制。 也可以通过将 EXE 或 DLL 文件加载到资源编辑器并直接编辑资源来本地化应用程序。 如有可能，每当您生成新版本的应用程序时，它都会要求重新应用那些更改。
...
从那以后，MFC 将从该 DLL 而非 myapp.exe 加载资源。 但是，所有资源都必须存在于该 DLL 中；MFC 不会搜索应用程序的实例来寻找给定的资源。 此方法同样适用于常规 MFC Dll 和 OLE 控件。 您的安装程序将根据用户想采用的资源区域设置复制相应版本的 MYRES.DLL。

创建纯资源 DLL 相对简单一些。 您创建 DLL 项目，向其添加 .RC 文件，然后添加必需的资源。 如果您有不使用此方法的现有项目，则可以从该项目中复制资源。 将资源文件添加到项目后，你差不多已做好生成项目的准备。 唯一必须执行的操作是将链接器选项设置为包括 **/NOENTRY**。 这会告知链接器 DLL 没有任何入口点，因为它没有代码，因此没有任何入口点。
...
到本文撰写之时为止，MFC 支持中文、德语、西班牙语、法语、意大利语、日语和朝鲜语。 包含这些本地化版本的文件位于 MFC\INCLUDE\L.*（“L”表示已本地化）目录中。 例如，德语位于 MFC\INCLUDE\L.DEU 中。 若要使你的应用程序使用这些 RC 文件而不是 MFC\INCLUDE 中的文件，请将添加 `/IC:\PROGRAM FILES\MICROSOFT VISUAL STUDIO .NET 2003\VC7\MFC\INCLUDE\L.DEU` 到 RC 命令行 (这只是一个示例; 你需要将选择的区域设置和 Visual C++) 中安装的目录进行替换。

如果应用程序以静态方式与 MFC 链接，以上指令将有效。 大多数应用程序以动态方式进行链接（因为这是 AppWizard 的默认设置）。 在这种情况下，不仅会动态链接代码，资源也是如此。 因此，您可以在应用程序中本地化资源，但 MFC 实现资源仍将从 MFC7x.DLL（或更高版本）加载或从 MFC7xLOC.DLL（如果存在）加载。 您可以从两个不同的角度达到此目的。

更复杂的方法是附带本地化的 MFC7xLOC.DLL（如 MFC7xDEU 用于德语、MFC7xESP.DLL 用于西班牙语，等等）或更高版本之一，并在用户安装您的应用程序时将相应的 MFC7xLOC.DLL 安装到系统目录中。 这对于开发人员和最终用户可能非常复杂，因此不建议采用。 有关此技术及其注意事项的详细信息，请参阅 [技术说明 56](../mfc/tn056-installation-of-localized-mfc-components.md) 。
