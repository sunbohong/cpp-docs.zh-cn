...
ms.locfileid: "97280581"
---
# <a name="mfc-library-versions"></a>MFC 库版本

MFC 库可用于支持 ANSI 单字节字符集和多字节字符集 (MBCS) 代码，以及支持 Unicode (编码为 UTF-16LE、Windows native 字符集) 的版本。 每个 MFC 版本都作为静态库或共享 DLL 提供。 还有一个较小的 MFC 静态库版本，它为对大小非常敏感且不需要这些控件的应用程序留下了用于对话框的 MFC 控件。 对于支持的体系结构（包括 x86、x64 和 ARM 处理器），MFC 库在调试和发布版本中均可用。 可以 ( .exe 文件) 和任意版本的 MFC 库创建应用程序。 还提供了一组用于与托管代码进行交互的 MFC 库。 MFC 共享 Dll 包含一个版本号，用于指示库的二进制文件兼容性。
...
|AFXNMCD.LIB|MFC Static-Link 没有 MFC 对话框控件的库，发布版本|
|AFXNMCDD.LIB|MFC Static-Link 没有 MFC 对话框控件的库，调试版本|
...
|MFCM *版本* U.DLL|带有 Windows 窗体控件的 MFC DLL，Unicode 发行版本|
...
如果动态链接到 MFC 的共享 DLL 版本，无论该版本是来自应用程序还是来自 MFC 扩展 DLL，都必须包含匹配的 MFC *版本*。DLL 或 MFC *版本* 在部署产品时U.DLL。

有关可与应用程序一起分发的 Visual C++ Dll 的列表，请参阅 [Microsoft Visual Studio 2017 和 Microsoft Visual Studio 2017 SDK 的可分发代码 (包括实用程序和 BuildServer 文件) ](/visualstudio/productinfo/2017-redistribution-vs) 或 [适用于 Visual Studio 2019 的可分发代码](/visualstudio/releases/2019/redistribution)。
...
通过链接到 MFC 静态库来创建 DLL 项目时，可以在不使用 MFC 共享 Dll 的情况下部署 DLL。 当 DLL 项目链接到导入库时，MFC *版本*。LIB 或 MFC *版本* U. 您必须部署匹配的 MFC 共享 DLL MFC *版本*。DLL 或 MFC *版本* U.DLL 与您的 dll 一起提供。 有关详细信息，请参阅 [dll](../build/dlls-in-visual-cpp.md)。
