---
description: 了解更多相关信息： MFC ActiveX 控件：在 ActiveX 控件中使用数据绑定
...
1. 传递给 `BoundPropertyChanged` 和函数的参数 `BoundPropertyRequestEdit` 是属性的 dispid，它是传递给中属性的 id ( # A1 特性的参数。IDL 文件。
...
1. 对于控件类的标头文件的公共部分，为成员变量 (构造函数) 添加以下定义：
