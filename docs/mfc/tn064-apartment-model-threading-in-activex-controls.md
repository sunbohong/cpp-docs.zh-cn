---
description: 了解有关以下内容的详细信息： TN064： ActiveX 控件中的 Apartment-Model 线程处理
...
本技术说明介绍了如何在 ActiveX 控件中启用单元模型线程处理。 请注意，仅 Visual C++ 版本4.2 或更高版本中支持单元模型线程处理。

## <a name="what-is-apartment-model-threading"></a>什么是 Apartment-Model 线程

单元模型是一种在多线程容器应用程序中支持嵌入对象（如 ActiveX 控件）的方法。 尽管应用程序可能有多个线程，但嵌入对象的每个实例都将分配给一个 "单元"，后者将只在一个线程上执行。 换句话说，对控件实例的所有调用都将在同一线程上发生。

但是，可以将相同类型的控件的不同实例分配给不同的单元。 因此，如果控件的多个实例共享公共 (（例如静态或全局数据) ）中的所有数据，则对此共享数据的访问需要由同步对象（如临界区）进行保护。

有关单元线程模型的完整详细信息，请参阅 *OLE 程序员参考* 中的 [进程和线程](/windows/win32/ProcThread/processes-and-threads)。

## <a name="why-support-apartment-model-threading"></a>为什么支持 Apartment-Model 线程
...
如果控件使用共享数据（如静态成员变量），则应使用临界区保护对该数据的访问，以防止多个线程同时修改数据。 若要为此目的设置关键部分，请 `CCriticalSection` 在控件的类中声明类的静态成员变量。 如果 `Lock` `Unlock` 代码访问共享数据，请使用此临界区对象的和成员函数。
...
`_strShared`然后，可以通过临界区保护对静态成员的访问：
...
支持单元模型线程的控件应通过在 *类 id* \\ **InprocServer32** 键下的类 id 注册表项中添加值为 "单元" 的命名值 "ThreadingModel" 来指示此功能。 若要使此密钥自动注册到你的控件，请将第六个参数中的 *afxregapartmentthreading 改* 标志传递给 `AfxOleRegisterControlClass` ：
...
如果在 Visual C++ 版本4.1 或更高版本中由 ControlWizard 生成了控件项目，则代码中已存在此标志。 无需进行任何更改即可注册线程模型。
