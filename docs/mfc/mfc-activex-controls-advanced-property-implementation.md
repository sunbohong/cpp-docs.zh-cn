...
如果要将属性设置为条件只读还是只写 (例如，仅当控件在特定模式下操作时) ，你可以像平常一样提供 Set 或 Get 函数，然后在 `SetNotSupported` 适当的位置调用或 `GetNotSupported` 函数。 例如：
...
若要指示在尝试获取或设置属性时出错，请使用 `COleControl::ThrowError` 函数，该函数采用 SCODE (状态代码) 作为参数。 可以使用预定义的 SCODE，也可以定义自己的。 有关定义自定义 SCODEs 的预定义 SCODEs 的列表，请参阅文章 ActiveX 控件：高级主题中的 [处理 Activex 控件](mfc-activex-controls-advanced-topics.md) 中的错误。
