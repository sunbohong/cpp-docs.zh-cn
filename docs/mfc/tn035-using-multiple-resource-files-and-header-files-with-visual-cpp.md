---
description: 了解详细信息： TN035：将多个资源文件和头文件用于 Visual C++
...
- **概述 Visual C++ 如何管理资源文件和头文件** 概述了资源集如何包括中的命令 Visual C++ 使你可以在同一项目中使用多个资源文件和头文件。
...
- **强制不可编辑的 Visual C++ 文件** 介绍了如何确保 Visual C++ 不会对自定义资源进行编辑和无意中重新格式化。

- **管理由多个 Visual C++ 编辑的共享的符号。RC 文件** 描述了如何在多个之间共享相同的符号。RC 文件以及如何避免分配重复的 ID 数值。

- **管理之间的依赖关系。RC、。CPP 和。H 文件** 描述 Visual C++ 如何避免不必要的重新编译。依赖于资源符号文件的 CPP 文件。

- **Visual C++ 如何管理集包含信息** 提供有关 Visual C++ 如何跟踪多个 (嵌套) 的技术详细信息。#Include 的 RC 文件和多个标头文件。RC 文件。
...
默认情况下，无论资源文件的名称为何（例如 MYAPP.RC），Visual C++ 始终将相应的头文件命名为 RESOURCE.H。 通过 Visual C++ 中的 "**视图**" 菜单使用 "**资源包含**" 命令，可以通过在 "**设置包括**" 对话框中更新符号头文件来更改此标头文件的名称。

### <a name="read-only-symbol-directives"></a>只读符号指令

虽然 Visual C++ 对于任何给定 .RC 文件仅编辑一个头文件，但 Visual C++ 支持引用在其他只读头文件中定义的符号。 通过 Visual C++ 中的 "**视图**" 菜单使用 "**资源包括**" 命令，可以指定任意数量的附加只读头文件作为 Read-Only 符号指令。 在“只读”限制下，当你在 .RC 文件中添加新资源时，可使用只读头文件中定义的符号；但如果删除此资源，符号仍将保留在只读头文件中。 无法更改分配给只读符号的数值。

### <a name="compile-time-directives"></a>编译时指令

Visual C++ 还支持嵌套资源文件，即通过 #include 指令将一个 .RC 文件包括在另一 .RC 文件中。 使用 Visual C++ 编辑给定 .RC 文件时，通过 #include 指令包括的文件中的任何资源将不可见。 但当你编译此 .RC 文件时，也将编译通过 #include 指令包括的文件。 使用 "Visual C++ 的"**视图**"菜单中的"**资源包括**"命令，可以指定任意数量的 #include。作为 Compile-Time 指令的 RC 文件。

请注意，如果您读入 Visual C++，会发生什么情况。RC 文件，#include 其他文件。 *未* 指定为 Compile-Time 指令的 RC 文件。 如果将先前通过文本编辑器手动维护的 .RC 文件置于 Visual C++ 中，则可能会发生此情况。 Visual C++ 读取通过 #include 指令包括的 .RC 文件时，会将通过 #include 指令包括的资源合并到父 .RC 文件。 保存父 .RC 文件时，#include 语句实际上将被通过 #include 指令包括的资源替代。 如果您不希望执行此合并，则应该从父级中删除 #include 语句。RC 文件，然后 *再* 将其读入 Visual C++;然后，使用 Visual C++ 将与 Compile-Time 指令相同的 #include 语句添加回。

Visual C++ 在中保存。RC 文件上述三种类型的设置包括 (符号头文件的信息、Read-Only 符号指令以及 #include 指令中的 Compile-Time) 指令 *和* TEXTINCLUDE 资源。 TEXTINCLUDE 资源是一种通常不需要处理的实现细节，介绍了 [Visual C++ 管理集包括信息的方式](#_mfcnotes_tn035_set_includes)。
...
## <a name="including-additional-header-files"></a><a name="_mfcnotes_tn035_including"></a> 包括其他头文件
...
您可以通过使用 **资源** include 命令将 #include 语句指定为第二个 Read-Only 符号指令来包括其他只读头文件，如下所示：
...
- 如果需要以其他专用数据编辑器可解释的二进制格式或文本格式保留用户定义的资源，则应将资源保留在单独 .RC 文件中，以防 Visual C++ 将格式更改为十六进制数据。 此.在 MFC 高级概念示例 [SPEAKN](../overview/visual-cpp-samples.md) 中，WAV (声音) 文件资源是一个很好的示例。
...
向导向导创建的 RES\MYAPP。RC2 文件是一个文件示例，其中包含您 *不* 希望意外读入 Visual C++ 的资源，然后将其写回，并丢失格式设置信息。 要避免此问题，请将以下各行置于 RES\MYAPP.RC2 文件的开头：
...
当 Visual C++ 编译时。RC 文件，它定义 `APSTUDIO_INVOKED` 和 `RC_INVOKED` 。 如果 AppWizard 创建的文件结构被破坏，并且 Visual C++ 读取上述 #error 行，则它将报告错误，并中止读取 .RC 文件。
...
ID_TOOLS_SPELL 符号保留在共享头文件 MYSHARED.H 中。 可使用文本编辑器手动维护此共享头文件；Visual C++ 不会直接编辑此文件。 在两个资源文件 MYSTRS.RC 中。RC 和 MYMENUS.RC。RC：指定 #include MYSHARED.H。在 MYAPP 的 Read-Only 指令中为 H。RC，如前文所述，使用 **资源包含** 命令。
...
`_APS_NEXT_RESOURCE_VALUE` 将用于对话框资源、菜单资源等的下一个符号值。 资源符号值的有效范围为 1 到 0x6FFF。

`_APS_NEXT_COMMAND_VALUE` 将用于命令标识的下一个符号值。 命令符号值的有效范围为 0x8000 到 0xDFFF。

`_APS_NEXT_CONTROL_VALUE` 将用于对话框控件的下一个符号值。 对话框控件符号值的有效范围为 8 到 0xDFFF。
...
现在，每次在同一项目中创建新的资源文件时，Visual C++ 定义相同的 `_APS_NEXT_` 值。 因此，如果你在两个不同 .RC 文件中添加比如多个对话框，则 Visual C++ 很可能会将相同 #define 值分配到不同对话框。 例如，第一个 .RC 文件中的 IDD_MY_DLG1 可能会获得与第二个 .RC 文件中的 IDD_MY_DLG2 相同的数字 101。

若要避免此问题，应在各自 .RC 文件中对四个 ID 域中的每个域保留不同的数值范围。 为此，请手动更新 `_APS_NEXT` 每个中的值。RC 文件， **然后再** 开始添加资源。 例如，如果第一个。RC 文件使用默认 `_APS_NEXT` 值，则您可能需要将以下 `_APS_NEXT` 值分配给第二个值。RC 文件：
...
Visual C++ 保存 .RC 文件时，还会保存对相应 RESOURCE.H 文件的符号更改。 引用 .RC 文件中的资源的任何 .CPP 文件必须通过 #include 指令包括 RESOURCE.H 文件，此文件通常位于你项目的主头文件中。 这将导致意外的副作用，因为开发环境的内部项目管理将浏览源文件的头依赖关系。 每次在 Visual C++ 中添加一个新符号时，所有。#Include 资源的 CPP 文件。需要重新编译 H。
...
## <a name="how-visual-c-manages-set-includes-information"></a><a name="_mfcnotes_tn035_set_includes"></a> Visual C++ 管理集的包含信息
...
TEXTINCLUDE 资源的目的是安全地将集包括信息存储在可在 Visual C++ 的 " **设置包括** " 对话框中轻松使用的形式。 TEXTINCLUDE 是由 Visual C++ 定义的 *资源类型* 。 Visual C++ 可识别具有资源标识符 1、2 和 3 的三种特定 TEXTINCLUDE 资源：
...
[按编号的技术说明](../mfc/technical-notes-by-number.md)\
