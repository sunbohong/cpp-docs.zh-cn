...
如 [MFC ActiveX 控件：属性](mfc-activex-controls-properties.md)中所述，通过 " [添加属性向导](../ide/adding-a-property-visual-cpp.md#names-add-property-wizard)" 可以轻松添加常用属性。 您可以选择 "字体" 属性，"添加属性向导" 会自动在控件的调度映射中插入常用字体条目。
