---
description: 了解详细信息：演练：向 MFC 项目添加 D2D 对象
...
ms.locfileid: "97344895"
---
# <a name="walkthrough-adding-a-d2d-object-to-an-mfc-project"></a>演练：向 MFC 项目添加 D2D 对象

本演练介绍如何将基本的 Direct2D (D2D) 对象添加到 Visual C++，Microsoft 基础类库 (MFC) project，然后将项目生成到打印 "Hello，World！" 的应用程序中。 在渐变背景上。
...
若要完成本演练，你必须安装 Visual Studio，并使用 **c + +** 工作负荷进行桌面开发，并 **为 x86 和 x64** 组件提供可选 Visual C++ MFC。
...
1. 在 " **源文件** " 文件夹中，打开 MFCD2DWalkthroughView。 在类的构造函数中 `CMFCD2DWalkthroughView` ，添加以下代码：
...
1. 在 " **消息** " 选项卡上的 " **消息** " 框中，选择， `WM_SIZE` 然后选择 " **添加处理程序**"。 此操作向 `OnSize` 类添加消息处理程序 `CMFCD2DWalkthroughView` 。
...
1. 在 " **添加自定义消息** " 对话框的 " **自定义 Windows 消息** " 框中，键入 *AFX_WM_DRAW2D*。 在 " **消息处理程序名称** " 框中，键入 *OnDraw2D*。 选择 " **已注册的消息** " 选项，然后选择 **"确定"**。 此操作向类添加 AFX_WM_DRAW2D 消息的消息处理程序 `CMFCD2DWalkthroughView` 。

1. 在 " **现有处理程序** " 框中，选择 `OnDraw2D` 。 选择 " **编辑代码** " 以显示 `CMFCD2DWalkthroughView::OnDraw2D` 方法。 将此代码用于 `CMFCD2DWalkthroughView::OnDrawD2D` 方法：
