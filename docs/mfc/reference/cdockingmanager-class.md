---
description: 了解详细信息： CDockingManager 类
...
|[CDockingManager::AdjustPaneFrames](#adjustpaneframes)|导致向所有窗格和窗口发送 WM_NCCALCSIZE 消息 `CPaneFrameWnd` 。|
...
`CDockingManager`类实现某些可用于查找窗格或窗口的服务 `CPaneFrameWnd` 。 通常不会直接调用这些服务，因为它们被包装在主框架窗口对象中。 有关详细信息，请参阅 [CPaneFrameWnd 类](../../mfc/reference/cpaneframewnd-class.md)。
...
下面的示例演示如何使用类中的各种方法 `CDockingManager` 配置 `CDockingManager` 对象。 该示例演示如何显示一个附加按钮，该按钮打开所有停靠窗格标题上的弹出菜单，以及如何设置对象的停靠模式。 此代码片段是 [Visual Studio 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
导致向所有窗格和窗口发送 WM_NCCALCSIZE 消息 `CPaneFrameWnd` 。
...
如果调整对象的对齐方式，则为 TRUE `CRect` ;否则为 FALSE。
...
|CS_DOCK_IMMEDIATELY|在 "即时" 模式下，指针位于停靠站点上 () 启用 DT_IMMEDIATE 样式，因此必须立即停靠该窗格。|
...
*bFloatMultiOnly*<br/>
中若要排除不是实例的帧，则为 TRUE `CMultiPaneFrameWnd` ;否则为 FALSE。
...
框架将在即将关闭当前主窗口时发送 WM_DESTROY 消息。 `CMFCPopupMenu`当 `CMFCPopupMenu` 对象处理 WM_DESTROY 消息时，重写此方法以处理属于框架窗口的对象中的通知。
...
*pExtra*<br/>
中指向 void 的指针，如果为，则为指向强制转换的指针， `CCmdUI` 如果 *nCode* 是 CN_UPDATE_COMMAND_UI。
...
## <a name="cdockingmanagerserialize"></a><a name="serialize"></a> CDockingManager：：串行化
