---
description: 了解详细信息： CMFCButton 类
title: CMFCButton 类
ms.date: 08/28/2018
f1_keywords:
- CMFCButton
...
|“属性”|描述|
|----------|-----------------|
|[CMFCButton：：清理](#cleanup)|重置内部变量并释放已分配的资源，如图像、位图和图标。|
...
|[CMFCButton::SetStdImage](#setstdimage)|使用 `CMenuImages` 对象设置按钮图像。|
...
|[CMFCButton：： System.windows.window.sizetocontent](#sizetocontent)|调整按钮的大小以包含其按钮文本和图像。|
...
|[CMFCButton：： m_bHighlightChecked](#m_bhighlightchecked)|指示当光标悬停在其上方时是否突出显示 BS_CHECKBOX 样式的按钮。|
...
其他类型的按钮是从类派生的 `CMFCButton` ，如支持超链接的 [CMFCURLLinkButton](../../mfc/reference/cmfclinkctrl-class.md) 类以及 `CMFCColorButton` 支持颜色选取器对话框的类。

对象的样式 `CMFCButton` 可以是 *三维*、 *平*、 *半平面* 或 *无边框*。 按钮文本可以在按钮的左侧、顶部或中心对齐。 在运行时，您可以控制按钮是显示文本、图像、文本还是显示图像。 还可以指定在光标悬停在按钮上时显示特定的光标图像。

直接在代码中或使用 " **MFC 类向导** " 工具和对话框模板创建一个按钮控件。 如果直接创建一个按钮控件，请在 `CMFCButton` 应用程序中添加一个变量，然后调用该对象的构造函数和 `Create` 方法 `CMFCButton` 。 如果使用 **MFC 类向导**，请向 `CButton` 应用程序中添加一个变量，然后将该变量的类型从更改 `CButton` 为 `CMFCButton` 。

若要在对话框应用程序中处理通知消息，请为每个通知添加消息映射项和事件处理程序。 对象发送的通知与 `CMFCButton` 对象发送的通知相同 `CButton` 。

## <a name="example"></a>示例

下面的示例演示如何使用类中的各种方法配置按钮的属性 `CMFCButton` 。 该示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
## <a name="cmfcbuttoncleanup"></a><a name="cleanup"></a> CMFCButton：：清理
...
此方法会影响您的应用程序中从类派生的所有按钮 `CMFCButton` 。
...
如果按钮 BS_AUTOCHECKBOX 或 BS_AUTORADIOBUTTON 样式，则为 TRUE;否则为 FALSE。
...
如果该按钮具有 BS_CHECKBOX 或 BS_AUTOCHECKBOX 样式，则为 TRUE; 否则为。否则为 FALSE。
...
指示当光标悬停在其上方时是否突出显示 BS_CHECKBOX 样式的按钮。
...
将 `m_bHighlightChecked` 成员设置为 TRUE，以指定框架将在鼠标悬停在 BS_CHECKBOX 样式按钮时将其突出显示。
...
将 `m_bRightImage` 成员设置为 TRUE，以指定框架将在按钮的文本标签右侧显示该按钮的图像。
...
将 `m_bTransparent` 成员设置为 TRUE，以指定该框架将使该按钮透明。 `CMFCButton`构造函数将此成员初始化为 FALSE。
...
下表列出了 `CMFCButton::m_nFlatStyle` 用于指定按钮外观的枚举值。

|值|描述|
|-----------|-----------------|
|BUTTONSTYLE_3D|默认情况下， () 按钮显示为具有较高的三维边。 单击该按钮时，该按钮将显示为深缩进。|
...
下面的示例演示如何 `m_nFlatStyle` 在类中设置成员变量的值 `CMFCButton` 。 此示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
*bMap3dColors*<br/>
中指定按钮背景的透明颜色;即按钮的图符。 如果为 TRUE，则使用颜色值 RGB (192、192、192) ;若为 FALSE，则使用由定义的颜色值 `AFX_GLOBAL_DATA::clrBtnFace` 。
...
使用此方法可为按钮背景 (面部) 定义新的填充颜色。 请注意，当 [CMFCButton：： m_bTransparent](#m_btransparent) 成员变量为 TRUE 时，不填充背景。
...
下面的示例演示如何 `SetImage` 在类中使用方法的各种版本 `CMFCButton` 。 该示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
下面的示例演示如何使用 `SetMouseCursor` 类中的方法 `CMFCButton` 。 示例是 [新控件示例](../../overview/visual-cpp-samples.md)中的代码的一部分。
...
*id*<br/>
中枚举中定义的一个按钮图像标识符 `CMenuImage::IMAGES_IDS` 。 图像值指定图像，如箭头、pin 和单选按钮。

*state*<br/>
中枚举中定义的按钮图像状态标识符之一 `CMenuImages::IMAGE_STATE` 。 图像状态指定按钮颜色，如黑色、灰色、浅灰色、白色和暗灰色。 默认值为 `CMenuImages::ImageBlack`。

*idDisabled*<br/>
中枚举中定义的一个按钮图像标识符 `CMenuImage::IMAGES_IDS` 。 图像指示该按钮已禁用。 默认值是 () 的第一个按钮图像 `CMenuImages::IdArrowDown` 。
