---
description: 了解详细信息： CDrawingManager 类
...
|“属性”|描述|
|----------|-----------------|
|[CDrawingManager：： CreateBitmap_32](#createbitmap_32)|创建应用程序可以直接写入的与32位设备无关的位图 (DIB) 。|
|[CDrawingManager：:D rawAlpha](#drawalpha)|显示具有透明或半透明像素的位图。|
|[CDrawingManager：:D rawRotated](#drawrotated)|将给定矩形内的源 DC 内容旋转 +/-90 度|
|[CDrawingManager：:D rawEllipse](#drawellipse)|绘制具有所提供的填充和边框颜色的椭圆。|
|[CDrawingManager：:D rawGradientRing](#drawgradientring)|绘制一圈，并用颜色渐变填充它。|
|[CDrawingManager：:D rawLine，CDrawingManager：:D rawLineA](#drawline_cdrawingmanager__drawlinea)|绘制线条。|
|[CDrawingManager：:D rawRect](#drawrect)|用提供的填充和边框颜色绘制矩形。|
|[CDrawingManager：:D rawShadow](#drawshadow)|绘制矩形区域的阴影。|
...
|[CDrawingManager：:P ixelAlpha](#pixelalpha)|确定半透明像素的最终颜色的帮助器方法。|
|[CDrawingManager：:P repareShadowMask](#prepareshadowmask)|创建可用作阴影的位图。|
...
*规格*\
中一个指示位图大小的 [CSize](../../atl-mfc-shared/reference/csize-class.md) 参数。

*pBits*\
弄指向数据指针的指针，该指针接收 DIB 的位值的位置。
...
如果任意一种颜色设置为-1，则此方法返回而不绘制椭圆。 如果边界矩形的任一维度为0，它也将返回，而不绘制椭圆。
...
如果将任何颜色设置为-1，则此方法返回而不绘制矩形。 如果矩形的任一维度为0，它也会返回。
...
*bRightShadow*<br/>
中指示如何绘制阴影的布尔型参数。 如果 *bRightShadow* 为 `TRUE` ，则 `DrawShadow` 在矩形右侧绘制阴影。
...
可以使用参数 *pBmpSaveBottom* 和 *pBmpSaveRight* 为下阴影和右阴影提供两个有效的位图。 如果这些 [CBitmap](../../mfc/reference/cbitmap-class.md) 对象具有附加的 GDI 对象， `DrawShadow` 则将使用这些位图作为阴影。 如果 `CBitmap` 参数没有附加的 GDI 对象，则 `DrawShadow` 绘制阴影并将位图附加到参数。 在以后对的调用中 `DrawShadow` ，您可以提供这些位图来加快绘制过程的速度。 有关 `CBitmap` 类和 GDI 对象的详细信息，请参阅 [图形对象](../../mfc/graphic-objects.md)。

如果其中一个参数为 `NULL` ，则 `DrawShadow` 将自动绘制阴影。

如果将 *bRightShadow* 设置为 FALSE，则将在矩形区域的下方和左侧绘制阴影。

### <a name="example"></a>示例

下面的示例演示如何使用类的 `DrawShadow` 方法 `CDrawingManager` 。 此代码段是 "组件 [表演示" 示例](../../overview/visual-cpp-samples.md)的一部分。
...
*bHorz*<br/>
中布尔型参数，指示是 `Fill4ColorsGradient` 水平渐变还是垂直渐变颜色。 TRUE 表示水平渐变。
...
如果 *nPercentage* 小于0或大于100，则此方法将生成断言失败。
...
中在 `FillGradient` 开始渐变之前用 *colorStart* 填充的矩形的百分比。

*nEndFlatPercentage*<br/>
中在 `FillGradient` 完成渐变后用 *colorFinish* 填充的矩形的百分比。

### <a name="example"></a>示例

下面的示例演示如何使用类的 `FillGradient` 方法 `CDrawingManager` 。 此代码片段是 [MS Office 2007 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
使用 *nAngle* 指定颜色渐变的方向。 指定颜色渐变的方向时，还可以指定颜色渐变的起始位置。 如果 *nAngle* 的值为0，则表示渐变从矩形顶部开始。 随着 *nAngle* 的增加，渐变的起始位置会根据角度以逆时针方向移动。

### <a name="example"></a>示例

下面的示例演示如何使用类的 `FillGradient2` 方法 `CDrawingManager` 。 此代码片段是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
如果 *nPercentage* 介于0到99之间，则 `HighlightRect` 使用 alpha 混合算法。 有关 alpha 混合的详细信息，请参阅 [Alpha 混合线条和填充](/dotnet/framework/winforms/advanced/alpha-blending-lines-and-fills)。 如果 *nPercentage* 为-1，则此方法使用默认突出显示级别。 如果 *nPercentage* 为100，则此方法不执行任何操作并返回 TRUE。

方法使用参数 *nTolerance* 来确定是否突出显示矩形区域。 若要突出显示该矩形，应用程序和 *clrTransparent* 的背景色之间的差异必须小于每个颜色组件中的 *nTolerance* (红、绿和蓝) 。
...
颜色可以表示为 " (HSV"、"饱和度" 和 "值") ，"HSL (色调"、"饱和度"、"发光度) " 或 "RGB (红色、绿色和蓝色) "。 有关颜色不同表示形式的详细信息，请参阅 [颜色](/windows/win32/uxguide/vis-color)。

此方法和 `CDrawingManager::HLStoRGB_TWO` 方法执行相同的操作，但需要为 *H* 参数提供不同的值。 在此方法中， *H* 是圆圈的百分比。 在 `CDrawingManager::HLStoRGB_TWO` 方法中， *H* 是0到360之间的度数值，两者都表示红色。 例如，对于 `HLStoRGB_ONE` ，对于 *H* ，值为0.25 等效于值90，值为 `HLStoRGB_TWO` 。
...
此方法和 [CDrawingManager：： HLStoRGB_ONE](#hlstorgb_one) 方法执行相同的操作，但需要为 *H* 参数提供不同的值。 在此方法中， *H* 为介于0到360之间的度数值，这两个值都表示红色。 在 [CDrawingManager：： HLStoRGB_ONE](#hlstorgb_one) 方法中， *H* 是圆圈的百分比。 例如，对于 `HLStoRGB_ONE` ，对于 *H* ，值为0.25 等效于值90，值为 `HLStoRGB_TWO` 。
...
*高*\
中一个介于0到360之间的数字，用于指示颜色的色调。

*些*\
中一个介于0和1之间的数字，表示颜色的饱和度。

*向量*\
中一个介于0和1之间的数字，用于指示颜色的值。
...
此方法可以反转此类拥有的任何设备上下文区域 `CDrawingManager` 。 如果 *bHorz* 设置为 TRUE，则此方法水平翻转区域。 否则，它会垂直翻转区域。
...
如果将 *nDepth* 设置为0，则此方法将退出并返回 NULL。 如果 *nDepth* 小于3，则阴影的宽度和高度将设置为3个像素。

## <a name="cdrawingmanagerrgbtohsl"></a><a name="rgbtohsl"></a> CDrawingManager::RGBtoHSL

将红色、绿色和蓝色 (RGB) 表示形式的颜色转换为 "色调"、"饱和度" 和 "亮度" (HSL) 表示形式。
...
*高*\
弄一个指针，指向一种双精度型，其中方法存储颜色的色调。

*些*\
弄一个指针，指向一种双精度型，其中方法存储颜色的饱和度。

*L*\
弄一个指针，指向一种双精度型，其中方法存储颜色的亮度。
...
*dblLumRatio*\
中新颜色发光度的比率。 `SmartMixColors` 在确定最终颜色之前，将混合颜色的发光度与此比率相乘。
...
如果 *版 k1* 或 *k2* 小于零，则此方法将失败并出现错误。 如果这两个参数均设置为0，则该方法返回 `RGB(0, 0, 0)` 。

加权比率的计算公式如下： (color1 \* 版 k1 + color2 \* k2) / (版 k1 + k2) 。 确定了加权比率后，方法将计算混合颜色的发光度。 然后，它通过 *dblLumRatio* 将发光度相乘。 如果该值大于1.0，则方法将混合颜色的发光度设置为新值。 否则，发光度设置为1.0。
