---
description: 了解详细信息： CDocObjectServerItem 类
...
若要使用 `CDocObjectServerItem` ，请确保派生类中的 [OnGetEmbeddedItem](../../mfc/reference/coleserverdoc-class.md#ongetembeddeditem) 重写 `COleServerDoc` 返回一个新的 `CDocObjectServerItem` 对象。 如果需要更改项中的任何功能，则可以创建自己的派生类的新实例 `CDocObjectServerItem` 。
...
*bAutoDelete*<br/>
指示在释放对象的链接时是否可将其删除。 如果对象是文档数据的组成部分，则将参数设置为 FALSE `CDocObjectServerItem` 。 如果对象是一个辅助结构，则将其设置为 TRUE，该结构用于标识可由框架删除的文档数据中的范围。
