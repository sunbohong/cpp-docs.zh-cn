...
|[CMFCMenuButton：： System.windows.window.sizetocontent](#sizetocontent)|根据按钮的文本和图像大小更改按钮的大小。|
...
|[CMFCMenuButton：： m_bDefaultClick](#m_bdefaultclick)| 允许) 处理按钮文本/图像上的默认 (。|

## <a name="remarks"></a>备注

`CMFCMenuButton`该类派生自[CMFCButton 类](../../mfc/reference/cmfcbutton-class.md)，后者又派生自[CButton 类](../../mfc/reference/cbutton-class.md)。 因此，您可以使用与 `CMFCMenuButton` 使用相同的方法在代码中使用 `CButton` 。

创建时 `CMFCMenuButton` ，必须将句柄传入关联的弹出菜单。 接下来，调用函数 `CMFCMenuButton::SizeToContent` 。 `CMFCMenuButton::SizeToContent` 检查按钮大小是否足以包含一个箭头，该箭头指向弹出窗口将出现的位置（即，在按钮的下方或右侧）。
...
如果 `m_bOSMenu` 为 TRUE，则框架将 `TrackPopupMenu` 为此对象调用继承的方法。 否则，框架将调用 [CContextMenuManager：： TrackPopupMenu](../../mfc/reference/ccontextmenumanager-class.md#trackpopupmenu)。
...
如果 `m_bStayPressed` 成员为 FALSE，则当使用单击按钮时，菜单按钮不会被按下。 在这种情况下，框架只显示弹出菜单。

如果 `m_bStayPressed` 成员为 TRUE，则当用户单击按钮时，菜单按钮将变为按下状态。 在用户关闭弹出菜单之后，可以通过选择或取消来保持按下状态。
...
如果调用此函数并且 *bCalcOnly* 为 TRUE，则 `SizeToContent` 将只计算按钮的新大小。
