---
description: 了解详细信息： CDumpContext 类
...
`CDumpContext` **<<** 对于 `CObject` 转储对象数据的指针，类具有重载插入 () 运算符。 如果需要将自定义转储格式用于派生对象，请重写 [CObject：:D ump](../../mfc/reference/cobject-class.md#dump)。 大多数 Microsoft 基础类实现了一个重写的 `Dump` 成员函数。
