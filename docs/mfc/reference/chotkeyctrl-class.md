---
description: 了解详细信息： CHotKeyCtrl 类
title: CHotKeyCtrl 类
ms.date: 11/04/2016
f1_keywords:
- CHotKeyCtrl
...
|[CHotKeyCtrl：：指向](#setrules)|为热键控件定义无效的组合和默认修饰符组合。|

## <a name="remarks"></a>备注

"热键控件" 是使用户能够创建热键的窗口。 "热键" 是用户可以按其快速执行操作的组合键。  (例如，用户可以创建激活给定窗口的热键，并将其放到 Z 顺序的顶部。 ) 热键控件显示用户的选择，并确保用户选择有效的键组合。

此控件 (，因此 `CHotKeyCtrl` 类) 仅适用于在 windows 95/98 和 WINDOWS NT 版本3.51 及更高版本下运行的程序。

当用户选择了组合键后，应用程序可以从控件中检索指定的组合键，并使用 WM_SETHOTKEY 消息在系统中设置热键。 每当用户在系统的任何部分按下热键时，WM_SETHOTKEY 消息中指定的窗口都将接收指定 SC_HOTKEY 的 WM_SYSCOMMAND 消息。 此消息将激活接收它的窗口。 热键始终有效，直到调用 WM_SETHOTKEY 的应用程序退出。

此机制不同于基于 WM_HOTKEY 消息和 Windows [RegisterHotKey](/windows/win32/api/winuser/nf-winuser-registerhotkey) 和 [UnregisterHotKey](/windows/win32/api/winuser/nf-winuser-unregisterhotkey) 函数的热键支持。

有关使用的详细信息 `CHotKeyCtrl` ，请参阅 [控件](../../mfc/controls-mfc.md) 和 [使用 CHotKeyCtrl](../../mfc/using-chotkeyctrl.md)。
...
可以通过 `CHotKeyCtrl` 两个步骤构造对象。 首先，调用构造函数，然后调用 `Create` ，它将创建热键控件并将其附加到 `CHotKeyCtrl` 对象。
...
调用此函数可创建 (子窗口) 的控件，并将其与 `CHotKeyCtrl` 对象关联。
...
当前选定的热键的本地化名称。 如果没有选定的热键，则 `GetHotKeyName` 返回空字符串。
...
*Vk* 参数指定的密钥的本地化名称。 如果该键没有映射名称，则 `GetKeyName` 返回空字符串。
...
## <a name="chotkeyctrlsetrules"></a><a name="setrules"></a> CHotKeyCtrl：：指向
