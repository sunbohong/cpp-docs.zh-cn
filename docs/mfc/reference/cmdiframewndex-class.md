---
description: 了解详细信息： CMDIFrameWndEx 类
...
ms.locfileid: "97207665"
---
# <a name="cmdiframewndex-class"></a>CMDIFrameWndEx 类

扩展 [CMDIFrameWnd](../../mfc/reference/cframewnd-class.md)的功能，Windows 多文档界面 (MDI) 框架窗口中。
...
下面的示例从派生一个类 `CMDIFrameWndEx` 。 此代码段来自 [DrawClient 示例： MFC Ribbon-Based OLE 对象绘图应用程序](../../overview/visual-cpp-samples.md)。
...
在此示例中， `g_strStartViewName` 可以是 "虚拟文档" (的名称（例如，"起始页" ) ，实际上不是从磁盘文件加载的）。 因此，我们需要特殊处理来处理这种情况。
...
在全屏模式下，将隐藏所有停靠控件栏、工具栏和菜单，并调整活动视图的大小以占据全屏。启用全屏模式时，必须指定启用或禁用它的命令 ID。 可以 `EnableFullScreenMode` 从主框架的函数中调用 `OnCreate` 。 当框架窗口切换到全屏模式时，框架会创建一个浮动工具栏，其中包含一个具有指定命令 ID 的按钮。如果要在屏幕上保留主菜单，请调用 [CMDIFrameWndEx：： EnableFullScreenMainMenu](#enablefullscreenmainmenu)。
...
选项卡标签可以位于帧的顶部或底部，具体取决于参数 *tabLocation* 的设置。 可以 `CMFCTabCtrl::LOCATION_BOTTOM`) 或指定 (默认设置 `CMFCTabCtrl::LOCATION_TOP` 。
...
使用此方法来插入菜单项，其命令调用 MDI 子窗口管理对话框 ( [CMFCWindowsManagerDialog 类](../../mfc/reference/cmfcwindowsmanagerdialog-class.md)) 。 新项将插入到由 *uiMenuId* 指定的菜单中。 `EnableWindowsDialog`处理 WM_CREATE 消息时调用。
...
使用此函数创建新的选项卡式窗口， (新的选项卡式组) 并向其中添加第一个选项卡。
...
此标志可能会导致不需要的效果， (如) 可见的后台应用程序。 因此，建议您仅在 MDI 选项卡激活过程中出现明显闪烁时才更改默认值。
...
- AFX_MDI_CAN_BE_DOCKED 将选项卡式文档切换为固定状态 (仅) 的选项卡式文档。
...
如果 [CDockingManager：： m_bHideDockingBarsInContainerMode](../../mfc/reference/cdockingmanager-class.md#m_bhidedockingbarsincontainermode) 为 TRUE (默认) ，则所有停靠窗格都将隐藏。
...
*bAdjustLayout*<br/>
中若要立即调整停靠布局，则为 TRUE。 如果为 FALSE，则只会在发生重绘事件时进行调整，原因是用户调整窗口大小时， (用户调整窗口的大小、拖动主框架等 ) 。
