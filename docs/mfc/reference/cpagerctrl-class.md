...
|[CPagerCtrl：： IsButtonDepressed](#isbuttondepressed)|指示当前页导航控件的指定按钮是否处于 `pressed` 状态。|
|[CPagerCtrl：： IsButtonGrayed](#isbuttongrayed)|指示当前页导航控件的指定按钮是否处于 `grayed` 状态。|
|[CPagerCtrl：： IsButtonHot](#isbuttonhot)|指示当前页导航控件的指定按钮是否处于 `hot` 状态。|
|[CPagerCtrl：： IsButtonInvisible](#isbuttoninvisible)|指示当前页导航控件的指定按钮是否处于 `invisible` 状态。|
|[CPagerCtrl：： IsButtonNormal](#isbuttonnormal)|指示当前页导航控件的指定按钮是否处于 `normal` 状态。|
...
例如，如果您的应用程序所具有的工具栏宽度不足以显示其所有项，则可以将工具栏分配给页导航控件，用户将能够向左或向右滚动工具栏以访问所有项。 Microsoft Internet Explorer 版本 4.0 ( # A0 版本 4.71) 引入了页导航控件。

`CPagerCtrl`类从[CWnd](../../mfc/reference/cwnd-class.md)类派生。 有关页导航控件的详细信息和说明，请参阅 [页导航控件](/windows/win32/Controls/pager-controls)。
...
使用 [CPagerCtrl：： create](#create) 或 [CPagerCtrl：： CreateEx](#createex) 方法创建页导航控件，并将其附加到 `CPagerCtrl` 对象。
...
*rect*\
中对 [矩形](/windows/win32/api/windef/ns-windef-rect) 结构的引用，该结构包含控件在工作区坐标中的位置和大小。

*pParentWnd*\
中指向 [CWnd](../../mfc/reference/cwnd-class.md) 对象的指针，该对象是控件的父窗口。 此参数不能为 NULL。
...
若要创建页导航控件，请声明一个 `CPagerCtrl` 变量，然后在该变量上调用 [CPagerCtrl：： Create](#create) 或 [CPagerCtrl：： CreateEx](#createex) 方法。
...
此方法发送 Windows SDK 中描述的 [PGM_FORWARDMOUSE](/windows/win32/Controls/pgm-forwardmouse) 消息。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBORDER](/windows/win32/Controls/pgm-getborder) 消息。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBKCOLOR](/windows/win32/Controls/pgm-getbkcolor) 消息。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBUTTONSIZE](/windows/win32/Controls/pgm-getbuttonsize) 消息。

如果页导航控件具有 PGS_HORZ 样式，则按钮大小确定页导航按钮的宽度，如果页导航控件具有 PGS_VERT 样式，则按钮大小决定页导航按钮的高度。 有关详细信息，请参阅 [页导航控件样式](/windows/win32/Controls/pager-control-styles)。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息。
...
指向 `IDropTarget` 当前页导航控件的接口的指针。

### <a name="remarks"></a>备注

`IDropTarget` 是为了支持应用程序中的拖放操作而实现的接口之一。

此方法发送 Windows SDK 中描述的 [PGM_GETDROPTARGET](/windows/win32/Controls/pgm-getdroptarget) 消息。 此方法的调用方负责 `Release` 在不再需要接口时调用 [IDropTarget](/windows/win32/api/oleidl/nn-oleidl-idroptarget) 接口的成员。
...
此方法发送 Windows SDK 中描述的 [PGM_GETPOS](/windows/win32/Controls/pgm-getpos) 消息。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息。 然后测试返回的状态是否 PGF_DEPRESSED。 有关详细信息，请参阅 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息的 "返回值" 部分。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息。 然后测试返回的状态是否 PGF_GRAYED。 有关详细信息，请参阅 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息的 "返回值" 部分。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息。 然后测试返回的状态是否 PGF_HOT。 有关详细信息，请参阅 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息的 "返回值" 部分。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息。 然后测试返回的状态是否 PGF_INVISIBLE。 有关详细信息，请参阅 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息的 "返回值" 部分。
...
此方法发送 Windows SDK 中描述的 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息。 然后测试返回的状态是否 PGF_NORMAL。 有关详细信息，请参阅 [PGM_GETBUTTONSTATE](/windows/win32/Controls/pgm-getbuttonstate) 消息的 "返回值" 部分。
...
此方法发送 Windows SDK 中描述的 [PGM_RECALCSIZE](/windows/win32/Controls/pgm-recalcsize) 消息。 因此，寻呼控件将发送 [PGN_CALCSIZE](/windows/win32/Controls/pgn-calcsize) 通知，以获得所包含窗口的可滚动维度。
...
此方法发送 Windows SDK 中描述的 [PGM_SETBKCOLOR](/windows/win32/Controls/pgm-setbkcolor) 消息。
...
此方法发送 Windows SDK 中描述的 [PGM_SETBORDER](/windows/win32/Controls/pgm-setborder) 消息。
...
此方法发送 Windows SDK 中描述的 [PGM_SETBUTTONSIZE](/windows/win32/Controls/pgm-setpos) 消息。

如果页导航控件具有 PGS_HORZ 样式，则按钮大小确定页导航按钮的宽度，如果页导航控件具有 PGS_VERT 样式，则按钮大小决定页导航按钮的高度。 默认按钮大小为滚动条宽度的三四分之三，最小按钮大小为12像素。 有关详细信息，请参阅 [页导航控件样式](/windows/win32/Controls/pager-control-styles)。
...
此方法发送 Windows SDK 中描述的 [PGM_SETCHILD](/windows/win32/Controls/pgm-setchild) 消息。
