---
description: 了解详细信息： CConnectionPoint 类
...
默认情况下， `COleControl` 派生类实现两个连接点：一个用于事件，另一个用于属性更改通知。 这些连接分别用于事件激发和通知接收器 (例如，当属性值发生更改时，控件的容器) 。 还为 OLE 控件提供了支持以实现其他连接点。 对于在控件类中实现的每个附加连接点，必须声明实现连接点的 "连接部分"。 如果实现一个或多个连接点，则还需要在控件类中声明单个 "连接映射"。
...
BEGIN_CONNECTION_PART 和 END_CONNECTION_PART 宏声明嵌入类， `XSampleConnPt` (派生自 `CConnectionPoint` 实现该特定连接点的) 。 如果要重写任何 `CConnectionPoint` 成员函数或添加自己的成员函数，请在这两个宏之间进行声明。 例如， `CConnectionPoint::GetIID` 当在两个宏之间放置成员函数时，CONNECTION_IID 宏会重写该成员函数。

第二个代码段插入 ( 的实现文件中。控件类的 CPP) 。 此代码实现连接映射，其中包括附加连接点 `SampleConnPt` ：
...
指向 (接收器) 的活动连接的数组的指针。 数组中的某些指针可能为 NULL。 使用强制转换运算符，可以安全地将此数组中的每个非 NULL 指针转换为指向接收器接口的指针。
