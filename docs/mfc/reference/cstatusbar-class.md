---
description: 了解详细信息： CStatusBar 类
title: CStatusBar 类
ms.date: 11/04/2016
f1_keywords:
- CStatusBar
...
|[CStatusBar：： CreateEx](#createex)|`CStatusBar`使用嵌入对象的其他样式创建对象 `CStatusBarCtrl` 。|
|[CStatusBar：:D rawItem](#drawitem)|当所有者描述的状态栏控件的可视方位更改时调用。|
...
使用[CStatusBar：： GetStatusBarCtrl](#getstatusbarctrl)（MFC 4.0 的一种新成员函数），可以利用 Windows 公共控件对状态栏自定义和其他功能的支持。 `CStatusBar` 成员函数为你带来了 Windows 公共控件的大多数功能;但是，当你调用时 `GetStatusBarCtrl` ，你可以为你的状态栏指定 Windows 95/98 状态栏的更多特性。 调用时 `GetStatusBarCtrl` ，它将返回对对象的引用 `CStatusBarCtrl` 。 有关使用 Windows 公共控件设计工具栏的详细信息，请参阅 [CStatusBarCtrl](../../mfc/reference/cstatusbarctrl-class.md) 。 有关公共控件的更多常规信息，请参阅 Windows SDK 中的 [公共控件](/windows/win32/Controls/common-controls-intro) 。
...
1. 调用 [create](#create) (或 [CreateEx](#createex)) 函数以创建状态栏窗口，并将其附加到 `CStatusBar` 对象。
...
有关使用的详细信息 `CStatusBar` ，请参阅文章 " [MFC 中](../../mfc/status-bar-implementation-in-mfc.md) 的状态栏实现" 和 " [技术说明31：控件条](../../mfc/tn031-control-bars.md)"。
...
## <a name="cstatusbarcreate"></a><a name="create"></a> CStatusBar：： Create
 ...
 (子窗口) 创建一个状态栏，并将其与对象关联 `CStatusBar` 。
...
调用此函数可创建一个状态栏 (子窗口) 并将其与 `CStatusBar` 对象关联。
...
- SBARS_SIZEGRIP 状态栏控件的状态栏的右端包含大小调整手柄。 大小调整手柄类似于大小调整边框；它是用户可以通过单击和拖动来重设父窗口大小的矩形区域。
...
`CreateEx`如果在创建嵌入的状态栏控件期间需要提供某些样式，请使用，而不是[创建](#create)。 例如，将 *dwCtrlStyle* 设置为 SBT_TOOLTIPS，以在状态栏对象中显示工具提示。
...
`itemAction`结构的成员 `DRAWITEMSTRUCT` 定义要执行的绘图操作。 重写此成员函数以实现所有者描述对象的绘制 `CStatusBar` 。 应用程序应还原在此成员函数终止之前为 *lpDrawItemStruct* 中提供的显示上下文选择的所有图形设备接口 (GDI) 对象。
...
*lpRect*<br/>
指向 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构或 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象，该对象将接收由 *nIndex* 指定的指示器的坐标。
...
- 不 SBPS_NOBORDERS 窗格周围的三维边框。

- SBPS_POPOUT 反转边框，使文本 "弹出"。

- SBPS_DISABLED 不绘制文本。

- SBPS_STRETCH 拉伸窗格来填充未使用的空间。 每个状态栏只有一个窗格可以具有此样式。
...
调用后 `SetPaneText` ，您必须添加一个 UI 更新处理程序，以便在状态栏中显示新文本。
