---
description: 了解详细信息： COlePasteSpecialDialog 类
...
如果要调用此对话框，请创建类的对象 `COlePasteSpecialDialog` 。 `COlePasteSpecialDialog`构造对象后，可以使用[AddFormat](#addformat)和[AddStandardFormats](#addstandardformats)成员函数将剪贴板格式添加到对话框中。 你还可以使用 [m_ps](#m_ps) 结构在对话框中初始化控件的值或状态。 `m_ps`结构的类型为 OLEUIPASTESPECIAL。
...
如果要通过设置 [m_ps](#m_ps) 结构的成员来初始化各种对话框控件，应在调用之前执行此操作 `DoModal` ，但在构造对话框对象之后。
...
用户选择的结构数组中的索引 `OLEUIPASTEENTRY` 。 执行粘贴操作时，应使用与所选索引对应的格式。
...
返回类型值由 `Selection` 类中声明的枚举类型指定 `COlePasteSpecialDialog` 。
