---
description: 了解详细信息： COleChangeSourceDialog 类
...
|“属性”|描述|
|----------|-----------------|
|[COleChangeSourceDialog：:D oModal](#domodal)|显示 "OLE 更改源" 对话框。|
...
如果要调用此对话框，请创建类的对象 `COleChangeSourceDialog` 。 `COleChangeSourceDialog`构造对象后，可以使用[m_cs](#m_cs)结构在对话框中初始化控件的值或状态。 `m_cs`结构的类型为[OLEUICHANGESOURCE](/windows/win32/api/oledlg/ns-oledlg-oleuichangesourcew)。 有关使用此对话框类的详细信息，请参阅 [DoModal](#domodal) 成员函数。
...
如果要通过设置 [m_cs](#m_cs) 结构的成员来初始化各种对话框控件，应在调用之前执行此操作 `DoModal` ，但在构造对话框对象之后。
