...
|[CPane：:D ockPaneStandard](#dockpanestandard)|使用大纲 (标准) 停靠停靠窗格。|
...
|[CPane：： IsLeftOf](#isleftof)|确定窗格是 (还是) 指定矩形的上方。|
...
如果无法创建)  ( [CPaneDivider 类](../../mfc/reference/cpanedivider-class.md) 的窗格分隔线，则为 FALSE; 否则为 FALSE。否则为 TRUE。
...
可用拉伸大小的计算方法是：从当前大小 ( [CWnd：： GetWindowRect](../../mfc/reference/cwnd-class.md#getwindowrect)) 中减去窗格的最小允许大小 ( [CPane：： GetMinSize](#getminsize)) 。
...
返回停靠在窗格 ( [类](../../mfc/reference/cdockingpanesrow-class.md)) 的停靠行。
...
一个 `CSize` 对象，该对象包含新矩形和旧矩形之间的宽度和高度差异 (旧矩形- *rectNew*) 。
...
如果该窗格是浮动窗格，则此方法会通知父袖珍框架，以将窗格大小调整到微型框架的大小。 此框架可确保最小框架的最小允许大小 ( [CPane：： GetMinSize](#getminsize)) ，并在必要时调整微型框架的大小。
...
当窗格处于浮动时，会将其放在 [CPaneFrameWnd](../../mfc/reference/cpaneframewnd-class.md) (微型帧) 窗口中。 在 `CPaneFrameWnd` 调用 [CPane：： CreateDefaultMiniframe](#createdefaultminiframe) 时，可以提供将使用的自定义派生类。
