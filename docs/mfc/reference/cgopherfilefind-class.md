---
description: 了解详细信息： CGopherFileFind 类
...
用于搜索 Internet 和本地文件的其他 MFC 类包括 [CFtpFileFind](../../mfc/reference/cftpfilefind-class.md) 和 [CFileFind](../../mfc/reference/cfilefind-class.md)。 与不同的是 `CGopherFileFind` ，这些类为用户提供了一个无缝机制来查找特定文件，无论服务器协议、文件类型或位置 (本地计算机还是远程服务器。 ) 请注意，不存在用于在 http 服务器上搜索的 MFC 类，因为 http 不支持搜索所需的直接文件操作。
...
此外，当与一起使用时 `CGopherFileFind` ， `CFileFind` 成员函数 [ISDOTS](../../mfc/reference/cfilefind-class.md#isdots) 始终为 FALSE。
...
*DwContext* 的默认值由 MFC 发送到 `CGopherFileFind` 创建该对象的 [CInternetSession](../../mfc/reference/cinternetsession-class.md)对象中的对象 `CGopherFileFind` 。 构造 `CGopherFileFind` 对象时，可以重写默认值，将上下文标识符设置为你选择的值。 上下文标识符返回到 [CInternetSession：： OnStatusCallback](../../mfc/reference/cinternetsession-class.md#onstatuscallback) ，以在标识它的对象上提供状态。 有关上下文标识符的详细信息，请参阅文章 [Internet 优先步骤： WinInet](../../mfc/wininet-basics.md) 。
...
- INTERNET_FLAG_USE_EXISTING 如果可能，请将现有连接重复用于新 `FindFile` 请求，而不是为每个请求创建一个新会话。
...
如果成功，则为非零值;如果不成功，则为0。 `GetCreationTime` 仅当从未在此对象上调用 [FindNextFile](#findnextfile) 时返回 0 `CGopherFileFind` 。
...
如果成功，则为非零值;如果不成功，则为0。 `GetLastAccessTime` 仅当从未在此对象上调用 [FindNextFile](#findnextfile) 时返回 0 `CGopherFileFind` 。
...
如果成功，则为非零值;如果不成功，则为0。 `GetLastWriteTime` 仅当从未在此对象上调用 [FindNextFile](#findnextfile) 时返回 0 `CGopherFileFind` 。
...
### <a name="example"></a>示例
  ...
  请参阅 [CFile：： GetLength](../../mfc/reference/cfile-class.md#getlength) 的示例 (基类实现) 。
