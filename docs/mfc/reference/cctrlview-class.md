---
description: 了解详细信息： CCtrlView 类
title: CCtrlView 类
ms.date: 11/04/2016
f1_keywords:
- CCtrlView
...
|[CCtrlView：:P reCreateWindow](#precreatewindow)|在创建附加到此 `CCtrlView` 对象的 Windows 窗口之前调用。|
...
类 `CCtrlView` 及其派生项、 [CEditView](../../mfc/reference/ceditview-class.md)、 [CListView](../../mfc/reference/clistview-class.md)、 [CTreeView](../../mfc/reference/ctreeview-class.md)和 [CRichEditView](../../mfc/reference/cricheditview-class.md)，将文档视图结构调整为 Windows 95/98 和 windows NT 版本3.51 及更高版本支持的新公共控件。 有关文档视图体系结构的详细信息，请参阅 [文档/视图体系结构](../../mfc/document-view-architecture.md)。
...
由框架调用，以 `CCtrlView` 使用指定的设备上下文绘制对象的内容。
...
派生自的每个类都 `CCtrlView` 将其自己的功能添加到其重写 `PreCreateWindow` 。 按照设计， `PreCreateWindow` 不会记录这些派生的。 若要确定适用于每个类和样式之间相互依赖关系的样式，可以检查应用程序的基类的 MFC 源代码。 如果选择重写 `PreCreateWindow` ，则可以通过使用从 MFC 源代码收集的信息来确定应用程序的基类中使用的样式是否提供所需的功能。
