---
description: 了解详细信息： CMFCVisualManager 类
...
|[CMFCVisualManager：： GetInstance](#getinstance)|返回一个指向对象的指针 `CMFCVisualManager` 。|
...
|[CMFCVisualManager：： OnDrawBrowseButton](#ondrawbrowsebutton)|当框架绘制属于编辑控件的浏览按钮时，由框架调用 ( [CMFCEditBrowseCtrl 类](../../mfc/reference/cmfceditbrowsectrl-class.md)) 。|
...
|[CMFCVisualManager：： OnDrawPaneCaption](#ondrawpanecaption)|当它绘制的标题时由框架调用 `CPane` 。|
...
|[CMFCVisualManager：： OnDrawRibbonSliderThumb](#ondrawribbonsliderthumb)|当框架绘制对象的滚动块时由框架调用 `CMFCRibbonSlider` 。|
|[CMFCVisualManager：： OnDrawRibbonSliderZoomButton](#ondrawribbonsliderzoombutton)|当框架在绘制对象的缩放按钮时由框架调用 `CMFCRibbonSlider` 。|
...
|[CMFCVisualManager：： OnDrawTabContent](#ondrawtabcontent)|当框架在绘制选项卡内部 (图像、文本) 时由框架调用。|
...
|[CMFCVisualManager：： OnFillTasksPaneBackground](#onfilltaskspanebackground)|当框架填充控件的背景时由框架调用 `CMFCTasksPane` 。|
...
由于 `CMFCVisualManager` 类控制应用程序的 GUI，因此每个应用程序可以有一个实例 `CMFCVisualManager` ，或者一个派生自的类的一个实例 `CMFCVisualManager` 。 应用程序也可以在不使用的情况下运行 `CMFCVisualManager` 。 使用静态方法 `GetInstance` 获取指向当前 `CMFCVisualManager` 派生对象的指针。

若要更改应用程序的外观，必须使用其他类，这些类提供用于绘制应用程序的所有可视元素的方法。 这些类的一些示例包括 [CMFCVisualManagerOfficeXP 类](../../mfc/reference/cmfcvisualmanagerofficexp-class.md)、 [CMFCVisualManagerOffice2003 类](../../mfc/reference/cmfcvisualmanageroffice2003-class.md)和 [CMFCVisualManagerOffice2007 类](../../mfc/reference/cmfcvisualmanageroffice2007-class.md)。 若要更改应用程序的外观，请将这些视觉对象管理器之一传递到方法中 `SetDefaultManager` 。 有关演示应用程序如何模拟 Microsoft Office 2003 外观的示例，请参阅 [CMFCVisualManagerOffice2003 类](../../mfc/reference/cmfcvisualmanageroffice2003-class.md)。
...
下面的示例演示如何检索对象的默认值 `CMFCVisualManager` 。 此代码段是 "任务" [窗格示例](../../overview/visual-cpp-samples.md)的一部分。
...
一个应用程序只能有一个 `CMFCVisualManager` 与其关联的对象。 这包括从类派生的任何对象 `CMFCVisualManager` 。 此方法返回指向关联对象的指针 `CMFCVisualManager` 。 如果应用程序没有关联的 `CMFCVisualManager` 对象，则此方法将创建一个对象并将其与应用程序相关联。
...
*bSmall*<br/>
中布尔型参数，指示是否 `GetNcBtnSize` 应检索小系统或大系统按钮的大小。 如果 *bSmall* 为 TRUE，则 `GetNcBtnSize` 返回小系统按钮的大小。 否则，它将返回大系统按钮的大小。
...
此方法的默认实现显示标准控制手柄。 若要自定义控制手柄的外观，请在派生自类的自定义类中重写此方法 `CMFCVisualManager` 。
...
*pBar*<br/>
中指向对象的指针 `CMFCCaptionBar` 。 框架将绘制此标题栏。
...
*pButton*<br/>
中指向对象的指针 `CMFCCaptionButton` 。 框架将绘制此标题按钮。
...
此方法的默认实现显示来自类的全局实例的小按钮 `CMenuImages` 。 这些按钮在的标头文件中列出 `CMenuImages` 。 一些示例包括 `CMenuImages::IdClose` 、、、 `CMenuImages::IdArowLeft` `CMenuImages::IdArowRight` `CMenuImages::IdArowDown` 、 `CMenuImages::IdArowUp` 和 `CMenuImages::IdPinHorz` 。
...
*pButton*<br/>
中指向对象的指针 `CMFCToolBarComboBoxButton` 。 框架将为此组合框按钮绘制下拉按钮。
...
*rect*<br/>
中指定对象边界的矩形 `CMFCToolBarEditBoxButton` 。
...
*pButton*<br/>
中指向对象的指针 `CMFCToolBarEditBoxButton` 。 框架绘制此编辑框按钮的边框。

### <a name="remarks"></a>备注

在派生的可视化管理器中重写此方法，以自定义对象的边框 `CMFCToolBarEditBoxButton` 。
...
边框的大小由 *rectBorderSize* 中的相应参数指定。 例如，工具栏的上边框宽度由指定 `rectBorderSize.top` 。
...
*pCtrl*<br/>
中指向对象的指针 `CMFCHeaderCtrl` 。 框架绘制此标头控件的边框。
...
如果 *bIsUp* 为 TRUE，则视觉对象管理器绘制一个向上排序箭头。 如果该参数为 FALSE，则视觉对象管理器绘制向下排序箭头。 `OnDrawHeaderCtrlSortArrow`在派生类中重写以自定义排序按钮的外观。
...
*pDC*<br/>
中指向对象的设备上下文的指针 `CMFCPopupMenu` 。

*pMenu*<br/>
中指向对象的指针 `CMFCPopupMenu` 。 框架在此弹出菜单周围绘制边框。
...
*pBar*<br/>
中指向对象的指针 `CDockablePane` 。 框架将为此窗格绘制标题。
...
在派生的可视化管理器中重写此方法，以自定义的外观 `CMFCRibbonButton` 。
...
*pButton*<br/>
指向类的指针 `CMFCRibbonCaptionButton` 。 框架将绘制此标题按钮。
...
*pCategory*<br/>
中指向对象的指针 `CMFCRibbonCategory` 。 框架将绘制此功能区类别。
...
*pTab*<br/>
中指向类的实例的指针 `CMFCRibbonTab` 。 框架将绘制此选项卡。
...
*pButton*<br/>
中指向 `CMFCRibbonButton` 位于功能区主面板上的的指针。 框架将为此按钮绘制边框。

### <a name="remarks"></a>备注

在派生的可视化管理器中重写此方法，以自定义 `CMFCRibbonButton` **主** 面板上的边框的外观。
...
*pPanel*<br/>
中指向的指针 `CMFCRibbonMainPanel` 。

*rect*<br/>
中指定的边界的矩形 `CMFCRibbonMainPanel` 。

### <a name="remarks"></a>备注

在派生的可视化管理器中重写此方法，以自定义框架的外观 `CMFCRibbonMainPanel` 。
...
*pPanel*<br/>
中指向对象的指针 `CMFCRibbonPanel` 。 框架绘制此功能区面板。
...
*pPanel*<br/>
中指向对象的指针 `CMFCRibbonPanel` 。 框架将绘制此功能区面板的标题。
...
*pProgress*<br/>
中指向对象的指针 `CMFCRibbonProgressBar` 。 框架将绘制此进度栏。
...
*pSlider*<br/>
中指向的指针 `CMFCRibbonSlider` 。 框架绘制此功能区滑块的滚动块。
...
在派生的可视化管理器中重写此方法，以自定义的滚动块的外观 `CMFCRibbonSlider` 。
...
*pSlider*<br/>
中指向对象的指针 `CMFCRibbonSlider` 。 框架绘制此功能区滑块。
...
*pBar*<br/>
中指向对象的指针 `CMFCStatusBar` 。 框架将绘制此状态栏对象。
...
在派生的可视化管理器中重写此方法，以自定义对象的外观 `CMFCStatusBar` 。
...
在派生的可视化管理器中重写此方法，以自定义上大小框的外观 `CMFCStatusBar` 。
...
*pTabWnd*<br/>
中指向对象的指针 `CMFCBaseTabCtrl` 。 框架绘制此选项卡控件。
...
*pButton*<br/>
中指向 [CMFCButton](../../mfc/reference/cmfcbutton-class.md) 对象的指针。 框架将为此实例绘制边框 `CMFCButton` 。
...
*pGroup*<br/>
中指向对象的指针 `CMFCTasksPaneTaskGroup` 。 框架将为此组绘制标题。
...
当对象处理 WM_PAINT 消息时，框架会调用此函数 `CMFCPopupMenu` ，并且必须绘制一个撕上的标题。
...
*pTabWnd*<br/>
中指向选项卡窗口的指针。 框架将为此删除一个帧 `CMFCBaseTabCtrl` 。
...
*pBar*<br/>
中指向对象的指针 `CBasePane` 。 框架将填充此窗格的背景。
...
此方法的默认实现用全局变量的3d 背景色填充条形的背景 `afxGlobalData` 。 在派生的可视化管理器中重写此方法，以自定义窗格的背景。

### <a name="example"></a>示例

下面的示例演示如何 `OnFillBarBackground` 在类中使用 `CMFCVisualManager` 。 此代码片段是 [Outlook 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
当框架填充类的实例的内部时，框架会调用此方法 `CMFCRibbonRichEditCtrl` 。
...
*pEdit*<br/>
中指向对象的指针 `CMFCRibbonRichEditCtrl` 。 框架填充此编辑控件的内部。
...
`CMFCRibbonRichEditCtrl` *PEdit* 指示的可以是功能区中组合框按钮的一部分。

在派生的可视化管理器中重写此方法，以自定义的外观 `CMFCRibbonRichEditCtrl` 。
...
在派生的可视化管理器中重写此方法，以自定义菜单栏的外观 `CMFCRibbonMainPanel` 。
...
在派生的可视化管理器中重写此方法，以自定义对象的外观 `CMFCTasksPane` 。
