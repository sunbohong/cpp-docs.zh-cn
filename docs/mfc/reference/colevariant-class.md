---
description: 了解详细信息： COleVariant 类
title: COleVariant 类
ms.date: 11/04/2016
f1_keywords:
- COleVariant
...
两个相关 MFC 类 [COleCurrency](../../mfc/reference/colecurrency-class.md) 和 [COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md) 将 variant 数据类型货币封装 ( `VT_CY`) 和日期 ( `VT_DATE`) 。 `COleVariant`类在 DAO 类中广泛使用; 有关此类的典型用法，请参阅这些类，例如[CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md)和[CDaoRecordset](../../mfc/reference/cdaorecordset-class.md)。
...
- **COleVariant (** *nSrc* **，** *vtSrc* **)** 将16位整数 (或布尔值) 复制到新的对象中。 参数 *vtSrc* 必须 VT_I2 或 VT_BOOL。

- **COleVariant (** *lSrc* **，** *vtSrc* **)** 将32位整数 (或 SCODE 值) 复制到新的对象中。 参数 *vtSrc* 必须为 VT_I4、VT_ERROR 或 VT_BOOL。
...
- **COleVariant (** *arrSrc* **)** 将 `CByteArray` 对象复制到 VT_EMPTY 的新对象中。

- **COleVariant (** *lbSrc* **)** 将 `CLongBinary` 对象复制到 VT_EMPTY 的新对象中。
