---
description: 了解详细信息： COlePropertyPage 类
...
|[COlePropertyPage：： GetPageSite](#getpagesite)|返回指向属性页接口的指针 `IPropertyPageSite` 。|
...
实现的子类时 `COlePropertyPage` ，子类的构造函数应使用 `COlePropertyPage` 构造函数来标识属性页所基于的对话框模板资源以及包含其标题的字符串资源。
...
指向 `IDispatch` 用于访问属性页上每个控件的属性的指针数组的指针。 调用方不能释放这些接口指针。

### <a name="remarks"></a>备注

每个属性页对象维护一个指针数组，这些指针指向 `IDispatch` 该页正在编辑的对象的接口。 此函数将其 *pnObjects* 参数设置为该数组中的元素数，并返回一个指向数组中第一个元素的指针。

## <a name="colepropertypagegetpagesite"></a><a name="getpagesite"></a> COlePropertyPage：： GetPageSite

获取指向属性页接口的指针 `IPropertyPageSite` 。
...
指向属性页的接口的指针 `IPropertyPageSite` 。

### <a name="remarks"></a>备注

协作控件和容器，使用户可以浏览和编辑控件属性。 控件提供了属性页，其中每个都是允许用户编辑一组相关属性的 OLE 对象。 容器提供显示属性页的属性框架。 对于每个页面，属性框架都提供一个支持接口的页面站点 `IPropertyPageSite` 。
...
如果在初始化对话框时需要进行任何特殊操作，请重写此方法。 默认实现调用 `CDialog::OnInitDialog` 并返回 FALSE。
