---
description: 了解详细信息： CTreeCtrl 类
...
|[CTreeCtrl：:D eleteAllItems](#deleteallitems)|删除树视图控件中的所有项。|
|[CTreeCtrl：:D eleteItem](#deleteitem)|删除树视图控件中的新项。|
...
|[CTreeCtrl：： System.windows.media.visualtreehelper.hittest](#hittest)|返回与对象相关的光标当前位置 `CTreeCtrl` 。|
...
|[CTreeCtrl：： SetIndent](#setindent)|设置树视图项与其父级) 的偏移量 (（以像素为单位）。|
...
可以通过 `CTreeCtrl` 两个步骤构造。 首先调用构造函数，然后调用 `Create` ，它将创建树视图控件并将其附加到 `CTreeCtrl` 对象。
...
调用此函数可创建 (子窗口) 的控件，并将其与 `CTreeCtrl` 对象关联。
...
*fCancelWithoutSave*\
中如果为 TRUE，则在结束编辑操作之前放弃对树视图项所做的更改; 如果为 FALSE，则在结束操作之前保存对树视图项所做的更改。
...
*nPart*\
中部件的标识符。 必须设置为 TVGIPR_BUTTON。

*lpRect*\
弄指向 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构的指针。 如果此方法成功，则结构将接收由 *hItem* 和 *nPart* 指定的部分的矩形坐标。
...
下面的代码示例定义了一个 `m_treeCtrl` 用于访问当前树视图控件的变量。 此代码示例还定义了一个无符号整数和多个 HTREEITEM 变量。 下面的示例中使用了这些变量。
...
*lpRect*<br/>
指向接收边框的 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构的指针。 坐标相对于树视图控件的左上角。
...
*nStateMask*<br/>
指示要检索的一个或多个状态的掩码。 有关 *nStateMask* 的可能值的详细信息，请参阅 `state` `stateMask` Windows SDK 中的 [TVITEM](/windows/win32/api/commctrl/ns-commctrl-tvitemw) 结构的和成员的讨论。
...
下面的代码示例定义了一个 `m_treeCtrl` 用于访问当前树视图控件的变量。 此代码示例还定义了一个无符号整数和多个 HTREEITEM 变量。 下一个示例中使用了这些变量中的一个或多个。
...
有关 `GetNextItem` 在循环中使用的示例，请参阅 [CTreeCtrl：:D eleteitem](#deleteitem)。
...
*pFlags*<br/>
指向一个整数的指针，该整数接收有关命中测试结果的信息。 它可以是 `flags` "备注" 部分中成员下面列出的一个或多个值。
...
调用此函数时， *pt* 参数指定要测试的点的坐标。 该函数返回位于指定点处的项的句柄; 如果没有项，则返回 NULL。 此外， *pFlags* 参数包含一个值，该值指示指定点的位置。 可能的值为：
...
|TVHT_ONITEMLABEL|在标签上 (与项关联) 字符串。|
...
辅助功能辅助工具是帮助残障人士使用计算机的应用程序。 接口使用可访问性标识符 `IAccessible` 来唯一指定窗口中的元素。 有关这些 api 的详细信息 `IAccessible` ，请参阅 [Microsoft Active Accessibility](/windows/win32/winauto/microsoft-active-accessibility)。
...
*hItem*\
中控件中的树视图项的句柄。 有关详细信息，请参阅 [TVITEMEX](/windows/win32/api/commctrl/ns-commctrl-tvitemexw)结构的 *hItem* 成员。
...
*dwExMask*\
中一个位掩码，指定当前树视图控件中的哪些样式受此方法的影响。 如果此参数为零，则将其忽略，并将 *dwExStyles* 参数的值分配给树视图控件。 指定零个或按位组合 (或在 [树视图控件扩展样式](/windows/win32/Controls/tree-view-control-window-extended-styles)中描述的样式) 。

*dwExStyles*\
中一个位掩码，指定当前树视图控件中要设置或清除的样式。 若要设置样式组合，请指定 " [树-视图控件扩展样式](/windows/win32/Controls/tree-view-control-window-extended-styles)" 中所述 (或样式) 的按位组合。 若要清除一组样式，请指定0。
...
*hItem*<br/>
要设置其属性的项的句柄。 查看 `hItem` `TVITEM` Windows SDK 中结构的成员。

*nMask*<br/>
整数，用于指定要设置的特性。 查看 `mask` 结构的成员 `TVITEM` 。

*lpszItem*<br/>
包含该项文本的字符串的地址。

*n*<br/>
树视图控件的图像列表中项的图像的索引。 查看 `iImage` 结构的成员 `TVITEM` 。

*nSelectedImage*<br/>
树视图控件的图像列表中项的选定图像的索引。 查看 `iSelectedImage` 结构的成员 `TVITEM` 。

*nState*<br/>
指定项的状态值。 查看 `State` 结构的成员 `TVITEM` 。

*nStateMask*<br/>
指定要设置的状态。 查看 `stateMask` 结构的成员 `TVITEM` 。
...
如果 `mask` 成员或 *nMask* 参数指定了 TVIF_TEXT 值，则该 `pszText` 成员或 *lpszItem* 为以 null 结尾的字符串的地址，并且该成员 `cchTextMax` 将被忽略。 如果 `mask` (或 *NMask*) 指定 TVIF_STATE 值，则 `stateMask` 成员或 *nStateMask* 参数指定要更改的项目状态，并且 `state` member 或 *nState* 参数包含这些状态的值。
...
*uStateEx*\
中项的扩展状态。 有关详细信息，请参阅 `uStateEx` [TVITEMEX](/windows/win32/api/commctrl/ns-commctrl-tvitemexw) 结构的成员。
...
有关 tooltip 和信息提示之间的区别的详细信息，请参阅 [tooltip And 信息提示](/windows/win32/uxguide/ctrl-tooltips-and-infotips)。
...
*LParam1* 和 *lParam2* 参数对应于 `lParam` 所比较的两个项的 [TVITEM](/windows/win32/api/commctrl/ns-commctrl-tvitemw)结构成员。 *LParamSort* 参数与结构的成员相对应 `lParam` `TV_SORTCB` 。
