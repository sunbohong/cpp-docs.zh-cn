---
description: 了解详细信息： AFX_GLOBAL_DATA 结构
...
主题定义应用程序的视觉样式。 如果 *hTheme* 参数为 NULL，或者不支持 [DrawThemeTextEx](/windows/win32/api/uxtheme/nf-uxtheme-drawthemetextex) 方法，或者 [桌面窗口管理器](/windows/win32/dwm/dwm-overview) 如果 (DWM) 组合处于禁用状态，则不使用主题来绘制文本。
...
使用下表中的符号对指定缓冲区中的一组转义符进行编码。 将 *bIsCharsList* 参数指定为 TRUE 可将 *strTag* 参数中的符号转换为实际转义字符。 下表使用 [_T ( # B1 ](../../c-runtime-library/data-type-mappings.md) 宏来指定符号和转义字符串。
...
dwFlags <br/>
中按位组合 (或指定要使用的方法参数) 标志。 指定 LWA_COLORKEY 使用 *crKey* 参数作为透明度颜色。 指定 LWA_ALPHA 使用 *bAlpha* 参数确定分层窗口的不透明度。
...
`ITaskbarList`如果创建任务条形列表对象成功，则为指向接口的指针;如果创建失败或当前操作系统小于 Windows 7，则为 NULL。
...
`ITaskbarList3`如果创建任务条形列表对象成功，则为指向接口的指针;如果创建失败或当前操作系统小于 Windows 7，则为 NULL。
...
释放通过和方法获取的接口 `GetITaskbarList` `GetITaskbarList3` 。
