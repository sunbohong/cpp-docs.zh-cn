...
创建一个实现的 COM 对象 `IBindStatusCallback` 。
...
或者，您的 COM 对象可以通过调用委托给 MFC 的实现 `CreateBindStatusCallback( NULL )` 。
...
异步的名字对象调用 `OnDataAvailable` ，用于在异步绑定操作过程中将数据提供给客户端。
...
- BSCF_FIRSTDATANOTIFICATION 标识给定绑定操作的第一次调用 `OnDataAvailable` 。

- BSCF_INTERMEDIATEDATANOTIFICATION 标识 `OnDataAvailable` 绑定操作的中间调用。
...
*pMoniker*<br/>
指向异步名字对象接口的指针 `IMoniker` ，精确的名字对象，它是文档自己的名字对象的组合，你可以使用检索它 `IOleClientSite::GetMoniker(OLEWHICHMK_CONTAINER)` ，以及从路径名称创建的名字对象。 控件可以使用此名字对象进行绑定，但这不是控件应保存的名字对象。

*pBindHost*<br/>
指向 `IBindHost` 接口的指针，该接口将用于基于可能的相对路径名创建名字对象。 如果绑定主机无效或未提供名字对象，则调用默认为 `Open(lpszFileName,pError)` 。 有关接口的说明 `IBindHost` ，请参阅 Windows SDK。

*pServiceProvider*<br/>
指向 `IServiceProvider` 接口的指针。 如果服务提供程序无效或无法为提供服务 `IBindHost` ，则调用默认为 `Open(lpszFileName,pError)` 。

*pUnknown*<br/>
指向 `IUnknown` 接口的指针。 如果 `IServiceProvider` 找到了，则函数将查询 `IBindHost` 。 如果服务提供程序无效或无法为提供服务 `IBindHost` ，则调用默认为 `Open(lpszFileName,pError)` 。
