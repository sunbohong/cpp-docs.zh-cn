...
|[CDC：：弦](#chord)|绘制一个 (闭合图形的弦形，该闭合图形由椭圆和直线段的交集) 。|
...
|[CDC：:D rawEscape](#drawescape)|访问不能通过图形设备接口直接提供的视频显示器的绘图功能 (GDI) 。|
...
|[CDC：： GetCurrentPosition](#getcurrentposition)|检索)  (逻辑坐标中的触笔的当前位置。|
...
|[CDC：： GetLayout](#getlayout)|检索设备上下文 (DC) 的布局。 布局可以从左到右 (默认) 或从右到左 (镜像) 。|
...
|[CDC：： GrayString](#graystring)|在给定位置绘制灰显 (灰色) 文本。|
...
|[CDC：:P olygon](#polygon)|绘制一个多边形，其中包含两个或多个点 (顶点) 连接的直线。|
...
通过对象的成员函数执行所有绘制操作 `CDC` 。 类为设备上下文操作提供成员函数，使用绘图工具、类型安全图形设备接口 (GDI) 对象选择和使用颜色和调色板。 它还提供了成员函数，用于获取和设置绘图属性、映射、使用视区、处理窗口区、转换坐标、使用区域、剪辑、绘制线条以及绘制简单形状、省略号和多边形。 还提供了成员函数用于绘制文本、使用字体、使用打印机转义、滚动和播放图元文件。
...
`CDC` 包含两个设备上下文， [m_hDC](#m_hdc) 和 [m_hAttribDC](#m_hattribdc)，在创建对象时，请 `CDC` 引用同一设备。 `CDC` 将所有的输出 GDI 调用定向到，对的 `m_hDC` 大多数特性调用 `m_hAttribDC` 。  (属性调用的示例是 `GetTextColor` ，而 `SetTextColor` 是输出调用。 ) 
...
如果使用打印管理器来启动打印作业，则调用会 `AbortDoc` 擦除整个后台打印作业-打印机不接收任何内容。 如果打印管理器未用于启动打印作业，则在调用之前，可能已将数据发送到打印机 `AbortDoc` 。 在这种情况下，打印机驱动程序会在可能的情况下重置打印机 () 并关闭打印作业。
...
*x1*<br/>
指定边框左上角的 x 坐标 (逻辑单元) 中。

*y1*<br/>
指定 (逻辑单元) 中边框的左上角的 y 坐标。

*x2*<br/>
指定在逻辑单元)  (的边框右下角的 x 坐标。

*y2*<br/>
指定在逻辑单元)  (的边框右下角的 y 坐标。

*x3*<br/>
指定定义弧线起点的点的 x 坐标，该点在逻辑单元) 中 (。 此点不必完全位于弧上。

*y3*<br/>
指定定义弧线起点的点的 y 坐标，该点 (逻辑单位) 。 此点不必完全位于弧上。

*x4*<br/>
指定定义弧线的端点的点的 x 坐标 (逻辑单位) 。 此点不必完全位于弧上。

*y4*<br/>
指定定义弧线的端点的点的 y 坐标 (逻辑单位) 。 此点不必完全位于弧上。

*lpRect*<br/>
指定逻辑单元)  (的边界矩形。 可以为此参数传递 LPRECT 或 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象。

*ptStart*<br/>
指定点的 x 和 y 坐标，该点定义弧线的起点 (逻辑单位) 。 此点不必完全位于弧上。可以为此参数传递 [点](/windows/win32/api/windef/ns-windef-point) 结构或 [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) 对象。

*ptEnd*<br/>
指定点的 x 和 y 坐标，该点定义弧的终点 (逻辑单位) 。 此点不必完全位于弧上。可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
*lpRect*<br/>
指定逻辑单元)  (的边界矩形。 可以为此参数传递指向 [RECT](/windows/win32/api/windef/ns-windef-rect) 数据结构或 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象的指针。

*ptStart*<br/>
指定点的 x 和 y 坐标，该点定义弧线的起点 (逻辑单位) 。 此点不必完全位于弧上。可以为此参数传递 [POINT](/windows/win32/api/windef/ns-windef-point) 数据结构或 [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) 对象。

*ptEnd*<br/>
指定点的 x 和 y 坐标，该点定义弧的终点 (逻辑单位) 。 此点不必完全位于弧上。可以 `POINT` 为此参数传递数据结构或 `CPoint` 对象。
...
`CDC::Arc`除了更新当前位置外，此函数与类似。 点 ( *x1*， *y1*) 和 ( *x2*， *y2*) 指定边框。 由给定边框形成的椭圆形定义弧线的曲线。圆弧按逆时针方向 (默认弧线方向，) 从边框中心到放射连线的点到 ( *x3*， *y3*) 。 该弧形结束时，从边框中心将放射连线从边框中心到 ( *x4*， *y4*) 。 如果起始点和终点相同，则绘制一个完整的椭圆。
...
当 `BitBlt` 函数将单色位图转换为彩色颜色时，它会将白色位 (1) 设置为背景色，黑色位 (0) 为前景色。 使用目标设备上下文的前景色和背景色。 若要将颜色转换为单色，请 `BitBlt` 将与背景色匹配的像素设置为白色，并将所有其他像素设置为黑色。 `BitBlt` 使用颜色设备上下文的前景色和背景色从颜色转换为单色。
...
*x1*<br/>
指定弦形边框左上角的 x 坐标 (逻辑单位) 。

*y1*<br/>
指定弦形边框的左上角的 y 坐标 (逻辑单位) 。

*x2*<br/>
指定下边框的边框右下角的 x 坐标 (逻辑单位) 。

*y2*<br/>
指定弦形边框右下角的 y 坐标 (逻辑单位) 。

*x3*<br/>
指定定义弦的起点的点的 x 坐标，该点 (逻辑单位) 。

*y3*<br/>
指定定义弦起点的点的 y 坐标，该点在逻辑单元) 中 (。

*x4*<br/>
指定定义弦的端点的点的 x 坐标，该点在逻辑单元) 中 (。

*y4*<br/>
指定定义弦的端点的点的 y 坐标 (逻辑单元) 。

*lpRect*<br/>
指定逻辑单元)  (的边界矩形。 可以为此参数传递 LPRECT 或 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象。

*ptStart*<br/>
指定点的 x 和 y 坐标，该点定义弦的起点 (逻辑单位) 。 此点不必完全位于弦上。 可以 `POINT` `CPoint` 为此参数传递结构或对象。

*ptEnd*<br/>
指定点的 x 和 y 坐标，该点定义弦的终点 (逻辑单位) 。 此点不必完全位于弦上。 可以为此参数传递 [点](/windows/win32/api/windef/ns-windef-point) 结构或 [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) 对象。
...
### <a name="remarks"></a>备注
 ...
 ( *x1*， *y1*) 和 ( *x2*， *y2*) 参数分别指定了一个矩形的左上角和右下角，这些矩形限定了弦形的椭圆。  ( *x3*， *y3*) 和 ( *x4*， *y4*) 参数指定与椭圆相交的行的端点。 使用所选笔绘制弦形，并使用所选画笔进行填充。

函数绘制的图形 `Chord` 最多可扩展到，但不包括右坐标和下坐标。 这意味着，图形的高度为 *y2*  -   ，而图形的宽度是 *x2*  -  *x1*。
...
函数通过从当前位置到图形的第一个点绘制直线来关闭图形 (通常情况下，由对成员函数的最近一次调用指定的点 `MoveTo`) 并使用 "线条联接" 样式连接这些直线。 如果使用 `LineTo` 成员函数（而不是）关闭某一图形 `CloseFigure` ，则使用 end cap 来创建角，而不是联接。 `CloseFigure` 只有在设备上下文中有一个开放路径方括号时才应调用。

除非使用此函数显式关闭，否则路径中的图形处于打开状态。 即使当前点和图形的起点相同也可以打开图形 (。在开始新的数字后 ) 添加到路径中的任何直线或曲线。 `CloseFigure`
...
*lpszDriverName*<br/>
指向以 null 结尾的字符串，该字符串指定无需扩展) 设备驱动程序的文件名 ( (例如，"EPSON" ) 。 还可以 `CString` 为此参数传递对象。

*lpszDeviceName*<br/>
指向以 null 结尾的字符串，该字符串指定要支持的特定设备的名称 (例如，"EPSON FX-80" ) 。 如果模块支持多个设备，则使用 *lpszDeviceName* 参数。 还可以 `CString` 为此参数传递对象。

*lpszOutput*<br/>
指向以 null 结尾的字符串，该字符串指定物理输出媒体 (文件或输出端口) 的文件或设备名称。 还可以 `CString` 为此参数传递对象。

*lpInitData*<br/>
指向 `DEVMODE` 包含设备驱动程序的设备特定初始化数据的结构。 Windows `DocumentProperties` 函数检索在给定设备中填充的此结构。 如果设备驱动程序在用户通过控制面板指定的任何)  (，则 *lpInitData* 参数必须为 NULL。
...
设备名称遵循以下约定： ) 建议 (，但可选。 Windows 将使用终止冒号，以便将以冒号结尾的设备名称映射到相同的端口，而不需要使用冒号。 驱动程序和端口名称不能包含前导空格或尾随空格。 GDI 输出函数不能与信息上下文一起使用。
...
*lpszDriverName*<br/>
指向以 null 结尾的字符串，该字符串指定无需扩展) 设备驱动程序的文件名 ( (例如，"EPSON" ) 。 可以 `CString` 为此参数传递对象。

*lpszDeviceName*<br/>
指向以 null 结尾的字符串，该字符串指定要支持的特定设备的名称 (例如，"EPSON FX-80" ) 。 如果模块支持多个设备，则使用 *lpszDeviceName* 参数。 可以 `CString` 为此参数传递对象。

*lpszOutput*<br/>
指向以 null 结尾的字符串，该字符串指定物理输出媒体 (文件或端口) 的文件或设备名称。 可以 `CString` 为此参数传递对象。

*lpInitData*<br/>
指向设备驱动程序的特定于设备的初始化数据。 如果设备驱动程序在用户通过控制面板指定的任何)  (，则 *lpInitData* 参数必须为 NULL。 有关 `CreateDC` 特定于设备的初始化的数据格式，请参阅。
...
由 `CWinApp` 空闲时间处理程序自动调用，将 `DeleteTempMap` 删除 `CDC` 由创建的任何临时对象 `FromHandle` ，但不会销毁设备上下文句柄， ( `hDC`) 临时与 `CDC` 对象关联。
...
如果设备上下文对象的映射模式为 MM_LOENGLISH、MM_HIENGLISH、MM_LOMETRIC 或 MM_HIMETRIC，则转换将基于物理英寸的像素数。 如果映射模式是其他非约束模式之一 (例如 MM_TEXT) ，则转换基于逻辑英寸中的像素数。
...
*lpRect*<br/>
指定逻辑单元)  (的边界矩形。 可以为此参数传递指向 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构或 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象的指针。
...
*大小*<br/>
指定从外边框的左上角到内部边框的左上角的位移， (即矩形的边框) 的粗细，。
...
*sizeLast*<br/>
指定从外边框的左上角到内部边框的左上角的位移 (即要重新绘制的原始矩形的边框) 的粗细。
...
访问不能通过图形设备接口直接提供的视频显示器的绘图功能 (GDI) 。
...
- DFC_BUTTON
  ...
  - 单选按钮的 DFCS_BUTTONRADIOIMAGE 图像 (不需要图像) 

  - 单选按钮 DFCS_BUTTONRADIOMASK 掩码 (不需要掩码) 
...
此代码在窗口右下角绘制大小控制手柄。 它适用于对话框的 `OnPaint` 处理程序，该处理程序没有样式，并且通常不包含其他控件 (如状态栏) ，可以为其分配大小。
...
*lpRect*<br/>
指向 [矩形结构或](/windows/win32/api/windef/ns-windef-rect) [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象，其中包含要在其中设置文本格式的逻辑坐标)  (。
...
如果已设置 TA_UPDATECP 文本对齐标志 (请参阅 [CDC：： SetTextAlign](#settextalign)) ， `DrawText` 将显示从当前位置开始，而不是在给定矩形的左侧显示的文本。 `DrawText` 如果已设置 TA_UPDATECP 标志，则将不会换行文本 (即，DT_WORDBREAK 标志将不会产生任何) 效果。
...
对于编译 Microsoft Foundation 程序，你通常将使用/GA 和/GEs 编译器选项。 /Gw 编译器选项不与 Microsoft 基础类一起使用。  (如果您确实使用 Windows 函数 `MakeProcInstance` ，则需要将返回的函数指针从 FARPROC 显式转换为此 API 所需的类型。 ) 回调注册接口现在是类型安全的 (必须传入一个函数指针，该指针指向特定回调) 的适当类型的函数。
...
如果函数成功，则为非零值;否则，如果无法完成填充，则如果给定点具有由 (*crColor* 指定的边界颜色，则为; 如果) 请求了 FLOODFILLBORDER，则为; 如果 () 请求 FLOODFILLSURFACE，则给定点不具有 *crColor* 指定的颜色，或者该点在剪辑区域外。
...
矩形区域可以是不透明的 (使用当前背景色) 填充，并且它可以是剪辑区域。
...
`FillRect` 类似于 [CDC：： FillSolidRect](#fillsolidrect);不过，使用 `FillRect` 画笔，因此可用于使用纯色、抖动颜色、阴影画笔或图案填充矩形。 `FillSolidRect` 仅使用 COLORREF 参数) 指定的纯色 (。 `FillRect` 通常比更慢 `FillSolidRect` 。
...
*lpRect*<br/>
指定逻辑单元)  (的边界矩形。 可以为此参数传递指向 [RECT](/windows/win32/api/windef/ns-windef-rect) 数据结构或对象的指针 `CRect` 。
...
`FillSolidRect` 非常类似于 [CDC：： FillRect](#fillrect);但是， `FillSolidRect` 只使用 COLORREF 参数) 指定的纯色 (，而使用 `FillRect` 画笔，因此可用于填充具有纯色、抖动颜色、阴影画笔或图案的矩形。 `FillSolidRect` 通常比更快 `FillRect` 。
...
绘制的边框与 `FrameRect` 成员函数使用相同坐标绘制的边框位于同一位置 `Rectangle` (如果 `Rectangle` 使用的是1个逻辑单元范围) 的笔。 不填充矩形的内部 `FrameRect` 。
...
画笔 (的当前起源) 为 [CPoint](../../atl-mfc-shared/reference/cpoint-class.md) 对象。

### <a name="remarks"></a>备注

初始画笔原点位于客户端区域 (0，0) 。 返回值指定此点在设备单位中相对于桌面窗口的原点。
...
*nMaxExtent*<br/>
指定对字符串进行处理) 的逻辑单元 (的最大范围。 如果已处理，则会忽略此区。 任何所需排序或标志符号数组的计算仅适用于包含的字符。 仅当在 *dwFlags* 参数中指定了 GCP_MAXEXTENT 值时，才使用此参数。 当函数处理输入字符串时，只有在总范围尚未超过最大值时，才会将每个字符及其范围添加到输出、范围和其他数组。 达到限制后，处理将停止。
...
若要将 ABC 宽度转换为字体设计单元，应用程序应创建 LOGFONT 结构成员中指定的高度 (字体 `lfHeight`) 等于[](/windows/win32/api/wingdi/ns-wingdi-logfontw) `ntmSizeEM` [NEWTEXTMETRIC](/windows/win32/api/wingdi/ns-wingdi-newtextmetricw)结构的成员中存储的值。  (`ntmSizeEM` 可以通过调用 [EnumFontFamilies](/windows/win32/api/wingdi/nf-wingdi-enumfontfamiliesw) Windows 函数来检索成员的值 ) 
...
*lpFloatBuffer*<br/>
指向用于接收字符宽度的缓冲区。 返回的宽度采用32位 IEEE 浮点格式。  (以字符的基准线度量宽度。 ) 
...
应用程序可以通过检查 OUTLINETEXTMETRIC 结构的成员来确定是否可以嵌入字体 `otmfsType` 。 [](/windows/win32/api/wingdi/ns-wingdi-outlinetextmetricw) 如果设置了的第1位 `otmfsType` ，则不允许嵌入该字体。 如果第1位为明文，则可以嵌入该字体。 如果设置了 bit 2，则嵌入为只读。
...
标志符号轮廓作为一系列轮廓返回。 每个轮廓由一个 [TTPOLYGONHEADER](/windows/win32/api/wingdi/ns-wingdi-ttpolygonheader) 结构定义，后跟所需的任意多个 `TTPOLYCURVE` 结构。 所有点都作为 [POINTFX](/windows/win32/api/wingdi/ns-wingdi-pointfx) 结构返回，表示绝对位置，而不是相对移动。 TTPOLYGONHEADER 结构成员指定的起始点 `pfxStart` 是轮廓[](/windows/win32/api/wingdi/ns-wingdi-ttpolygonheader)轮廓开始的点。 下面的 [TTPOLYCURVE](/windows/win32/api/wingdi/ns-wingdi-ttpolycurve) 结构可以是折线记录或样条记录。 折线记录是一系列的点;点之间绘制的线条描述字符的轮廓。 样条记录表示 TrueType (的二次曲线，即) 二次 b 样条。
...
[CSize](../../atl-mfc-shared/reference/csize-class.md)对象中逻辑单元)  (字符串的尺寸。
...
由于某些设备不会将字符放在常规单元数组中 (也就是说，它们会使字符) 的字符间距，字符串中字符的范围的总和可能不会等于字符串的范围。
...
在 [CSize](../../atl-mfc-shared/reference/csize-class.md) 对象中返回的逻辑单元)  (字符串的维度。

### <a name="remarks"></a>备注

当前剪辑区域不影响返回的宽度和高度 `GetOutputTextExtent` 。

由于某些设备不会将字符放在常规单元数组中 (也就是说，它们将执行字偶间距调整) ，字符串中字符的范围总和可能不会等于字符串的范围。
...
*lpPoints*<br/>
指向[](/windows/win32/api/windef/ns-windef-point) `CPoint` 放置直线终结点和曲线控制点的点数据结构或对象的数组。
...
PT_BEZIERTO 类型总是以三组出现。 位于它们前面的路径中的点定义 Bzier 曲线的起点。 前两 PT_BEZIERTO 点是控制点，第三个 PT_BEZIERTO 点是 (如硬编码的) 的终点。

   可以使用按位运算符 **或**) 来将 PT_LINETO 或 PT_BEZIERTO 类型与以下标志 (结合，以指示相应点为图形中的最后一个点并且应关闭该图形：
...
如果 *nCount* 参数为非零值，则枚举的点数。 如果 *nCount* 为0，则 (路径中的总点数，并将 `GetPath` 内容不写入缓冲区) 。 如果 *nCount* 为非零值且小于路径中的点数，则返回值为-1。
...
默认情况下， `GetTextExtent` 假定其检索了维度的文本沿水平线设置 (也就是说，行距为 0) 。 如果创建的字体指定非零行距，则必须显式转换文本角度以获取字符串的尺寸。
...
*nCount*<br/>
以字节为单位指定缓冲区 (大小) 。 如果字样名称长度超过此参数指定的字节数，则将截断名称。
...
在给定位置绘制灰色 (灰色) 文本，方法是在内存位图中书写文本，使位图变暗，然后将位图复制到显示器。
...
*nCount*<br/>
指定要输出的字符数。 如果此参数为0，则 `GrayString` 计算字符串的长度， (假设 *lpData* 是指向字符串) 的指针。 如果 *nCount* 为1，并且 *lpfnOutput* 指向的函数返回0，则显示图像，但不会显示为灰色。
...
*nWidth*<br/>
指定包含字符串的矩形的逻辑单元)  (宽度。 如果 *nWidth* 为0，则 `GrayString` 计算区域的宽度，假设 *lpData* 是指向字符串的指针。

*nHeight*<br/>
指定包含字符串的矩形的高度 (以逻辑单位) 。 如果 *nHeight* 为0，则 `GrayString` 计算区域的高度，假设 *lpData* 是指向字符串的指针。
...
应用程序可以在不调用成员函数的情况下，在支持纯灰色颜色的设备上绘制灰显 (灰色) 字符串 `GrayString` 。 系统颜色 COLOR_GRAYTEXT 是用于绘制禁用文本的灰色系统颜色。 应用程序可以调用 `GetSysColor` Windows 函数来检索 COLOR_GRAYTEXT 的颜色值。 如果颜色不是 0 (黑色) ，则应用程序可以调用 `SetTextColor` 成员函数将文本颜色设置为颜色值，然后直接绘制字符串。 如果检索的颜色为黑色，则应用程序必须调用 `GrayString` ，使文本变暗 (灰色) 。

如果 *lpfnOutput* 为 NULL，则 GDI 将使用 Windows [TextOut](/windows/win32/api/wingdi/nf-wingdi-textoutw) 函数，而 *lpData* 被假定为要输出的字符的远端指针。 如果成员函数无法处理要输出的字符 `TextOut` (例如，该字符串存储为位图) ，则该应用程序必须提供其自己的输出函数。
...
绘制从当前位置到 *x* 和 *y* 指定的点（但不包括） (或 *点*) 的直线。
...
如果源、模式和目标位图的颜色格式不同，则此函数将转换模式或源格式，或同时转换两者，以匹配目标格式。 如果掩码位图不是单色位图，则会出现错误。 在记录增强型图元文件时，会发生错误 (并且如果源设备上下文标识增强型图元文件设备上下文，则函数返回 0) 。 并非所有设备都支持 `MaskBlt` 。 应用程序应调用 `GetDeviceCaps` 来确定设备是否支持此功能。 如果未提供掩码位图，此函数的行为与 `BitBlt` 使用前台光栅操作代码完全相同。 掩码位图中的像素偏移量映射到源设备上下文位图中 (0，0) 的点。 这适用于掩码位图包含一组掩码的情况;通过调整发送到的像素偏移量和矩形大小，应用程序可以轻松地将任何一个应用程序应用到 blitting 任务 `MaskBlt` 。
...
*x3*<br/>
指定在逻辑单元) 中，弧线起点 (的 x 坐标。 此点不必完全位于弧上。

*y3*<br/>
指定以逻辑单位)  (的弧形起点的 y 坐标。 此点不必完全位于弧上。

*x4*<br/>
在逻辑单元) 中指定弧形端点 (的 x 坐标。 此点不必完全位于弧上。

*y4*<br/>
指定圆弧的端点 (在逻辑单元) 中的 y 坐标。 此点不必完全位于弧上。
...
弧线中心是由 *x1*、 *y1*、 *x2* 和 *y2* 指定的边框的中心 (或 *lpRect*) 。 圆弧的起点和终点由 *x3*、 *y3*、 *x4* 和 (*Y4* 指定，或由 *ptStart* 和 *ptEnd*) 指定。

弧线用所选笔绘制，以逆时针方向移动。 从每个端点到弧线中心绘制两条额外直线。 用当前画笔填充扇形区区域。 如果 *x3* 等于 *x4* ， *y3* 等于 *y4*，则结果是一个椭圆，其中的单个线条从椭圆中心到点 ( *x3*， *y3*) 或 ( *x4*， *y4*) 。
...
的第二个版本 `PlayMetaFile` 显示存储在给定的增强格式图元文件中的图片。 当应用程序调用第二个版本的时 `PlayMetaFile` ，Windows 将使用增强型图元文件头中的图片框架将图片映射到 *lpBounds* 参数指向的矩形。  (可以在调用之前，通过在输出设备中设置世界变换来剪切或旋转此图片 `PlayMetaFile` 。图中包含矩形边缘的 ) 点。 在播放增强型图元文件之前，可以通过在输出设备中定义剪辑区域来裁剪增强型图元文件。
...
平行四边形 (D) 的第四个顶点是通过将 A、B 和 C)  (A、B 和 C 视为向量，计算 D = B + C-A 来定义的。
...
*lpPoints*<br/>
指向一个 [点](/windows/win32/api/windef/ns-windef-point) 数据结构数组，这些结构包含样条 () 的端点和控制点。
...
在调用此成员函数之前，必须确保已选择已在设备上下文中选择的所有对象)  (已选择的所有对象。
...
*x3*<br/>
指定用于绘制 (逻辑单位) 的圆角的椭圆宽度。

*y3*<br/>
指定用于绘制 (逻辑单位) 的圆角的椭圆高度。

*lpRect*<br/>
指定逻辑单元中的边框。 可以 `CRect` 向此参数传递对象或指向 `RECT` 结构的指针。

*情况*<br/>
*点* 的 x 坐标指定在逻辑单位)  (绘制圆角的椭圆宽度。 *Point* 的 y 坐标指定在逻辑单位)  (绘制圆角的椭圆高度。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
通过将状态信息 (如剪辑区域、选定对象和映射模式) 复制到 Windows 维护的上下文堆栈，从而保存设备上下文的当前状态。
...
如果 *lpRectUpdate* 为 NULL，则 Windows 不会计算更新矩形。 如果 *pRgnUpdate* 和 *LPRECTUPDATE* 都为 NULL，则 Windows 不会计算更新区域。 如果 *pRgnUpdate* 不为 NULL，则 Windows 将假定其包含滚动过程所发现的区域的有效指针 (由 `ScrollDC` 成员函数) 定义。 如果需要，可以将 *lpRectUpdate* 中返回的更新区域传递给 `CWnd::InvalidateRgn` 。
...
- RGN_AND 新的剪辑区域包括当前剪辑区域和当前路径) 重叠区域 (重叠区域。

- RGN_COPY 新的剪辑区域为当前路径。

- RGN_DIFF 新的剪辑区域包含当前剪辑区域的区域，并且排除当前路径中的区域。

- RGN_OR 新的剪辑区域包含当前剪辑区域和当前路径) 的联合 (组合区域。
...
- RGN_COPY 新的剪辑区域是 *pRgn* 标识的区域的副本。 此功能与的第一个版本是相同的 `SelectClipRgn` 。 如果由 *pRgn* 标识的区域为 null，则新的剪辑区域将成为)  (空区域的默认剪辑区域。
...
回调注册接口现在是类型安全的 (必须传入一个函数指针，该指针指向特定回调) 的正确函数类型。
...
- DCB_ACCUMULATE 将 *lpRectBounds* 指定的矩形添加到边框 (使用矩形联合运算) 。

- DCB_DISABLE 关闭边界堆积。

- DCB_ENABLE 开启边界累积。  (禁用了界限累积的默认设置。 ) 
...
*x*<br/>
指定新原点) 的 x 坐标 (。 此值必须在0-7 的范围内。

*y*<br/>
指定新原点的设备单位) 的 y 坐标 (。 此值必须在0-7 的范围内。
...
画笔原点的默认坐标是 (0，0) 。 若要更改画笔的原点，请调用 `UnrealizeObject` 对象的函数 `CBrush` ，调用 `SetBrushOrg` ，然后调用 `SelectObject` 成员函数以在设备上下文中选择画笔。
...
失败时返回0。 要获得更多的错误信息，请调用 GetLastError。
...
如果失败，GDI_ERROR。 要获得更多的错误信息，请调用 GetLastError。
...
如果 **(LAYOUT_RTL** ) 调用 SetLayout，则 `SetLayout` 会将映射模式自动更改为 MM_ISOTROPIC。 因此，对 [GetMapMode](#getmapmode) 的后续调用将返回 MM_ISOTROPIC 而不是 MM_TEXT。
...
- MM_TWIPS 每个逻辑单元都转换为点的1/20。  (，因为点为1/72 英寸，twip 是1/1440 英寸。 ) 正 x 右对齐;正 y 向上。
...
MM_HIENGLISH、MM_HIMETRIC、MM_LOENGLISH、MM_LOMETRIC 和 MM_TWIPS 模式对于必须以物理意义单位（如英寸或毫米 (）绘制的应用程序很有用。 MM_ISOTROPIC 模式确保了1:1 纵横比，这在保持图像的准确形状非常重要时非常有用。 MM_ANISOTROPIC 模式允许单独调整 x 和 y 坐标。

> [!NOTE]
> 如果调用 [SetLayout](#setlayout) 将 DC (设备上下文) 改为从右到左的布局，则 `SetLayout` 会自动将映射模式改为 MM_ISOTROPIC。
...
实际绘制点的颜色的 RGB 值。 如果使用该颜色的近似值，此值可以与 *crColor* 指定的值不同。 如果函数失败 (如果该点位于剪辑区域外) ，则返回值为-1。
...
- R2_MERGEPENNOT 像素是笔颜色和屏幕颜色反转 (最后一个像素 = (不是屏幕像素) 或笔) 的组合。

- R2_MASKPENNOT 像素是笔和反转屏幕的颜色的组合 (最终像素 = (不是屏幕像素) 和笔) 。

- R2_MERGENOTPEN 像素是屏幕颜色和笔颜色反转 (final 像素 = (不是笔) 或屏幕像素) 的组合。

- R2_MASKNOTPEN 像素是屏幕和笔反转的颜色的组合 (final 像素 = (不是笔) 和屏幕像素) 。

- R2_MERGEPEN 像素是笔颜色和屏幕颜色 (最终像素 = 笔或屏幕像素) 的组合。

- R2_NOTMERGEPEN 像素是 R2_MERGEPEN 颜色 (最后一个像素 = 不 (钢笔或屏幕像素) # A3 的反转。

- R2_MASKPEN 像素是笔和屏幕共有的颜色组合 (最终像素 = 笔和屏幕像素) 。

- R2_NOTMASKPEN 像素是 R2_MASKPEN 颜色 (最终像素 = 不 (笔和屏幕像素) # A3 的反转。

- R2_XORPEN 像素是笔中或屏幕中的颜色的组合，但不是在 (final 象素 = 触色屏幕像素) 中。

- R2_NOTXORPEN 像素是 R2_XORPEN 颜色反转 (最后一个像素 = 不 (笔 XOR 屏幕像素) # A3。
...
*nCharExtra*<br/>
指定要添加到每个字符)  (逻辑单元中的额外空间量。 如果当前映射模式不为 `MM_TEXT` ，则将转换 *nCharExtra* 并将其舍入到最接近的像素。
...
*nBreakExtra*<br/>
指定要添加到 (逻辑单元) 中的文本行的总额外空间。 如果当前映射模式不为 `MM_TEXT` ，则此参数指定的值将转换为当前映射模式并舍入到最近的设备单元。
...
`SetTextJustification`调用成员函数后，对文本输出函数的调用 (如 `TextOut`) 在指定的中断字符数之间均匀分配指定的额外空间。 换行符通常是 (ASCII 32) 的空格字符，但是可以通过字体作为其他字符进行定义。

成员函数 `GetTextExtent` 通常与一起使用 `SetTextJustification` 。 `GetTextExtent` 计算在对齐前给定行的宽度。 应用程序可以通过在 `GetTextExtent` 对齐后从字符串的宽度减去返回的值，来确定要在 nBreakExtra 参数中指定的空间量。
...
*x*<br/>
指定视区原点的设备单位) 的 x 坐标 (。 该值必须在设备坐标系统范围内。

*y*<br/>
指定视区原点的设备单位) 的 y 坐标 (。 该值必须在设备坐标系统范围内。
...
窗口的先前范围 (逻辑单元) 为 `CSize` 对象。 如果发生错误，则返回的对象的 x 坐标和 y 坐标都 `CSize` 设置为0。
...
*lpnTabStopPositions*<br/>
指向一个数组，其中包含在逻辑单元)  (的制表位位置。 制表位必须按递增顺序排序;最小的 x 值应是数组中的第一项。

*nTabOrigin*<br/>
指定 (以逻辑单元) 展开选项卡的起始位置的 x 坐标。
