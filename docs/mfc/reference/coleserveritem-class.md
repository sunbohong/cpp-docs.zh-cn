---
description: 了解详细信息： COleServerItem 类
...
`COleServerItem`类定义了多个可重写的成员函数，这些函数由 OLE 系统动态链接库调用 (dll) ，通常用于响应来自容器应用程序的请求。 这些成员函数允许容器应用程序以各种方式间接操作项，例如显示、执行其谓词或以各种格式检索其数据。
...
必须已经实现了 [OnDraw](#ondraw) 成员函数才能为项提供表示格式 (图元文件图片) 。 若要支持其他转换格式，请使用[GetDataSource](#getdatasource)返回的[COleDataSource](../../mfc/reference/coledatasource-class.md)对象进行注册，并覆盖[OnRenderData](#onrenderdata)成员函数以提供要支持的格式的数据。
...
*dwEffects*<br/>
确定拖动操作在拖动操作中允许的效果 (复制、移动和链接) 的组合。
...
调用此函数以用要复制到剪贴板的所有数据填充指定的 [COleDataSource](../../mfc/reference/coledatasource-class.md) 对象（如果您调用了 [CopyToClipboard](#copytoclipboard) ） (相同的数据也将在调用 [system.windows.dragdrop.dodragdrop](#dodragdrop)) 时传输。
...
有关详细信息，请参阅 Windows SDK 中的[IDataObject：：：：](/windows/win32/api/objidl/nf-objidl-idataobject-getdata) FORMATETC 和[](/windows/win32/api/objidl/ns-objidl-formatetc) 。
