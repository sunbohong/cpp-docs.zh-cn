---
description: 了解详细信息： CSplitterWnd 类
...
拆分器窗口支持 (除了窗格可能) 的滚动条之外的特殊滚动条。 这些滚动条是对象的子项 `CSplitterWnd` ，并与窗格共享。
...
*pContext*<br/>
指向用于创建视图的创建上下文的指针 (通常 *是传递到* 父框架的重写 [CFrameWnd：： OnCreateClient](../../mfc/reference/cframewnd-class.md#oncreateclient) 成员函数（在其中创建拆分窗口) ）。
...
如果拆分窗口具有 SPLS_DYNAMIC_SPLIT 样式) ，则框架将调用此成员函数以实现动态拆分窗口的逻辑 (。 它可以自定义，以及虚拟函数 [CreateView](#createview)，以实现更高级的动态拆分器。
...
*bDoScroll*<br/>
确定是否发生指定的滚动操作。 如果 *bDoScroll* 为 TRUE (也就是说，如果存在子窗口，并且拆分窗口) 滚动范围，则可以执行指定的滚动操作;如果 *bDoScroll* 为 FALSE (也就是说，如果不存在子窗口，或拆分视图没有滚动范围) ，则不会发生滚动。
...
`SplitColumn` 如果拆分窗口具有 SPLS_DYNAMIC_SPLIT 样式) ，则框架将调用来实现动态拆分窗口的逻辑 (。 它可以自定义，以及虚拟函数 [CreateView](#createview)，以实现更高级的动态拆分器。
...
`SplitRow` 如果拆分窗口具有 SPLS_DYNAMIC_SPLIT 样式) ，则框架将调用来实现动态拆分窗口的逻辑 (。 它可以自定义，以及虚拟函数 [CreateView](#createview)，以实现更高级的动态拆分器。
