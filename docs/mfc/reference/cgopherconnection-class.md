---
description: 了解详细信息： CGopherConnection 类
...
类 `CGopherConnection` 包含构造函数和其他三个成员函数，用于管理 gopher 服务： [OpenFile](#openfile)、 [CreateLocator](#createlocator)和 [GetAttribute](#getattribute)。

若要与 gopher Internet 服务器通信，必须先创建 [CInternetSession](../../mfc/reference/cinternetsession-class.md)的实例，然后调用 [CInternetSession：： GetGopherConnection](../../mfc/reference/cinternetsession-class.md#getgopherconnection)，这将创建 `CGopherConnection` 对象并返回指向该对象的指针。 永远不会 `CGopherConnection` 直接创建对象。
...
永远不会 `CGopherConnection` 直接创建。 相反，可以调用 [CInternetSession：： GetGopherConnection](../../mfc/reference/cinternetsession-class.md#getgopherconnection)，它将创建一个 `CGopherConnection` 对象并返回指向该对象的指针。
...
为了从 gopher 服务器中检索信息，应用程序必须首先获取 gopher 定位符。 然后，应用程序必须将定位符视为不透明标记 (即，应用程序可以使用定位符，但不会直接操作或将其与) 进行比较。 通常，应用程序使用定位符对 [CGopherFileFind：： FindFile](../../mfc/reference/cgopherfilefind-class.md#findfile) 成员函数进行调用以检索特定的信息片段。
