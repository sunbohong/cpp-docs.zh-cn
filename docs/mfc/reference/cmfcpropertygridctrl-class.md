---
description: 了解详细信息： CMFCPropertyGridCtrl 类
...
|“属性”|描述|
|----------|-----------------|
|`CMFCPropertyGridCtrl::accHitTest`|由框架调用以检索屏幕上给定点处的子元素或子对象。  (将重写 [CWnd：： accHitTest](../../mfc/reference/cwnd-class.md#acchittest)。 ) |
|`CMFCPropertyGridCtrl::accLocation`|由框架调用以检索指定对象的当前屏幕位置。  (将重写 [CWnd：： accLocation](../../mfc/reference/cwnd-class.md#acclocation)。 ) |
|[CMFCPropertyGridCtrl：： accSelect](#accselect)|由框架调用以修改选定内容或移动指定对象的键盘焦点。  (将重写 [CWnd：： accSelect](../../mfc/reference/cwnd-class.md#accselect)。 ) |
...
|[CMFCPropertyGridCtrl：:D eleteProperty](#deleteproperty)|从属性网格控件中删除指定属性。|
...
|[CMFCPropertyGridCtrl：：展开](#expandall)|展开或折叠所有属性网格控件节点。|
|[CMFCPropertyGridCtrl：： FindItemByData](#finditembydata)|检索与用户定义的 DWORD 值相关联的属性。|
|`CMFCPropertyGridCtrl::get_accChild`|由框架调用以检索指定子级的 `IDispatch` 接口地址。  (将替代 [CWnd：： get_accChild](../../mfc/reference/cwnd-class.md#get_accchild)。 ) |
|[CMFCPropertyGridCtrl：： get_accChildCount](#get_accchildcount)|由框架调用调用以检索属于该对象的子级的个数。  (将替代 [CWnd：： get_accChildCount](../../mfc/reference/cwnd-class.md#get_accchildcount)。 ) |
|`CMFCPropertyGridCtrl::get_accDefaultAction`|由框架调用以检索描述对象默认操作的字符串。  (将替代 [CWnd：： get_accDefaultAction](../../mfc/reference/cwnd-class.md#get_accdefaultaction)。 ) |
|`CMFCPropertyGridCtrl::get_accDescription`|由框架调用以检索描述指定对象的可视外观的字符串。  (将替代 [CWnd：： get_accDescription](../../mfc/reference/cwnd-class.md#get_accdescription)。 ) |
|[CMFCPropertyGridCtrl：： get_accFocus](#get_accfocus)|由框架调用以检索具有键盘焦点的对象。  (将替代 [CWnd：： get_accFocus](../../mfc/reference/cwnd-class.md#get_accfocus)。 ) |
|[CMFCPropertyGridCtrl：： get_accHelp](#get_acchelp)|由框架调用以检索对象的 `Help` 属性字符串。  (将替代 [CWnd：： get_accHelp](../../mfc/reference/cwnd-class.md#get_acchelp)。 ) |
|[CMFCPropertyGridCtrl：： get_accHelpTopic](#get_acchelptopic)|由框架调用以检索与指定对象关联的 **WinHelp** 文件的完整路径以及该文件内相应主题的标识符。  (将替代 [CWnd：： get_accHelpTopic](../../mfc/reference/cwnd-class.md#get_acchelptopic)。 ) |
|[CMFCPropertyGridCtrl：： get_accKeyboardShortcut](#get_acckeyboardshortcut)|由框架调用以检索指定对象的快捷键或访问键。  (将替代 [CWnd：： get_accKeyboardShortcut](../../mfc/reference/cwnd-class.md#get_acckeyboardshortcut)。 ) |
|`CMFCPropertyGridCtrl::get_accName`|由框架调用以检索指定对象的名称。  (将替代 [CWnd：： get_accName](../../mfc/reference/cwnd-class.md#get_accname)。 ) |
|`CMFCPropertyGridCtrl::get_accRole`|由框架调用以检索描述指定对象的角色的信息。  (将替代 [CWnd：： get_accRole](../../mfc/reference/cwnd-class.md#get_accrole)。 ) |
|[CMFCPropertyGridCtrl：： get_accSelection](#get_accselection)|由框架调用以检索该对象的选定子级。  (将替代 [CWnd：： get_accSelection](../../mfc/reference/cwnd-class.md#get_accselection)。 ) |
|`CMFCPropertyGridCtrl::get_accState`|由框架调用以检索指定对象的当前状态。  (将替代 [CWnd：： get_accState](../../mfc/reference/cwnd-class.md#get_accstate)。 ) |
|`CMFCPropertyGridCtrl::get_accValue`|由框架调用以检索指定对象的值。  (将替代 [CWnd：： get_accValue](../../mfc/reference/cwnd-class.md#get_accvalue)。 ) |
...
|[CMFCPropertyGridCtrl：： GetScrollBarCtrl](#getscrollbarctrl)|检索指向属性网格控件中的滚动条控件的指针。  (将重写 [CWnd：： GetScrollBarCtrl](../../mfc/reference/cwnd-class.md#getscrollbarctrl)。 ) |
...
|[CMFCPropertyGridCtrl：： System.windows.media.visualtreehelper.hittest](#hittest)|检索一个指针，该指针指向与属性网格控件项相对应的属性，如果在项中指定点，则为。 此方法还指示属性网格控件中包含该点的区域。|
...
|`CMFCPropertyGridCtrl::PreTranslateMessage`|在将窗口消息调度到[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)和[DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) Windows 函数之前，由[CWinApp](../../mfc/reference/cwinapp-class.md)类用于转换窗口消息。 （重写 [CWnd::PreTranslateMessage](../../mfc/reference/cwnd-class.md#pretranslatemessage)。）|
...
下面的示例演示如何使用类中的各种方法配置属性网格控件对象 `CMFCPropertyGridCtrl` 。 该示例演示如何启用标头控件，启用 "说明" 区域，以及设置属性网格控件的外观。 该示例还演示了如何为控件设置按字母模式，从而使控件按属性名对它所包含的所有属性进行排序，以及如何为属性网格控件的各种元素设置自定义颜色。 此示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
*dwStyle*<br/>
中按位组合 (或) 的 [窗口样式](../../mfc/reference/styles-used-by-mfc.md#window-styles)。
...
下面的示例演示如何使用 `Create` 类中的方法 `CMFCPropertyGridCtrl` 。 此示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
## <a name="cmfcpropertygridctrlexpandall"></a><a name="expandall"></a> CMFCPropertyGridCtrl：：展开
...
*pnArea*<br/>
[in，out]指向变量的指针 `ClickArea` 。 此方法返回时，该变量指示包含指定点的 *属性区域* 。 有关属性区域的详细信息，请参阅 "备注"。
...
如果在属性网格控件的宽度中显示组名，则为 TRUE;如果组名称被 () 控件的列的右侧截断，则为 FALSE。
...
默认情况下，此方法将 [AFX_WM_PROPERTY_CHANGED](../../mfc/reference/afx-messages.md) 消息发送给属性网格控件的所有者。
