---
description: 了解详细信息： CContextMenuManager 类
...
|[CContextMenuManager::SetDontCloseActiveMenu](#setdontcloseactivemenu)|控制在 `CContextMenuManager` 显示新的快捷菜单时是否关闭活动快捷菜单。|
...
不应 `CContextMenuManager` 手动创建对象。 应用程序的框架将创建 `CContextMenuManager` 对象。 但是，在初始化应用程序时，应调用 [CWinAppEx：： InitContextMenuManager](../../mfc/reference/cwinappex-class.md#initcontextmenumanager) 。 初始化上下文管理器后，使用方法 [CWinAppEx：： GetContextMenuManager](../../mfc/reference/cwinappex-class.md#getcontextmenumanager) 获取应用程序的上下文管理器的指针。

可以通过调用在运行时创建快捷菜单 `AddMenu` 。 如果希望在不首先接收用户输入的情况下显示菜单，请调用 `ShowPopupMenu` 。 `TrackPopupMenu` 当您想要创建菜单并等待用户输入时使用。 `TrackPopupMenu` 返回所选命令的索引; 如果用户退出但不选择任何内容，则返回0。

`CContextMenuManager`还可以将其状态保存并加载到 Windows 注册表中。

## <a name="example"></a>示例

下面的示例演示如何向对象添加菜单 `CContextMenuManager` ，以及如何在 `CContextMenuManager` 对象显示新的弹出菜单时关闭活动的弹出菜单。 此代码片段是 [自定义页面示例](../../overview/visual-cpp-samples.md)的一部分。
...
如果 *uiMenuResId* 无效或中已存在具有相同名称的另一个菜单，则此方法将失败 `CContextMenuManager` 。
...
在大多数情况下，不应手动创建 `CContextMenuManager` 。 应用程序的框架将创建 `CContextMenuManager` 对象。 你应在应用程序的初始化过程中调用 [CWinAppEx：： InitContextMenuManager](../../mfc/reference/cwinappex-class.md#initcontextmenumanager) 。 若要获取指向上下文管理器的指针，请调用 [CWinAppEx：： GetContextMenuManager](../../mfc/reference/cwinappex-class.md#getcontextmenumanager)。
...
如果此方法找到与 *lpszName* 匹配的菜单，则会将 `GetMenuByName` 该菜单资源 ID 存储在参数 *puiOrigResID* 中。
...
此方法会清除弹出菜单并将其从中移除 `CContextMenuManager` 。
...
默认情况下，将 `CContextMenuManager` 关闭活动的弹出菜单。
...
此方法类似于方法 [CContextMenuManager：： TrackPopupMenu](#trackpopupmenu) ，因为这两个方法都显示一个快捷菜单。 但是， `TrackPopupMenu` 将返回选定菜单命令的索引。

如果参数 *bAutoDestroy* 为 FALSE，则必须手动调用继承的 `DestroyMenu` 方法以释放内存资源。 的默认实现 `ShowPopupMenu` 不使用参数 *bAutoDestroy*。 提供此方法是为了供将来使用，或者用于从类派生的自定义类 `CContextMenuManager` 。
