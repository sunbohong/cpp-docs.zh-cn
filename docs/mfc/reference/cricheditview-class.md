---
description: 了解详细信息： CRichEditView 类
...
|[CRichEditView：： QueryAcceptData](#queryacceptdata)|查询查看上的数据 `IDataObject` 。|
|[CRichEditView：： WrapChanged](#wrapchanged)|基于的值调整此 rich edit 视图的目标输出设备 `m_nWordWrap` 。|
...
*lpchrg*<br/>
指向 [CHARRANGE](/windows/win32/api/richedit/ns-richedit-charrange) 结构的指针，该结构指定 (和 OLE 项) 要复制到 *lplpdataobj* 指定的数据对象的字符范围。
...
*lpRichDataObj*<br/>
指向 [IDataObject](/windows/win32/api/objidl/nn-objidl-idataobject) 对象的指针，该对象包含 rich edit 控件中的剪贴板数据 ( [IRichEditOle：： GetClipboardData](/windows/win32/api/richole/nf-richole-iricheditole-getclipboarddata)) 。

*lplpdataobj*<br/>
一个指针，指向用于接收对象地址的指针变量，该 `IDataObject` 对象表示在 *lpchrg* 参数中指定的范围。 如果返回错误，则忽略 *lplpdataobj* 的值。
...
如果返回值指示 success，则 `IRichEditOleCallback::GetClipboardData` 返回 `IDataObject` 由 *lplpdataobj* 访问的; 否则，它将返回 *lpRichDataObj* 访问的值。 重写此函数以提供您自己的剪贴板数据。 此函数的默认实现将返回 E_NOTIMPL。
...
*lpoleobj*<br/>
指向结构的指针，该 `OLEOBJECT` 结构指定选定的第一个 ole 对象（如果选定内容包含一个或多个 ole 项）。 如果所选内容不包含任何项，则 *lpoleobj* 为 NULL。 `OLEOBJECT`结构保存指向 OLE 对象 v 表的指针。
...
调用此函数可获取一个指针，该指针指向 `CRichEditDoc` 与此视图关联的。
...
指向与您的对象相关联的 [CRichEditDoc](../../mfc/reference/cricheditdoc-class.md) 对象的指针 `CRichEditView` 。

## <a name="cricheditviewgetinplaceactiveitem"></a><a name="getinplaceactiveitem"></a> CRichEditView：： GetInPlaceActiveItem

调用此函数可获取此对象中当前就地激活的 OLE 项 `CRichEditView` 。
...
调用此函数可检索与对象关联的 [CRichEditCtrl](../../mfc/reference/cricheditctrl-class.md) 对象 `CRichEditView` 。
...
调用此函数可检索 (`CRichEditCntrItem` 此对象中当前选定) 对象的 OLE 项 `CRichEditView` 。
...
指向对象中选定的 [CRichEditCntrItem](../../mfc/reference/cricheditcntritem-class.md) 对象的指针 `CRichEditView` ;如果未在此视图中选择任何项，则为 NULL。

## <a name="cricheditviewgettextlength"></a><a name="gettextlength"></a> CRichEditView：： GetTextLength

调用此函数可检索此对象中文本的长度 `CRichEditView` 。
...
*uCodePage*<br/>
用于 ANSI 代码页的翻译 (CP_ACP 的代码页，用于 Unicode) 1200。
...
调用此函数可将指定的文件作为 [CRichEditCntrItem](../../mfc/reference/cricheditcntritem-class.md) 对象插入到丰富的编辑视图中 () 。
...
调用此函数可在中查找文本 `CRichEditView` 。 重写此函数以更改派生视图类的搜索特性。
...
此函数的默认实现将调用 [CView：： OnUpdate](../../mfc/reference/cview-class.md#onupdate) 成员函数，该函数不包含提示信息 (也就是说，使用 *lHint* 参数的默认值0，为 *pHint* 参数) 使用 NULL。 重写此函数以执行需要文档相关信息的任何一次性初始化。 例如，如果应用程序具有固定大小的文档，则可以使用此函数根据文档大小初始化视图的滚动限制。 如果你的应用程序支持可变大小的文档，请使用在 `OnUpdate` 每次更改文档时更新滚动限制。
...
通常，可以通过创建 [COleStreamFile](../../mfc/reference/colestreamfile-class.md) 来实现此目的 `IStorage` 。 `COleStreamFile`可以附加到存档和[CObject：：串行化](../../mfc/reference/cobject-class.md#serialize)以加载数据。
...
默认实现将纸张大小设置为 (printer) 的输出设备的物理高度和宽度。 如果没有与 *dcPrinter* 关联的设备上下文，则默认实现会将纸张大小设置为 8.5 x 11 英寸。
...
有关 HRESULT 和的详细信息 `IDataObject` ，请参阅 Windows SDK 中的 [COM 错误代码](/windows/win32/com/structure-of-com-error-codes) 和 [IDataObject](/windows/win32/api/objidl/nn-objidl-idataobject)的结构。
...
调用此函数可设置此对象中新文本的字符格式设置特性 `CRichEditView` 。
...
如果 [m_nWordWrap](#m_nwordwrap) 为 `WrapToTargetDevice` ，则应在使用此函数后调用 [WrapChanged](#wrapchanged) 来调整打印特征。

请注意， [system.drawing.printing.printdocument.printpage>](#printpage) 使用的边距相对于物理页，而不是逻辑页。 因此，零的边距经常会剪裁文本，因为许多打印机在页面上都有不可打印的区域。 若要避免剪裁文本，应在打印之前调用 "使用 `SetMargins` 设置合理的打印机边距"。
...
调用此函数可设置此对象中当前选定内容的段落格式特性 `CRichEditView` 。
...
当打印特征更改 ( [SetMargins](#setmargins) 或 [SetPaperSize](#setpapersize)) 时调用此函数。
...
重写此函数以修改富编辑视图对 [m_nWordWrap](#m_nwordwrap) 或打印特征 ( [OnPrinterChanged](#onprinterchanged)) 的更改的响应方式。
