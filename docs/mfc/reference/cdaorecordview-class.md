---
description: 了解详细信息： CDaoRecordView 类
...
> [!NOTE]
> DAO 数据库类不同于基于开放式数据库连接的 MFC 数据库类 (ODBC) 。 所有 DAO 数据库类名称都具有 "CDao" 前缀。 你仍可以使用 DAO 类访问 ODBC 数据源;DAO 类通常提供出色的功能，因为它们使用 Microsoft Jet 数据库引擎。
...
`CDaoRecordView::OnInitialUpdate` 调用 `CWnd::UpdateData` ，它调用 `CWnd::DoDataExchange` 。 此初始调用会 `DoDataExchange` 将 `CDaoRecordView` (间接) 的控件连接到 `CDaoRecordset` 由 ClassWizard 创建的字段数据成员。 在调用基类成员函数之前，不能使用这些数据成员 `CFormView::OnInitialUpdate` 。
...
如果用户移动到第一条记录，框架将禁用任何用户界面对象 (例如，菜单项或工具栏按钮，) 您必须将其移动到第一个记录或上一条记录。
...
> [!CAUTION]
> 此函数的结果是可靠的，但视图可能无法检测记录集的结尾，直到用户移过它。 用户可能必须超出最后一条记录，才能知道，记录视图必须禁用所有用户界面对象，才能移动到下一条或最后一条记录。 如果用户移过最后一条记录，然后移回最后一条记录 (或在) 之前，记录视图可以跟踪用户在记录集中的位置，并正确地禁用用户界面对象。
