---
description: 了解详细信息： CDaoDatabase 类
...
ms.locfileid: "97250967"
---
# <a name="cdaodatabase-class"></a>CDaoDatabase 类

表示使用 (DAO) 的数据访问对象连接到 Access 数据库。 DAO 受 Office 2013 的支持。 DAO 3.6 是最终版本，被视为已过时。
...
|“属性”|描述|
|----------|-----------------|
|[CDaoDatabase：： CDaoDatabase](#cdaodatabase)|构造 `CDaoDatabase` 对象。 调用将 `Open` 对象连接到数据库。|
...
|[CDaoDatabase：： Execute](#execute)|执行操作查询。 `Execute`为返回结果的查询调用会引发异常。|
|[CDaoDatabase：： GetConnect](#getconnect)|返回用于将对象连接到数据库的连接字符串 `CDaoDatabase` 。 用于 ODBC。|
...
|[CDaoDatabase：： GetQueryTimeout](#getquerytimeout)|返回数据库查询操作超时之前的秒数。会影响对 ODBC 数据源的所有后续打开、添加、更新和编辑操作以及其他操作 (仅) 例如 `Execute` 调用。|
|[CDaoDatabase：： GetRecordsAffected](#getrecordsaffected)|返回受上次更新、编辑或添加操作或通过调用影响的记录数 `Execute` 。|
...
|[CDaoDatabase：： IsOpen](#isopen)|如果对象当前已连接到数据库，则返回非零 `CDaoDatabase` 值。|
|[CDaoDatabase：： Open](#open)|建立与数据库的连接。|
|[CDaoDatabase：： SetQueryTimeout](#setquerytimeout)|设置在 ODBC 数据源上 (的数据库查询操作仅) 超时之前的秒数。影响所有后续的已打开、添加、更新和删除操作。|
...
有关支持的数据库格式的信息，请参阅 [GetName](../../mfc/reference/cdaoworkspace-class.md#getname) 成员函数。 在 `CDaoDatabase` 给定的 "工作区" 中，可以有一个或多个对象处于活动状态，由 [CDaoWorkspace](../../mfc/reference/cdaoworkspace-class.md) 对象表示。 工作区维护一组打开的数据库对象，称为数据库集合。

## <a name="usage"></a>使用情况

创建记录集对象时，可以隐式创建数据库对象。 但您也可以显式创建数据库对象。 若要使用显式使用现有数据库 `CDaoDatabase` ，请执行以下操作之一：

- 构造一个 `CDaoDatabase` 对象，并将指针传递到打开的 [CDaoWorkspace](../../mfc/reference/cdaoworkspace-class.md) 对象。

- 或构造一个 `CDaoDatabase` 对象而不指定工作区 (MFC 创建一个临时工作区对象) 。

创建新的 Microsoft Jet (。MDB) 数据库，构造 `CDaoDatabase` 对象并调用其 [Create](#create) 成员函数。 不要 `Open` 在之后调用 `Create` 。

若要打开现有数据库，请构造一个 `CDaoDatabase` 对象并调用其 [open](#open) 成员函数。

这些方法中的任何一种都将 DAO 数据库对象追加到工作区的数据库集合，并打开与该数据的连接。 然后，在连接的数据库上构造用于操作的 [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md)、 [CDaoTableDef](../../mfc/reference/cdaotabledef-class.md)或 [CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md) 对象时，为这些对象传递构造函数，以指向 `CDaoDatabase` 对象的指针。 使用完连接后，调用 [Close](#close) 成员函数并销毁 `CDaoDatabase` 对象。 `Close` 关闭之前未关闭的任何记录集。

## <a name="transactions"></a>事务

在工作区级别提供数据库事务处理-请参阅类的 [BeginTrans](../../mfc/reference/cdaoworkspace-class.md#begintrans)、 [CommitTrans](../../mfc/reference/cdaoworkspace-class.md#committrans)和 [Rollback](../../mfc/reference/cdaoworkspace-class.md#rollback) 成员函数 `CDaoWorkspace` 。
...
调用此成员函数以确定对象是否 `CDaoDatabase` 允许更新。
...
如果 `CDaoDatabase` 对象允许更新，则为非零; 否则为0，指示打开对象时在 *bReadOnly* 中传递了 TRUE， `CDaoDatabase` 或者数据库本身是只读的。 请参见 [开放式](#open) 成员函数。
...
*pWorkspace*<br/>
指向 `CDaoWorkspace` 对象的指针，该对象将包含新的数据库对象。 如果接受默认值 NULL，则构造函数将创建一个 `CDaoWorkspace` 使用默认 DAO 工作区的临时对象。 可以通过 [m_pWorkspace](#m_pworkspace) 数据成员获取指向工作区对象的指针。
...
你可能会发现， `CDaoDatabase` 在文档类中嵌入对象会很方便。

> [!NOTE]
> `CDaoDatabase`如果在不传递指向现有对象的指针的情况下打开[CDaoRecordset](../../mfc/reference/cdaorecordset-class.md)对象，也会隐式创建一个对象 `CDaoDatabase` 。 关闭 recordset 对象时，将关闭此数据库对象。
...
在调用此成员函数之前，最好自行关闭这些对象。 关闭 `CDaoDatabase` 对象会将其从关联 [工作区](../../mfc/reference/cdaoworkspace-class.md)中的数据库集合中删除。 由于不 `Close` 会销毁 `CDaoDatabase` 对象，因此可以通过打开相同的数据库或不同的数据库重用该对象。

> [!CAUTION]
> 如果在[](../../mfc/reference/cdaorecordset-class.md#update)关闭数据库之前存在挂起的编辑) 和 `Close` 所有打开的记录集对象上的成员函数，则调用 Update 成员函数 (。 如果退出声明堆栈上的 [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md) 或对象的函数，则 `CDaoDatabase` 数据库将关闭，所有未保存的更改将丢失，所有挂起的事务都将回滚，并且对数据的任何挂起的编辑都将丢失。

> [!CAUTION]
> 如果在任何记录集对象处于打开状态时尝试关闭某个数据库对象，或者如果在打开某个工作区对象的任何数据库对象处于打开状态时尝试关闭该对象，则这些记录集对象将关闭，并且将回滚所有挂起的更新或编辑。 如果尝试在工作区对象的任何数据库对象处于打开状态时将其关闭，则该操作将关闭属于该特定工作区对象的所有数据库对象，这可能会导致关闭的记录集对象处于关闭状态。 如果未关闭数据库对象，则 MFC 会在调试版本中报告断言失败。
...
创建新的 Microsoft Jet (。MDB) 数据库，请在构造对象之后调用此成员函数 `CDaoDatabase` 。
...
*lpszName*<br/>
字符串表达式，是要创建的数据库文件的名称。 它可以是完整路径和文件名，如 "C： \\ \MYDB。MDB "。 必须提供名称。 如果未提供文件扩展名，则为。附加 MDB。 如果网络支持 (UNC) 的统一命名约定，则还可以指定网络路径，例如 " \\ \\ \\ \MYSERVER \\ \MYSHARE \\ \MYDIR \\ \MYDB"。 只有 Microsoft Jet (。可以使用此成员函数创建 MDB) 数据库文件。 字符串文本中需要 (双反斜杠，因为 " \\ " 是 c + + 转义符。 ) 

*lpszLocale*<br/>
一个字符串表达式，用于指定用于创建数据库的排序顺序。 默认值为 `dbLangGeneral`。 可能的值为：
...
- `dbLangNordic` 北欧语言 (仅) Microsoft Jet 数据库引擎版本1。0
...
`Create` 创建数据库文件和基础 DAO 数据库对象，并初始化 c + + 对象。 对象将追加到关联的工作区的数据库集合。 数据库对象处于打开状态;不要 `Open*` 在之后调用 `Create` 。
...
调用此成员函数以建立数据库中主表中的一个或多个字段之间的关系，以及一个或多个表中的一个或多个字段 (数据库) 中的另一个表。
...
*lpszForeignTable*<br/>
关系中外表的名称。 如果该表不存在，MFC 将引发类型为的异常 `CDaoException` 。

*lAttributes*<br/>
一个长整型值，该值包含关系类型的相关信息。 您可以使用此值来强制实施引用完整性。 您可以使用按位 "或" 运算符 ( **&#124;**) 将以下任意 (值组合在一起，前提是该组合可以) ：
...
- `dbRelationUpdateCascade` 更新将级联 (有关级联的详细信息，请参阅 "备注") 。

- `dbRelationDeleteCascade` 删除将级联。

*lpszField*<br/>
指向以 null 结尾的字符串的指针，该字符串包含由 *lpszTable*) 命名 (主表中的字段的名称。

*lpszForeignField*<br/>
指向以 null 结尾的字符串的指针，该字符串包含由 *lpszForeignTable*) 命名的外部 (表中的字段的名称。
...
此操作创建基础 DAO 关系对象，但这是 MFC 实现的详细信息，因为关系对象的 MFC 封装包含在类中 `CDaoDatabase` 。 MFC 不提供关系的类。
...
有关创建 querydef 对象的信息，请参阅类 [CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md)。 构造对象时，querydef 对象将与特定对象相关联 `CDaoDatabase` `CDaoQueryDef` ，并向其传递指向数据库对象的指针。
...
有关创建 tabledef 对象的信息，请参阅 [CDaoTableDef](../../mfc/reference/cdaotabledef-class.md)类。 构造对象时，tabledef 对象将与特定对象相关联 `CDaoDatabase` `CDaoTableDef` ，并向其传递指向数据库对象的指针。
...
*nOptions*<br/>
一个整数，指定与查询的完整性相关的选项。 您可以使用按位 "或" 运算符 ( **&#124;**) 组合任何以下常量， (提供这种组合是有意义的，例如，不能 `dbInconsistent` 与) 合并 `dbConsistent` ：

- `dbDenyWrite` 拒绝其他用户的写入权限。

- `dbInconsistent` (默认) 不一致的更新。
...
> [!NOTE]
> 如果同时 `dbInconsistent` `dbConsistent` 包含和，或者两者都不包括，则结果为默认值。 有关这些常量的说明，请参阅 DAO 帮助中的 "执行方法" 主题。
...
> [!TIP]
> 给定语法正确的 SQL 语句和适当的权限， `Execute` 即使不能修改或删除单个行，成员函数也不会失败。 因此， `dbFailOnError` 使用 `Execute` 成员函数运行 update 或 delete 查询时，请始终使用选项。 此选项将导致 MFC 引发 [CDaoException](../../mfc/reference/cdaoexception-class.md) 类型的异常，并在受影响的任何记录被锁定且无法更新或删除时回滚所有成功的更改。 请注意，你始终可以调用 `GetRecordsAffected` 来查看受影响的记录数。

调用数据库对象的 [GetRecordsAffected](#getrecordsaffected) 成员函数，以确定受最近调用影响的记录数 `Execute` 。 例如， `GetRecordsAffected` 返回有关执行操作查询时删除、更新或插入的记录数的信息。 当级联更新或删除有效时，返回的计数不会反映相关表中的更改。

`Execute` 不返回记录集。 `Execute`在选择记录的查询上使用将导致 MFC 引发类型为的异常 `CDaoException` 。  (没有 `ExecuteSQL` 类似于的成员函数 `CDatabase::ExecuteSQL` 。 ) 
...
如果已在 ODBC 数据源上成功调用 [Open](#open) ，则为连接字符串;否则为空字符串。 用于 Microsoft Jet (。MDB) 数据库中，除非将其设置为与 `dbSQLPassThrough` 用于 [Execute](#execute) 成员函数的选项一起使用，或者用于打开记录集，否则字符串始终为空。
...
> [!NOTE]
> 连接字符串用于根据需要将附加信息传递给 ODBC 和某些 ISAM 驱动程序。 它不用于。MDB 数据库。 对于 Microsoft Jet 数据库基表，连接字符串为空字符串 ( "" ) ，除非将其用于 SQL 传递查询，如以上返回值中所述。

有关如何创建连接字符串的说明，请参阅 [Open](#open) 成员函数。 在调用中设置连接字符串后 `Open` ，您可以在以后使用它来检查设置，以确定数据库的类型、路径、用户 ID、密码或 ODBC 数据源。
...
如果你的网络支持 (UNC) 的统一命名约定，则还可以指定网络路径，例如 " \\ \\ \\ \MYSERVER \\ \MYSHARE \\ \MYDIR \\ \MYDB。MDB "。 字符串文本中需要 (双反斜杠，因为 " \\ " 是 c + + 转义符。 ) 

例如，您可能希望在标题中显示此名称。 如果在检索名称时出现错误，MFC 将引发类型为 [CDaoException](../../mfc/reference/cdaoexception-class.md)的异常。

> [!NOTE]
> 为了在访问外部数据库时获得更好的性能，我们建议你将外部数据库表附加到 Microsoft Jet 数据库 (。MDB) 而不是直接连接到数据源。
...
|包含的目录。DBF 文件 (s) |dBASE 数据库|
|包含的目录。XLS 文件|Microsoft Excel 数据库|
|包含的目录。PDX 文件 (s) |Paradox 数据库|
|包含格式正确的文本数据库文件的目录|文本格式数据库|

对于 ODBC 数据库（如 SQL Server 和 Oracle），数据库的连接字符串会标识 ODBC 注册的 (DSN) 的数据源名称。
...
- AFX_DAO_PRIMARY_INFO (默认) 名称中，键入
...
由于网络访问问题、过多的查询处理时间等，操作可能会超时。 设置生效时，它会影响对与此对象关联的任何记录集的所有打开、添加、更新和删除操作 `CDaoDatabase` 。 可以通过调用 [SetQueryTimeout](#setquerytimeout)来更改当前超时设置。 在打开后更改记录集的查询超时值不会更改记录集的值。 例如，后续 [移动](../../mfc/reference/cdaorecordset-class.md#move) 操作不使用新值。 初始化数据库引擎时，最初设置默认值。
...
返回的值包括通过运行操作查询所删除、更新或插入的记录数 `Execute` 。 当级联更新或删除有效时，返回的计数不会反映相关表中的更改。
...
- AFX_DAO_PRIMARY_INFO (默认) 名称、表、外部表
...
> [!NOTE]
> 如果将关系对象的属性设置为激活级联操作 (`dbRelationUpdateCascades` 或 `dbRelationDeleteCascades`) ，则在对相关主键表进行更改时，Microsoft Jet 数据库引擎会自动更新或删除一个或多个其他表中的记录。 例如，假设您在 Customers 表和 Orders 表之间建立了级联删除关系。 删除 Customers 表中的记录时，也会删除与该客户相关的 Orders 表中的记录。 此外，如果您在 Orders 表和其他表之间建立了级联删除关系，则当您从 Customers 表中删除记录时，将自动删除这些表中的记录。
...
- AFX_DAO_PRIMARY_INFO (默认) 名称，可更新，特性
...
返回的值以 "主要. 次要" 格式表示版本号;例如 "3.0"。 产品版本号 (例如，3.0) 包含版本号 (3) 、句点和发布号 (0) 。 版本截止日期为1.0、1.1、2.0 和3.0。
...
包含一个指针，该指针指向对象基础的 DAO 数据库对象的 OLE 接口 `CDaoDatabase` 。
...
必须调用此成员函数来初始化 `CDaoDatabase` 表示现有数据库的新构造对象。
...
*lpszName*<br/>
作为现有 Microsoft Jet ( 的名称的字符串表达式。MDB) 数据库文件。 如果文件名有扩展名，则它是必需的。 如果你的网络支持 (UNC) 的统一命名约定，则还可以指定网络路径，例如 " \\ \\ \\ \MYSERVER \\ \MYSHARE \\ \MYDIR \\ \MYDB。MDB "。 字符串文本中需要 (双反斜杠，因为 " \\ " 是 c + + 转义符。 ) 
...
- 否则，不引用现有数据库或有效的 ODBC 数据源名称，MFC 将引发类型为的异常 `CDaoException` 。

> [!NOTE]
> 有关 DAO 错误代码的详细信息，请参阅 DAOERR。H 文件。 有关相关信息，请参阅 DAO 帮助中的 "可捕获的数据访问错误" 主题。

*bExclusive*<br/>
一个布尔值，如果要以独占方式 (非共享) 访问打开数据库，则该值为 TRUE; 如果要为共享访问打开数据库，则为 FALSE。 如果省略此参数，则将为共享访问打开数据库。

*bReadOnly*<br/>
一个布尔值，如果要打开数据库以进行只读访问，则为 TRUE; 如果要打开数据库以进行读/写访问，则为 FALSE。 如果省略此参数，则会打开数据库以进行读/写访问。 所有依赖记录集均继承此属性。

*lpszConnect*<br/>
用于打开数据库的字符串表达式。 此字符串构成 ODBC 连接参数。 必须提供独占和只读参数以提供源字符串。 如果数据库是 Microsoft Jet 数据库 (。MDB) 中，此字符串为空 ( "" ) 。 默认值（ **_T** ( "" ) ）的语法为 Unicode 以及应用程序的 ANSI 版本提供可移植性。
...
- 如果要打开 Microsoft Jet (。MDB) 数据库中，使用 *lpszName* 参数，并为 *lpszConnect* 参数传递空字符串或传递格式为 ";PWD = password "如果数据库是受密码保护的 (。MDB 数据库只) 。
...
> [!NOTE]
> 为了获得更好的性能，以便在访问外部数据库（包括 ISAM 数据库和 ODBC 数据源）时获得更好的性能，建议您将外部数据库表附加到 Microsoft Jet engine 数据库 (。MDB) 而不是直接连接到数据源。

例如，如果 DBMS 主机不可用，则连接尝试可能会超时。 如果连接尝试失败， `Open` 则会引发类型为 [CDaoException](../../mfc/reference/cdaoexception-class.md)的异常。

其余备注仅适用于 ODBC 数据库：

如果数据库是 ODBC 数据库，而您的调用中的参数 `Open` 未包含足够的信息来建立连接，ODBC 驱动程序将打开一个对话框，以获取用户所需的信息。 当你调用时 `Open` ，你的连接字符串 *lpszConnect* 将私下存储，并通过调用 [GetConnect](#getconnect) 成员函数提供。

如果需要，您可以在调用 `Open` 以从用户那里获取信息（如密码）之前打开自己的对话框，然后将该信息添加到您传递到的连接字符串中 `Open` 。 或者，您可能想要保存您传递 (传递的连接字符串) 以便您可以在下次应用程序对对象调用时重用它 `Open` `CDaoDatabase` 。

你还可以将连接字符串用于多个级别的登录授权 (分别用于不同的 `CDaoDatabase` 对象) 或传达其他特定于数据库的信息。
...
由于网络访问问题、过多查询处理时间等，操作可能会超时。 `SetQueryTimeout`如果要更改查询超时值，请在打开记录集之前或在调用记录集的[AddNew](../../mfc/reference/cdaorecordset-class.md#addnew)、 [Update](../../mfc/reference/cdaorecordset-class.md#update)或[Delete](../../mfc/reference/cdaorecordset-class.md#delete)成员函数之前调用。 此设置会影响[](../../mfc/reference/cdaorecordset-class.md#open) `AddNew` `Update` `Delete` 对与此对象关联的任何记录集 `CDaoDatabase` 的所有后续打开的、、和调用。 在打开后更改记录集的查询超时值不会更改记录集的值。 例如，后续 [移动](../../mfc/reference/cdaorecordset-class.md#move) 操作不使用新值。
