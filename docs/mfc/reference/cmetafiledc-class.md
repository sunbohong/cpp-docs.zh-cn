---
description: 了解详细信息： CMetaFileDC 类
title: CMetaFileDC 类
ms.date: 11/04/2016
f1_keywords:
- CMetaFileDC
...
若要实现 Windows 图元文件，请首先创建一个 `CMetaFileDC` 对象。 调用 `CMetaFileDC` 构造函数，然后调用 [Create](#create) 成员函数，该函数会创建一个 Windows 图元文件设备上下文，并将其附加到 `CMetaFileDC` 对象。

接下来，向该对象发送一个要 `CMetaFileDC` 用于重播的 CDC GDI 命令序列。 仅可使用那些创建输出的 GDI 命令，如 `MoveTo` 和 `LineTo` 。
...
您还可以实现 `CMetaFileDC` 对象，以便它可以处理两个输出调用和属性 GDI 调用（如） `GetTextExtent` 。 此类图元文件更灵活，并且可以更轻松地重复使用常规 GDI 代码，这通常由输出和属性调用的混合组成。 `CMetaFileDC`类从 CDC 继承两个设备上下文 `m_hDC` 和 `m_hAttribDC` 。 `m_hDC`设备上下文处理所有[cdc](../../mfc/reference/cdc-class.md) gdi 输出调用， `m_hAttribDC` 设备上下文处理所有 cdc gdi 属性调用。 通常，这两个设备上下文引用相同的设备。 对于 `CMetaFileDC` ，默认情况下，属性 DC 设置为 NULL。

创建指向屏幕、打印机或设备而不是图元文件的第二个设备上下文，然后调用 `SetAttribDC` 成员函数以将新的设备上下文与相关联 `m_hAttribDC` 。 此时，对信息的 GDI 调用将定向到新的 `m_hAttribDC` 。 输出 GDI 调用将转入 `m_hDC` ，后者表示图元文件。
...
当应用程序不再需要增强型图元文件句柄时，应通过调用 Win32 函数来释放该句柄 `DeleteEnhMetaFile` 。

## <a name="cmetafiledccmetafiledc"></a><a name="cmetafiledc"></a> CMetaFileDC：： CMetaFileDC

`CMetaFileDC`按两个步骤构造对象。
...
首先，调用构造函数 `CMetaFileDC` ，然后调用 `Create` ，这将创建 Windows 图元文件设备上下文并将其附加到 `CMetaFileDC` 对象。
...
*lpszFileName*<br/>
指向以 null 值结束的字符串。 指定要创建的增强型图元文件的文件名。 如果此参数为 NULL，则增强型图元文件将基于内存，当对象被销毁或调用 Win32 函数时，它的内容将丢失 `DeleteEnhMetaFile` 。

*lpBounds*<br/>
指向 [RECT](/windows/win32/api/windef/ns-windef-rect) 数据结构或 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象，它以 .01-毫米的增量) 指定要存储在增强型图元文件中的图像的 (尺寸。
...
LpBounds 参数指向的数据结构的左侧成员和顶级成员 `RECT` 必须分别小于右成员和下成员。 沿矩形边缘的点包括在图片中。 如果 *lpBounds* 为 NULL，图形设备接口 (GDI) 计算可包含应用程序绘制的图片的最小矩形的尺寸。 应尽可能提供 *lpBounds* 参数。

*LpszDescription* 参数指向的字符串在应用程序名称和图片名称之间必须包含 null 字符，并且必须以两个空字符（例如 "XYZ Graphics Editor\0Bald Eagle\0\0"）结尾，其中 \ 0 表示 null 字符。 如果 *lpszDescription* 为 NULL，则增强型图元文件头中没有对应的条目。

应用程序使用此函数创建的 DC 将图形图片存储在增强型图元文件中。 标识此 DC 的句柄可传递给任何 GDI 函数。

应用程序在增强型图元文件中存储图片后，可以通过调用函数在任何输出设备上显示该图片 `CDC::PlayMetaFile` 。 显示图片时，Windows 使用由 *lpBounds* 参数指向的矩形，并使用来自引用设备的解析数据来定位和缩放图片。 此函数返回的设备上下文包含与任何新 DC 关联的相同的默认属性。
