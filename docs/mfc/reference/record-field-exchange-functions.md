...
如果你尚未在基于 ODBC 的类中实现批量行提取，或者使用基于 DAO 的类 (过时的) ，则 ClassWizard 将重写 `DoFieldExchange` 或的成员函数， `CRecordset` `CDaoRecordset` 方法是：对 ODBC 类 (调用 RFX 函数) 或为记录集中的每个字段数据成员调用 DAO 类 (的 DFX 函数。
...
函数的第二个版本使用对结构的引用 `TIMESTAMP_STRUCT` 。 在调用之前，必须自行设置此结构。 对于此版本，不提供对话框数据交换 (DDX) 支持或代码向导支持。 函数的第三个版本的工作方式类似于第一个版本，只不过它采用对 [COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md) 对象的引用。
...
使用类 [CLongBinary](clongbinary-class.md) 在对象的字段数据成员 `CRecordset` 和 ODBC 类型 SQL_LONGVARBINARY 或 SQL_LONGVARCHAR 的数据源中记录的列之间传输 (BLOB) 数据的二进制大型对象。
...
有关详细信息，请参阅文章 [记录集：批量提取记录 (ODBC) ](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md) 并 [记录字段交换 (RFX) ](../../data/odbc/record-field-exchange-rfx.md)。
...
数据在 DAO (中的类型 DAO_CHAR 之间进行映射，或者，如果定义了符号 _UNICODE，则 DAO_WCHAR) 并在记录集中键入 [CString](../../atl-mfc-shared/reference/cstringt-class.md) 。  n
