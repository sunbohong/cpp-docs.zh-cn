---
description: 了解详细信息： COleControl 类
title: COleControl 类
ms.date: 08/27/2018
f1_keywords:
- COleControl
...
|[COleControl：： OnGetViewExtent](#ongetviewextent)|重写以检索控件显示区域的大小， (可用于启用两遍绘图) 。|
...
|[COleControl：： OnWindowlessMessage](#onwindowlessmessage)|处理窗口消息 (鼠标和键盘消息) ，无窗口控件。|
...
控件不需要窗口。 Windows 提供的服务可以通过单个共享窗口轻松地提供 (通常是容器的) 和一个调度代码。 在此对象上，有一个窗口通常是不必要的。

使用无窗口激活时，具有窗口) 的容器 (负责提供其他已由控件自己的窗口提供的服务。 例如，如果您的控件需要查询键盘焦点、查询鼠标捕获或获取设备上下文，则这些操作由容器管理。 `COleControl`无窗口操作成员函数在容器上调用这些操作。

启用无窗口激活时，容器会将输入消息委托给控件的 `IOleInPlaceObjectWindowless` 接口， (扩展 [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject) 的扩展，) 。 `COleControl`当调整鼠标坐标后，此接口的实现将通过控件的消息映射调度这些消息。 您可以通过将相应条目添加到消息映射来处理这些消息，如普通窗口消息。
...
如果容器处于用户模式，则为非零值;否则为 0 (设计模式) 。 如果此属性不受支持，则此函数返回 TRUE。
...
*pPoint*<br/>
指向 OLE 工作区点的指针，该指针将转换为父 (容器) 的坐标。

### <a name="remarks"></a>备注

输入 *pPoint* 是相对于 OLE 控件的工作区的起点 (控件) 的工作区的左上角。 在 output *pPoint* 上，相对于) 容器的父级 (左上角的原点。
...
无窗口对象在不事先检查插入符号是否部分或完全隐藏了重叠对象的情况下，不能安全地显示插入符号。 为了实现此目的，对象可以使用 `ClipCaretRect` 来获取插入符号调整后的 (降低) ，以确保它适合于剪辑区域。
...
在处理 stock 错误事件之后，由框架调用 (除非事件处理程序已禁止显示错误) 。
...
序列化或初始化控件的区的状态 (其在 HIMETRIC units) 中的维度。
...
您可以通过将相应条目添加到消息映射来处理这些消息，就像普通的窗口消息一样。 在处理程序中，避免使用 `m_hWnd` 成员变量 (或) 使用成员变量，而不首先检查其值是否为非 NULL。
...
返回值指定当前外观设置作为 **`short`** (VT_I2) 值（如果成功）。 如果控件的外观为平面，则此值为零; 如果控件的外观为三维，则为1。
...
如果控件已激活且无窗口，则 **`this`** 当控件当前具有鼠标捕获时，将返回 (由控件的容器) 确定，如果没有捕获，则返回 NULL。

否则，将返回 `CWnd` 具有鼠标捕获的对象 (与) 相同 `CWnd::GetCapture` 。
...
|名称|描述|
|-|-|
|`fastBeginPaint`|如果设置，则使用为 OLE 控件而不是 [BeginPaint](/windows/win32/api/winuser/nf-winuser-beginpaint) API 定制的 begin 画图函数 (默认情况下) 设置。|
...
- OLEDC_OFFSCREEN 通知容器：对象要呈现到屏幕外位图，随后应将该位图复制到屏幕上。 当对象要执行的绘图操作生成很多闪烁时，对象应使用此标志。 容器可随意接受此请求。 但是，如果未设置此标志，则容器必须手动取回屏幕 DC。 这允许对象执行直接屏幕操作，例如通过 **XOR** 运算) 显示选择 (。
...
如果控件已激活且无窗口，则 **`this`** 当控件当前具有键盘焦点时，如果控件的容器)  (确定，则返回; 如果该控件没有焦点，则返回 NULL。

否则，将返回 `CWnd` 焦点与)  (相同的对象 `CWnd::GetFocus` 。
...
`GetStockTextMetrics`如果成功，函数会将 lptm 所指向的结构初始化为 `TEXTMETRIC` 有效的指标信息，如果不成功，则用零填充结构。  在绘制控件时使用此函数，而不是 [GetTextMetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) ，因为控件（如任何嵌入的 OLE 对象）可能需要将其呈现到图元文件中。
...
另一种情况是，如果您的 VBX 控件在其 VBM_SAVEPROPERTY 消息处理程序) 中保存了专有的二进制数据 (，并且您的 OLE 控件以不同的格式保存其二进制数据。 如果希望 OLE 控件与 VBX 控件向后兼容，则可以 `IsConvertingVBX` 通过区分 VBX 控件或 OLE 控件是否正在加载来使用函数来读取旧格式和新格式。

在控件的 `DoPropExchange` 函数中，可以检查此条件，如果为 true，则执行特定于此转换的加载代码 (例如前面的示例) 。 如果控件未进行转换，则可以执行正常加载代码。 此功能仅适用于从 VBX 对应项转换的控件。
...
框架对的调用的 `IDispatch::Invoke` 实现 `IsInvokeAllowed` ，以确定是否可以调用给定函数 (由 `dispid`) 标识。 OLE 控件的默认行为是仅当控件已初始化时才允许调用自动化方法;不过， `IsInvokeAllowed` 是一个虚函数，如有必要，可能会被重写 (例如，将控件用作自动化服务器) 时。
...
如果支持优化的绘图，则在绘制完成时，控件不需要选择 (笔、画笔、字体等 ) 到设备上下文中的旧对象。
...
*hWndParent*<br/>
控件的父窗口的句柄。 如果执行谓词 (或 windows) 创建窗口，则应将 *hWndParent* 用作父项。
...
有关合并 OLE 资源的详细信息，请参阅文章 [ (OLE) 的菜单和资源 ](../../mfc/menus-and-resources-ole.md)。
...
- DVEXTENT_CONTENT 查询控件大小，以使内容完全适合 (对齐大小) 
...
对象的大小由转换为一个 `OnGetViewRect` 矩形，从特定位置开始 (默认为显示) 的左上角。 如果控件使用两遍绘图，并且它的透明和透明部分具有不同的尺寸，则重写此函数。
...
|VIEWSTATUS_SOLIDBKGND|对象的纯色背景 (包含纯色，而非画笔图案) 。 只有在设置 VIEWSTATUS_OPAQUE 并仅适用于与内容相关的方面，而不是 DVASPECT_ICON 或 DVASPECT_DOCPRINT 时，此位才有意义。|
...
请注意，窗口客户端坐标 (像素) 用于传递鼠标光标位置。 这是通过同时传递同一坐标系统中的对象的边框来实现的。
...
- HITRESULT_CLOSE *ptlLoc* 位于 OLE 对象内部或外部，但接近于要在其中被视为的对象。 小型、瘦或详细对象可能会使用此值。 即使某个点位于对象的边框外，它仍可能会关闭 (需要此方法才能) 命中小对象。
...
*prcLoc*<br/>
指向结构的指针，该 `RECT` 结构指定要检查的命中的矩形 (与对象矩形重叠) 相对于对象的左上角。
...
查询对象的显示矩形是否与给定矩形中的任何点重叠， (命中) 的矩形。 `QueryHitRect` 可以重写以测试非矩形对象的命中。
...
由于无窗口对象没有窗口，因此它们需要一个机制让容器向它们分派消息。 无窗口 OLE 对象通过接口上的方法从其容器获取消息 `OnWindowMessage` `IOleInPlaceObjectWindowless` (扩展 [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject) 以实现无窗口支持) 。 `OnWindowMessage` 不采用 `HWND` 参数。
...
如果 *bHitTest* 为 FALSE，则返回 HTNOWHERE。 如果 *bHitTest* 为 TRUE，则返回父 (容器在 OLE 控件的工作区中) 点着陆的位置，它是以下鼠标命中测试值之一：
...
- HTERROR 在屏幕背景上，或在 windows (与 HTNOWHERE 之间的分隔线上，只是 `DefWndProc` windows 函数发出系统提示音，指出) 错误。
...
输入 *pPoint* 相对于) 容器的父级 (左上角的原点。 Output *pPoint* 相对于 OLE 控件的工作区 (左上角) 控件工作区的左上角。
...
*sAppearance*<br/>
一个 **`short`** (VT_I2 用于控件外观的) 值。 如果值为零，则将控件的外观设置为平面，值为1时，将控件的外观设置为三维。
