---
description: 了解详细信息： CDaoQueryDef 类
...
|[CDaoQueryDef：： GetConnect](#getconnect)|返回与 querydef 关联的连接字符串。 连接字符串标识数据源。 仅适用于 SQL 传递查询的 (;否则为空字符串。 ) |
...
|[CDaoQueryDef::GetODBCTimeout](#getodbctimeout)|当执行 querydef 时，返回 odbc 查询) 使用的 ODBC (的超时值。 这将确定允许查询操作完成的时间长度。|
...
|[CDaoQueryDef::SetODBCTimeout](#setodbctimeout)|设置执行 querydef 时 odbc 查询) 所 (使用的超时值。|
...
> [!NOTE]
> DAO 数据库类不同于基于开放式数据库连接的 MFC 数据库类 (ODBC) 。 所有 DAO 数据库类名称都具有 "CDao" 前缀。 你仍可以使用 DAO 类访问 ODBC 数据源。 通常，基于 DAO 的 MFC 类比基于 ODBC 的 MFC 类更强大;基于 DAO 的类可以通过其自己的数据库引擎来访问数据，包括通过 ODBC 驱动程序。 基于 DAO 的类还支持数据定义语言 (DDL) 操作，例如通过类添加表，而无需直接调用 DAO。
...
不能使用 querydef 来创建记录集 (querydef) 最常见的用法，除非它处于打开状态。 通过调用或，将 querydef 置于打开状态 `Open` `Create` 。

## <a name="external-databases"></a>外部数据库

Querydef 对象是使用外部数据库引擎的本机 SQL 方言的首选方法。 例如，你可以在 Microsoft SQL Server) 上创建一个 Transact-sql 查询 (，并将其存储在 querydef 对象中。 如果需要使用不基于 Microsoft Jet 数据库引擎的 SQL 查询，则必须提供指向外部数据源的连接字符串。 使用有效连接字符串的查询将绕过数据库引擎，并将查询直接传递到外部数据库服务器进行处理。
...
- 如果对象表示临时 querydef (不会保存在数据库) 中，请调用 `Create` ，传递查询名称的空字符串。 调用后 `Create` ，通过直接设置其特性来初始化 querydef。 不要调用 `Append`。
...
关闭 querydef 会释放基础 DAO 对象，但不会销毁保存的 DAO querydef 对象或 c + + `CDaoQueryDef` 对象。 这不同于 [CDaoDatabase：:D eletequerydef](../../mfc/reference/cdaodatabase-class.md#deletequerydef)，如果不是临时 querydef) ，则会从 DAO (的数据库的 QueryDefs 集合中删除 querydef。
...
*nOptions*<br/>
一个整数，它确定查询的特征。 有关相关信息，请参阅 DAO 帮助中的 "执行方法" 主题。 可以使用按位 "或" 运算符 ( **&#124;**) 将以下常量组合在一起：
...
`Execute` 不适用于返回记录的查询，如 select 查询。 `Execute` 通常用于大容量操作查询，如 **UPDATE**、 **INSERT** 或 **SELECT INTO**，或 (DDL) 操作的数据定义语言。
...
此函数仅与 ODBC 数据源和某些 ISAM 驱动程序一起使用。 它不与 Microsoft Jet ( 一起使用。MDB) 数据库;在这种情况下， `GetConnect` 将返回一个空字符串。 有关详细信息，请参阅 [SetConnect](#setconnect)。
...
Querydef 必须处于打开状态，然后才能使用它来调用 [Execute](#execute) 或创建 [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md) 对象。 若要将 querydef 添加到 "打开状态" 调用，请 [创建](#create) 新 querydef 的 () 或 [打开](#open) 现有 querydef) 的 (。
...
此成员函数可让你覆盖在连接的数据源上的后续操作之前的默认秒数 "超时"。 由于网络访问问题、过多的查询处理时间等，操作可能会超时。 `SetODBCTimeout`如果要更改查询超时值，请在使用此 querydef 执行查询前调用。  (为 ODBC 重用连接，则相同连接上的所有客户端的超时值都是相同的。 ) 
...
的典型用法 `SetSQL` 是设置用于 SQL 传递查询的 querydef 对象。  (针对目标 DBMS 的 SQL 传递查询的语法，请参阅 DBMS 的文档。 ) 
