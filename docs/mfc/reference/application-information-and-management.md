...
ms.locfileid: "88832302"
---
# <a name="application-information-and-management"></a>应用程序信息和管理

编写应用程序时，将创建单个 [CWinApp](../../mfc/reference/cwinapp-class.md)派生对象。 有时，可能需要从派生的对象外部获取有关此对象的信息 `CWinApp` 。 或者你可能需要访问其他全局 "管理器" 对象。
...
|[AfxFreeLibrary](#afxfreelibrary)|将加载的动态链接库的引用计数递减 (DLL) 模块。 当引用计数达到零时，模块将被取消映射。|
|[AfxGetApp](#afxgetapp)|返回一个指向应用程序的单个对象的指针 `CWinApp` 。|
...
|[AfxWinInit](#afxwininit)|由 MFC 提供的函数调用 `WinMain` ，作为基于 GUI 的应用程序的 [CWinApp](../../mfc/reference/cwinapp-class.md) 初始化的一部分，用于初始化 MFC。 必须直接为使用 MFC 的控制台应用程序调用。|
...
- CREATE_SUSPENDED 启动线程，该线程的挂起计数为1。 如果要在 `CWinThread` 线程开始运行之前初始化对象的任何成员数据，如 [m_bAutoDelete](../../mfc/reference/cwinthread-class.md#m_bautodelete) 或派生类的任何成员，请使用 CREATE_SUSPENDED。 初始化完成后，请使用 [CWinThread：： ResumeThread](../../mfc/reference/cwinthread-class.md#resumethread) 启动运行的线程。 在调用之前，该线程不会执行 `CWinThread::ResumeThread` 。
...
的第一种形式是 `AfxBeginThread` 创建工作线程。 第二个窗体创建一个线程，该线程可用作用户界面线程或工作线程。

`AfxBeginThread` 创建一个新的 `CWinThread` 对象，调用其 [CreateThread](../../mfc/reference/cwinthread-class.md#createthread) 函数以开始执行线程，并返回指向该线程的指针。 在整个过程中进行检查，以确保在创建过程中的任何部分都不会正确释放所有对象。 若要结束线程，请从线程内调用 [AfxEndThread](#afxendthread) ，或从工作线程的控制函数返回。

应用程序必须启用多线程处理;否则，此函数将失败。 有关启用多线程处理的详细信息，请参阅 [/md、/mt、/ld (使用运行时库) ](../../build/reference/md-mt-ld-use-run-time-library.md)。

有关的详细信息 `AfxBeginThread` ，请参阅文章 [多线程处理：创建辅助线程](../../parallel/multithreading-creating-worker-threads.md) 和 [多线程处理：创建用户界面线程](../../parallel/multithreading-creating-user-interface-threads.md)。
...
有关的详细信息 `AfxEndThread` ，请参阅文章 [多线程处理：终止线程](../../parallel/multithreading-terminating-threads.md)。
...
`AfxFindResourceHandle` 查找特定的资源，并返回包含该资源的模块的句柄。 资源可能位于加载的任何 MFC 扩展 DLL 中。 `AfxFindResourceHandle` 将告知您哪个 DLL 包含该资源。
...
`AfxFreeLibrary` `AfxLoadLibrary` `FreeLibrary` `LoadLibrary` 如果你的应用程序使用多个线程，请确保使用和 (而不是 Win32 函数，并) 。 使用 `AfxLoadLibrary` 和 `AfxFreeLibrary` 可确保在加载并卸载 MFC 扩展 DLL 时执行的启动和关闭代码不会损坏全局 MFC 状态。
...
`AfxGetInstanceHandle` 始终返回可执行文件 ( 的 HINSTANCE。EXE) ，除非它是从与 MFC 的 USRDLL 版本链接的 DLL 中调用的。 在这种情况下，它会将 HINSTANCE 返回到 DLL。
...
如果为 TRUE，则指示将注册表信息定向到 HKCU 节点。 FALSE 表示应用程序将注册表信息写入默认节点。 默认节点 **HKEY_CLASSES_ROOT** (**HKCR**) 。
...
> [!NOTE]
> 如果要 `AfxGetThread` 从 Visual C++ 版本4.2、5.0 或6.0 中迁移从调用的 MFC 项目， `AfxGetThread` 则在找不到任何线程时调用 [AfxGetApp](#afxgetapp) 。 在最新版本的编译器中， `AfxGetThread` 如果未找到任何线程，则返回 NULL。 如果需要应用程序线程，则必须调用 `AfxGetApp`。
...
调用此函数可对应用程序 (版本 1.0) 初始化 rich edit 控件。
...
`AfxInitRichEdit` 加载 RICHED32.DLL 以初始化 rich edit 控件的1.0 版。 若要使用 rich edit 控件的2.0 和3.0 版本，需要加载 RICHED20.DLL。 它通过调用 [AfxInitRichEdit2](#afxinitrichedit2)进行加载。

若要将现有 Visual C++ 应用程序中的 rich edit 控件更新到版本2.0，请打开。RC 文件作为文本，将每个 rich edit 控件的类名从 "RICHEDIT" 更改为 "RichEdit20a"。 然后，将对的调用替换为 `AfxInitRichEdit` `AfxInitRichEdit2` 。

如果尚未为进程初始化库，此函数还会初始化公共控制库。 如果直接在 MFC 应用程序中使用 rich edit 控件，请调用此函数以确保 MFC 正确地初始化了丰富的编辑控件运行时。 如果调用 `Create` [CRichEditCtrl](../../mfc/reference/cricheditctrl-class.md)、 [CRichEditView](../../mfc/reference/cricheditview-class.md)或 [CRichEditDoc](../../mfc/reference/cricheditdoc-class.md)方法，则通常不需要调用此函数，但在某些情况下，可能需要这样做。
...
调用此函数以加载 RICHED20.DLL 并初始化 Rich Edit 控件 2.0 版。 如果调用 `Create` [CRichEditCtrl](../../mfc/reference/cricheditctrl-class.md)、 [CRichEditView](../../mfc/reference/cricheditview-class.md)或 [CRichEditDoc](../../mfc/reference/cricheditdoc-class.md)方法，则通常不需要调用此函数，但在某些情况下，可能需要这样做。
...
`TRUE`如果*pWnd*派生自，则此方法返回 `CMFCToolBar` 。 此方法在您必须验证函数或方法参数是否是 `CMFCToolBar` 对象时很有用。
...
*lpszModuleName*\
指向以 null 结尾的字符串，该字符串包含 (的模块的名称。DLL 或。EXE 文件) 。 指定的名称是模块的文件名。

如果字符串指定了一个路径，但该文件不在指定的目录中，则该函数将失败。

如果未指定路径并且省略了文件扩展名，则默认扩展名为。附加了 DLL。 但是，文件名字符串可以包含一个尾随的点字符 (. ) ，以指示该模块名称不包含扩展名。 如果未指定路径，则该函数将使用 [桌面应用程序的搜索顺序](/windows/win32/dlls/dynamic-link-library-search-order#search-order-for-desktop-applications)。
...
每个进程都维护每个已加载库模块的引用计数。 每次调用时，此引用计数都将递增 `AfxLoadLibrary` ，每次调用时，都将减少 `AfxFreeLibrary` 。 当引用计数达到零时，将从调用进程的地址空间取消映射模块，并且句柄不再有效。

`AfxLoadLibrary` `AfxFreeLibrary` `LoadLibrary` `FreeLibrary` 如果你的应用程序使用多个线程，并且它动态加载 MFC 扩展 DLL，则一定要使用和 (而不是 Win32 函数和) 。 使用 `AfxLoadLibrary` 并 `AfxFreeLibrary` 确保加载并卸载 MFC 扩展 DLL 时执行的启动和关闭代码不会损坏全局 MFC 状态。

`AfxLoadLibrary`在应用程序中使用需要动态链接到 MFC 的 DLL 版本。 `AfxLoadLibrary`仅当 MFC 作为 DLL 链接到应用程序时，才会包含 Afxdll_ 的头文件。 此要求是设计使然，因为必须链接到 MFC 的 DLL 版本才能使用或创建 MFC 扩展 Dll。
...
每个进程都维护每个已加载库模块的引用计数。 每次调用时，此引用计数都将递增 `AfxLoadLibraryEx` ，每次调用时，都将减少 `AfxFreeLibrary` 。 当引用计数达到零时，将从调用进程的地址空间取消映射模块，并且句柄不再有效。

`AfxLoadLibraryEx` `AfxFreeLibrary` `LoadLibraryEx` `FreeLibrary` 如果应用程序使用多个线程并且动态加载 MFC 扩展 DLL，请确保使用和 (而不是 Win32 函数，并) 。 使用 `AfxLoadLibraryEx` 和 `AfxFreeLibrary` 可确保在加载并卸载 MFC 扩展 DLL 时执行的启动和关闭代码不会损坏全局 MFC 状态。

`AfxLoadLibraryEx`在应用程序中使用需要动态链接到 MFC 的 DLL 版本。 `AfxLoadLibraryEx`仅当 MFC 作为 DLL 链接到应用程序时，才会包含 Afxdll_ 的头文件。 此要求是设计使然，因为必须链接到 MFC 的 DLL 版本才能使用或创建 MFC 扩展 Dll。
...
在非 DLL 生成中， `AfxRegisterClass` 标识符定义为映射到 Windows 函数的宏 `RegisterClass` ，因为在应用程序中注册的类会自动取消注册。 如果你使用 `AfxRegisterClass` 而不是 `RegisterClass` ，则可以使用你的代码，而无需在应用程序和 DLL 中进行更改。
...
包含类名的以 null 结尾的字符串。 可以将此类名称传递到 `Create` `CWnd` 或其他 **CWnd**派生类中的成员函数以创建窗口。 该名称由 Microsoft 基础类库生成。
...
由 `AfxRegisterWndClass` 为类注册的名称仅依赖于参数。 如果使用相同的参数调用 `AfxRegisterWndClass` 多次，它只会在首次调用时注册类。 以后 `AfxRegisterWndClass` 用相同的参数调用时，将返回已注册的类名。

如果使用相同的参数为多个 CWnd 派生类调用 `AfxRegisterWndClass`，而不是为每个类获得单独的窗口类，则每个类都将共享同一窗口类。 如果使用 CS_CLASSDC 类样式，此共享可能会导致问题。 除了多个 CS_CLASSDC 窗口类，最终只需要一个 CS_CLASSDC 的窗口类。 使用该类的所有 c + + 窗口共享同一 DC。 若要避免此问题，请调用 [AfxRegisterClass](#afxregisterclass) 来注册该类。

有关窗口类注册和函数的详细信息，请参阅技术说明 [TN001：窗口类注册](../../mfc/tn001-window-class-registration.md) `AfxRegisterWndClass` 。
...
*bEnable*\
中如果为 TRUE，则指示将注册表信息定向到 HKCU 节点。 FALSE 表示应用程序将注册表信息写入默认节点。 默认节点 **HKEY_CLASSES_ROOT** (**HKCR**) 。
...
*lpwsaData*\
指向 [WSADATA](/windows/win32/api/winsock2/ns-winsock2-wsadata) 结构的指针。 如果 *lpwsaData* 不等于 NULL，则 `WSADATA` 通过调用来填充结构的地址 `WSAStartup` 。 此函数还确保在应用程序终止前为您调用 `WSACleanup`。
...
如果您 `AfxWinInit` 亲自调用，则应声明一个类的实例 `CWinApp` 。 对于控制台应用程序，您可以选择不从派生您自己的类 `CWinApp` ，而是直接使用的实例 `CWinApp` 。 如果你决定将应用程序的所有功能保留在 **主**实现中，则此方法适用。
