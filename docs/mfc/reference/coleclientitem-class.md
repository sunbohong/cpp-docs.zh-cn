---
description: 了解详细信息： COleClientItem 类
...
|[COleClientItem：： GetClipboardData](#getclipboarddata)|通过调用成员函数，获取将放置在剪贴板上的数据 `CopyToClipboard` 。|
|[COleClientItem：： GetDocument](#getdocument)|返回 `COleDocument` 包含当前项的对象。|
...
|[COleClientItem：： GetType](#gettype)|返回 OLE 项 (嵌入、链接或静态) 的类型。|
...
|[COleClientItem：： Reload.sql](#reload)|在调用后重新加载项 `ActivateAs` 。|
...
|[COleClientItem：： SetLinkUpdateOptions](#setlinkupdateoptions)| (高级功能) 设置链接项的更新模式。|
...
> [!NOTE]
> Windows SDK 将嵌入项和链接项称为 "对象"，并将项的类型表示为 "类"。 此引用使用术语 "item" 来区分 OLE 实体与相应的 c + + 对象，并使用术语 "type" 将 OLE 类别与 c + + 类区分开来。
...
如果使用 Microsoft 基础类库编写服务器应用程序，则此函数将导致执行相应对象的 [OnDoVerb](../../mfc/reference/coleserveritem-class.md#ondoverb) 成员函数 `COleServerItem` 。

如果主谓词是 Edit 并且在 *nVerb* 参数中指定了 zero，则启动服务器应用程序以允许编辑该 OLE 项。 如果容器应用程序支持就地激活，则可以就地执行编辑。 如果容器不支持就地激活 (或者) 指定打开谓词，则将在单独的窗口中启动服务器，并可在该窗口中进行编辑。 通常，当容器应用程序的用户双击 OLE 项时， *nVerb* 参数中主要谓词的值将决定用户可以执行的操作。 但是，如果服务器仅支持一个操作，它将执行该操作，而无论在 *nVerb* 参数中指定了哪个值。
...
*rDataObject*<br/>
对 `COleDataObject` 对象的引用，该对象将进行初始化，以允许访问 OLE 项中的数据。
...
检查容器应用程序是否可以从给定的对象创建嵌入的对象 `COleDataObject` 。
...
如果容器可以从对象创建嵌入的对象，则 `COleDataObject` 为非零; 否则为0。
...
检查容器应用程序是否可以从给定的对象创建链接的对象 `COleDataObject` 。
...
如果容器可以从对象创建链接对象，则为非零值 `COleDataObject` 。
...
*dwCloseOption*<br/>
指定在 OLE 项返回到已加载状态时，该 OLE 项保存在什么情况下的标志。 可以具有以下一个值：
...
构造一个 `COleClientItem` 对象并将其添加到容器文档的文档项的集合中，这些文档项仅构造 c + + 对象，而不执行任何 OLE 初始化。
...
*lpFormatEtc*<br/>
指向 [FORMATETC](/windows/win32/api/objidl/ns-objidl-formatetc) 结构的指针，如果 OLERENDER_FORMAT 或 OLERENDER_DRAW *，则使用* 该结构。 仅当要指定超出 *cfFormat* 指定的剪贴板格式的其他格式信息时，才为此参数提供值。 如果省略此参数，则将对结构中的其他字段使用默认值 `FORMATETC` 。
...
通常，在 "编辑" 菜单上的 "粘贴" 命令的消息处理程序中调用此函数。  (如果 [CanPaste](#canpaste) 成员函数返回非零值，则该框架将启用粘贴命令。 ) 
...
数据传输操作（如从剪贴板或拖放操作中进行粘贴）提供的 `COleDataObject` 对象包含服务器应用程序提供的信息。 它通常用于重写 [CView：： system.windows.uielement.ondrop](../../mfc/reference/cview-class.md#ondrop)。
...
当用户指示应创建链接时，在删除操作期间调用此。 它还可用于处理编辑粘贴命令。 在 `COleClientItem::CreateLinkFromClipboard` 选择了 Link 选项后，和中的框架将调用该方法 [： COlePasteSpecialDialog：： CreateItem](../../mfc/reference/colepastespecialdialog-class.md#createitem) 。
...
静态项包含表示数据，而不包含本机数据;因此，不能对其进行编辑。 这基本上与 [CreateStaticFromClipboard](#createstaticfromclipboard) 相同，只是可以从任意剪贴板创建静态项 `COleDataObject` ，而不只是从剪贴板创建。
...
如果你的应用程序支持撤消，请不要调用， `Deactivate` 而应调用 [DeactivateUI](#deactivateui)。
...
此函数调用 [Release](#release) 成员函数，该函数反过来删除项的 c + + 对象，并从文档中永久删除 OLE 项。 如果 OLE 项是嵌入的，则该项的本机数据将被删除。 它始终关闭正在运行的服务器;因此，如果项是打开的链接，则此函数会将其关闭。
...
*lpBounds*<br/>
指向 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象或 `RECT` 结构的指针，该对象或结构定义用于在设备上下文) 确定的逻辑单元中绘制 OLE (项的边框。
...
函数可以使用由的 [OnDraw](../../mfc/reference/coleserveritem-class.md#ondraw) 成员函数创建的 OLE 项的图元文件表示形式 `COleServerItem` 。

通常，用于 `Draw` 屏幕显示，将屏幕设备上下文作为 *pDC* 传递。 在这种情况下，只需指定前两个参数。
...
*lpSize*<br/>
指向结构的指针 `SIZE` 或将接收大小信息的 [CSize](../../atl-mfc-shared/reference/csize-class.md) 对象。
...
此函数提供与 [GetExtent](#getextent)相同的信息。 但是，您可以调用 `GetCachedExtent` 来在处理其他 OLE 处理程序的过程中获取范围信息，如 [OnChange](#onchange)。 维度以 MM_HIMETRIC 单位为单位。

这是可能的，因为 `GetCachedExtent` 使用 [IViewObject2](/windows/win32/api/oleidl/nn-oleidl-iviewobject2) 接口，而不是使用 [IOleObject](/windows/win32/api/oleidl/nn-oleidl-ioleobject) 接口来获取此项的范围。 `IViewObject2`COM 对象缓存上一次对 IViewObject 的调用中使用的范围信息[：:D raw](/windows/win32/api/oleidl/nf-oleidl-iviewobject-draw)。
...
`GetClipboardData` 称为 [OnGetClipboardData](#ongetclipboarddata)的默认实现。 `OnGetClipboardData`仅当你除了提供的数据格式之外还需要提供数据格式时，重写 `CopyToClipboard` 。 在 `COleDataSource` 调用之前或之后将这些格式置于对象中 `CopyToClipboard` ，然后将该 `COleDataSource` 对象传递给 [COleDataSource：： SetClipboard](../../mfc/reference/coledatasource-class.md#setclipboard) 函数。 例如，如果想要将 OLE 项的容器文档中的该位置伴随在剪贴板上，则需要定义自己的格式以传递该信息，并将其放在 `COleDataSource` 调用之前 `CopyToClipboard` 。
...
通过此指针，可以访问 `COleDocument` 作为参数传递给 `COleClientItem` 构造函数的对象。
...
*lpSize*<br/>
指向结构的指针 `SIZE` 或 `CSize` 将接收大小信息的对象。
...
如果使用 Microsoft 基础类库编写服务器应用程序，则此函数将导致调用相应对象的 [OnGetExtent](../../mfc/reference/coleserveritem-class.md#ongetextent) 成员函数 `COleServerItem` 。 请注意，检索到的大小可能不同于 [SetExtent](#setextent) 成员函数最后设置的大小;指定的大小 `SetExtent` 被视为建议。 维度以 MM_HIMETRIC 单位为单位。

> [!NOTE]
> 不要在 `GetExtent` 处理 OLE 处理程序的过程中调用，如 [OnChange](#onchange)。 改为调用 [GetCachedExtent](#getcachedextent) 。
...
`COleClientItem::ItemState`枚举的值，可以是下列值之一： `emptyState` 、 `loadedState` 、 `openState` 、 `activeState` 、 `activeUIState` 。 有关这些状态的信息，请参阅文章 [容器： Client-Item 状态](../../mfc/containers-client-item-states.md)。

### <a name="remarks"></a>备注

若要在 OLE 项的状态发生更改时得到通知，请使用 [OnChange](#onchange) 成员函数。

有关详细信息，请参阅文章 [容器： Client-Item 状态](../../mfc/containers-client-item-states.md)。
...
- OLEUPDATE_ONCALL 在) 调用 [UpdateLink](#updatelink) 成员函数时，仅在容器应用程序 (的请求上更新链接项。 此选项支持 "链接" 对话框中的 "手动链接-更新" 单选按钮。
...
- USERCLASSTYPE_SHORT 一个短名称 (15 个字符的最大) ，以便在弹出菜单和 "编辑链接" 对话框中使用。
...
`OnActivateUI`当对象进入活动 UI 状态时，框架会调用。
...
*dwParam*<br/>
如果 *nCode* 是 OLE_SAVED 或 OLE_CLOSED，则不使用此参数。 如果 OLE_CHANGED *nCode* ，则此参数指定已更改的 OLE 项的特性。 有关可能的值，请参阅 [COleClientItem：:D raw](#draw)的 *dwParam* 参数。 如果 OLE_CHANGED_STATE *nCode* ，则此参数是 `COleClientItem::ItemState` 枚举值并描述所输入的状态。 它可以具有以下值之一： `emptyState` 、 `loadedState` 、 `openState` 、 `activeState` 或 `activeUIState` 。

### <a name="remarks"></a>备注

 (如果使用 Microsoft 基础类库编写服务器应用程序，则会调用此函数以响应 `Notify` 或的成员函数 `COleServerDoc` `COleServerItem` 。 ) 当 *nCode* 为 OLE_CHANGED 或 OLE_SAVED 时，默认实现会将容器文档标记为已修改。
...
当 OLE 项从就地活动状态转换 () 为加载状态时由框架调用 `activeState` ，这意味着在就地激活后将其停用。
...
如果服务器重复调用， *pMenuShared* 上的子菜单可能由多个复合菜单共享 `OnInsertMenus` 。 因此，不应删除重写中的任何子菜单 `OnRemoveMenus` ; 只应分离它们。
...
当就地激活开始和结束时，由框架调用两次;第一次安装复合菜单时，第二次 (将 *holemenu* 等于 NULL) 将其删除。
...
*pMenuShared*<br/>
一个指针，指向通过调用 [OnInsertMenus](#oninsertmenus) 成员函数和函数构造的复合菜单 `InsertMenu` 。

*holemenu*<br/>
函数返回的菜单描述符的句柄 `OleCreateMenuDescriptor` ，或者为 NULL （如果要移除调度代码）。
...
当容器应用程序支持指向嵌入 (项的链接时，如果已从 [COleLinkingDoc](../../mfc/reference/colelinkingdoc-class.md)) 派生文档类，则可以使用此方法。 此函数在就地激活期间调用，或者当 OLE 项为链接源并且用户要编辑它时调用。 默认实现激活容器文档中的第一个视图。 重写此函数以滚动文档，使 OLE 项可见。
...
默认实现不会更改框架窗口标题。 如果需要为应用程序提供不同的框架标题（例如 "docname 中的 *服务器应用* 项 ("），请重写此函数，例如 "  -   Microsoft Excel-REPORT.DOC 中的 Microsoft Excel-电子表格 ) "。  这是一种高级的可重写。
...
## <a name="coleclientitemreload"></a><a name="reload"></a> COleClientItem：： Reload.sql
...
`Reload`通过调用[ActivateAs](#activateas)将项激活为另一类型的项后调用该函数。
...
调用 `Run` 成员函数以在激活项之前启动服务器应用程序。 这是通过 [Activate](#activate) 和 [DoVerb](#doverb)自动完成的，因此通常不需要调用此函数。 如果在执行[DoVerb](#doverb)之前需要运行服务器以设置项属性（如[SetExtent](#setextent)），请调用此函数。
...
此函数由 "更改" 图标 (和直接调用 "更改图标" 对话框) 的其他对话框自动调用，以在用户请求时启用图标显示方位。
...
如果使用 Microsoft 基础类库编写服务器应用程序，这将导致调用相应对象的 [OnSetExtent](../../mfc/reference/coleserveritem-class.md#onsetextent) 成员函数 `COleServerItem` 。 然后，OLE 项可以相应地调整其显示。 维度必须以 MM_HIMETRIC 单位为单位。 当用户调整 OLE 项的大小或支持某种布局协商时，请调用此函数。
...
如果使用 Microsoft 基础类库编写服务器应用程序，则此函数将调用[](../../mfc/reference/coleserverdoc-class.md#onsethostnames) `COleServerDoc` 包含该 OLE 项的文档的 OnSetHostNames 成员函数。 在编辑 OLE 项时，将在窗口标题中使用此信息。 每次加载容器文档时，框架都会为文档中的所有 OLE 项调用此函数。 `SetHostNames` 仅适用于嵌入项。 每次激活嵌入式 OLE 项进行编辑时都不需要调用此函数。
...
此函数的参数包含 OLE 系统用于标识目标设备的信息。 `PRINTDLG`结构包含 Windows 用于初始化常见 "打印" 对话框的信息。 用户关闭对话框后，Windows 将返回有关此结构中的用户选择的信息。 `m_pd` [CPrintDialog](../../mfc/reference/cprintdialog-class.md)对象的成员是 `PRINTDLG` 结构。
