---
description: 了解详细信息：为反射消息定义消息处理程序
title: 为反射消息定义消息处理程序
ms.date: 09/07/2019
f1_keywords:
...
例如，利用此功能，您可以创建一个将重绘自身的列表框，而不是依赖于父窗口来执行此操作 (所有者) 。 有关反射消息的详细信息，请参阅 [处理反射的消息](../../mfc/handling-reflected-messages.md)。
...
1. 选择要为其定义处理程序的反射消息。 使用等号标记 (=) 来标记反射的消息。

1. 单击 "类向导" 右侧列中的单元，将建议的处理程序名称显示为 " \<add> *HandlerName*"。  (例如， **= WM_CTLCOLOR** 消息处理程序建议 \<add> **CTLCOLOR**) 。
