...
ms.locfileid: "97219403"
---
# <a name="mapping-messages-to-functions"></a>将消息映射到函数

使用 [类向导](mfc-class-wizard.md) 可以将消息处理程序绑定 (MFC 用户界面类的成员函数) 到应用程序的资源生成的消息。 它们使用 [MFC 消息映射](../../mfc/messages-and-commands-in-the-framework.md) 来创建绑定。

当你使用类视图创建从一个框架类派生的新类时，它会自动将一个完整且功能的类放置在标头中 () ，并 () 指定的文件。
