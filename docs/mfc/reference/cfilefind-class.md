---
description: 了解详细信息： CFileFind 类
title: CFileFind 类
ms.date: 11/04/2016
f1_keywords:
- CFileFind
...
|[CFileFind：： IsNormal](#isnormal)|确定找到的文件是否为正常 (换言之，不) 其他特性。|
...
`CFileFind` 用于搜索特定服务器类型的其他两个 MFC 类的基类：专用于 `CGopherFileFind` gopher 服务器，并 `CFtpFileFind` 专用于 FTP 服务器。 这三个类共同提供了一个无缝机制，使客户端能够在本地计算机或远程服务器上查找文件，而不考虑服务器协议、文件类型或位置。

下面的代码将枚举当前目录中的所有文件，并打印每个文件的名称：

[!code-cpp[NVC_MFCFiles#31](../../atl-mfc-shared/reference/codesnippet/cpp/cfilefind-class_1.cpp)]

为了简单起见，此代码使用 c + + 标准库 `cout` 类。 `cout` `CListBox::AddString` 例如，在具有图形用户界面的程序中，可以使用调用来替换该行。
...
调用后 `Close` ，在调用 FindFile 之前，无需创建新的 `CFileFind` 实例[](#findfile)即可开始新的搜索。
...
在调用 `FindFile` 开始文件搜索后，调用 [FindNextFile](#findnextfile) 检索后续文件。 在 `FindNextFile` 调用以下任何属性成员函数之前，必须至少调用一次：
...
在 `FindNextFile` 调用以下任何属性成员函数之前，必须至少调用一次：
...
如果成功，则为非零值;如果不成功，则为0。 `GetCreationTime` 仅当从未在此对象上调用 [FindNextFile](#findnextfile) 时返回 0 `CFileFind` 。

### <a name="remarks"></a>备注

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetCreationTime` 。
...
`GetFileName` 是 `CFileFind` 返回某种格式的文件名的三个成员函数之一。 下面的列表描述了三种方法和它们之间的区别：

- `GetFileName` 返回文件名（包括扩展名）。 例如，调用 `GetFileName` 以生成有关文件的用户消息 *c:\myhtml\myfile.txt* 返回 *myfile.txt* 的文件名。

- [GetFilePath](#getfilepath) 返回文件的整个路径。 例如，调用 `GetFilePath` 以生成有关文件的用户消息 *c:\myhtml\myfile.txt* 返回 *c:\myhtml\myfile.txt* 文件路径。

- [GetFileTitle](#getfiletitle) 返回文件名，不包括文件扩展名。 例如，调用 `GetFileTitle` 以生成有关文件的用户消息 *c:\myhtml\myfile.txt* 返回文件标题 *myfile.txt*。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetFilePath` 。

`GetFilePath` 是 `CFileFind` 返回某种格式的文件名的三个成员函数之一。 下面的列表描述了三种方法和它们之间的区别：

- [GetFileName](#getfilename) 返回文件名（包括扩展名）。 例如，调用 `GetFileName` 以生成有关文件的用户消息 *c:\myhtml\myfile.txt* 返回 *myfile.txt* 的文件名。

- `GetFilePath` 返回文件的整个路径。 例如，调用 `GetFilePath` 以生成有关文件的用户消息将 `c:\myhtml\myfile.txt` 返回文件路径 `c:\myhtml\myfile.txt` 。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetFileTitle` 。

`GetFileTitle` 是 `CFileFind` 返回某种格式的文件名的三个成员函数之一。 下面的列表描述了三种方法和它们之间的区别：

- [GetFileName](#getfilename) 返回文件名（包括扩展名）。 例如，调用 `GetFileName` 以生成有关文件的用户消息 *c:\myhtml\myfile.txt* 返回 *myfile.txt* 的文件名。

- [GetFilePath](#getfilepath) 返回文件的整个路径。 例如，调用 `GetFilePath` 以生成有关文件的用户消息 *c:\myhtml\myfile.txt* 返回 *c:\myhtml\myfile.txt* 文件路径。

- `GetFileTitle` 返回文件名，不包括文件扩展名。 例如，调用 `GetFileTitle` 以生成有关文件的用户消息 *c:\myhtml\myfile.txt* 返回文件标题 *myfile.txt*。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetFileURL` 。

`GetFileURL` 与成员函数 [GetFilePath](#getfilepath)类似，不同之处在于它以形式返回 URL `file://path` 。 例如，调用 `GetFileURL` 获取 *myfile.txt* 的完整 url 返回 url `file://c:\myhtml\myfile.txt` 。
...
如果成功，则为非零值;如果不成功，则为0。 `GetLastAccessTime` 仅当从未在此对象上调用 [FindNextFile](#findnextfile) 时返回 0 `CFileFind` 。

### <a name="remarks"></a>备注

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetLastAccessTime` 。
...
如果成功，则为非零值;如果不成功，则为0。 `GetLastWriteTime` 仅当从未在此对象上调用 [FindNextFile](#findnextfile) 时返回 0 `CFileFind` 。

### <a name="remarks"></a>备注

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetLastWriteTime` 。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetLength` 。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `GetRoot` 。

此成员函数返回用于启动搜索的驱动器说明符和路径名。 例如，在调用 [FindFile](#findfile) 时， `*.dat` `GetRoot` 将返回一个空字符串。 将路径（如）传递 `c:\windows\system\*.dll` 给 `FindFile` `GetRoot` 返回的结果 `c:\windows\system\` 。
...
应用程序将 FILE_ATTRIBUTE_ARCHIVE、 [WIN32_FIND_DATA](/windows/win32/api/minwinbase/ns-minwinbase-win32_find_dataw) 结构中标识的文件属性标记为备份或删除存档文件。

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsArchived` 。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsCompressed` 。
...
作为目录的文件标记为 FILE_ATTRIBUTE_DIRECTORY [WIN32_FIND_DATA](/windows/win32/api/minwinbase/ns-minwinbase-win32_find_dataw) 结构中标识的文件属性。

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsDirectory` 。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsDots` 。
...
用 FILE_ATTRIBUTE_HIDDEN 标记的隐藏文件， [WIN32_FIND_DATA](/windows/win32/api/minwinbase/ns-minwinbase-win32_find_dataw) 结构中标识的文件属性。 普通目录列表中不包含隐藏的文件。

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsHidden` 。
...
标记为 FILE_ATTRIBUTE_NORMAL 的文件， [WIN32_FIND_DATA](/windows/win32/api/minwinbase/ns-minwinbase-win32_find_dataw) 结构中标识的文件属性。 普通文件未设置其他属性。 所有其他文件属性都覆盖此属性。

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsNormal` 。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsReadOnly` 。
...
系统文件标记有 FILE_ATTRIBUTE_SYSTEM、 [WIN32_FIND_DATA](/windows/win32/api/minwinbase/ns-minwinbase-win32_find_dataw) 结构中标识的文件属性。 系统文件是的一部分，或由操作系统独占使用。

在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsSystem` 。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `IsTemporary` 。
...
*dwMask*<br/>
为找到的文件指定一个或多个在 [WIN32_FIND_DATA](/windows/win32/api/minwinbase/ns-minwinbase-win32_find_dataw) 结构中标识的文件属性。 若要搜索多个属性，请使用按位 "或" ( # A0) 运算符。 以下属性的任意组合都是可接受的：

- FILE_ATTRIBUTE_ARCHIVE 文件是存档文件。 应用程序使用此属性将文件标记为备份或删除。

- FILE_ATTRIBUTE_COMPRESSED 压缩文件或目录。 对于文件，这意味着文件中的所有数据都已压缩。 对于目录，这意味着压缩是新创建的文件和子目录的默认值。

- FILE_ATTRIBUTE_DIRECTORY 该文件是一个目录。

- FILE_ATTRIBUTE_NORMAL 文件没有设置其他属性。 仅当单独使用时，此特性才有效。 所有其他文件属性都覆盖此属性。
...
在调用之前，必须至少调用 [FindNextFile](#findnextfile) `MatchesMask` 。
