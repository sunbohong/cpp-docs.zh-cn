---
description: 了解详细信息： CGdiObject 类
title: CGdiObject 类
ms.date: 11/04/2016
f1_keywords:
- CGdiObject
...
该 `UnrealizeObject` 函数不应与 stock 对象一起使用。 `UnrealizeObject`每当使用[CDC：： SetBrushOrg](../../mfc/reference/cdc-class.md#setbrushorg)函数)  (设置新的画笔来源时，都必须调用函数。 `UnrealizeObject`对于当前选定的画笔或当前选择的任何显示上下文调色板，不能调用函数。
