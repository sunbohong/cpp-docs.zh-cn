---
description: 了解详细信息： CMFCMenuBar 类
title: CMFCMenuBar 类
ms.date: 10/18/2018
f1_keywords:
- CMFCMenuBar
...
|[CMFCMenuBar：： Create](#create)|创建一个菜单控件，并将其附加到 `CMFCMenuBar` 对象。|
|[CMFCMenuBar：： CreateEx](#createex)|创建 `CMFCMenuBar` 具有其他样式选项的对象。|
|[CMFCMenuBar：： CreateFromMenu](#createfrommenu)|初始化 `CMFCMenuBar` 对象。 接受充当填充的模板的 HMENU 参数 `CMFCMenuBar` 。|
...
|[CMFCMenuBar：： GetHMenu](#gethmenu)|返回附加到对象的菜单的句柄 `CMFCMenuBar` 。|
...
|[CMFCMenuBar：： LoadState](#loadstate)|`CMFCMenuBar`从注册表加载对象的状态。|
...
|[CMFCMenuBar：： SaveState](#savestate)|将对象的状态保存 `CMFCMenuBar` 到注册表中。|
...
`CMFCMenuBar` 支持用于显示最近使用的菜单项对象的选项。 如果启用此选项，则在 `CMFCMenuBar` 第一次查看时仅显示可用命令的子集。 此后，最近使用的命令与命令的原始子集一起显示在一起。 此外，用户始终可以展开菜单来查看所有可用命令。 因此，每个可用命令都配置为经常显示，或仅在最近选择的命令时显示。

若要使用 `CMFCMenuBar` 对象，请将其嵌入主窗口框架对象。 处理消息时 `WM_CREATE` ，请调用 `CMFCMenuBar::Create` 或 `CMFCMenuBar::CreateEx` 。 无论使用哪种 create 函数，都将传入指向主框架窗口的指针。 然后通过调用 [CFrameWndEx：： EnableDocking](../../mfc/reference/cframewndex-class.md#enabledocking)启用插接。 通过调用 CFrameWndEx，停靠此菜单 [：:D ockpane](../../mfc/reference/cframewndex-class.md#dockpane)。
...
构造 `CMFCMenuBar` 对象之后，必须调用 `Create` 。 此方法将创建 `CMFCMenuBar` 控件并将其附加到 `CMFCMenuBar` 对象。
...
*rcBorders*<br/>
中一个 `CRect` 参数，该参数指定对象边框的大小 `CMFCMenuBar` 。
...
下面的示例演示如何使用类的 `CreateEx` 方法 `CMFCMenuBar` 。 此代码片段是 [IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
初始化 [CMFCMenuBar](../../mfc/reference/cmfcmenubar-class.md) 对象。 此方法对 `CMFCMenuBar` HMENU 参数后的对象建模。
...
返回的字体是应用程序的全局参数。 为所有对象维护两种全局字体 `CMFCMenuBar` 。 这些单独的字体用于水平和垂直菜单栏。
...
`CMFCMenuBar`可以将对象配置为显示所有命令，或仅显示部分命令。 有关此功能的详细信息，请参阅 [CMFCMenuBar 类](../../mfc/reference/cmfcmenubar-class.md)。

`IsShowAllCommands` 将告诉您如何为对象配置此功能 `CMFCMenuBar` 。 若要控制显示的菜单命令，请使用方法 [CMFCMenuBar：： SetShowAllCommands](#setshowallcommands) 和 [CMFCMenuBar：： SetRecentlyUsedMenus](#setrecentlyusedmenus)。
...
默认情况下，所有 `CMFCMenuBar` 对象使用选项在短暂延迟后显示完整菜单。 此选项不能在类中以编程方式进行更改 `CMFCMenuBar` 。 但是，用户可以通过使用 " **自定义** " 对话框，在自定义工具栏期间更改行为。
