...
|[CDHtmlDialog：:D DX_DHtml_AxControl](#ddx_dhtml_axcontrol)|在 HTML 页上的某个 ActiveX 控件的成员变量和属性值之间交换数据。|
|[CDHtmlDialog：:D DX_DHtml_CheckBox](#ddx_dhtml_checkbox)|在 HTML 页上的成员变量和复选框之间交换数据。|
|[CDHtmlDialog：:D DX_DHtml_ElementText](#ddx_dhtml_elementtext)|在 HTML 页上的成员变量和任何 HTML 元素属性之间交换数据。|
|[CDHtmlDialog：:D DX_DHtml_Radio](#ddx_dhtml_radio)|在 HTML 页上的成员变量和单选按钮之间交换数据。|
|[CDHtmlDialog：:D DX_DHtml_SelectIndex](#ddx_dhtml_selectindex)|获取或设置 HTML 页上的列表框的索引。|
|[CDHtmlDialog：:D DX_DHtml_SelectString](#ddx_dhtml_selectstring)|基于 HTML 页上的当前索引) 获取或设置列表框项 (的显示文本。|
|[CDHtmlDialog：:D DX_DHtml_SelectValue](#ddx_dhtml_selectvalue)|基于 HTML 页上的当前索引) 获取或设置列表框项 (的值。|
|[CDHtmlDialog：:D estroyModeless](#destroymodeless)|销毁无模式对话框。|
...
|[CDHtmlDialog：： GetControlDispatch](#getcontroldispatch)|检索 `IDispatch` 嵌入在 HTML 文档中的 ActiveX 控件上的接口。|
|[CDHtmlDialog：： GetControlProperty](#getcontrolproperty)|检索指定 ActiveX 控件的请求的属性。|
|[CDHtmlDialog：： GetCurrentUrl](#getcurrenturl)|检索与当前文档关联 (URL) 的统一资源定位符。|
...
|[CDHtmlDialog：： IsExternalDispatchSafe](#isexternaldispatchsafe)|指示主机的接口是否 `IDispatch` 可安全编写脚本。|
|[CDHtmlDialog：： LoadFromResource](#loadfromresource)|将指定的资源加载到 WebBrowser 控件中。|
|[CDHtmlDialog：：导航](#navigate)|导航到指定的 URL。|
...
|[CDHtmlDialog：： Onframewindowactivate 调用](#onframewindowactivate)|当激活或停用框架窗口时由框架调用。|
...
|[CDHtmlDialog：： SetElementHtml](#setelementhtml)|设置 `innerHTML` HTML 元素的属性。|
|[CDHtmlDialog：： SetElementProperty](#setelementproperty)|设置 HTML 元素的属性。|
|[CDHtmlDialog：： SetElementText](#setelementtext)|设置 `innerText` HTML 元素的属性。|
...
基于 HTML 页上的当前索引) 获取或设置列表框项 (的显示文本。
...
从对象分离无模式对话框 `CDHtmlDialog` 并销毁对象。
...
*ppDORet*<br/>
请*ppDORet*参阅 `IDocHostUIHandler::FilterDataObject` Windows SDK 中的 ppDORet。
...
检索 `IDispatch` 嵌入在 [GetDHtmlDocument](#getdhtmldocument)返回的 HTML 文档中的 ActiveX 控件上的接口。
...
*ppDropTarget*<br/>
请*ppDropTarget*参阅 `IDocHostUIHandler::GetDropTarget` Windows SDK 中的 ppDropTarget。
...
检索 `innerHTML` 由 *szElementId*标识的 HTML 元素的属性。
...
`innerHTML`如果找不到元素，则为*szElementId*标识的 HTML 元素的属性。
...
检索 `innerText` 由 *szElementId*标识的 HTML 元素的属性。
...
`innerText`由*szElementId*标识的 HTML 元素的属性，如果找不到该属性或元素，则为 NULL。
...
*dw*<br/>
请*dw*参阅 `IDocHostUIHandler::GetOptionKeyPath` Windows SDK 中的 dw。
...
## <a name="cdhtmldialognavigate"></a><a name="navigate"></a> CDHtmlDialog：：导航
...
## <a name="cdhtmldialogonframewindowactivate"></a><a name="onframewindowactivate"></a> CDHtmlDialog：： Onframewindowactivate 调用
...
此消息将在执行、或调用期间发送到对话框，该对话框会在 `Create` `CreateIndirect` `DoModal` 对话框显示之前立即发生。

如果在初始化对话框时需要执行特殊处理，请重写此成员函数。 在重写的版本中，首先调用基类， `OnInitDialog` 但忽略其返回值。 通常会从重写的成员函数返回 TRUE。

Windows `OnInitDialog` 通过所有 Microsoft 基础类库对话框通用的标准全局对话框过程（而不是通过消息映射）调用函数，因此不需要此成员函数的消息映射项。
...
*pUIWindow*<br/>
请*pUIWindow*参阅 `IDocHostUIHandler::ResizeBorder` Windows SDK 中的 pUIWindow。

*fFrameWindow*<br/>
请*fFrameWindow*参阅 `IDocHostUIHandler::ResizeBorder` Windows SDK 中的 fFrameWindow。
...
*ppt*<br/>
请*ppt*参阅 `IDocHostUIHandler::ShowContextMenu` Windows SDK 中的 ppt。

*pcmdtReserved*<br/>
请*pcmdtReserved*参阅 `IDocHostUIHandler::ShowContextMenu` Windows SDK 中的 pcmdtReserved。

*pdispReserved*<br/>
请*pdispReserved*参阅 `IDocHostUIHandler::ShowContextMenu` Windows SDK 中的 pdispReserved。
...
*pActiveObject*<br/>
请参阅 Windows SDK 中的 *d pActiveObject* in `IDocHostUIHandler::ShowUI` 。

*pCommandTarget*<br/>
请*pCommandTarget*参阅 `IDocHostUIHandler::ShowUI` Windows SDK 中的 pCommandTarget。

*pFrame*<br/>
请*pFrame*参阅 `IDocHostUIHandler::ShowUI` Windows SDK 中的 pFrame。

*pDoc*<br/>
请*pDoc*参阅 `IDocHostUIHandler::ShowUI` Windows SDK 中的 pDoc。
...
*pguidCmdGroup*<br/>
请*pguidCmdGroup*参阅 `IDocHostUIHandler::TranslateAccelerator` Windows SDK 中的 pguidCmdGroup。

*nCmdID*<br/>
请*nCmdID*参阅 `IDocHostUIHandler::TranslateAccelerator` Windows SDK 中的 nCmdID。
...
*pchURLIn*<br/>
请*pchURLIn*参阅 `IDocHostUIHandler::TranslateUrl` Windows SDK 中的 pchURLIn。

*ppchURLOut*<br/>
请*ppchURLOut*参阅 `IDocHostUIHandler::TranslateUrl` Windows SDK 中的 ppchURLOut。
