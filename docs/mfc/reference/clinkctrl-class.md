...
|[CLinkCtrl：： System.windows.media.visualtreehelper.hittest](#hittest)|确定用户是否单击了指定的链接。|
...
此控件 (，因此 `CLinkCtrl` 类) 仅适用于在 WINDOWS XP 和更高版本下运行的程序。
...
*dwStyle*<br/>
指定链接控件的样式。 应用任何控件样式组合。 有关详细信息，请参阅中的 [常见控件样式](/windows/win32/Controls/common-control-styles) `Windows SDK` 。
...
可以通过 `CLinkCtrl` 两个步骤构造对象。 首先，调用构造函数，然后调用 `Create` ，它将创建链接控件并将其附加到 `CLinkCtrl` 对象。 如果要在控件中使用扩展的 windows 样式，请调用 [CLinkCtrl：： CreateEx](#createex) 而不是 `Create` 。

不推荐使用该方法的第二种形式 `Create` 。 使用指定 *lpszLinkMarkup* 参数的第一个窗体。

### <a name="example"></a>示例

下面的代码示例定义了两个名为 `m_Link1` 和的变量， `m_Link2` 用于访问两个链接控件。
...
创建具有扩展样式的链接控件，并将其附加到 `CLinkCtrl` 对象。
...
不推荐使用该方法的第二种形式 `CreateEx` 。 使用指定 *lpszLinkMarkup* 参数的第一个窗体。
...
链接文本的首选高度（以像素为单位）。 返回值与结构的 *cy* 成员的值相同 `SIZE` 。

### <a name="remarks"></a>备注

有关方法的示例 `GetIdealSize` ，请参见 [CLinkCtrl：： Create](#create)中的示例。

此方法发送 Windows SDK 中描述的 [LM_GETIDEALSIZE](/windows/win32/Controls/lm-getidealsize) 消息。
...
*stateMask*<br/>
描述要获取哪个状态项的标志组合。 有关值的列表，请参阅 `state` [LITEM](/windows/win32/api/commctrl/ns-commctrl-litem) 结构中成员的说明。 允许的项与中允许的项相同 `state` 。
...
*stateMask*<br/>
描述要设置的状态项的标志的组合。 有关值的列表，请参阅 `state` [LITEM](/windows/win32/api/commctrl/ns-commctrl-litem) 结构中成员的说明。 允许的项与中允许的项相同 `state` 。
