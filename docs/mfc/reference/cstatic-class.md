...
如果在对话框 `CStatic` 资源) 中创建 (通过对话框资源的对象，则 `CStatic` 当用户关闭对话框时，对象会自动销毁。
...
重写此函数以实现所有者描述的对象的绘图 `CStatic` (控件具有 SS_OWNERDRAW) 样式。
