...
|[CBrush：： FromHandle](#fromhandle)|`CBrush`当给定 Windows 对象的句柄时，返回指向对象的指针 `HBRUSH` 。|
...
|名称|描述|
|----------|-----------------|
|[CBrush：： operator HBRUSH](#operator_hbrush)|返回附加到对象的 Windows 句柄 `CBrush` 。|

## <a name="remarks"></a>备注

若要使用 `CBrush` 对象，请构造 `CBrush` 对象并将其传递给 `CDC` 需要画笔的任何成员函数。
...
- HS_BDIAGONAL 向下向右 (从左向右) 45 度

- HS_CROSS 水平和垂直交叉影线

- 45度 HS_DIAGCROSS 交叉影线

- HS_FDIAGONAL 向上阴影 (从左到右) 45 度
...
`CBrush` 具有四个重载构造函数。不带参数的构造函数将构造一个未初始化 `CBrush` 的对象，必须先对其进行初始化，然后才能使用该对象。

如果使用不带参数的构造函数，则必须 `CBrush` 用 [CreateSolidBrush](#createsolidbrush)、 [CreateHatchBrush](#createhatchbrush)、 [CreateBrushIndirect](#createbrushindirect)、 [CreatePatternBrush](#createpatternbrush)或 [CreateDIBPatternBrush](#createdibpatternbrush)初始化生成的对象。 如果使用采用参数的构造函数之一，则无需进行进一步的初始化。 如果遇到错误，带参数的构造函数可能会引发异常，而不带参数的构造函数将始终会成功。

具有单个 [COLORREF](/windows/win32/gdi/colorref) 参数的构造函数使用指定的颜色构造纯色画笔。 颜色指定一个 RGB 值，并且可以使用 WINDOWS .H 中的 RGB 宏来构造。

具有两个参数的构造函数构造一个阴影画笔。 *NIndex* 参数指定阴影模式的索引。 *CrColor* 参数指定颜色。

带参数的构造函数 `CBitmap` 构造一个带图案的画笔。 参数用于标识位图。 假定已通过使用 [CBitmap：： CreateBitmap](../../mfc/reference/cbitmap-class.md#createbitmap)、 [CBitmap：： CreateBitmapIndirect](../../mfc/reference/cbitmap-class.md#createbitmapindirect)、 [CBitmap：： LoadBitmap](../../mfc/reference/cbitmap-class.md#loadbitmap)或 [CBitmap：： CreateCompatibleBitmap](../../mfc/reference/cbitmap-class.md#createcompatiblebitmap)创建了该位图。 填充模式中要使用的位图的最小大小为 8 x 8 像素。
...
使用单色 (1 平面创建的画笔，使用当前文本和背景色绘制每个像素1位) 位图。 设置为0的位所表示的像素将用当前文本颜色绘制。 设置为1的位设置为1的像素将用当前背景颜色绘制。
...
*nUsage*<br/>
指定 `bmiColors[]` [BITMAPINFO](/windows/win32/api/wingdi/ns-wingdi-bitmapinfo) 数据结构的字段 ("打包的 DIB" ) 的一部分是否包含显式的 RGB 值或索引到当前实现的逻辑调色板。 参数必须是下列值之一：
...
- 在第一个版本中，若要获取 DIB 的句柄，请调用 Windows `GlobalAlloc` 函数以分配全局内存块，然后使用打包的 DIB 填充内存。
...
当应用程序在单色设备上下文中选择双色 DIB 模式画笔时，Windows 将忽略 DIB 中指定的颜色，而是使用设备上下文的当前文本和背景色显示模式画笔。 映射到 DIB 颜色表) 中偏移量 0 (第一种颜色的像素使用文本颜色显示。 使用背景色显示映射到颜色表中偏移量 1 (第二种颜色) 的像素。

有关使用以下 Windows 函数的信息，请参阅 Windows SDK：

- [CreateDIBPatternBrush](/windows/win32/api/wingdi/nf-wingdi-createdibpatternbrush) (此函数仅用于兼容为3.0 之前的 Windows 版本编写的应用程序;使用 `CreateDIBPatternBrushPt` 函数。 ) 
...
*crColor*<br/>
指定画笔的前景色作为 RGB 颜色 (阴影) 的颜色。 有关详细信息，请参阅 Windows SDK 中的 [COLORREF](/windows/win32/gdi/colorref) 。
...
使用单色位图创建的画笔 (1 色平面，每像素1位) 使用当前文本和背景颜色绘制。 位设置为0的像素由当前文本颜色绘制。 设置为1的位设置为1的像素将用当前背景颜色绘制。
...
*crColor*<br/>
指定画笔颜色的 [COLORREF](/windows/win32/gdi/colorref) 结构。 颜色指定一个 RGB 值，并且可以使用 WINDOWS .H 中的 RGB 宏来构造。
...
当应用程序使用创建的画笔完成后 `CreateSolidBrush` ，应从设备上下文中选择画笔。
...
当应用程序使用创建的画笔完成后 `CreateSysColorBrush` ，应从设备上下文中选择画笔。
...
`CBrush`给定 Windows [HBRUSH](#operator_hbrush)对象的句柄时，返回指向对象的指针。
...
如果 `CBrush` 对象尚未附加到句柄， `CBrush` 则会创建并附加一个临时对象。 此临时 `CBrush` 对象仅在下一次应用程序的事件循环中有空闲时间之前有效。 此时，将删除所有临时图形对象。 换言之，只有在处理一条窗口消息的过程中，临时对象才有效。
...
使用此运算符可获取对象的附加 Windows GDI 句柄 `CBrush` 。
...
如果成功，则为对象表示的 Windows GDI 对象的句柄 `CBrush` ; 否则为 NULL。
