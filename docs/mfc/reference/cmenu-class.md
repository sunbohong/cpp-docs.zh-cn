...
|[CMenu：:D eleteTempMap](#deletetempmap)|删除 `CMenu` 成员函数创建的任何临时 `FromHandle` 对象。|
|[CMenu：:D estroyMenu](#destroymenu)|销毁附加到对象的菜单 `CMenu` ，并释放菜单占用的任何内存。|
|[CMenu：:D etach](#detach)|从对象分离一个 Windows 菜单句柄 `CMenu` 并返回该句柄。|
|[CMenu：:D rawItem](#drawitem)|当所有者描述的菜单的视觉方面发生变化时，由框架调用。|
|[CMenu：： EnableMenuItem](#enablemenuitem)|启用、禁用或变暗 (菜单项) 的灰色。|
|[CMenu：： FromHandle](#fromhandle)|返回一个指向 `CMenu` 给定 Windows 菜单句柄的对象的指针。|
...
|[CMenu：： GetSafeHmenu](#getsafehmenu)|返回 `m_hMenu` 此对象包装的 `CMenu` 。|
...
|名称|描述|
|----------|-----------------|
|[CMenu：： m_hMenu](#m_hmenu)|指定附加到对象的 Windows 菜单的句柄 `CMenu` 。|

## <a name="remarks"></a>备注

它提供用于创建、跟踪、更新和销毁菜单的成员函数。

将 `CMenu` 堆栈帧上的对象创建为本地对象，然后调用 `CMenu` 的成员函数以根据需要操作新菜单。 接下来，调用 [CWnd：： SetMenu](../../mfc/reference/cwnd-class.md#setmenu) ，将菜单设置为窗口，后跟 `CMenu` 对象的 [分离](#detach) 成员函数的调用。 该 `CWnd::SetMenu` 成员函数将窗口的菜单设置为新菜单，导致重绘窗口以反映菜单更改，同时还将菜单的所有权传递给窗口。 对 `Detach` 从对象分离 HMENU 的调用 `CMenu` ，以便当本地 `CMenu` 变量超出范围时， `CMenu` 对象析构函数不会尝试销毁不再拥有的菜单。 销毁窗口时，菜单本身会自动销毁。
...
*nIDNewItem*<br/>
指定新菜单项的命令 ID，或者，如果 " *nFlags* " 设置为 "MF_POPUP"，则菜单将处理 `HMENU` 弹出菜单的 () 。 如果 *nFlags* 设置为 MF_SEPARATOR，则将忽略 *nIDNewItem* 参数 (不需要) 。
...
|------------|-----------------------------------|
|MF_OWNERDRAW|包含应用程序提供的32位值，应用程序可使用该值来维护与菜单项关联的附加数据。 当应用程序处理 WM_MEASUREITEM 和 WM_DRAWITEM 消息时，此32位值可用于应用程序。 该值存储在 `itemData` 与这些消息一起提供的结构的成员中。|
...
应用程序可以通过在 *nFlags* 中设置值来指定菜单项的状态。 当 *nIDNewItem* 指定一个弹出菜单时，它将成为它要追加到的菜单的一部分。 如果该菜单被销毁，则追加的菜单也会被销毁。 追加的菜单应该与 `CMenu` 对象分离，以避免冲突。 请注意，MF_STRING 和 MF_OWNERDRAW 对于的位图版本无效 `AppendMenu` 。

以下列表描述了可在 *nFlags* 中设置的标志：

- MF_CHECKED 作为 MF_UNCHECKED 的切换，以将默认复选标记置于该项旁边。 当应用程序提供了检查标记位图 (查看 [SetMenuItemBitmaps](#setmenuitembitmaps) 成员函数) 时，将显示 "选中标记" 位图。

- MF_UNCHECKED 使用 MF_CHECKED 进行切换，以便删除项旁边的复选标记。 当应用程序提供了检查标记位图 (查看 `SetMenuItemBitmaps`) 成员函数时，将显示 "勾号 off" 位图。
...
只要窗口中的菜单发生更改 (无论是否) 显示窗口，应用程序应调用 [CWnd：:D rawmenubar](../../mfc/reference/cwnd-class.md#drawmenubar)。
...
如果菜单已附加到对象，则不应调用此函数 `CMenu` 。 菜单句柄存储在 `m_hMenu` 数据成员中。
...
*nIDFirst*<br/>
将 (指定为 ID 或偏移量，具体取决于单选按钮组中第一个菜单项) *nFlags* 的值。

*nIDLast*<br/>
将 (指定为 ID 或偏移量，具体取决于单选按钮组中的最后一个菜单项) *nFlags* 的值。

*nIDItem*<br/>
将 (指定为 ID 或偏移量，具体取决于将使用单选按钮检查的组中的项) *nFlags* 的值。
...
菜单最初为空。 可以使用 `AppendMenu` 或成员函数添加菜单项 `InsertMenu` 。
...
菜单最初为空。 可以使用 `AppendMenu` 或成员函数添加菜单项 `InsertMenu` 。 应用程序可以将弹出菜单添加到现有菜单或弹出菜单。 `TrackPopupMenu`成员函数可用于将此菜单显示为浮动菜单上的浮动菜单和跟踪选项。
...
如果菜单项具有关联的弹出菜单，则会 `DeleteMenu` 销毁弹出菜单的句柄，并释放弹出菜单使用的内存。

只要窗口中的菜单发生更改 (无论是否) 显示窗口，应用程序必须调用 [CWnd：:D rawmenubar](../../mfc/reference/cwnd-class.md#drawmenubar)。
...
由 `CWinApp` 空闲时间处理程序自动调用，将删除 `CMenu` [FromHandle](#fromhandle) 成员函数创建的任何临时对象。
...
`DeleteTempMap``CMenu`在删除对象之前，分离附加到临时对象的 Windows 菜单对象 `CMenu` 。
...
菜单在 `CMenu` 销毁之前从对象分离。 `DestroyMenu`在析构函数中自动调用 Windows 函数 `CMenu` 。
...
从对象分离一个 Windows 菜单 `CMenu` 并返回该句柄。
...
`itemAction`结构的成员 `DRAWITEMSTRUCT` 定义要执行的绘图操作。 重写此成员函数以实现所有者描述对象的绘制 `CMenu` 。 应用程序应还原在此成员函数终止之前为 *lpDrawItemStruct* 中提供的显示上下文选择的所有图形设备接口 (GDI) 对象。
...
以前的状态 ( MF_DISABLED、MF_ENABLED 或 MF_GRAYED) ，如果无效，则为-1。

### <a name="remarks"></a>备注

[CreateMenu](#createmenu)、 [InsertMenu](#insertmenu)、 [ModifyMenu](#modifymenu)和[LoadMenuIndirect](#loadmenuindirect)成员函数还可以设置菜单项) 启用、禁用或灰显的状态 (。

使用 MF_BYPOSITION 值需要应用程序使用正确的 `CMenu` 。 如果 `CMenu` 使用菜单栏的，则顶级菜单项 (菜单栏中的项) 受影响。 若要按位置设置弹出菜单或嵌套的弹出菜单中项的状态，应用程序必须指定 `CMenu` 弹出菜单的。

当应用程序指定 MF_BYCOMMAND 标志时，Windows 将检查从属于的所有弹出菜单项 `CMenu` ; 因此，除非存在重复的菜单项，否则使用 `CMenu` 菜单栏的就足够了。
...
返回一个指向对象的指针，该指针指向具有 `CMenu` 菜单的 Windows 句柄的对象。
...
如果 `CMenu` 对象尚未附加到 Windows 菜单对象， `CMenu` 则会创建并附加一个临时对象。
...
如果具有一个，则为当前与关联的上下文帮助 ID `CMenu` ; 否则为零。
...
*nPos*<br/>
指定要检索其 ID 的菜单项 (从零开始的) 位置。

### <a name="return-value"></a>返回值

如果函数成功，则为弹出菜单中指定项的项 ID。 如果指定项是一个弹出菜单 (相对于弹出菜单) 中的项，则返回值为-1。 如果 *nPos* 对应于一个分隔符菜单项，则返回值为0。
...
*fByPos*<br/>
值，该值指定的含义 `nIDItem` 。 默认情况下， `ByPos` 为 FALSE，表示 uItem 是菜单项标识符。 如果未 `ByPos` 设置为 FALSE，则指示菜单项位置。
...
此成员函数实现 Win32 函数 [GetMenuItemInfo](/windows/win32/api/winuser/nf-winuser-getmenuiteminfow)的行为，如 Windows SDK 中所述。 请注意，在的 MFC 实现中 `GetMenuItemInfo` ，不使用菜单的句柄。
...
如果指定的项不存在，则值为0xFFFFFFFF。 如果 *nId* 标识一个弹出菜单，则高序位字节包含弹出菜单中的项数，而低序位字节包含与弹出菜单相关联的菜单标志。 否则，返回值为以下列表中值的掩码 (布尔或)  (此掩码描述 *nId* 标识) 的菜单项的状态：

- MF_CHECKED 作为 MF_UNCHECKED 的切换，以将默认复选标记置于该项旁边。 当应用程序提供了检查标记位图 (查看 `SetMenuItemBitmaps`) 成员函数时，将显示 "选中标记" 位图。
...
- MF_UNCHECKED 使用 MF_CHECKED 进行切换，以便删除项旁边的复选标记。 当应用程序提供了检查标记位图 (查看 `SetMenuItemBitmaps`) 成员函数时，将显示 "勾号 off" 位图。 请注意，此常量的值为 0;使用此值时，应用程序不应对0进行测试。
...
*nMaxCount*<br/>
指定要复制的标签)  (的最大长度。 如果标签的长度超过在 *nMaxCount* 中指定的最大值，则会截断额外的字符。
...
返回此对象包装的 HMENU `CMenu` 或 NULL `CMenu` 指针。
...
指向一个对象的指针，该 `CMenu` 对象的 `m_hMenu` 成员包含弹出菜单的句柄（如果位于给定位置的弹出菜单中）; 否则为 NULL。 如果 `CMenu` 对象不存在，则创建一个临时对象。 `CMenu`不应存储返回的指针。
...
*nIDNewItem*<br/>
指定新菜单项的命令 ID，或者，如果 " *nFlags* " 设置为 "MF_POPUP"，则菜单将处理弹出菜单 ( HMENU) 。 如果 *nFlags* 设置为 MF_SEPARATOR，则将忽略 *nIDNewItem* 参数 (不需要) 。
...
|nFlags|LpszNewItem 的解释|
|------------|-----------------------------------|
|MF_OWNERDRAW|包含应用程序提供的32位值，应用程序可使用该值来维护与菜单项关联的附加数据。 此32位值可用于应用程序中 `itemData` 由 [WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem) 和 [WM_DRAWITEM](/windows/win32/Controls/wm-drawitem) 消息提供的结构成员。 当菜单项最初显示或更改时，将发送这些消息。|
...
只要窗口中的菜单发生更改 (无论是否) 显示窗口，应用程序应调用 `CWnd::DrawMenuBar` 。

当 *nIDNewItem* 指定一个弹出菜单时，它将成为插入它的菜单的一部分。 如果该菜单被销毁，则插入的菜单还将被销毁。 插入的菜单应该与 `CMenu` 对象分离，以避免冲突。

如果活动的多文档界面 (MDI) 子窗口已最大化，并且应用程序通过调用此函数并指定了 MF_BYPOSITION 标志，将弹出菜单插入到 MDI 应用程序的菜单中，则会将该菜单向左移动一个位置而不是预期的位置。 出现这种情况的原因是活动 MDI 子窗口的 "控制" 菜单被插入到 MDI 框架窗口菜单栏的第一个位置。 若要正确地定位菜单，应用程序必须向要使用的位置值加1。 应用程序可以使用 WM_MDIGETACTIVE 消息来确定当前处于活动状态的子窗口是否处于最大化状态。
...
*lpMenuItemInfo*<br/>
请参阅 Windows SDK 中的 *lpmii* 说明 `InsertMenuItem` 。

*fByPos*<br/>
请参阅 Windows SDK 中的 *fByPosition* 说明 `InsertMenuItem` 。
...
*lpMenuTemplate*<br/>
指向菜单模板 (该模板为单个 [MENUITEMTEMPLATEHEADER](/windows/win32/api/winuser/ns-winuser-menuitemtemplateheader) 结构，) 的一个或多个 [MENUITEMTEMPLATE](/windows/win32/api/winuser/ns-winuser-menuitemtemplate) 结构的集合。 有关这两个结构的详细信息，请参阅 Windows SDK。
...
`mtOption`标志应包括弹出列表中最后一项的 MF_END 以及主列表中最后一项的。 `AppendMenu`有关其他标志，请参阅成员函数。 `mtId`当在中指定 MF_POPUP 时，必须从 MENUITEMTEMPLATE 结构中省略此成员 `mtOption` 。

为 MENUITEMTEMPLATE 结构分配的空间必须足够大，以便 `mtString` 将菜单项的名称包含为以 null 结尾的字符串。
...
指定附加到对象的 Windows 菜单的 HMENU 句柄 `CMenu` 。
...
*nIDNewItem*<br/>
指定修改后的菜单项的命令 ID，或者，如果将 *nFlags* 设置为 MF_POPUP，则菜单将处理弹出菜单 (HMENU) 。 如果 *nFlags* 设置为 MF_SEPARATOR，则将忽略 *nIDNewItem* 参数 (不需要) 。
...
|MF_STRING|包含指向以 null 结尾的字符串或的长指针 `CString` 。|
|MF_SEPARATOR|忽略 *lpszNewItem* 参数 (不需要) 。|
...
只要窗口中的菜单发生更改 (无论是否) 显示窗口，应用程序应调用 `CWnd::DrawMenuBar` 。 若要更改现有菜单项的属性，使用 `CheckMenuItem` 和成员函数的速度要快得多 `EnableMenuItem` 。
...
测试左侧的 menu 对象是否与 HMENU 值相等 (，) 到右侧的菜单对象。
...
每当窗口中的菜单发生更改时 (不管是否) 显示窗口，应用程序必须调用 `CWnd::DrawMenuBar` 。
...
*lpMenuItemInfo*<br/>
请参阅 Windows SDK 中的 *lpmii* 说明 `SetMenuItemInfo` 。

*fByPos*<br/>
请参阅 Windows SDK 中的 *fByPosition* 说明 `SetMenuItemInfo` 。
...
*pWnd*<br/>
标识拥有弹出菜单的窗口。 即使指定了 TPM_NONOTIFY 标志，此参数也不能为 NULL。 此窗口从菜单接收所有 WM_COMMAND 消息。 在 Windows 版本3.1 及更高版本中，在返回之前，窗口不会接收 WM_COMMAND 消息 `TrackPopupMenu` 。 在 Windows 3.0 中，窗口在返回前收到 WM_COMMAND 消息 `TrackPopupMenu` 。
...
如果未在 *fuFlags* 参数中指定 TPM_RETURNCMD，则如果函数成功，则返回值为非零; 否则返回值为0。 要获得更多的错误信息，请调用 GetLastError。
