---
description: 了解详细信息： COleConvertDialog 类
...
|“属性”|描述|
|----------|-----------------|
|[COleConvertDialog：:D oConvert](#doconvert)|执行在对话框中指定的转换。|
|[COleConvertDialog：:D oModal](#domodal)|显示 "OLE 更改项" 对话框。|
...
- CF_SELECTCONVERTTO 指定最初在调用对话框时，将选择 "转换为" 单选按钮。 这是默认值。

- CF_SELECTACTIVATEAS 指定最初在调用对话框时将选择 "激活方式" 按钮。

- CF_SETCONVERTDEFAULT 指定 `clsidConvertDefault` `m_cv` 当选择 "转换为" 单选按钮时，结构成员指定的 CLSID 将用作类列表框中的默认选择。

- CF_SETACTIVATEDEFAULT 指定 `clsidActivateDefault` `m_cv` 当选择 "激活为" 单选按钮时，结构成员指定的 CLSID 将用作类列表框中的默认选择。
...
*pParentWnd*<br/>
指向 "父对象" 或 "所有者" 窗口对象 (`CWnd` ") " 对话框对象所属的类型 "。 如果为 NULL，则对话框的父窗口设置为主应用程序窗口。
...
如果要通过设置 [m_cv](#m_cv) 结构的成员来初始化各种对话框控件，应在调用之前执行此操作 `DoModal` ，但在构造对话框对象之后。
...
返回类型值由 `Selection` 类中声明的枚举类型指定 `COleConvertDialog` 。
...
- `COleConvertDialog::convertItem` 如果 "转换为" 单选按钮已选中，则用户选择了其他要转换为的项，并 `DoModal` 返回 IDOK。

- `COleConvertDialog::activateAs` 如果选中了 "激活方式" 按钮，则用户选择了一个不同的项来激活，并 `DoModal` 返回 IDOK。
