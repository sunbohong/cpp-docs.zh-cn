---
description: 了解详细信息： CMonikerFile 类
...
|[CMonikerFile：:D etach](#detach)|`IMoniker`从此 `CMonikerFile` 对象分离。|
...
名字对象包含的信息与文件的路径名非常类似。 如果有指向名字对象接口的指针 `IMoniker` ，则可以获取对已标识文件的访问权限，而无需使用有关文件实际位置的任何其他特定信息。

从派生 `COleStreamFile` ， `CMonikerFile` 获取一个名字对象或字符串表示形式，它可以将其转换为名字对象，并绑定到该名字对象为其名称的流。 然后，你可以读取和写入该流。 的真正用途 `CMonikerFile` 是提供对的对的简单访问 `IStream` `IMoniker` ，以便你无需自行绑定到流，而是具有 `CFile` 流功能。

`CMonikerFile` 不能用于绑定到流之外的任何内容。 如果要绑定到存储或对象，则必须 `IMoniker` 直接使用接口。
...
一个指针，指向要绑定的绑定上下文 [IBindCtx](/windows/win32/api/objidl/nn-objidl-ibindctx) ，如果成功，则为; 否则为。否则为 NULL。 如果使用接口打开实例，将 `IBindHost` 从中检索绑定上下文 `IBindHost` 。 如果没有 `IBindHost` 接口或接口未能返回绑定上下文，则将创建一个绑定上下文。 有关 [IBindHost](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/ms775076\(v=vs.85\)) 接口的说明，请参阅 Windows SDK。
...
指向当前名字对象接口的指针 ( [IMoniker](/windows/win32/api/objidl/nn-objidl-imoniker)) 。

### <a name="remarks"></a>备注

由于不 `CMonikerFile` 是接口，因此返回的指针不会 (通过 [AddRef](/windows/win32/api/unknwn/nf-unknwn-iunknown-addref)) 递增引用计数，并在释放对象时释放标记 `CMonikerFile` 。 如果要保存到名字对象或自行发布，则必须 `AddRef` 这样做。
...
*LpszURL* 参数不能在 Macintosh 上使用。 只能 `Open` 在 Macintosh 上使用的 pMoniker 形式。
