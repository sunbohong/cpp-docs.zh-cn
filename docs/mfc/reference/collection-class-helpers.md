...
ms.locfileid: "97331384"
---
# <a name="collection-class-helpers"></a>集合类帮助器

集合类 `CMap` 、 `CList` 和 `CArray` 使用模板化全局 helper 函数，以便对元素进行比较、复制和序列化。 作为基于、和的类实现的一部分 `CMap` `CList` `CArray` ，你必须根据需要重写这些函数，使其适合你的映射、列表或数组中所存储的数据类型。 有关重写 helper 函数（如）的信息 `SerializeElements` ，请参阅文章 [集合：如何进行 Type-Safe 收集](../../mfc/how-to-make-a-type-safe-collection.md)。 请注意， `ConstructElements` 和已 `DestructElements` 被弃用。
...
由 [CList：： Find] 直接调用 (not_found CList # clist__find，并通过 [cmap__lookup](cmap-class.md#lookup) 和 [cmap__operator &#91;&#93;](cmap-class.md#operator_at)间接调用。
...
C + + 语言定义 `==` (、、) 等简单类型的比较运算符 () **`char`** ， **`int`** 但不 **`float`** 定义类和结构的比较运算符。 如果要使用 `CompareElements` 或来实例化使用它的一个集合类，则必须 `CompareElements` 使用返回适当值的版本定义比较运算符或重载。
...
有关实现此和其他帮助器函数的信息，请参阅文章 [集合：如何进行 Type-Safe 收集](../how-to-make-a-type-safe-collection.md)。
...
请参阅文章 [集合：如何建立 Type-Safe 集合](../how-to-make-a-type-safe-collection.md)中的示例。
