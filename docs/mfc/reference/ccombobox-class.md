---
title: CComboBox 类
description: MFC Windows 组合框类 CComboBox 的 API 参考
ms.date: 08/27/2020
f1_keywords:
- CComboBox
- AFXWIN/CComboBox
...
|[CComboBox：： Cut](#cut)|在编辑控件中删除 (剪切) 当前选择（如果有），并以 CF_TEXT 格式将删除的文本复制到剪贴板。|
...
|[CComboBox：： FindStringExact](#findstringexact)|查找组合框中与指定字符串匹配的第一个列表框字符串 () 。|
...
|[CComboBox：： GetDroppedControlRect](#getdroppedcontrolrect)|检索下拉组合框) 列表框中的可见 (的屏幕坐标。|
|[CComboBox：： GetDroppedState](#getdroppedstate)|确定下拉组合框的列表框是否可见 (下拉) 。|
...
|[CComboBox：： SetItemHeight](#setitemheight)|设置组合框中的列表项的高度，或组合框的编辑控件 (或静态文本) 部分的高度。|
...
如果 "静态" 或 "编辑" 控件中显示列表框中的任何) ，则当前选定的项 (。 此外，如果组合框具有下拉列表样式，则用户可以键入列表中某一项的起始字符，列表框（如果可见）将突出显示带有该初始字符的下一项。
...
如果要将组合框发送的 Windows 通知消息处理到其父 (通常是派生自) 的类 `CDialog` ，请将消息映射项和消息处理程序成员函数添加到每个消息的父类。
...
- ON_CBN_DROPDOWN 组合框的列表框将要下拉 (使其) 可见。 此通知消息仅适用于具有 CBS_DROPDOWN 或 CBS_DROPDOWNLIST 样式的组合框。
...
- ON_CBN_SELENDCANCEL (Windows 3.1 及更高版本。 ) 指示应取消用户的选择。 用户单击某个项，然后单击另一个窗口或控件以隐藏组合框的列表框。 此通知消息将在 CBN_CLOSEUP 通知消息之前发送，以指示应忽略用户的选择。 即使 CBN_CLOSEUP 通知消息未发送 (，也会发送 CBN_SELENDCANCEL 或 CBN_SELENDOK 通知消息，如 CBS_SIMPLE 样式) 的组合框。

- ON_CBN_SELENDOK 用户选择一个项，然后按 ENTER 键或单击向下箭头键以隐藏组合框的列表框。 此通知消息将在 CBN_CLOSEUP 消息之前发送，以指示应将用户的选择视为有效。 即使 CBN_CLOSEUP 通知消息未发送 (，也会发送 CBN_SELENDCANCEL 或 CBN_SELENDOK 通知消息，如 CBS_SIMPLE 样式) 的组合框。
...
如果在对话框 `CComboBox` 资源) 中创建 (通过对话框资源的对象，则 `CComboBox` 当用户关闭对话框时，对象会自动销毁。
...
在组合框编辑控件中删除 (剪切) 当前选择（如果有），并以 CF_TEXT 格式将删除的文本复制到剪贴板。
...
默认情况下，此成员函数不执行任何操作。 重写此成员函数以实现所有者描述对象的绘制 `CComboBox` 。 在此成员函数终止之前，应用程序应还原为 *lpDrawItemStruct*中提供的显示上下文选择的所有图形设备接口 (GDI) 对象。
...
调用 `FindStringExact` 成员函数以查找组合框中 (的第一个列表框字符串) 与 *lpszFind*中指定的字符串相匹配。
...
项数。 返回的计数比索引从零开始)  (一个大于最后一个项的索引值。 如果发生错误，则 CB_ERR。
...
调用 `GetDroppedControlRect` 成员函数以检索下拉组合框的可见 () 列表框的屏幕坐标。
...
- 按向下键将显示列表框 (F4 禁用了 F4) 。

当项列表不可见时，将禁用 "在静态控件中滚动" (禁用箭头键) 。
...
检索应用程序提供的与指定组合框项关联的32位值作为 () 的指针 **`void`** <strong>\*</strong> 。
...
字符串的长度 (（以字节) 为单位），不包括终止 null 字符。 如果 *nIndex* 未指定有效的索引，则返回值为 CB_ERR。

### <a name="remarks"></a>备注

此成员函数的第二种形式 `CString` 用该项的文本填充对象。
如果 *nIndex* 无效，则此函数将引发 `E_INVALIDARG` 异常 (错误代码： `-2147024809` `0x80070057`) 。
...
此函数有助于加快 (超过 100) 项的列表框的初始化速度。 它预分配指定的内存量，以便后续的 [AddString](#addstring)、 [InsertString](#insertstring)和 [Dir](#dir) 函数可以使用最短的时间。 您可以对参数使用估算值。 如果最好，则分配一些额外的内存;如果你低估，则会将常规分配用于超出预分配数量的项。
...
仅当字符串的初始字符 (起始点) 与前缀字符串中的字符相匹配时，才会选择字符串。
...
如有必要，列表框会将字符串滚动到 view (如果列表框) 显示。 组合框的编辑控件中的文本将更改以反映新的选择。 列表框中以前的任何选择都将被删除。
...
将与组合框中的指定项关联的32位值设置为 () 的指定指针 **`void`** <strong>\*</strong> 。
...
调用 `SetItemHeight` 成员函数，以设置组合框中的列表项的高度或编辑控件 (或静态文本) 部分的高度。
...
*nIndex*<br/>
指定是否设置组合框的列表项高度或编辑控件 (或静态文本) 部分的高度。
...
*nNewLocale*<br/>
新的区域设置标识符 (LCID) 为组合框设置的值。

### <a name="return-value"></a>返回值

先前的区域设置标识符 (此组合框的 LCID) 值。

### <a name="remarks"></a>备注

如果 `SetLocale` 未调用，则从系统获取默认区域设置。 可以使用 "控制面板" 的 "区域 (或" 国际) 应用程序 "修改此系统默认区域设置。
