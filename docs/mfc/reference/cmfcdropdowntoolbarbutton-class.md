---
description: 了解详细信息： CMFCDropDownToolbarButton 类
...
|[CMFCDropDownToolbarButton：:D ropDownToolbar](#dropdowntoolbar)|打开下拉工具栏。|
...
|`CMFCDropDownToolbarButton::OnCancelMode`|由框架调用，用于处理 [WM_CANCELMODE](/windows/win32/winmsg/wm-cancelmode) 消息。 （重写 `CMCToolBarButton::OnCancelMode`。）|
...
|[CMFCDropDownToolbarButton：：串行化](#serialize)|从存档中读取此对象或将其写入存档。  (重写 [CMFCToolBarButton：：串行化](../../mfc/reference/cmfctoolbarbutton-class.md#serialize)。 ) |
...
与 `CMFCDropDownToolBarButton` 普通按钮的不同之处在于，它在按钮的右下角有一个小箭头。 用户从下拉工具栏中选择某个按钮后，框架将在顶级工具栏按钮上显示其图标， (按钮右下角的小箭头) 。

有关如何实现下拉工具栏的信息，请参阅 [CMFCDropDownToolBar 类](../../mfc/reference/cmfcdropdowntoolbar-class.md)。

`CMFCDropDownToolBarButton`对象可以导出到[CMFCToolBarMenuButton 类](../../mfc/reference/cmfctoolbarmenubutton-class.md)对象，并显示为带有弹出菜单的菜单按钮。
...
调用此方法可将另一个工具栏按钮复制到此工具栏按钮。 *src* 的类型必须为 `CMFCDropDownToolbarButton` 。
...
下面的示例演示如何构造类的对象 `CMFCDropDownToolbarButton` 。 此代码片段是 [Visual Studio 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
如果父工具栏、 `m_pToolBar` 、为 NULL 或基类实现返回 FALSE，则此方法将失败。
...
此方法通过清除文本标签 ( [CMFCToolBarButton：： m_strText](../../mfc/reference/cmfctoolbarbutton-class.md#m_strtext)) 并将[CMFCToolBarButton：： m_bText](../../mfc/reference/cmfctoolbarbutton-class.md#m_btext)和[CMFCToolBarButton：： M_BUSERBUTTON](../../mfc/reference/cmfctoolbarbutton-class.md#m_buserbutton)数据成员设置为 FALSE，重写[CMFCToolBarButton：： OnChangeParentWnd](../../mfc/reference/cmfctoolbarbutton-class.md#onchangeparentwnd)) 的基类 ( 实现。
...
当用户单击工具栏按钮时，此方法将创建一个等待 [CMFCDropDownToolbarButton：： m_uiShowBarDelay](#m_uishowbardelay) 数据成员所指定时间长度的计时器，然后使用 [CMFCDropDownToolbarButton：:D ropdowntoolbar](#dropdowntoolbar) 方法打开下拉工具栏。 此方法会在用户第二次单击工具栏按钮时关闭下拉工具栏。
...
此方法通过调用 [CMFCDropDownToolbarButton：： OnClick](#onclick)方法，并将 *BDELAY* 设置为 FALSE 来扩展基类实现 ( [CMFCToolBarButton：： OnContextHelp](../../mfc/reference/cmfctoolbarbutton-class.md#oncontexthelp)) 。 此方法返回由 [CMFCDropDownToolbarButton：： OnClick](#onclick)返回的值。
...
此方法由 "自定义" 对话框 ( " **命令** " 选项卡) 当需要按钮在所有者描述列表框上显示时。

此方法通过将该按钮的文本标签更改为按钮的名称，从而扩展了基类实现 ( [CMFCToolBarButton：： OnDrawOnCustomizeList](../../mfc/reference/cmfctoolbarbutton-class.md#ondrawoncustomizelist))  (也就是说，将该按钮的名称更改为传递给构造) 函数的 *lpszName* 参数的值。

## <a name="cmfcdropdowntoolbarbuttonserialize"></a><a name="serialize"></a> CMFCDropDownToolbarButton：：串行化
...
此方法通过序列化父工具栏的资源 ID 来扩展基类实现 ( [CMFCToolBarButton：：串行化](../../mfc/reference/cmfctoolbarbutton-class.md#serialize)) 。 加载存档时 ( [CArchive：： IsLoading](../../mfc/reference/carchive-class.md#isloading) 返回非零值) ，则此方法 `m_pToolBar` 会将数据成员设置为包含序列化资源 ID 的工具栏。
