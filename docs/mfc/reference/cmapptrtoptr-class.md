---
description: 了解详细信息： CMapPtrToPtr 类
...
|[CMapPtrToPtr：： IsEmpty](../../mfc/reference/cmapstringtoob-class.md#isempty)|测试空映射条件 (不) 任何元素。|
...
`CMapPtrToPtr` 合并了 IMPLEMENT_DYNAMIC 宏，以支持运行时类型访问和转储到 `CDumpContext` 对象。 如果需要 (指针值) 各个地图元素的转储，则必须将转储上下文的深度设置为1或更大。
