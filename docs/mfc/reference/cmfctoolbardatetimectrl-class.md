---
description: 了解详细信息： CMFCToolBarDateTimeCtrl 类
...
|[CMFCToolBarDateTimeCtrl::GetByCmd](#getbycmd)|检索 `CMFCToolBarDateTimeCtrl` 应用程序中具有指定命令 ID 的第一个对象。|
...
有关如何使用日期和时间选取器控件的示例，请参阅 ToolbarDateTimePicker 示例项目。 有关如何向工具栏添加控件按钮的详细信息，请参阅 [演练：将控件放置在工具栏上](../../mfc/walkthrough-putting-controls-on-toolbars.md)。
...
默认情况下，框架不允许用户在自定义过程中拉伸工具栏按钮。 此方法通过允许用户在自定义过程中拉伸日期和时间工具栏按钮，扩展了基类实现 ( [CMFCToolBarButton：： CanBeStretched](../../mfc/reference/cmfctoolbarbutton-class.md#canbestretched)) 。
...
*iImage*<br/>
中工具栏的对象中的图像的索引 `CMFCToolBarImages` 。

*dwStyle*<br/>
中 `CMFCToolBarDateTimeCtrlImpl` 用户单击按钮时创建的窗口的样式。
...
此对象将初始化为系统日期和时间。 内部对象的窗口样式 `CMFCToolBarDateTimeCtrlImpl` 包括 *dwStyle* 参数以及 WS_CHILD 和 WS_VISIBLE 样式。 不能使用更改这些样式 `CMFCToolBarDateTimeCtrl::SetStyle` 。 用于 `SetStyle` 更改控件的样式 `CMFCToolBarDateTimeCtrl` 。

### <a name="example"></a>示例

下面的示例演示如何构造类的对象 `CMFCToolBarDateTimeCtrl` 。 此代码片段是 [工具栏日期时间选取器示例](../../overview/visual-cpp-samples.md)的一部分。
...
调用此方法可将另一个工具栏按钮复制到此工具栏按钮。 *src* 的类型必须为 `CMFCToolBarDateTimeCtrl` 。
...
此方法通过加载与控件的命令 ID 关联的字符串资源来重写基类实现 ( [CMFCToolBarButton：： ExportToMenuButton](../../mfc/reference/cmfctoolbarbutton-class.md#exporttomenubutton)) 。 有关字符串资源的详细信息，请参阅 [CStringT：： LoadString](../../atl-mfc-shared/reference/cstringt-class.md#loadstring)。

## <a name="cmfctoolbardatetimectrlgetbycmd"></a><a name="getbycmd"></a> CMFCToolBarDateTimeCtrl::GetByCmd

检索 `CMFCToolBarDateTimeCtrl` 应用程序中具有指定命令 ID 的第一个对象。
...
`CMFCToolBarDateTimeCtrl`应用程序中具有指定命令 id 的第一个对象，如果没有 `CMFCToolBarDateTimeCtrl` 指定的命令 id，则为 NULL。
...
`CMFCToolBarDateTimeCtrl` `m_pWndDateTime` 向工具栏中插入对象时，类将初始化数据成员 `CMFCToolBarDateTimeCtrl` 。
...
方法设置 [NMDATETIMECHANGE](/windows/win32/api/commctrl/ns-commctrl-nmdatetimechange) 结构成员 dwFlags，以指示日期和时间选取器是否设置为日期和时间。 如果值等于 GDT_NONE，则将控件设置为 `no date` status，并使用 DTS_SHOWNONE 样式。 如果返回的值等于 GDT_VALID，则系统时间将成功存储在目标位置。
...
如果框架找不到与命令 ID *uiCmd* 匹配的工具栏按钮，则在第一个重载中返回值为零，并在其他重载中 GDT_NONE。 如果找到工具栏按钮，则返回值与该按钮上对 [CMFCToolBarDateTimeCtrl：： GetTime](#gettime) 的调用中的返回值相同。 当找到按钮时，可能会出现零或 GDT_NONE 的返回值，这表明对的调用 `GetTime` 未返回有效日期，原因是其他原因。
...
如果按钮处理 WM_COMMAND 消息，则为 TRUE; 如果为 FALSE，则指示应由父工具栏处理消息。

### <a name="remarks"></a>备注

当框架即将向父窗口发送 [WM_COMMAND](/windows/win32/menurc/wm-command) 消息时，框架会调用此方法。

此方法通过处理[DTN_DATETIMECHANGE](/windows/win32/Controls/dtn-datetimechange)通知 ( [CMFCToolBarButton：： NotifyCommand](../../mfc/reference/cmfctoolbarbutton-class.md#notifycommand)) 扩展基类实现。 它更新内部时间状态，并更新 `CMFCToolBarDateTimeCtrl` 具有相同命令 ID 的所有对象的时间属性。
...
此方法通过重新创建内部对象来重写基类实现 ( [CMFCToolBarButton：： OnChangeParentWnd](../../mfc/reference/cmfctoolbarbutton-class.md#onchangeparentwnd)) `CMFCToolBarDateTimeCtrlImpl` 。
...
此方法通过返回非零值（如果内部对象可见）来重写基类实现 [CMFCToolBarButton：： OnClick](../../mfc/reference/cmfctoolbarbutton-class.md#onclick) `CMFCToolBarDateTimeCtrlImpl` 。
...
此方法通过更新内部对象的位置来重写默认类实现 ( [CMFCToolBarButton：： OnMove](../../mfc/reference/cmfctoolbarbutton-class.md#onmove)) `CMFCToolBarDateTimeCtrlImpl` 。
...
此方法通过在 *bShow* 为 TRUE 的情况下显示按钮，将基类实现 ( 扩展) ： [： OnShow](../../mfc/reference/cmfctoolbarbutton-class.md#onshow) 。 否则，此方法将隐藏按钮。
...
此方法通过更新内部对象的大小和位置来重写默认类实现 ( [CMFCToolBarButton：： OnSize](../../mfc/reference/cmfctoolbarbutton-class.md#onsize)) `CMFCToolBarDateTimeCtrlImpl` 。
...
此方法通过显示与按钮关联的工具提示文本，扩展了基类实现 ( [CMFCToolBarButton：： OnUpdateToolTip](../../mfc/reference/cmfctoolbarbutton-class.md#onupdatetooltip)) 。 如果该按钮未水平停靠，则此方法不执行任何操作并返回 FALSE。
