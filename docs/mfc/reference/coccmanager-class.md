---
description: 了解详细信息： COccManager 类
title: COccManager 类
ms.date: 11/04/2016
f1_keywords:
- COccManager
...
|[COccManager::CreateDlgControls](#createdlgcontrols)|创建由关联的对象承载的 ActiveX 控件 `COleContainer` 。|
...
基类 `CNoTrackObject` 是一个未记录的基类 (位于 AFXTLS 中。H) 。 旨在供 MFC 框架使用，从类派生的类 `CNoTrackObject` 不受内存泄漏检测的阻止。 不建议直接从派生 `CNoTrackObject` 。
...
## <a name="coccmanagercreatesite"></a><a name="createsite"></a> COccManager：：网站
...
每个控件容器都可以托管多个站点。 创建具有多个调用的其他站点 `CreateSite` 。
...
的默认行为 `IsDialogMessage` 是检查键盘消息，并将其转换为相应对话框的选定内容。 例如，按 TAB 键时，将选择下一个控件或一组控件。
...
*pHandlerInfo*<br/>
如果不为 NULL，则 `OnEvent` 填充 `pTarget` 结构的和 `pmf` 成员， `AFX_CMDHANDLERINFO` 而不是分派命令。 通常，此参数应为 NULL。
...
此内存由对的调用分配 `SplitDialogTemplate` ，并用于对话框中的任何托管 ActiveX 控件。
...
> [!NOTE]
> 为新模板分配的内存在函数中释放 `PostCreateDialog` 。
