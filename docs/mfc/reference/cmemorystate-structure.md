---
description: 了解详细信息： CMemoryState 结构
...
如果您怀疑您的程序有内存泄漏，则可以使用 `Checkpoint` 、 `Difference` 和 `DumpStatistics` 函数来发现内存状态 () 在程序执行中的两个不同点分配的对象之间的差异。 此信息可用于确定某个函数是否正在清理它分配的所有对象。

如果只是了解分配和解除分配不平衡的位置不能提供足够的信息，则可以使用 `DumpAllObjectsSince` 函数转储自上一次调用后分配的所有对象 `Checkpoint` 。 如果你使用 DEBUG_NEW 进行分配) ，并显示对象的派生、其地址和大小，则此转储显示分配的顺序，即分配对象的源文件和行 (。 `DumpAllObjectsSince` 还会调用每个对象的 `Dump` 函数，以提供有关其当前状态的信息。
...
`Dump`为从类派生的类型的所有对象调用函数，该类型派生自 `CObject` (，并在最后一个[检查点](#checkpoint)调用该对象时仍被分配) `CMemoryState` 。
