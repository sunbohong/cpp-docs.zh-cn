---
description: 了解详细信息： CMFCToolBarImages 类
...
|[CMFCToolBarImages::CreateFromImageList](#createfromimagelist)| ( [CImageList 类](../../mfc/reference/cimagelist-class.md)) 的图像列表初始化工具栏图像。|
...
|[CMFCToolBarImages：:D raw](#draw)| (按钮) 绘制单个工具栏图像。|
...
|[CMFCToolBarImages：： GetImageSize](#getimagesize)|检索存储在内存中的工具栏图像大小 (源大小) ，或在屏幕上绘制 (目标大小) 的工具栏图像的大小。|
...
|[CMFCToolBarImages::SetImageSize](#setimagesize)| (源大小) 设置工具栏图像的大小。|
...
|名称|描述|
|----------|-----------------|
|[CMFCToolBarImages：： m_bDisableTrueColorAlpha](#m_bdisabletruecoloralpha)|如果 (32 位颜色) ，则为 TRUE。|

## <a name="remarks"></a>备注

由管理的工具栏图像的完整位图 `CMFCToolbarImages` 包含一个或多个小工具栏图像 (按钮) 固定大小。

## <a name="example"></a>示例

下面的示例演示如何 `CMFCToolBarImages` 使用类中的各种方法配置对象 `CMFCToolBarImages` 。 该示例演示如何设置工具栏图像的大小，如何加载图像，以及如何设置图像的透明颜色。 此代码片段是 [Visual Studio 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
## <a name="cmfctoolbarimagesaddimage"></a><a name="addimage"></a> CMFCToolBarImages：： Shapes.addimage
...
*bSetBitPerPixel*<br/>
中如果 `CMFCToolBarImages`  对象使用颜色深度 (新图像的每像素位数) ，则为 TRUE;如果 `CMFCToolbarImages` 对象保持当前颜色深度，则为 FALSE。

*imageList*<br/>
中对 `CMFCToolbarImages` 包含要添加的图像的对象的引用。

*nIndex*<br/>
中要添加的图像的源对象中的索引 `CMFCToolbarImages` 。

### <a name="return-value"></a>返回值

`CMFCToolBarImages`成功添加新位图后对象维护的工具栏图像数; 如果操作失败，则为-1。

## <a name="cmfctoolbarimagescleanup"></a><a name="cleanup"></a> CMFCToolBarImages：：清理
...
如果映像已成功删除，则为 TRUE;如果图像索引无效、 `CMFCToolbarImages` 对象为临时对象、 `CMFCToolbarImages` 对象不包含用户定义的图像或发生其他错误，则为 FALSE。
...
*ds*<br/>
中对 `CAfxDrawState` 传递给方法的对象的引用 `PrepareDrawImage` 。
...
检索存储在内存中的工具栏图像大小 (源大小) ，或在屏幕上绘制 (目标大小) 的工具栏图像的大小。
...
工具栏图像以单个位图的行的形式存储在单个位图 *中。* 若要在图像中查找工具栏图像，请将图像的索引乘以工具栏图像的宽度 (参见 [CMFCToolBarImages：： GetImageSize](#getimagesize)) ，以获取图像内图像的水平偏移量。
...
> [!NOTE]
> 如果要撤消更改，则必须从源中重新加载映像。 为此，可以调用[CMFCToolBarImages：： Load](#load)或[CMFCToolBarImages：： UpdateImage](#updateimage) (仅适用于用户定义的图像) ，或通过调用[CMFCToolBarImages：： AddIcon](#addicon)或[CMFCToolBarImages：： shapes.addimage](#addimage)调用[CMFCToolBarImages：： Clear](#clear)并再次添加映像。
...
`CMFCToolbarImages`当从只读文件加载带有工具栏图像的位图，或使用方法在中复制位图时，对象为只读 `CMFCToolBarImages::CopyTemp` 。
...
如果对象有效，则为 TRUE `CMFCToolBarImages`  ; 否则为 FALSE。

### <a name="remarks"></a>备注

`CMFCToolBarImages`当使用工具栏图像的位图的句柄为空时，对象无效。
...
*ds*<br/>
中对结构的引用 `CAfxDrawState` ，它在图像呈现阶段之间存储已分配的资源。
...
调用此方法后，可以调用 [CMFCToolBarImages：:D raw](#draw) 任意次。 完成绘制后，必须调用 [CMFCToolBarImages：： EndDrawImage](#enddrawimage) 以释放分配的资源 `PrepareDrawImage` 。
