---
description: 了解详细信息： COleDocObjectItem 类
...
|“属性”|描述|
|----------|-----------------|
|[COleDocObjectItem：:D oDefaultPrinting](#dodefaultprinting)|使用默认打印机设置打印容器应用程序的文档。|
...
- `COleDocument`派生的类仍保留当前嵌入项的列表; 但是，这些项可能是 `COleDocObjectItem` 派生项。
...
由于活动文档容器拥有 " **帮助** " 菜单，因此容器负责将服务器 **帮助** 菜单消息转发到服务器。 此集成由处理 `COleDocObjectItem` 。
...
*pContainerDoc*<br/>
指向 `COleDocument` 作为活动文档容器的对象的指针。 若要启用 IMPLEMENT_SERIALIZE，此参数必须为 NULL。 通常使用非 NULL 文档指针构造 OLE 项。
...
返回的指针上的引用计数在 `IOleDocumentView` 此函数返回之前不会递增。
