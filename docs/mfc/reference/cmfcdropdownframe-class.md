...
框架使用此类向和类提供框架行为 `CMFCDropDownToolbar` `CMFCDropDownToolbarButton` 。 有关这些类的详细信息，请参阅 [CMFCDropDownToolBar 类](../../mfc/reference/cmfcdropdowntoolbar-class.md) 和 [CMFCDropDownToolbarButton 类](../../mfc/reference/cmfcdropdowntoolbarbutton-class.md)。

## <a name="example"></a>示例

下面的示例演示如何从类中检索指向对象的指针 `CMFCDropDownFrame` `CFrameWnd` ，以及如何将子下拉工具栏窗口设置为自动销毁。
...
`CMFCDropDownFrame`类创建提供的参数的副本 `CMFCDropDownToolBar` 。 此方法从参数将按钮图像和按钮状态复制 `pWndOriginToolbar` 到 `m_pWndOriginToolbar` 数据成员。
