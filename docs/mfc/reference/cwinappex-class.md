---
description: 了解详细信息： CWinAppEx 类
title: CWinAppEx 类
ms.date: 11/04/2016
f1_keywords:
- CWinAppEx
...
|[CWinAppEx：： ExitInstance](#exitinstance)|由框架在成员函数内调用 `Run` 以退出应用程序实例。  (重写 [CWinApp：： ExitInstance](../../mfc/reference/cwinapp-class.md#exitinstance)。 ) |
...
|[CWinAppEx：： GetObject](#getobject)|`CObject`从注册表读取与指定值关联的数据。|
...
|[CWinAppEx：： GetSectionObject](#getsectionobject)|`CObject`从注册表读取与指定的键和值相关联的数据。|
...
|[CWinAppEx：： WriteSectionObject](#writesectionobject)|将从类派生的数据写入 `CObject` 指定注册表项的值。|
...
MFC 框架提供的很多功能取决于 `CWinAppEx` 类。 您可以 `CWinAppEx` 通过以下两种方式之一将类合并到您的应用程序中：

- `CWinAppEx`在主线程中构造一个类。

- 从派生主应用程序类 `CWinAppEx` 。

将应用程序集成 `CWinAppEx` 到应用程序中后，可以初始化任何一个应用程序管理器。 使用应用程序管理器之前，必须通过调用相应的 initialize 方法对其进行初始化。 若要获取指向特定管理器的指针，请调用关联的 get 方法。 `CWinAppEx`类管理以下应用程序管理器： [CMouseManager 类](../../mfc/reference/cmousemanager-class.md)、 [CContextMenuManager 类](../../mfc/reference/ccontextmenumanager-class.md)、 [CKeyboardManager 类](../../mfc/reference/ckeyboardmanager-class.md)、 [CUserToolsManager 类](../../mfc/reference/cusertoolsmanager-class.md)和[CMenuTearOffManager 类](../../mfc/reference/cmenutearoffmanager-class.md)。
...
此方法清除注册表的特定部分中的应用程序数据。 您可以使用参数 *lpszSectionName* 指定要清除的部分。 如果 *lpszSectionName* 为 NULL，则此方法将使用存储在对象中的默认注册表路径 `CWinAppEx` 。 若要获取默认的注册表路径，请使用 [CWinAppEx：： GetRegistryBase](#getregistrybase)。
...
`CWinAppEx`类具有初始化方法，提供用于将应用程序信息保存和加载到注册表并控制全局应用程序设置的功能。 它还使你能够使用全局管理器，如 [CKeyboardManager 类](../../mfc/reference/ckeyboardmanager-class.md) 和 [CUserToolsManager 类](../../mfc/reference/cusertoolsmanager-class.md)。 每个应用程序只能有一个类的实例 `CWinAppEx` 。
...
如果 `CMenuTearOffManager` 成功创建并初始化了，则为 TRUE;如果发生错误或已存在，则为 FALSE `CMenuTearOffManager` 。

### <a name="remarks"></a>备注

使用此函数可在应用程序中启用脱离菜单。 应从调用此函数 `InitInstance` 。
...
*pToolRTC*<br/>
中 `CUserToolsManager` 对象用于创建新用户工具的类。
...
如果该方法创建并初始化一个对象，则为 TRUE `CUserToolsManager` ;如果方法失败或对象已存在，则为 FALSE `CUserToolsManager` 。
...
此方法从派生自的注册表读取数据 `CObject` 。 若要将 `CObject` 数据写入注册表，请使用 [CWinAppEx：： WriteObject](#writeobject) 或 [CWinAppEx：： WriteSectionObject](#writesectionobject)。
...
*obj*<br/>
弄对的引用 `CObject` 。 方法使用此 `CObject` 存储注册表数据。
...
此方法从注册表读取数据。 读取的数据为 `CObject` 数据，或者是从派生的类的数据 `CObject` 。 若要将 `CObject` 数据写入注册表，请使用 [CWinAppEx：： WriteObject](#writeobject) 或 [CWinAppEx：： WriteSectionObject](#writesectionobject)。
...
指向全局对象的指针 `CUserToolsManager` ;如果没有为应用程序启用用户工具管理，则为 NULL。

### <a name="remarks"></a>备注

检索指向对象的指针之前 `CUserToolsManager` ，必须通过调用 [CWinAppEx：： EnableUserTools](#enableusertools)初始化管理器。
...
如果方法创建 CContextMenuManager 对象，则为非零值;如果该对象已存在，则为 0 `CContextMenuManager` 。

### <a name="remarks"></a>备注

如果调用 [CWinAppEx：： GetContextMenuManager](#getcontextmenumanager)，则该方法的默认实现将调用 `InitContextMenuManager` 。

如果你的应用程序已具有上下文菜单管理器并调用 `InitContextMenuManager` ，则你的应用程序将有一个 [断言](diagnostic-services.md#assert) 失败。 因此， `InitContextMenuManager` 如果直接创建对象，则不应调用 `CContextMenuManager` 。 如果使用的不是自定义 `CContextMenuManager` ，则应使用 `GetContextMenuManager` 来创建 `CContextMenuManager` 对象。
...
如果该方法创建对象，则为非零 `CKeyboardManager` ; 如果该对象已存在，则为 0 `CKeyboardManager` 。

### <a name="remarks"></a>备注

如果调用 [CWinAppEx：： GetKeyboardManager](#getkeyboardmanager)，则该方法的默认实现将调用 `InitKeyboardManager` 。

如果你的应用程序已经有一个键盘管理器，而你调用了 `InitKeyboardManager` ，则你的应用程序将有一个 [断言](diagnostic-services.md#assert) 失败。 因此， `InitKeyboardManager` 如果直接创建对象，则不应调用 `CKeyboardManager` 。 如果使用的不是自定义 `CKeyboardManager` ，则应使用 `GetKeyboardManager` 来创建 `CKeyboardManager` 对象。
...
如果该方法创建对象，则为非零 `CMouseManager` ; 如果该对象已存在，则为 0 `CMouseManager` 。

### <a name="remarks"></a>备注

如果调用 [CWinAppEx：： GetMouseManager](#getmousemanager)，则该方法的默认实现将调用 `InitMouseManager` 。

如果你的应用程序已经有一个鼠标管理器，而你调用了 `InitMouseManager` ，则你的应用程序将有一个 [断言](diagnostic-services.md#assert) 失败。 因此， `InitMouseManager` 如果直接创建对象，则不应调用 `CMouseManager` 。 如果使用的不是自定义 `CMouseManager` ，则应使用 `GetMouseManager` 来创建 `CMouseManager` 对象。
...
如果该方法创建对象，则为非零 `CShellManager` ; 如果该对象已存在，则为 0 `CShellManager` 。

### <a name="remarks"></a>备注

如果调用 [CWinAppEx：： GetShellManager](#getshellmanager)，则该方法的默认实现将调用 `InitShellManager` 。

如果你的应用程序已具有 shell 管理器并调用 `InitShellManager` ，则应用程序将引发 [断言](diagnostic-services.md#assert) 失败。 因此， `InitShellManager` 当您直接创建对象时，请不要调用 `CShellManager` 。 如果使用的不是自定义 `CShellManager` ，请使用 `GetShellManager` 创建 `CShellManager` 对象。
...
如果该方法创建对象，则为非零 `CTooltipManager` ; 如果该对象已存在，则为 0 `CTooltipManager` 。

### <a name="remarks"></a>备注

如果调用 [CWinAppEx：： GetTooltipManager](#gettooltipmanager)，则该方法的默认实现将调用 `InitTooltipManager` 。

如果你的应用程序已具有工具提示管理器并调用 `InitTooltipManager` ，则你的应用程序将有一个 [断言](diagnostic-services.md#assert) 失败。 因此， `InitTooltipManager` 如果直接创建对象，则不应调用 `CTooltipManager` 。 如果使用的不是自定义 `CTooltipManager` ，则应使用 `GetTooltipManager` 来创建 `CTooltipManager` 对象。
...
*pFrameImpl*<br/>
中指向对象的指针 `CFrameImpl` 。 方法将注册表中的状态信息应用于此框架窗口。
...
调用的默认实现 `CFrameImpl::OnLoadFrame` `LoadState` 。
...
*pFrameImpl*<br/>
中指向对象的指针 `CFrameImpl` 。 此帧将保存到 Windows 注册表中。
...
若要支持弹出菜单，必须有一个 [CContextMenuManager](../../mfc/reference/ccontextmenumanager-class.md) 对象。 如果尚未初始化 `CContextMenuManager` 对象，则 `ShowPopupMenu` 将失败。
