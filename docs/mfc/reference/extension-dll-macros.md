...
description: 用于管理 Dll 的 MFC 宏和函数的参考指南。
ms.date: 11/30/2020
helpviewer_keywords:
...
| [`AfxGetStaticModuleState`](#afxgetstaticmodulestate) | 在初始化之前设置模块状态，并在清理后还原以前的模块状态。 |
...
使用 `AFX_EXT_CLASS` 宏，用于生成 MFC 扩展 DLL 的相同头文件 () 可与链接到 DLL 的可执行文件一起使用。
...
调用此宏时， *`pModuleState`* 为其余包含作用域的其余部分的有效模块状态。 离开作用域后，将自动还原以前的有效模块状态。

`AFX_MODULE_STATE`结构包含模块的全局数据，即推送或弹出模块状态的部分。

默认情况下，MFC 使用主应用程序的资源句柄来加载资源模板。 如果 DLL 中有一个已导出的函数（例如在 DLL 中启动对话框的函数），则资源模板存储在 DLL 模块中。 务必切换模块状态，以使用正确的句柄。 可以通过将以下代码添加到函数的开头来切换状态：

```cpp
AFX_MANAGE_STATE(AfxGetStaticModuleState( ));
```

此宏将当前模块状态替换为从返回的状态， [`AfxGetStaticModuleState`](#afxgetstaticmodulestate) 直到当前范围结束。

有关模块状态和 MFC 的详细信息，请参阅 [管理 mfc 模块的状态数据](../managing-the-state-data-of-mfc-modules.md) 和 [技术说明 58](../tn058-mfc-module-state-implementation.md)。

> [!NOTE]
> MFC 为程序集创建激活上下文时，它使用 [`AfxWinInit`](application-information-and-management.md#afxwininit) 创建上下文并 `AFX_MANAGE_STATE` 激活和停用上下文。 另请注意，为 `AFX_MANAGE_STATE` 静态 MFC 库和 Mfc dll 启用了，以便允许在用户 DLL 所选的正确激活上下文中执行 mfc 代码。 有关详细信息，请参阅 [MFC 模块状态中的激活上下文支持](../support-for-activation-contexts-in-the-mfc-module-state.md)。
...
如果要生成 OLE 控件并使用 `COleControlModule` ，则不应调用， `AfxOleInitModule` 因为调用的 `InitInstance` 成员函数 `COleControlModule` `AfxOleInitModule` 。
...
如果将标志设置 (这是默认) 并且线程进入 MFC 模块 (参阅 [`AFX_MANAGE_STATE`](#afx_manage_state)) ，则激活模块的上下文。

如果未设置标志，则不会在输入时激活模块的上下文。

模块的上下文由其清单确定，通常嵌入在模块资源中。
...
调用此函数可在初始化之前设置模块状态，并在清理后还原以前的模块状态。
...
此宏将当前模块状态替换为从返回的状态， `AfxGetStaticModuleState` 直到当前范围结束。
...
*`state`*<br/>
对[ `AFX_EXTENSION_MODULE` 结构](afx-extension-module-structure.md)结构的引用，该结构结构将包含初始化后的 MFC 扩展 DLL 模块的状态。 该状态包含一个运行时类对象的副本，这些对象已被 MFC 扩展 DLL 初始化为输入之前执行的正常静态静态对象构造的一部分 `DllMain` 。
...
`TRUE` 如果 MFC 扩展 DLL 已成功初始化，则为;否则为 `FALSE` 。
...
- `CDynLinkLibrary`如果 DLL 将导出[ `CRuntimeClass` 结构](cruntimeclass-structure.md)对象或有其自己的自定义资源，请创建对象。

`AfxTermExtensionModule`如果每个进程都与 mfc 扩展 dll 分离，则可以调用来清理 mfc 扩展 dll (在进程退出时发生，或者当调用) 卸载 DLL 时 `AfxFreeLibrary` 。
...
调用此函数可在每个进程与 DLL 分离时，调用此函数以清理 MFC 扩展 DLL (发生在进程退出时或调用) 卸载 DLL 时 `AfxFreeLibrary` 。
...
*`state`*<br/>
对 [`AFX_EXTENSION_MODULE`](afx-extension-module-structure.md) 包含 MFC 扩展 DLL 模块状态的结构的引用。
...
如果你的应用程序动态加载和释放 MFC 扩展 Dll，请确保调用 `AfxTermExtensionModule` 。 由于大多数 MFC 扩展 Dll 都不会动态地 (加载，因此它们通过其导入库进行链接) ， `AfxTermExtensionModule` 通常不需要调用。

MFC 扩展 Dll 需要 [`AfxInitExtensionModule`](#afxinitextensionmodule) 在其中调用 `DllMain` 。 如果 DLL 导出 [`CRuntimeClass`](cruntimeclass-structure.md) 对象或具有其自己的自定义资源，则还需要 `CDynLinkLibrary` 在中创建一个对象 `DllMain` 。
