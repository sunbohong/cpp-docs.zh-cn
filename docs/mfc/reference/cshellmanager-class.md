---
description: 了解详细信息： CShellManager 类
...
类的方法 `CShellManager` 均处理 pidl。 PIDL 是 shell 对象的唯一标识符。

不应 `CShellManager` 手动创建对象。 应用程序的框架会自动创建它。 但是，你应在应用程序的初始化过程中调用 [CWinAppEx：： InitShellManager](../../mfc/reference/cwinappex-class.md#initshellmanager) 。 若要获取应用程序的 shell 管理器的指针，请调用 [CWinAppEx：： GetShellManager](../../mfc/reference/cwinappex-class.md#getshellmanager)。
...
下面的示例演示如何使用方法检索对对象的引用 `CShellManager` `CWinAppEx::GetShellManager` 以及如何使用 `BrowseForFolder` 方法。 此代码片段是 [资源管理器示例](../../overview/visual-cpp-samples.md)的一部分。
...
在大多数情况下，无需 `CShellManager` 直接创建。 默认情况下，框架为你创建一个。 若要获取指向的指针 `CShellManager` ，请调用 [CWinAppEx：： GetShellManager](../../mfc/reference/cwinappex-class.md#getshellmanager)。 如果 `CShellManager` 手动创建，则必须使用方法 [CWinAppEx：： InitShellManager](../../mfc/reference/cwinappex-class.md#initshellmanager)对其进行初始化。
