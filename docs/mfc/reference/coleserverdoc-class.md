---
description: 了解详细信息： COleServerDoc 类
...
|[COleServerDoc：： GetClientSite](#getclientsite)|检索指向基础接口的指针 `IOleClientSite` 。|
...
服务器文档可以包含 [COleServerItem](../../mfc/reference/coleserveritem-class.md) 对象，这些对象表示嵌入或链接项的服务器接口。 如果容器启动服务器应用程序以编辑嵌入项，则会将该项作为其自己的服务器文档加载; `COleServerDoc` 对象只包含一个 `COleServerItem` 对象，其中包含整个文档。 当容器启动服务器应用程序以编辑链接项时，将从磁盘加载现有文档;文档内容的一部分将突出显示以指示链接项。

`COleServerDoc` 对象也可以包含 [COleClientItem](../../mfc/reference/coleclientitem-class.md) 类的项。 这允许您创建容器-服务器应用程序。 在为对象提供服务时，框架提供用于正确存储项的函数 `COleClientItem` `COleServerItem` 。

如果你的服务器应用程序不支持链接，则服务器文档将始终只包含一个服务器项，这会将整个嵌入对象表示为文档。 如果你的服务器应用程序支持链接，则每次将选定内容复制到剪贴板时，它都必须创建一个服务器项。

若要使用 `COleServerDoc` ，请从中派生类并实现 [OnGetEmbeddedItem](#ongetembeddeditem) 成员函数，使服务器能够支持嵌入项。 从中派生一个类 `COleServerItem` ，以实现文档中的项，并从返回该类的对象 `OnGetEmbeddedItem` 。
...
您 `COleServerDoc` 的应用程序支持的每种服务器文档都需要一个派生类。 例如，如果您的服务器应用程序支持工作表和图表，则需要两个 `COleServerDoc` 派生类。
...
默认情况下，不 `COleServerDoc` 支持活动文档 (也称为 DocObjects) 。 若要启用此支持，请参阅 [GetDocObjectServer](#getdocobjectserver) 和类 [CDocObjectServer](../../mfc/reference/cdocobjectserver-class.md)。
...
此函数由 [COleServerItem：： OnShow](../../mfc/reference/coleserveritem-class.md#onshow)的默认实现调用。 如果你的应用程序为就地激活 (如播放) ，则调用此函数。
...
必须调用 [COleLinkingDoc：： Register](../../mfc/reference/colelinkingdoc-class.md#register) 才能打开与 OLE 的通信。 如果在应用程序中使用 [COleTemplateServer](../../mfc/reference/coletemplateserver-class.md) ，则 `COleLinkingDoc::Register` 通过 `COleLinkingDoc` 的 `OnNewDocument` 、和实现调用 `OnOpenDocument` `OnSaveDocument` 。
...
指向的指针 `CDocObjectServer` ;如果操作失败，则为 NULL。

### <a name="remarks"></a>备注

激活 DocObject 服务器后，返回非 NULL 指针将显示客户端可以支持 DocObjects。 默认实现返回 NULL。

支持 DocObjects 的文档的典型实现只是分配新的 `CDocObjectServer` 对象并将其返回给调用方。 例如：
...
*lpClipRect*<br/>
指向结构的指针， `RECT` 或用于 `CRect` 接收项的剪辑矩形坐标的对象。
...
*lpPosRect*<br/>
指向结构的指针， `RECT` 或用于 `CRect` 接收该项坐标的对象。

### <a name="remarks"></a>备注

坐标以像素为单位，相对于容器应用程序窗口的工作区。

项的位置可以与当前剪裁矩形进行比较，以确定该项的可见范围 (或在屏幕上不可见) 。
...
*lpSizeNum*<br/>
指向类的对象的指针 `CSize` ，它将保存缩放系数的分子。 可以为 NULL。

*lpSizeDenom*<br/>
指向类的对象的指针 `CSize` ，它将保存缩放系数的分母。 可以为 NULL。
...
缩放系数（以像素为单位）是项大小与当前范围的比例。 如果容器应用程序未设置项的区，则会使用 [COleServerItem：： OnGetExtent](../../mfc/reference/coleserveritem-class.md#ongetextent)) 确定的自然范围 (。

函数将它的前两个参数设置为项的 "缩放系数" 的分子和分母。 如果未就地编辑项，则函数会将这些参数设置为默认值 100% (或 1:1) 并返回零。 有关详细信息，请参阅技术说明40： [MFC/OLE In-Place 调整大小和缩放](../../mfc/tn040-mfc-ole-in-place-resizing-and-zooming.md)。
...
如果对象处于 `COleServerDoc` 活动状态，则为非零; 否则为0。
...
通常，在用户更改某些全局属性（如服务器文档的维度）后，调用此函数。 如果某个 OLE 项已链接到带有自动链接的文档，则该项将更新以反映所做的更改。 在用 Microsoft 基础类库编写的容器应用程序中，调用的 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 成员函数 `COleClientItem` 。
...
当用户从 "文件" 菜单中选择 "关闭" 命令时， `NotifyClosed` 将调用 `COleServerDoc` [OnCloseDocument](../../mfc/reference/cdocument-class.md#onclosedocument) 成员函数的实现。 在用 Microsoft 基础类库编写的容器应用程序中，调用的 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 成员函数 `COleClientItem` 。
...
当用户从 "文件" 菜单中选择 "另存为" 命令时， `NotifyRename` 将调用 `COleServerDoc` [onopendocument](../../mfc/reference/cdocument-class.md#onsavedocument) 成员函数的实现。 此函数通知 OLE 系统 Dll，进而通知容器。 在用 Microsoft 基础类库编写的容器应用程序中，调用的 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 成员函数 `COleClientItem` 。
...
当用户从 "文件" 菜单中选择 "保存" 命令时， `NotifySaved` 将通过 `COleServerDoc` 的 [onopendocument](../../mfc/reference/cdocument-class.md#onsavedocument)实现来调用。 此函数通知 OLE 系统 Dll，进而通知容器。 在用 Microsoft 基础类库编写的容器应用程序中，调用的 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 成员函数 `COleClientItem` 。
...
`COleCmdUI` 可用于启用、更新和设置 DocObject 用户界面命令的其他属性。 初始化命令后，可以通过执行这些命令 `OnExecOleCmd` 。

框架在尝试转换和调度 OLE 文档命令之前调用函数。 不需要重写此函数来处理标准 OLE 文档命令，但是，如果您想要处理您自己的自定义命令或处理接受参数或返回结果的命令，则必须提供此函数的重写。

大多数命令不采用参数或返回值。 对于大多数命令，调用方可以传递 *pvarargIn* 和 *PvarargOut* 的 null 值。 对于需要输入值的命令，调用方可以声明和初始化 VARIANTARG 变量，并将指针传递到 *pvarargIn* 中的变量。 对于需要单个值的命令，可以直接将参数存储在 VARIANTARG 中并将其传递给函数。 必须使用受支持的类型之一在 VARIANTARG 中对多个参数进行打包 `IDispatch` ，如和 )  (。
...
## <a name="coleserverdoconframewindowactivate"></a><a name="onframewindowactivate"></a> COleServerDoc：： Onframewindowactivate 调用
...
如果你的应用程序支持撤消，则重写此函数。 通常，您将执行撤消操作，然后通过调用来激活该项 `ActivateInPlace` 。 如果容器应用程序是用 Microsoft 基础类库编写的，则调用将 `COleClientItem::ReactivateAndUndo` 导致调用此函数。
...
*lpRectBorder*<br/>
指向结构的指针 `RECT` 或 `CRect` 指定边框坐标的对象。

*lpUIWindow*<br/>
指向 `IOleInPlaceUIWindow` 拥有当前就地编辑会话的类的对象的指针。
...
*lpPosRect*<br/>
指向结构的指针 `RECT` 或 `CRect` 指定就地框架窗口相对于容器应用程序工作区的位置的对象。

*lpClipRect*<br/>
指向结构的指针 `RECT` 或 `CRect` 指定就地框架窗口相对于容器应用程序的工作区的剪辑矩形的对象。
...
`OnShowDocument`当必须隐藏或显示服务器文档时，框架将调用函数。
...
如果 *bShow* 为 TRUE，则默认实现将激活服务器应用程序（如有必要），并使容器应用程序滚动其窗口，使项可见。 如果 *bShow* 为 FALSE，则默认实现通过调用来停用项 `OnDeactivate` ，然后销毁或隐藏已为文档创建的所有框架窗口（第一个除外）。 如果没有任何可见文档，则默认实现将隐藏服务器应用程序。
...
*lpPosRect*<br/>
指向结构的指针 `RECT` 或 `CRect` 包含该项的新位置的对象。

### <a name="remarks"></a>备注

`UpdateAllItems`当就地活动项中的数据发生更改时，通常将此函数与)  (一起调用。 在此调用之后，容器可能会或不会通过调用来执行更改 `OnSetItemRects` 。 生成的位置可能与请求的位置不同。
...
此函数由自动调用 `OnUpdateDocument` 。 请注意，此函数会导致在磁盘上更新项，因此通常仅作为特定用户操作的结果调用。

## <a name="coleserverdocscrollcontainerby"></a><a name="scrollcontainerby"></a> COleServerDoc：： ScrollContainerBy

调用 `ScrollContainerBy` 成员函数以按所指示的量（以像素为单位）滚动容器文档 `sizeScroll` 。
...
通常在用户更改服务器文档后调用此函数。 如果某个 OLE 项已链接到带有自动链接的文档，则该项将更新以反映所做的更改。 在用 Microsoft 基础类库编写的容器应用程序中，调用的 [OnChange](../../mfc/reference/coleclientitem-class.md#onchange) 成员函数 `COleClientItem` 。

此函数 `OnUpdate` 为文档的每个项（发送项除外，传递 *pHint*、 *lHint* 和 *nDrawAspect*）调用成员函数。 使用这些参数将信息传递到有关对文档所做的修改的项。 您可以使用 *lHint* 对信息进行编码，也可以定义一个 `CObject` 派生类，用于存储有关修改的信息并使用 *pHint* 传递该类的对象。 重写 `OnUpdate` 派生类中的成员函数 `COleServerItem` ，以优化每个项的更新，具体取决于其演示是否已更改。
