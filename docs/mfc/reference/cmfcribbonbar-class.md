---
description: 了解详细信息： CMFCRibbonBar 类
...
下面的示例演示如何使用 `AddCategory` 类中的方法 `CMFCRibbonBar` 。
...
指向新创建的类别的指针; 如果 `CreateObject` *pRTI* 的方法无法创建指定的类别，则为 NULL。
...
下面的示例演示如何使用 `AddMainCategory` 类中的方法 `CMFCRibbonBar` 。
...
下面的示例演示如何使用类的 `Create` 方法 `CMFCRibbonBar` 。
...
中如果要测试功能区面板的标题，则为 TRUE; 否则为。如果设置为 FALSE，则不测试功能区面板的标题。 有关详细信息，请参阅备注部分。
...
下面的示例演示如何使用 `SetApplicationButton` 类中的方法 `CMFCRibbonBar` 。
...
下面的示例演示如何使用 `SetQuickAccessCommands` 类中的方法 `CMFCRibbonBar` 。
...
下面的示例演示如何使用 `SetQuickAccessDefaultState` 类中的方法 `CMFCRibbonBar` 。
...
启用或禁用对功能区的 Windows 7 外观 (小型矩形应用程序按钮) 。
