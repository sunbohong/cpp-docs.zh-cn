---
description: 了解详细信息： CMFCToolBarFontComboBox 类
...
|“属性”|描述|
|----------|-----------------|
|[CMFCToolBarFontComboBox::GetFontDesc](#getfontdesc)|返回一个指向 `CMFCFontInfo` 组合框中指定索引的对象的指针。|
...
*nFontType*<br/>
中组合框包含的字体类型。 此参数可以是下列值 (布尔值或) 的组合：
...
*nCharSet*<br/>
中如果设置为 DEFAULT_CHARSET，组合框将包含所有字符集中所有唯一命名的字体。  (如果有两个具有相同名称的字体，组合框会包含其中一个 ) 。如果设置为有效的字符集值，则组合框只包含指定字符集中的字体。 有关可能的字符集的列表，请参阅 [LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw) 。

*dwStyle*<br/>
中组合框的样式。  (参见 [组合框样式](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles)) 
...
通常， `CMFCToolBarFontComboBox` 对象会在单个共享对象中存储可用字体的列表 `CObList` 。 如果使用构造函数的第二个重载，并提供指向 *pLstFontsExternal* 的有效指针，则该 `CMFCToolBarFontComboBox` 对象将改为使用 `CObList` 可用字体填充 *pLstFontsExternal* 指向的内容。
...
返回一个指向 `CMFCFontInfo` 组合框中指定索引的对象的指针。
...
如果 `m_nFontHeight` 变量是0，则会根据组合框的默认字体自动计算高度。 高度包括基线上方的字符的上移量和基线下的字符的下降量。
...
如果 *bExact* 为 TRUE，则此方法将选择与指定为 *lpszName* 的名称完全匹配的字体。 如果 *bExact* 为 FALSE，此方法将选择一个字体，该字体以指定为 *lpszName* 的文本开头，并使用指定为 *nCharSet* 的字符集。 如果将 *nCharSet* 设置为 DEFAULT_CHARSET，则将忽略字符集，并且仅 *lpszName* 将用于选择字体。
