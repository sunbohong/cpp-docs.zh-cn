...
第一版在 `InsertAt` 数组中的指定索引处插入一个元素 (或元素的多个副本) 。
...
在此过程中，它将 (此索引中现有元素的索引) 递增，并将其上移所有的元素。
...
为不是的数组调用的第一个运算符 **`const`** 可在右 (r-值) 或赋值语句的左 (左值) 使用。 第二个（为 **`const`** 数组调用）仅可在右侧使用。
...
当) 调用[SetSize](#setsize)或[FreeExtra](#freeextra)时， [CArray](../../mfc/reference/carray-class.md)实现使用此方法将旧数据复制到新 (缓冲区。 默认实现仅复制数据。
