...
|[CFile：： Read](#read)|从当前文件位置的文件中读取 (未缓冲的) 数据。|
|[CFile：： Remove](#remove)| (静态函数) 删除指定的文件。|
|[CFile：： Rename](#rename)|将指定的文件重命名 (静态函数) 。|
...
|[CFile：： SetStatus](#setstatus)| (静态、虚函数) 设置指定文件的状态。|
|[CFile：： UnlockRange](#unlockrange)|解锁文件中的字节范围。|
|[CFile：： Write](#write)|将文件中 (无缓冲) 数据写入当前文件位置。|
...
若要返回文件的完整路径，包括名称，请调用 [GetFilePath](#getfilepath)。 若要将文件的标题返回 ( `myfile`) ，请调用 [GetFileTitle](#getfiletitle)。
...
若要仅返回 () 的文件的名称 `myfile.wri` ，请调用 [GetFileName](#getfilename)。 若要将文件的标题返回 (`myfile`) ，请调用 [GetFileTitle](#getfiletitle)。
...
*nOpenFlags*<br/>
定义文件的共享和访问模式的 UINT。 它指定打开文件时要执行的操作。 可以使用按位 "或" ( **&#124;** ) "运算符来组合选项。 需要一个访问权限和一个共享选项; `modeCreate` 和 `modeNoInherit` 模式是可选的。 有关模式选项的列表，请参阅 [CFile](#cfile) 构造函数。
