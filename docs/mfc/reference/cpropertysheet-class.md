---
description: 了解详细信息： CPropertySheet 类
...
即使不 `CPropertySheet` 是派生自 [CDialog](../../mfc/reference/cdialog-class.md)，管理 `CPropertySheet` 对象也像管理 `CDialog` 对象。 例如，创建属性表需要两部分构造：调用构造函数，然后为模式属性表调用 [DoModal](#domodal) 或为无模式属性表 [创建](#create) 。 `CPropertySheet` 具有两种类型的构造函数： [CPropertySheet：：构造](#construct) 和 [CPropertySheet：： CPropertySheet](#cpropertysheet)。

构造 `CPropertySheet` 对象时，某些 [窗口样式](../../mfc/reference/styles-used-by-mfc.md#window-styles) 可能会导致出现首次异常。 这会导致系统在创建工作表之前尝试更改属性表的样式。 若要避免此异常，请确保在创建时设置以下样式 `CPropertySheet` ：
...
`Window Styles`将禁止任何其他用户，不应启用它们。

交换对象和外部对象之间的数据与与 `CPropertySheet` 对象交换数据类似 `CDialog` 。 重要的区别在于，属性表的设置通常是对象的成员变量， `CPropertyPage` 而不是对象本身的成员变量 `CPropertySheet` 。
...
有关如何使用对象的详细信息 `CPropertySheet` ，请参阅文章 [属性表和属性页](../../mfc/property-sheets-and-property-pages-in-mfc.md)。
...
使用添加属性页时 `AddPage` ， `CPropertySheet` 是的父级 `CPropertyPage` 。 若要从属性页访问属性表，请调用 [CWnd：： GetParent](../../mfc/reference/cwnd-class.md#getparent)。

在创建要调用的属性表窗口之前，无需等待 `AddPage` 。 通常，你将在 `AddPage` 调用 [DoModal](#domodal) 或 [Create](#create)前调用。

如果在 `AddPage` 显示 "属性" 页后调用，则选项卡行将反映新添加的页面。

### <a name="example"></a>示例

[!code-cpp[NVC_MFCDocView#129](../../mfc/codesnippet/cpp/cpropertysheet-class_1.cpp)]

## <a name="cpropertysheetconstruct"></a><a name="construct"></a> CPropertySheet：：构造
...
如果尚未调用某个类构造函数，则调用此成员函数。 例如，在 `Construct` 声明或分配对象数组时调用 `CPropertySheet` 。 对于数组，必须调用 `Construct` 数组中的每个成员。

若要显示属性表，请调用 [DoModal](#domodal) 或 [Create](#create)。 第一个参数中包含的字符串将放在属性表的标题栏中。

如果使用上面列出的第三个或第四个原型， `Construct` 并为 *hbmWatermark*、 *HpalWatermark* 和/或 *hbmHeader* 参数传递有效值，则可以自动显示水印和/或标题图像。

### <a name="example"></a>示例

下面的示例演示了在什么情况下应调用 `Construct` 。
...
如果你有多个参数 (例如，如果你使用数组) ，请使用 [构造](#construct) 而不是 `CPropertySheet` 。

如果使用上面的第三个或第四个原型， `CPropertySheet` 并为 *hbmWatermark*、 *HpalWatermark* 和/或 *hbmHeader* 参数传递有效值，则可以自动显示水印和/或标头图像。
...
默认样式（通过将-1 作为 *dwStyle* 传递）实际上是 WS_SYSMENU&#124;WS_POPUP&#124;WS_CAPTION&#124;DS_MODALFRAME&#124;DS_CONTEXTHELP&#124;WS_VISIBLE。 默认的扩展窗口样式（由0作为 *dwExStyle* 表示）实际上 WS_EX_DLGMODALFRAME。

`Create`成员函数将在创建属性表后立即返回。 若要销毁属性表，请调用 [CWnd：:D estroywindow](../../mfc/reference/cwnd-class.md#destroywindow)。

使用对的调用显示的无模式属性表 `Create` 没有 "确定"、"取消"、"立即应用" 和 "帮助" 按钮作为模式属性表。 用户必须创建所需的按钮。
...
如果函数成功，则为 IDOK 或 IDCANCEL;否则为0或-1。 如果属性表已建立为向导 (参阅 [SetWizardMode](#setwizardmode)) ，则 `DoModal` 返回 ID_WIZFINISH 或 IDCANCEL。

### <a name="remarks"></a>备注

返回值对应于关闭属性表的控件的 ID。 此函数返回后，对应于属性表和所有页面的窗口将被销毁。 对象本身将仍然存在。 通常，在返回 IDOK 后，将从 [CPropertyPage](../../mfc/reference/cpropertypage-class.md) 对象中检索数据 `DoModal` 。
...
> [!NOTE]
> 如果正在使用异步异常处理模型进行编译，则必须由操作系统处理此异常。 有关异常处理模型的详细信息，请参阅 [/EH (异常处理模型) ](../../build/reference/eh-exception-handling-model.md)。 在这种情况下，不要 `CPropertySheet::DoModal` 使用 c + + try-catch 块包装调用，其中 catch 处理所有异常，例如 `catch (...)` 。 此块将处理适用于操作系统的异常，并导致不可预知的行为。 但是，您可以安全地将 c + + 异常处理用于特定异常类型或结构化异常处理，在这种情况下，将访问冲突异常传递到操作系统。
...
默认情况下，如果属性表具有的选项卡数多于在属性表宽度的单个行中容纳的选项卡，则选项卡将在多个行中堆叠。 若要使用滚动选项卡而不是堆栈选项卡，请 `EnableStackedTabs` 在调用 [DoModal](#domodal)或 [Create](#create)之前，调用 *bStacked* 设置为 FALSE。

`EnableStackedTabs`创建模式或无模式属性表时，必须调用。 若要将此样式并入 `CPropertySheet` 派生类中，请为 WM_CREATE 编写消息处理程序。 在 [CWnd：： OnCreate](../../mfc/reference/cwnd-class.md#oncreate)的重写版本中，调用 `EnableStackedTabs( FALSE )` 基类实现前调用。
...
使用此结构可以在构造后，初始化属性表的外观，但在使用 [DoModal](#domodal) 成员函数显示它之前。 例如，将的 *dwSize* 成员设置 `m_psh` 为要使用的属性表的大小。
...
在关闭窗口的所有者之前，不会销毁 [CPropertyPage](../../mfc/reference/cpropertypage-class.md) 对象本身 `CPropertySheet` 。
...
例如， `SetActivePage` 如果某一页上的用户操作应导致另一个页面成为活动页面，则使用。
...
`SetWizardButtons`仅在对话框处于打开状态后调用; 在 `SetWizardButtons` 调用[DoModal](#domodal)之前，无法调用。 通常，应 `SetWizardButtons` 从 [CPropertyPage：： OnSetActive](../../mfc/reference/cpropertypage-class.md#onsetactive)调用。
...
`SetWizardMode`在调用[DoModal](#domodal)前调用。 调用后 `SetWizardMode` ， `DoModal` 如果用户以 "完成" 按钮) 或 IDCANCEL 关闭，则将返回 ID_WIZFINISH (。
