---
description: 了解详细信息： CMFCFontComboBox 类
...
若要 `CMFCFontComboBox` 在对话框中使用对象，请将 `CMFCFontComboBox` 变量添加到对话框类。 然后，在 `OnInitDialog` 对话框类的方法中调用 [CMFCFontComboBox：： Setup](#setup) 方法以初始化组合框控件中的项列表。
...
下面的示例演示如何使用 `SelectFont` 类中的方法 `CMFCFontComboBox` 。 此示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
*nFontType*<br/>
中指定字体类型。 默认值为 DEVICE_FONTTYPE、RASTER_FONTTYPE 和 TRUETYPE_FONTTYPE 的按位组合 (或) 。
...
下面的示例演示如何使用 `Setup` 类中的方法 `CMFCFontComboBox` 。 此示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
