---
description: 了解详细信息： CMFCBaseTabCtrl 类
...
|[CMFCBaseTabCtrl::PreTranslateMessage](#pretranslatemessage)|在将窗口消息调度到[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)和[DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) Windows 函数之前，由[CWinApp](../../mfc/reference/cwinapp-class.md)类用于转换窗口消息。 （重写 [CWnd::PreTranslateMessage](../../mfc/reference/cwnd-class.md#pretranslatemessage)。）|
...
将图标添加到受保护成员的图标列表 `CMap m_mapAddedIcons` 。
...
*图标*<br/>
中受保护成员中图标的从零开始的索引 `CImageList m_Images` 。
...
`CDockablePane`如果 `CreateWrapper` 成功创建 *pWndToWrap* 的包装器类，则为从类派生的包装器的指针。 如果该方法失败，它将 retruns *pWndToWrap*。

### <a name="remarks"></a>备注

选项卡式窗口可以停靠派生自的任何对象 `CWnd` 。 但是，为了使对象可 `CMFCBaseTabCtrl Class` 停靠，上的每个对象都 `CMFCBaseTabCtrl` 必须可分离。 因此， `CMFCBaseTabCtrl` 会自动包装不是从派生的任何对象 `CDockablePane` 。
...
如果 *pWndToWrap* 是从派生的 `CDockablePane` ，则此方法不会创建包装。 相反，它将失败并返回 *pWndToWrap*。
...
在中 `CMFCBaseTabCtrl Class` ，此方法是纯虚函数，并且没有实现。 如果从派生类 `CMFCBaseTabCtrl` ，则必须实现此函数。
...
默认情况下，不支持此方法 `CMFCBaseTabCtrl Class` 。 `CMFCBaseTabCtrl`如果自定义选项卡控件支持选项卡滚动，则应在派生自的自定义类中实现此函数。 [CMFCTabCtrl 类](../../mfc/reference/cmfctabctrl-class.md)支持此方法。
...
在 `CMFCBaseTabCtrl` 类中，此方法是纯虚函数：如果从派生类，则必须实现此方法 `CMFCBaseTabCtrl` 。
...
如果 *iStartFrom* 大于或等于选项卡控件中的选项卡数，将 `GetFirstVisibleTab` 自动失败。
...
*rectTabAreaTop*<br/>
中对对象的引用 `CRect` 。 `GetTabArea` 使用此对象存储顶部选项卡区域的大小和位置。

*rectTabAreaBottom*<br/>
中对对象的引用 `CRect` 。 `GetTabArea` 使用此对象存储底部选项卡区域的大小和位置。

### <a name="remarks"></a>备注

`GetTabArea`返回后， `CRect` 参数包含选项卡区域在选项卡控件的工作区坐标中的大小和位置。 如果选项卡控件的顶部或底部没有选项卡区域， `rectTabAreaTop` 或为 `rectTabAreaBottom` 空。
...
*strLabel*<br/>
弄对对象的引用 `CString` 。 此方法在此参数中存储选项卡的标签。
...
*rect*<br/>
弄对对象的引用 `CRect` 。 此方法在此参数中存储选项卡的大小和位置。
...
此方法检索指向 `CWnd` 通过使用方法 [CMFCBaseTabCtrl：： AddTab](#addtab) 或 [CMFCBaseTabCtrl：： InsertTab](#inserttab)添加的对象的直接指针。 `GetTabWndNoWrapper` 将检索指向已添加的的指针 `CWnd` ，即使框架添加了对象的包装。 有关包装和 [CMFCBaseTabCtrl 类](../../mfc/reference/cmfcbasetabctrl-class.md)的详细信息，请参阅 [CMFCBaseTabCtrl：： CreateWrapper](#createwrapper)。
...
默认情况下，在派生自的自定义类中不支持 Microsoft OneNote 样式 `CMFCBaseTabCtrl Class` 。 但是，它在类中受支持 `CMFCTabCtrl` 。
...
## <a name="cmfcbasetabctrlondrop"></a><a name="ondrop"></a> CMFCBaseTabCtrl：： System.windows.uielement.ondrop
...
在中 `CMFCBaseTabCtrl Class` ，此方法是纯虚函数。 如果从派生类 `CMFCBaseTabCtrl` ，则必须实现此函数。
...
如果 [CMFCBaseTabCtrl：： m_bAutoDestroyWindow](#m_bautodestroywindow) 为 TRUE，则会 `RemoveTab` 销毁与指定选项卡关联的 [CWnd](../../mfc/reference/cwnd-class.md) 对象。
...
## <a name="cmfcbasetabctrlserialize"></a><a name="serialize"></a> CMFCBaseTabCtrl：：串行化
...
*iTab*<br/>
中选项卡的从零开始的索引。 `SetActiveTab` 使此索引的选项卡处于活动状态。
...
默认情况下，框架从 [GetSysColor](/windows/win32/api/winuser/nf-winuser-getsyscolor)获取文本颜色。 使用方法重写此默认颜色 `SetActiveTabTextColor` 。
...
使用 [CMFCBaseTabCtrl：： AddTab](#addtab) 和 [CMFCBaseTabCtrl：： InsertTab](#inserttab)方法将选项卡添加到选项卡控件。 添加选项卡时，该选项卡上的每个控件都必须是可停靠的。 不是从派生的任何对象都 `CDockablePane` 必须进行包装。 `AddTab` 并 `InsertTab` 为这些对象创建包装。 默认包装类是 [CDockablePaneAdapter 类](../../mfc/reference/cdockablepaneadapter-class.md)。 使用方法， `SetDockingBarWrapperRTC` 可以更改用作包装类的类。 您提供的包装类必须派生自 `CDockablePaneAdapter` 。
...
仅当 *bShow* 为 TRUE 时，才应用参数 *bActivate* 。 如果 *bActivate* 为 TRUE，并且如果 `ShowTab` 成功， `ShowTab` 则会将消息 AFX_WM_CHANGE_ACTIVE_TAB 发送到选项卡窗口的父级。
