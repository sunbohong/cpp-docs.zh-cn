---
description: 了解有关以下内容的详细信息： CArchive 类
title: CArchive 类
ms.date: 11/04/2016
f1_keywords:
- CArchive
...
ms.locfileid: "97118753"
---
# <a name="carchive-class"></a>CArchive 类

允许你将复杂的对象网络以永久二进制格式保存 (通常是在这些对象被删除后仍然存在的磁盘存储) 。
...
构造 `CArchive` 对象时，可以将其附加到类 (的对象 `CFile` 或表示打开的文件的派生类) 。 你还可以指定是否将存档用于加载或存储。 `CArchive`对象不仅可处理基元类型，还可以处理为序列化而设计的[CObject](../../mfc/reference/cobject-class.md)派生类的对象。 可序列化的类通常具有 `Serialize` 成员函数，并且通常使用 [DECLARE_SERIAL](../../mfc/reference/run-time-object-model-services.md#declare_serial) 和 [IMPLEMENT_SERIAL](../../mfc/reference/run-time-object-model-services.md#implement_serial) 宏，如类中所述 `CObject` 。

重载提取 ( **>>**) 和插入 ( **<<**) 运算符是支持基元类型和派生类的方便的存档编程接口 `CObject` 。
...
仅当 `CArchive` 加载对象 ( [CArchive：： IsLoading](#isloading) 返回非零) 时，调用此函数才有效。 它应该是函数中的第一个调用 `Serialize` ，只调用一次。  ( UINT) 的返回值指示版本号未知。

`CObject`派生类可以使用按位 **或**) 与架构版本本身一起使用的 VERSIONABLE_SCHEMA 组合 (， (在 IMPLEMENT_SERIAL 宏) 中，以创建一个 "无版本冲突对象"，即，其 `Serialize` 成员函数可以读取多个版本的对象。 不 VERSIONABLE_SCHEMA) 的默认框架功能 (是在版本不匹配时引发异常。
...
此指针的常见用法是将有关序列化过程的其他信息传递到要序列化的所有对象。 这是通过以下方式实现的：在要序列化的文档中初始化包含文档 (`CDocument` 派生类) ，以便文档中的对象可以访问文档（如有必要）。 序列化期间，对象也使用此指针 `COleClientItem` 。

当用户发出文件打开或保存命令时，框架会将 *m_pDocument* 设置为要序列化的文档。 如果将对象链接和嵌入 (OLE) 容器文档的原因不是文件打开或保存的原因，则必须显式设置 *m_pDocument*。 例如，在将容器文档序列化到剪贴板时，可以执行此操作。
...
[CStringT](../../atl-mfc-shared/reference/cstringt-class.md)插入运算符 ( # A2) 支持诊断转储并存储到存档。
...
如果在类实现中使用 IMPLEMENT_SERIAL 宏，则为 `CObject` 调用受保护函数 (重载的提取运算符， `ReadObject` 并) 非零运行时类指针。 此函数反过来调用 `Serialize` 类的函数。

[CStringT](../../atl-mfc-shared/reference/cstringt-class.md)提取运算符 ( # A2) 支持从存档加载。
...
此函数通常由 `CArchive` 提取 ( **>>** [CObject](../../mfc/reference/cobject-class.md) 指针重载) 运算符。 `ReadObject`反过来，会调用 `Serialize` 存档类的函数。
...
在具有 *n 每天* 参数的成员函数版本中，缓冲区最多可包含 *n 每天* 个字符的限制。 读取由回车换行符对停止。 尾随换行符始终会被删除。 在任一情况下，都将 ( "\ 0" ) 追加 null 字符。
...
确切的对象编码方法 (包括 ASCII 类名的存在) 是实现详细信息，并可能在库的未来版本中发生更改。
...
终止 null 字符 ( "\ 0" ) 未写入文件;也不会自动写入一个换行符。
