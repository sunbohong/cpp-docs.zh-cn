---
description: 了解详细信息： CMFCColorBar 类
...
|[CMFCColorBar：： Create](#create)|创建一个颜色栏控件窗口，并将其附加到 `CMFCColorBar` 对象。|
...
|[CMFCColorBar：： SetPropList](#setproplist)|将 `m_pWndPropList` 受保护的数据成员设置为指向属性网格控件的指定指针。|
...
通常不会 `CMFCColorBar` 直接创建对象。 相反， [CMFCColorMenuButton 类](../../mfc/reference/cmfccolormenubutton-class.md) (在菜单和工具栏) 使用，而 [CMFCColorButton 类](../../mfc/reference/cmfccolorbutton-class.md) 将创建 `CMFCColorBar` 对象。
...
- 如果使用 [CMFCColorMenuButton 类](../../mfc/reference/cmfccolormenubutton-class.md)) 创建，则支持 "撕" 状态 (。

将功能合并 `CMFCColorBar` 到应用程序中：

1. 创建一个常规菜单按钮并为其分配 ID，例如 ID_CHAR_COLOR。

1. 在框架窗口类中，重写 [CFrameWndEx：： OnShowPopupMenu](../../mfc/reference/cframewndex-class.md#onshowpopupmenu) 方法，并将常规菜单按钮替换为 [CMFCColorMenuButton 类](../../mfc/reference/cmfccolormenubutton-class.md) 对象，方法是调用 [CMFCToolBar：： ReplaceButton](../../mfc/reference/cmfctoolbar-class.md#replacebutton))  (。

1. 设置所有样式，并 `CMFCColorBar` 在创建 [CMFCColorMenuButton 类](../../mfc/reference/cmfccolormenubutton-class.md) 期间启用或禁用对象的功能。 在 `CMFCColorMenuButton` `CMFCColorBar` 框架调用方法后，对象动态创建对象 `CreatePopupMenu` 。

用户单击颜色栏控件按钮时，框架将使用 `ON_COMMAND` 宏来通知颜色栏控件的父级。 在宏中，"命令 ID" 参数是在步骤1中分配给颜色栏控件按钮的值， (此示例中 ID_CHAR_COLOR) 。 有关详细信息，请参阅 [CMFCColorMenuButton 类](../../mfc/reference/cmfccolormenubutton-class.md)、 [CMFCColorButton 类](../../mfc/reference/cmfccolorbutton-class.md)、 [CMFCColorPickerCtrl 类](../../mfc/reference/cmfccolorpickerctrl-class.md)、 [CFrameWndEx 类](../../mfc/reference/cframewndex-class.md)和 [CMFCToolBar 类](../../mfc/reference/cmfctoolbar-class.md) 。

## <a name="example"></a>示例

下面的示例演示如何使用类中的各种方法来配置颜色条 `CMFCColorBar` 。 方法设置水平和垂直边距，启用 "其他" 按钮，创建颜色栏控件窗口，并设置当前选定的颜色。 此示例是 [新控件示例](../../overview/visual-cpp-samples.md)的一部分。
...
*src*<br/>
中 `CMFCColorBar` 要复制到新对象的现有对象 `CMFCColorBar` 。
...
*bSquareButtons*\
中如果为 TRUE，则指定颜色条控件上的按钮的形状是正方形;否则为 FALSE。 默认值为 TRUE。

*bCenterButtons*\
中如果为 TRUE，则指定颜色条控件按钮表面上的内容居中;否则为 FALSE。 默认值为 TRUE。

### <a name="remarks"></a>备注

## <a name="cmfccolorbarcreate"></a><a name="create"></a> CMFCColorBar：： Create

创建一个颜色栏控件窗口，并将其附加到 `CMFCColorBar` 对象。
...
*bVertDock*\
中若要为垂直停靠的颜色栏控件执行计算，则为 TRUE;否则，请为水平停靠的控件执行计算。
...
当用户选择一种新颜色时，框架会在 WM_COMMAND 消息中发送命令 ID 以通知对象的父级 `CMFCColorBar` 。
...
*nNumColumns*\
中如果颜色栏控件包含文档颜色，则为文档颜色网格中显示的列数。 否则，不使用此值。
...
*pPalette*\
中指向调色板对象的指针，或为 NULL。 如果此参数为 NULL，此方法将使用操作系统的默认调色板。
...
此方法在应用程序的所有对象中更改指定颜色的名称 `CMFCColorBar` 。
...
将 `m_pWndPropList` 受保护的数据成员设置为指向属性网格控件的指定指针。
...
此方法将 WM_SETMESSAGESTRING 消息发送给颜色栏控件的所有者。
