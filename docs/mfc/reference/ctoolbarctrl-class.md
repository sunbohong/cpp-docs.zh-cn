---
description: 了解详细信息： CToolBarCtrl 类
...
|[CToolBarCtrl：：不确定](#indeterminate)|设置或清除工具栏控件中指定按钮 (灰色) 状态。|
...
|[CToolBarCtrl：： IsButtonIndeterminate](#isbuttonindeterminate)|指示工具栏控件中指定按钮的状态是否为不确定的 (灰色) 。|
...
|[CToolBarCtrl：： SetButtonStructSize](#setbuttonstructsize)|指定结构的大小 `TBBUTTON` 。|
...
`CToolBarCtrl` 对象包含几个重要的内部数据结构：一个按钮图像位图列表、一个图像列表、一个按钮标签字符串列表以及一个结构列表， `TBBUTTON` 这些结构将图像和/或字符串与按钮的位置、样式、状态和命令 ID 相关联。 这些数据结构的每个元素均由从零开始的索引引用。 `CToolBarCtrl`必须先设置这些数据结构，然后才能使用对象。 字符串列表只能用于按钮标签;不能从工具栏检索字符串。
...
1. 如果要在不是的所有者窗口中提供工具栏按钮的工具提示 `CFrameWnd` ，则需要按照 [处理工具提示通知](../../mfc/handling-tool-tip-notifications.md)中所述，在工具栏的所有者窗口中处理 TTN_NEEDTEXT 消息。 如果工具栏的父窗口是从派生的 `CFrameWnd` ，则会显示工具提示，无需任何额外的工作，因为 `CFrameWnd` 它提供了默认的处理程序。
...
有关使用的详细信息 `CToolBarCtrl` ，请参阅 [控件](../../mfc/controls-mfc.md) 和 [使用 CToolBarCtrl](../../mfc/using-ctoolbarctrl.md)。
...
*pBitmap*<br/>
指向 `CBitmap` 对象的指针，该对象包含要添加的按钮图像或图像。
...
在将位图添加到工具栏之前，可以使用 Windows API [CreateMappedBitmap](/windows/win32/api/commctrl/nf-commctrl-createmappedbitmap) 映射颜色。 如果传递指向对象的指针 `CBitMap` ，则必须确保在销毁工具栏后才销毁位图。
...
*lpButtons*<br/>
结构数组的地址 `TBBUTTON` ，其中包含有关要添加的按钮的信息。 数组中必须有与 *nNumButtons* 所指定的按钮相同数量的元素。
...
*LpButtons* 指针指向结构的数组 `TBBUTTON` 。 每个 `TBBUTTON` 结构将添加的按钮与按钮的样式、图像和/或字符串、命令 ID、状态和用户定义的数据相关联：
...
不应将对象传递 `CString` 给此函数，因为在中不能有多个 null 字符 `CString` 。
...
当父窗口的大小发生更改时，或者在工具栏大小发生更改时，应调用此函数 (例如，设置按钮或位图大小时，或者) 添加字符串。
...
*rect*<br/>
根据需要指定工具栏控件的大小和位置。 它可以是 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象或 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构。
...
可以通过 `CToolBarCtrl` 两个步骤构造。 首先，调用构造函数，然后调用 `Create` ，它创建工具栏控件并将其附加到 `CToolBarCtrl` 对象。 将以下窗口样式应用到工具栏控件。
...
若要使用扩展工具栏样式，请在调用后调用 [SetExtendedStyle](#setextendedstyle) `Create` 。 若要创建具有扩展窗口样式的工具栏，请调用 [CToolBarCtrl：： CreateEx](#createex) 而不是 `Create` 。
...
创建一个 (子窗口) 的控件，并将其与 `CToolBarCtrl` 对象关联。
...
使用 `CreateEx` 由 Windows 扩展样式指定的扩展 Windows 样式，而不是 [Create](#create) **WS_EX_**。 `CreateEx` 使用由 *dwExStyle* 指定的扩展 Windows 样式创建控件。 使用 [SetExtendedStyle](#setextendedstyle)设置特定于控件的扩展样式。 例如，使用将 `CreateEx` 此类样式设置为 WS_EX_CONTEXTHELP，但使用将 `SetExtendedStyle` 此类样式设置为 TBSTYLE_EX_DRAWDDARROWS。 有关详细信息，请参阅 Windows SDK 中 [工具栏扩展样式](/windows/win32/Controls/toolbar-extended-styles) 中所述的样式。
...
*lpButton*<br/>
`TBBUTTON`用于接收按钮信息副本的结构的地址。 有关结构的信息，请参阅 [CToolBarCtrl：： AddButtons](#addbuttons) `TBBUTTON` 。
...
*lpColorScheme*\
弄指向接收配色方案信息的 [COLORSCHEME](/windows/win32/api/commctrl/ns-commctrl-colorscheme) 结构的指针。 此方法返回时，结构描述了 toolbar 控件的突出显示颜色和阴影颜色。
...
此成员函数实现 Win32 消息 [TB_GETDISABLEDIMAGELIST](/windows/win32/Controls/tb-getdisabledimagelist)的行为，如 Windows SDK 中所述。 的 MFC 实现 `GetDisabledImageList` 使用 `CImageList` 包含工具栏控件按钮图像的对象，而不是使用图像列表的句柄。
...
*lpRect*<br/>
[矩形](/windows/win32/api/windef/ns-windef-rect)结构或接收边框坐标的[CRect](../../atl-mfc-shared/reference/crect-class.md)对象的地址。
...
*ptbm*<br/>
指向对象的 [TBMETRICS](/windows/win32/api/commctrl/ns-commctrl-tbmetrics) 结构的指针 `CToolBarCtrl` 。
...
*pnHorzPadding*\
弄一个整数，接收工具栏控件的水平填充（以像素为单位）。

*pnVertPadding*\
弄一个整数，接收工具栏控件的垂直填充（以像素为单位）。
...
*lpRect*<br/>
一个指针，指向用于接收边框信息的 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构。
...
*ppt*<br/>
[指向结构的](/windows/win32/api/windef/ns-windef-point)指针，该结构包含成员中的命中测试的 x 坐标和该命中 `x` 测试的 y 坐标 `y` 。 坐标相对于工具栏的工作区。
...
*lpButton*<br/>
`TBBUTTON`包含要插入的按钮信息的结构的地址。 有关结构的说明，请参阅 [CToolBarCtrl：： AddButtons](#addbuttons) `TBBUTTON` 。
...
*ppt*<br/>
指向一个 [点](/windows/win32/api/windef/ns-windef-point) 结构的指针，该结构包含命中测试坐标（相对于工具栏的工作区）。
...
*pReplaceBitmap*\
中指向 [TBREPLACEBITMAP](/windows/win32/api/commctrl/ns-commctrl-tbreplacebitmap) 结构的指针，该结构描述要替换的位图和新位图。
...
*nSize*<br/>
结构的大小（以字节为单位） `TBBUTTON` 。

### <a name="remarks"></a>备注

如果您想要在结构中存储额外数据， `TBBUTTON` 则可以从中派生新的结构 `TBBUTTON` ，添加所需的成员，或创建包含 `TBBUTTON` 结构作为其第一个成员的新结构。 然后，可以调用此函数来告知工具栏控制新结构的大小。

有关结构的详细信息，请参阅 [CToolBarCtrl：： AddButtons](#addbuttons) `TBBUTTON` 。
...
*lpColorScheme*\
中指向 [COLORSCHEME](/windows/win32/api/commctrl/ns-commctrl-colorscheme) 结构的指针，该结构描述 toolbar 控件的突出显示颜色和阴影颜色。
...
*pImageList*<br/>
指向对象的指针，该 `CImageList` 对象包含 toolbar 控件用于显示禁用的按钮图像的图像。
...
此成员函数实现 Win32 消息 [TB_SETDISABLEDIMAGELIST](/windows/win32/Controls/tb-setdisabledimagelist)的行为，如 Windows SDK 中所述。 的 MFC 实现 `SetDisabledImageList` 使用 `CImageList` 包含工具栏控件的禁用按钮图像的对象，而不是使用图像列表的句柄。
...
*dwDTFlags*<br/>
在 Win32 函数中指定的一个或多个 DT_ 标志的组合， `DrawText` 用于指示如何绘制按钮文本。 绘制按钮文本时，此值将传递给 `DrawText` 。
...
此成员函数实现 Win32 消息 [TB_SETDRAWTEXTFLAGS](/windows/win32/Controls/tb-setdrawtextflags)的行为，如 Windows SDK 中所述。 此成员函数在 Win32 函数中设置标记，该函数在 `DrawText` 指定的矩形中绘制文本，并根据标志的设置方式进行设置。
...
*pImageList*<br/>
指向对象的指针，该 `CImageList` 对象包含工具栏控件用来显示热按钮图像的图像。
...
的 MFC 实现 `SetHotImageList` 使用 `CImageList` 包含工具栏控件的热按钮图像的对象，而不是使用图像列表的句柄。 当指针位于其上方时，热按钮将突出显示。
...
*pImageList*<br/>
指向对象的指针，该 `CImageList` 对象包含 toolbar 控件用于显示处于默认状态的按钮图像的图像。
...
的 MFC 实现 `SetImageList` 使用 `CImageList` 包含工具栏控件按钮图像的对象，而不是使用图像列表的句柄。
...
*nHorzPadding*\
中指定工具栏控件的水平填充（以像素为单位）。

*nVertPadding*\
中指定工具栏控件的垂直填充（以像素为单位）。
...
*iImageID*\
中图像列表的从零开始的索引。 如果只使用一个图像列表，请将此参数设置为零。

*pImageList*\
中指向包含新图像列表的 [CImageList](../../mfc/reference/cimagelist-class.md) 的指针。
...
*lpRect*<br/>
指向将接收工具栏的新边框的 [CRect](../../atl-mfc-shared/reference/crect-class.md) 对象或 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构。
...
如果按钮可以排列，则给定数量的行对工具栏有效，因此，除了最后一行) 以外，所有行都具有相同数目的按钮 (。 例如，包含四个按钮的工具栏无法调整为三行大小，因为最后两行必须更短。 如果尝试将其设置为三行大小，则如果 *bLarger* 为 TRUE，则会获得四行，如果 *bLarger* 为 FALSE，则返回两行。

如果工具栏中有分隔符，则给定行数有效的规则将更复杂。 计算布局后，按钮会将 (按钮分组，并在组中的第一个按钮之前加上一个分隔符，而) 组中的最后一个按钮将永远不会在多行上分解，除非该组不能放在一行上。

如果某个组不适合某个行，则下一组将从下一行开始，即使它适合于大组结束的行。 此规则的目的是使大组之间的隔离更为明显。 生成的垂直分隔符计为行。

另请注意， `SetRows` 成员函数将始终选择生成最小工具栏大小的布局。 创建具有 TBSTYLE_WRAPABLE 样式的工具栏，然后调整控件大小，只需在控件的宽度后应用上面所述的方法。
