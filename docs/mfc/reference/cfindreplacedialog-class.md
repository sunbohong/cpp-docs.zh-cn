---
description: 了解详细信息： CFindReplaceDialog 类
...
若要构造 `CFindReplaceDialog` 对象，请使用提供的构造函数 (没有) 的参数。 由于这是无模式对话框，因此请使用 **`new`** 运算符（而不是堆栈）在堆上分配对象。
...
为了让父窗口收到查找/替换请求的通知，您必须使用 Windows [RegisterWindowMessage](/windows/win32/api/winuser/nf-winuser-registerwindowmessagew) 函数，该函数的返回值为应用程序实例特有的消息号。 框架窗口应具有一个消息映射条目，该条目声明回调函数 ( `OnFindReplace` 在下面的示例中，该函数用于处理此注册消息的) 。 下面的代码片段举例说明如何为名为的框架窗口类执行此 `CMyRichEditView` 操作：
