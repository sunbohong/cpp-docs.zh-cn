---
description: 了解详细信息： CMFCToolBarEditBoxButton 类
...
ms.locfileid: "97163894"
---
# <a name="cmfctoolbareditboxbutton-class"></a>CMFCToolBarEditBoxButton 类

一个工具栏按钮，其中包含 ( [CEdit 类](../../mfc/reference/cedit-class.md)) 的编辑控件。
...
|[CMFCToolBarEditBoxButton::GetByCmd](#getbycmd)|检索 `CMFCToolBarEditBoxButton` 应用程序中具有指定命令 ID 的第一个对象。|
...
|`CMFCToolBarEditBoxButton::SetACCData`|`CAccessibilityData`用工具栏按钮中的可访问性数据填充提供的对象。  (重写 [CMFCToolBarButton：： SetACCData](../../mfc/reference/cmfctoolbarbutton-class.md#setaccdata)。 ) |
...
有关详细信息，请参阅 [演练：将控件放置在工具栏上](../../mfc/walkthrough-putting-controls-on-toolbars.md)。

## <a name="example"></a>示例

下面的示例演示了如何使用 `CMFCToolBarEditBoxButton` 类中的各种方法。 该示例演示了如何指定用户可以在自定义过程中拉伸按钮、指定在用户单击该按钮时显示该按钮的边框、设置 "文本框" 控件中的文本、在应用程序中指定编辑框按钮的平面样式外观，以及指定工具栏编辑框控件的样式。
...
默认情况下，框架不允许用户在自定义过程中拉伸工具栏按钮。 此方法通过允许用户在自定义过程中拉伸编辑框工具栏按钮，扩展了基类实现 ( [CMFCToolBarButton：： CanBeStretched](../../mfc/reference/cmfctoolbarbutton-class.md#canbestretched)) 。
...
调用此方法可将另一个工具栏按钮复制到此工具栏按钮。 *src* 的类型必须为 `CMFCToolBarEditBoxButton` 。
...
可以通过 `CMFCToolBarEditBoxButton` 两个步骤构造对象。 首先调用构造函数，然后调用 `CreateEdit` ，它创建 Windows 编辑控件并将其附加到 `CMFCToolBarEditBoxButton` 对象。

## <a name="cmfctoolbareditboxbuttongetbycmd"></a><a name="getbycmd"></a> CMFCToolBarEditBoxButton::GetByCmd

检索 `CMFCToolBarEditBoxButton` 应用程序中具有指定命令 ID 的第一个对象。
...
`CMFCToolBarEditBoxButton`应用程序中具有指定命令 ID 的第一个对象，如果不存在这样的对象，则为 NULL。
...
当 [CMFCVisualManager：： OnDrawEditBorder](../../mfc/reference/cmfcvisualmanager-class.md#ondraweditborder) 方法在对象周围绘制边框时，将调用此方法 `CMFCToolBarEditBoxButton` 。
...
指向按钮包含的 [CEdit 类](../../mfc/reference/cedit-class.md) 控件的指针。 如果尚未创建控件，则为 NULL `CEdit` 。

### <a name="remarks"></a>备注

可以 `CEdit` 通过调用 [CMFCToolBarEditBoxButton：： CreateEdit](#createedit)创建控件。
...
此方法通过处理[EN_UPDATE](/windows/win32/Controls/en-update)通知 ( [CMFCToolBarButton：： NotifyCommand](../../mfc/reference/cmfctoolbarbutton-class.md#notifycommand)) 扩展基类实现。 对于具有与此对象相同的命令 ID 的每个编辑框，它会将其文本标签设置为此对象的文本标签。
...
此方法通过从具有与此对象相同的命令 ID 的任何工具栏中的 "编辑框" 控件中复制属性，来扩展基类实现 ( [CMFCToolBarButton：： OnAddToCustomizePage](../../mfc/reference/cmfctoolbarbutton-class.md#onaddtocustomizepage)) 。 如果没有任何工具栏具有与此对象相同的命令 ID，则此方法不执行任何操作。
...
此方法通过重新创建内部对象来重写基类实现 ( [CMFCToolBarButton：： OnChangeParentWnd](../../mfc/reference/cmfctoolbarbutton-class.md#onchangeparentwnd)) `CEdit` 。
...
此方法通过返回非零值（如果内部对象可见）来重写基类实现 ( [CMFCToolBarButton：： OnClick](../../mfc/reference/cmfctoolbarbutton-class.md#onclick)) `CEdit` 。
...
此方法通过将提供的设备上下文的文本和背景色设置为全局文本和背景颜色，来重写基类实现 ( [CMFCToolBarButton：： OnCtlColor](../../mfc/reference/cmfctoolbarbutton-class.md#onctlcolor)) 。
...
此方法通过更新内部对象的位置来重写默认类实现 ( [CMFCToolBarButton：： OnMove](../../mfc/reference/cmfctoolbarbutton-class.md#onmove)) `CEdit`
...
此方法通过更新内部对象的大小和位置来重写默认类实现 [CMFCToolBarButton：： OnSize](../../mfc/reference/cmfctoolbarbutton-class.md#onsize) `CEdit` 。
...
此方法通过显示与按钮编辑部分关联的工具提示文本，将基类实现 ( 扩展) ： [： OnUpdateToolTip](../../mfc/reference/cmfctoolbarbutton-class.md#onupdatetooltip) 。 如果内部 `CEdit` 对象为 NULL 或对象的窗口句柄不 `CEdit` 标识现有窗口，则此方法不执行任何操作并返回 FALSE。
...
如果设置了文本，则为非零值;如果 `CMFCToolBarEditBoxButton` 具有指定命令 ID 的控件不存在，则为0。
