...
|[CTabCtrl：： Create](#create)|创建选项卡控件，并将其附加到对象的实例 `CTabCtrl` 。|
|[CTabCtrl：： CreateEx](#createex)|创建具有指定 Windows 扩展样式的选项卡控件，并将其附加到对象的实例 `CTabCtrl` 。|
...
|[CTabCtrl：： SetPadding](#setpadding)|设置选项卡控件中每个选项卡的图标和标签周围)  (填充的空间量。|
...
有关使用的详细信息 `CTabCtrl` ，请参阅 [控件](../../mfc/controls-mfc.md) 和 [使用 CTabCtrl](../../mfc/using-ctabctrl.md)。
...
*lpRect*<br/>
指向 [矩形结构的指针，该结构](/windows/win32/api/windef/ns-windef-rect) 指定给定矩形并接收计算矩形。
...
可以通过 `CTabCtrl` 两个步骤构造对象。 首先，调用构造函数，然后调用，它将 `Create` 创建选项卡控件，并将其附加到 `CTabCtrl` 对象。
...
创建一个 (子窗口) 的控件，并将其与 `CTabCtrl` 对象关联。
...
`CreateEx` 使用由 *dwExStyle* 指定的扩展 Windows 样式创建控件。 使用 [SetExtendedStyle](#setextendedstyle)设置特定于控件的扩展样式。 例如，使用将 `CreateEx` 此类样式设置为 WS_EX_CONTEXTHELP，但使用将 `SetExtendedStyle` 此类样式设置为 TCS_EX_FLATSEPARATORS。 有关详细信息，请参阅在 Windows SDK 中的 [选项卡控件扩展样式](/windows/win32/Controls/tab-control-extended-styles) 中所述的样式。
...
默认情况下，此成员函数不执行任何操作。 重写此成员函数以实现所有者描述对象的绘制 `CTabCtrl` 。
...
发送消息时，该 `mask` 成员指定要返回的属性。 如果 `mask` 成员指定 TCIF_TEXT 值，则 `pszText` 成员必须包含接收项文本的缓冲区的地址，并且 `cchTextMax` 成员必须指定缓冲区的大小。

- `mask`

   指定 `TCITEM` 要检索或设置的结构成员的值。 此成员可以为零或以下值的组合：
  ...
  - TCIF_RTLREADING 在 `pszText` 希伯来语或阿拉伯语系统上使用从右到左的阅读顺序显示的文本。
...
- `cchTextMax`
   ...
   所指向的缓冲区的大小 `pszText` 。 如果结构未接收信息，则忽略此成员。

- `iImage` 在选项卡控件的图像列表中编制索引，或者，如果选项卡没有图像，则为-1。

- `lParam`

   与选项卡关联的应用程序定义数据。如果每个选项卡的应用程序定义的数据超过四个字节，应用程序必须定义一个结构，并使用它而不是 `TCITEM` 结构。 应用程序定义的结构的第一个成员必须是 [TCITEMHEADER](/windows/win32/api/commctrl/ns-commctrl-tcitemheaderw)结构。 `TCITEMHEADER`结构与结构完全相同 `TCITEM` ，但不包含 `lParam` 成员。 结构大小与结构大小之间的差异 `TCITEMHEADER` 应该等于每个选项卡的额外字节数。
...
*lpRect*<br/>
一个指针，指向用于接收该选项卡的边框的 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构。这些坐标使用视区的当前映射模式。
...
选项卡控件创建具有 TCS_TOOLTIPS 样式的工具提示控件。 还可以使用成员函数向选项卡控件分配工具提示控件 `SetToolTips` 。
...
*nMask*<br/>
指定 `TCITEM` 要设置的结构特性。 可以为零或以下值的组合：
...
*pTabCtrlItem*<br/>
指向 [TCITEM](/windows/win32/api/commctrl/ns-commctrl-tcitemw) 结构的指针，该结构包含新的项特性。 `mask`成员指定要设置的特性。 如果 `mask` 成员指定 TCIF_TEXT 值，则该 `pszText` 成员为以 null 结尾的字符串的地址，并且该成员 `cchTextMax` 将被忽略。
...
可以通过调用来获取与选项卡控件关联的工具提示控件 `GetToolTips` 。
