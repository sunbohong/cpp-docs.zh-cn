...
|[CProgressCtrl：： StepIt](#stepit)|按步长增量 (移动进度栏控件的当前位置，请参阅 [SetStep](#setstep)) 并重绘条形图以反映新位置。|
...
有关使用的详细信息 `CProgressCtrl` ，请参阅 [控件](../../mfc/controls-mfc.md) 和 [使用 CProgressCtrl](../../mfc/using-cprogressctrl.md)。
...
构造对象后 `CProgressCtrl` ，调用 `CProgressCtrl::Create` 以创建进度栏控件。
...
创建一个进度栏控件，并将其附加到 `CProgressCtrl` 对象。
...
如果 `CProgressCtrl` 已成功创建对象，则为 TRUE; 否则为 FALSE。

### <a name="remarks"></a>备注

可以通过 `CProgressCtrl` 两个步骤构造对象。 首先，调用构造函数，该构造函数将创建 `CProgressCtrl` 对象，然后调用 `Create` ，这将创建进度栏控件。
...
创建一个 (子窗口) 的控件，并将其与 `CProgressCtrl` 对象关联。
...
`SetBarColor`仅当 Windows Vista[主题](/windows/win32/Controls/visual-styles-overview)无效时，方法才会设置进度栏颜色。
