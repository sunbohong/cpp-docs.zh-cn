---
description: 了解详细信息： CTypedPtrArray 类
...
*BASE_CLASS*<br/>
类型化指针数组类的基类;必须是 [CObArray](../../mfc/reference/cobarray-class.md) 或 [CPtrArray](../../mfc/reference/cptrarray-class.md)) 的数组类 (。
...
为 not 的数组调用的第一个运算符 **`const`** 可在右 (r-值) 或赋值语句的左 (左值) 使用。 第二个为 **`const`** 数组调用，只能在右侧使用。
