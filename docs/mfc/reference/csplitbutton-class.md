...
此 `CSplitButton` 类派生自 [CButton](../../mfc/reference/cbutton-class.md) 类。 拆分按钮控件是一个按钮控件，其样式为 BS_SPLITBUTTON。 当用户单击下拉箭头时，它会显示一个自定义菜单。 有关详细信息，请参阅 [按钮样式](/windows/win32/Controls/button-styles)中的 BS_SPLITBUTTON 和 BS_DEFSPLITBUTTON 样式。
...
创建具有指定样式的拆分按钮控件，并将其附加到当前 `CSplitButton` 对象。
...
*pMenu*\
中指向指定子菜单的 [CMenu](../../mfc/reference/cmenu-class.md) 对象的指针。 `CSplitButton` `CMenu` 当对象超出范围时，对象将删除该对象及其关联的 HMENU `CSplitButton` 。
...
*pResult*\
[out] (未使用;不返回任何值。 [BCN_DROPDOWN](/windows/win32/Controls/bcn-dropdown) 通知 ) 返回值。

### <a name="remarks"></a>备注

当用户单击拆分按钮控件上的下拉箭头时，系统将发送一个 BCN_DROPDOWN 通知消息，该消息将由 `OnDropDown` 方法处理。 但是， `CSplitButton` 对象不会将 BCN_DROPDOWN 通知转发到包含拆分按钮控件的控件。 因此，包含控件无法支持自定义操作来响应通知。

若要实现包含控件支持的自定义操作，请使用样式为 BS_SPLITBUTTON 而不是对象的 [CButton](../../mfc/reference/cbutton-class.md) 对象 `CSplitButton` 。 然后为对象中的 BCN_DROPDOWN 通知实现处理程序 `CButton` 。 有关详细信息，请参阅 [按钮样式](../../mfc/reference/styles-used-by-mfc.md#button-styles)。

若要实现拆分按钮控件本身支持的自定义操作，请使用 [消息反射](../../mfc/tn062-message-reflection-for-windows-controls.md)。 从类派生你自己的类 `CSplitButton` 并将其命名为，例如，CMySplitButton。 然后，将以下消息映射添加到你的应用程序以处理 BCN_DROPDOWN 通知：
