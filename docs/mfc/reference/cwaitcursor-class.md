---
description: 了解详细信息： CWaitCursor 类
title: CWaitCursor 类
ms.date: 11/04/2016
f1_keywords:
- CWaitCursor
...
当对象超出范围时 (在将对象声明) 的块的末尾处 `CWaitCursor` ，其析构函数将游标设置为上一个游标。 换句话说，对象会自动执行必要的清理。
...
您可以利用这一事实：在块的末尾调用析构函数 (可能在函数末尾之前) 使等待游标仅在函数的一部分中处于活动状态。 下面的第二个示例演示了此方法。
