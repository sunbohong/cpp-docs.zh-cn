---
description: 了解详细信息： COleLinksDialog 类
...
|“属性”|描述|
|----------|-----------------|
|[COleLinksDialog：:D oModal](#domodal)|显示 OLE "编辑链接" 对话框。|
...
如果要调用此对话框，请创建类的对象 `COleLinksDialog` 。 `COleLinksDialog`构造对象后，可以使用[m_el](#m_el)结构在对话框中初始化控件的值或状态。 `m_el`结构的类型为 OLEUIEDITLINKS。 有关使用此对话框类的详细信息，请参阅 [DoModal](#domodal) 成员函数。
...
如果要通过设置 [m_el](#m_el) 结构的成员来初始化各种对话框控件，应在调用之前执行此操作 `DoModal` ，但在构造对话框对象之后。
...
此函数仅构造 `COleLinksDialog` 对象。 若要显示该对话框，请调用 [DoModal](#domodal) 函数。
