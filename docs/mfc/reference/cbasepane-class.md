---
description: 了解详细信息： CBasePane 类
title: CBasePane 类
ms.date: 11/06/2018
f1_keywords:
- CBasePane
...
|“属性”|描述|
|----------|-----------------|
|`CBasePane::accHitTest`|由框架调用以检索屏幕上给定点处的子元素或子对象。  (将重写 [CWnd：： accHitTest](../../mfc/reference/cwnd-class.md#acchittest)。 ) |
|`CBasePane::accLocation`|由框架调用以检索指定对象的当前屏幕位置。  (将重写 [CWnd：： accLocation](../../mfc/reference/cwnd-class.md#acclocation)。 ) |
|[CBasePane：： AccNotifyObjectFocusEvent](#accnotifyobjectfocusevent)|`CBasePane` 不使用此方法。|
|`CBasePane::accSelect`|由框架调用以修改选定内容或移动指定对象的键盘焦点。  (将重写 [CWnd：： accSelect](../../mfc/reference/cwnd-class.md#accselect)。 ) |
...
|`CBasePane::get_accChild`|由框架调用以检索指定子级的 `IDispatch` 接口地址。  (将替代 [CWnd：： get_accChild](../../mfc/reference/cwnd-class.md#get_accchild)。 ) |
|`CBasePane::get_accChildCount`|由框架调用以检索属于该对象的子级的数目。  (将替代 [CWnd：： get_accChildCount](../../mfc/reference/cwnd-class.md#get_accchildcount)。 ) |
|`CBasePane::get_accDefaultAction`|由框架调用以检索描述对象默认操作的字符串。  (将替代 [CWnd：： get_accDefaultAction](../../mfc/reference/cwnd-class.md#get_accdefaultaction)。 ) |
|`CBasePane::get_accDescription`|由框架调用以检索描述指定对象的可视外观的字符串。  (将替代 [CWnd：： get_accDescription](../../mfc/reference/cwnd-class.md#get_accdescription)。 ) |
|`CBasePane::get_accFocus`|由框架调用以检索具有键盘焦点的对象。  (将替代 [CWnd：： get_accFocus](../../mfc/reference/cwnd-class.md#get_accfocus)。 ) |
|`CBasePane::get_accHelp`|由框架调用以检索对象的 Help 属性字符串。  (将替代 [CWnd：： get_accHelp](../../mfc/reference/cwnd-class.md#get_acchelp)。 ) |
|[CBasePane：： get_accHelpTopic](#get_acchelptopic)|由框架调用以检索与指定对象关联的 WinHelp 文件的完整路径以及该文件中相应主题的标识符。  (将替代 [CWnd：： get_accHelpTopic](../../mfc/reference/cwnd-class.md#get_acchelptopic)。 ) |
|`CBasePane::get_accKeyboardShortcut`|由框架调用以检索对象的指定快捷键。  (将替代 [CWnd：： get_accKeyboardShortcut](../../mfc/reference/cwnd-class.md#get_acckeyboardshortcut)。 ) |
|`CBasePane::get_accName`|由框架调用以检索指定对象的名称。  (将替代 [CWnd：： get_accName](../../mfc/reference/cwnd-class.md#get_accname)。 ) |
|`CBasePane::get_accParent`|由框架调用以检索对象的 `IDispatch` 父对象的接口。  (将替代 [CWnd：： get_accParent](../../mfc/reference/cwnd-class.md#get_accparent)。 ) |
|`CBasePane::get_accRole`|由框架调用以检索描述指定对象的角色的信息。  (将替代 [CWnd：： get_accRole](../../mfc/reference/cwnd-class.md#get_accrole)。 ) |
|[CBasePane：： get_accSelection](#get_accselection)|由框架调用以检索该对象的选定子级。  (将替代 [CWnd：： get_accSelection](../../mfc/reference/cwnd-class.md#get_accselection)。 ) |
|`CBasePane::get_accState`|由框架调用以检索指定对象的当前状态。  (将替代 [CWnd：： get_accState](../../mfc/reference/cwnd-class.md#get_accstate)。 ) |
|`CBasePane::get_accValue`|由框架调用以检索指定对象的值。  (将替代 [CWnd：： get_accValue](../../mfc/reference/cwnd-class.md#get_accvalue)。 ) |
...
|`CBasePane::PreTranslateMessage`|在将窗口消息调度到[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)和[DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) Windows 函数之前，由[CWinApp](../../mfc/reference/cwinapp-class.md)类用于转换窗口消息。 （重写 [CWnd::PreTranslateMessage](../../mfc/reference/cwnd-class.md#pretranslatemessage)。）|
...
- 如果你想要创建一个静态窗格，该窗格将无法浮动，并阻止任何其他窗格在 (类似于 OutlookDemo 示例中的 Outlook 栏) ，请将其创建为非浮动，并重写 [CBasePane：:D oesallowdyninsertbefore](#doesallowdyninsertbefore) 返回 FALSE。 如果创建的窗格没有 AFX_CBRS_FLOAT 样式，则默认实现将返回 FALSE。
...
下面的示例演示了如何使用 `CBasePane` 类中的各种方法。 该示例演示如何从类检索窗格， `CFrameWndEx` 以及如何设置停靠模式、窗格对齐方式和窗格样式。 此代码来自 [Word Pad 示例](../../overview/visual-cpp-samples.md)。
...
*bStretch*<br/>
中指示是否应将栏拉伸到帧的大小。 如果栏不是停靠栏 (不能停靠) ，则 *bStretch* 参数为非零值，当停靠或浮动 (可用于停靠) 时，该参数为0。
...
此方法检查 AFX_CBRS_RESIZE 标志，该标志默认情况下在中指定 `CBasePane::OnCreate` 。 如果未指定此标志，则停靠管理器将在内部将窗格标记为可移动，而不是将其停靠。
...
只有特定派生类型的对象（ `CBasePane` 如 [CDockablePane 类](../../mfc/reference/cdockablepane-class.md)）才能转换为选项卡式文档。
...
*dwStyleEx*<br/>
中扩展样式 (参阅 [CWnd：： CreateEx](../../mfc/reference/cwnd-class.md#createex) 以了解详细信息) 。
...
*dwStyle*<br/>
中窗口样式 (参阅 [CWnd：： CreateEx](../../mfc/reference/cwnd-class.md#createex)) 。
...
创建类的窗口 `lpszClassName` 。 如果指定 WS_CAPTION，则此方法将清除 WS_CAPTION 样式位并将设置 `CBasePane::m_bHasCaption` 为 TRUE，因为库不支持带有标题的窗格。

您可以使用子窗口样式和 MFC 控件条的任意组合 (CBRS_) 样式。
...
调用此函数可将一个窗格停靠到另一个窗格或停靠栏 ( 由 *pDockBar* 指定的 [CDockSite 类](../../mfc/reference/cdocksite-class.md)) ; 如果 *pDockBar* 为 NULL，则为主帧。
...
如果无法创建 [CPaneDivider 类](../../mfc/reference/cpanedivider-class.md)) 的窗格分隔 ( 线，则此方法将失败。 否则，它始终返回 TRUE。
...
例如，假设您的应用程序创建一个停靠在框架左侧的窗格， (如 Outlook bar) 。 若要防止用户在第一个窗格左侧停靠另一个窗格，请重写此方法并返回 FALSE。
...
默认实现将调用当前的可视管理器以填充后台 ( [CMFCVisualManager：： OnFillBarBackground](../../mfc/reference/cmfcvisualmanager-class.md#onfillbarbackground)) 。
...
*varChild*<br/>
中指定要检索的帮助主题是否为对象的对象或该对象的一个子元素。 此参数可以是 CHILDID_SELF (为对象) 获取帮助主题，或使用子 ID (获取对象) 的某个子元素的帮助主题。
...
如果 `CBasePane::m_dockMode` 未定义 (DT_UNDEFINED) ，则将从全局停靠模式获取停靠模式 (`AFX_GLOBAL_DATA::m_dockModeGlobal`) 。
...
如果面板停靠，则为指向的指针 `CDockingPanesRow` ; 如果它是浮动的，则为 NULL。
...
控件条形样式的组合 (包括在创建时由 [CBasePane：： SetPaneStyle](#setpanestyle) 方法设置 CBRS_ 样式) 。
...
使用此函数可检索指向父选项卡式窗口的指针。 有时不能调用 `GetParent` ，因为窗格可以位于停靠包装器 ( [CDockablePaneAdapter 类](../../mfc/reference/cdockablepaneadapter-class.md)) 或窗格适配器 ( [CDockablePaneAdapter 类](../../mfc/reference/cdockablepaneadapter-class.md)) 中。 通过使用 `GetParentTabWnd` ，你将能够在这些情况下检索有效指针， (假设父项是选项卡式窗口) 。
...
状态栏 ( [CMFCStatusBar 类](../../mfc/reference/cmfcstatusbar-class.md)) 并且无法调整停靠栏 ( [CDockSite 类](../../mfc/reference/cdocksite-class.md)) 。
...
如果窗格不是选项卡式 (参阅 [CBasePane：： IsTabbed](#istabbed)) ，则此方法检查 WS_VISIBLE 样式。 如果窗格是选项卡式，则此方法检查父选项卡式窗口的可见性。 如果父窗口可见，则函数使用 [CMFCBaseTabCtrl：： IsTabVisible](../../mfc/reference/cmfcbasetabctrl-class.md#istabvisible)检查窗格选项卡的可见性。
...
框架在将窗格的状态保存到注册表时调用此方法。 `SaveState`在派生类中重写以存储附加信息。
...
最初，插接模式由 [CDockingManager：： m_dockModeGlobal](../../mfc/reference/cdockingmanager-class.md#m_dockmodeglobal)全局定义。 您可以使用方法分别为每个窗格设置停靠模式 `SetDockingMode` 。
...
*pWndInsertAfter*<br/>
中标识 `CWnd` `CWnd` 在 Z 顺序中位于此对象之前的对象。 有关详细信息，请参阅 [CWnd：： SetWindowPos](../../mfc/reference/cwnd-class.md#setwindowpos)。
...
如果 *pWndInsertAfter* 为 NULL，则此方法将调用 [CWnd：： SetWindowPos](../../mfc/reference/cwnd-class.md#setwindowpos)。 如果 *pWndInsertAfter* 为非 NULL，则此方法将调用 `DeferWindowPos` 。
