---
description: 了解详细信息： CPropertyPage 类
...
|[CPropertyPage：：构造](#construct)|构造 `CPropertyPage` 对象。 `Construct`如果要在运行时指定参数，或者使用数组，请使用。|
|[CPropertyPage：： GetPSP](#getpsp)|检索与对象关联的 Windows [PROPSHEETPAGE](/windows/win32/api/prsht/ns-prsht-propsheetpagea_v2) 结构 `CPropertyPage` 。|
...
与标准对话框一样，你可以 `CPropertyPage` 为属性表中的每一页派生一个类。 若要使用 `CPropertyPage` 派生对象，请先创建 [CPropertySheet](../../mfc/reference/cpropertysheet-class.md) 对象，然后为属性表中的每一页创建一个对象。 对工作表中的每一页调用 [CPropertySheet：： AddPage](../../mfc/reference/cpropertysheet-class.md#addpage) ，然后通过对模式属性表调用 [CPropertySheet：:D omodal](../../mfc/reference/cpropertysheet-class.md#domodal) ，或为无模式属性表调用 [CPropertySheet：： Create](../../mfc/reference/cpropertysheet-class.md#create) 来显示属性表。
...
有关将属性表作为向导建立的详细信息，请参阅 [CPropertySheet：： SetWizardMode](../../mfc/reference/cpropertysheet-class.md#setwizardmode)。 有关使用对象的详细信息 `CPropertyPage` ，请参阅文章 [属性表和属性页](../../mfc/property-sheets-and-property-pages-in-mfc.md)。
...
## <a name="cpropertypageconstruct"></a><a name="construct"></a> CPropertyPage：：构造
...
`Construct`如果未调用某个其他类构造函数，则调用。 `Construct`成员函数是灵活的，因为可以将 parameter 语句保留为空白，然后在代码中的任意位置指定多个参数和构造。

`Construct`当使用数组时，必须使用，并且必须 `Construct` 为数组的每个成员调用，以便为数据成员分配正确的值。
...
如果你有多个参数 (例如，如果你使用数组) ，请使用 [CPropertySheet：：构造](../../mfc/reference/cpropertysheet-class.md#construct) 而不是 `CPropertyPage` 。
...
检索与对象关联的 Windows [PROPSHEETPAGE](/windows/win32/api/prsht/ns-prsht-propsheetpagea_v2) 结构 `CPropertyPage` 。
...
有关此结构的详细信息（包括其成员的列表），请参阅 `PROPSHEETPAGE` Windows SDK 中的。
...
当框架调用此函数时，将接受对属性表中的所有属性页所做的更改，属性表将保留焦点，并 `OnApply` (值 1) 返回 TRUE。 在 `OnApply` 框架可以调用之前，必须先调用 [SetModified](#setmodified) 并将其参数设置为 TRUE。 这将在用户在属性页上进行更改后立即激活 "立即应用" 按钮。

重写此成员函数以指定用户单击 "立即应用" 按钮时程序采取的操作。 重写时，函数应返回 TRUE 以接受更改，并返回 FALSE 以防止更改生效。

调用的默认实现 `OnApply` `OnOK` 。
...
此成员函数的默认实现将设置从属性页的控件复制到属性页的成员变量。 如果数据由于对话数据验证而未成功更新 (DDV) 错误，则页面将保持焦点。
...
当用户选择 "确定" 或 "立即应用" 按钮时，框架会从属性页接收 [PSN_APPLY](/windows/win32/Controls/psn-apply) 通知。 `OnOK`如果调用[CPropertySheet：:P ressbutton](../../mfc/reference/cpropertysheet-class.md#pressbutton) ，则不会调用，因为在这种情况下，属性页不会发送通知。

重写此成员函数可在用户关闭整个属性表时，实现特定于当前活动页的其他行为。

此成员函数的默认实现将该页标记为 "clean"，以反映已在函数中更新数据 `OnKillActive` 。
...
当用户在向导中单击 " **完成** " 按钮时，框架会调用此函数;当 `OnWizardFinish`)  (的非零值返回 TRUE 时，可以销毁属性表 (但实际上不会对) 进行销毁。 调用 `DestroyWindow` 以销毁属性表。 不要从调用 `DestroyWindow` `OnWizardFinish` ; 这样做将导致堆损坏或其他错误。
...
框架跟踪哪些页是 "脏的"，即您已调用的属性页 `SetModified( TRUE )` 。 如果调用某个页面，将始终启用 "立即应用" 按钮 `SetModified( TRUE )` 。 当你为某个页面调用时，"立即应用" 按钮将被禁用 `SetModified( FALSE )` ，但前提是所有其他页面均未 "更新"。
