---
description: 了解详细信息： CMFCToolBarComboBoxButton 类
...
ms.locfileid: "97163907"
---
# <a name="cmfctoolbarcomboboxbutton-class"></a>CMFCToolBarComboBoxButton 类

包含组合框控件的工具栏按钮)  ( [CComboBox 类](../../mfc/reference/ccombobox-class.md) 。
...
|[CMFCToolBarComboBoxButton::AddSortedItem](#addsorteditem)|向组合框列表中添加一个项。 列表中项的顺序由指定 `Compare` 。|
...
|[CMFCToolBarComboBoxButton：:D eleteItem](#deleteitem)|删除组合框列表中的项。|
...
有关详细信息，请参阅 [演练：将控件放置在工具栏上](../../mfc/walkthrough-putting-controls-on-toolbars.md)。 有关组合框工具栏按钮的示例，请参阅示例项目 VisualStudioDemo。

## <a name="example"></a>示例

下面的示例演示了如何使用 `CMFCToolBarComboBoxButton` 类中的各种方法。 该示例演示如何启用 "编辑" 和 "组合框"，设置应用程序中组合框按钮的垂直位置，在下拉列表框时设置列表框的高度，设置应用程序中组合框按钮的平面样式外观，并设置组合框按钮的编辑框中的文本。 此代码片段是 [Visual Studio 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
将指定的状态复制 `CMFCToolBarComboBoxButton` 到当前对象。
...
列表框中当前选定项的索引;如果未选择任何项，则为或 CB_ERR。
...
列表框中当前选定项的索引;否则，如果未选择任何项或未找到组合框按钮，则 CB_ERR。
...
如果方法成功，则为与项关联的数据;否则，如果指定的索引无效，则为 0; 如果找不到组合框按钮，则 CB_ERR。
...
## <a name="cmfctoolbarcomboboxbuttonserialize"></a><a name="serialize"></a> CMFCToolBarComboBoxButton：：串行化
