---
description: 了解详细信息： COleDropSource 类
...
若要使用 `COleDropSource` 对象，只需调用构造函数。 这可以简化确定哪些事件（例如鼠标单击）开始使用 COleDataSource 的拖动操作的过程 [：:D odragdrop](../../mfc/reference/coledatasource-class.md#dodragdrop)、 [COleClientItem：:D odragdrop](../../mfc/reference/coleclientitem-class.md#dodragdrop)或 [COleServerItem：:D odragdrop](../../mfc/reference/coleserveritem-class.md#dodragdrop) 函数。 这些函数将 `COleDropSource` 为您创建一个对象。 你可能需要修改可 `COleDropSource` 重写函数的默认行为。 框架将在适当的时间调用这些成员函数。
...
*bEscapePressed*<br/>
指示自上次调用后是否已按下了 ESC 键 `COleDropSource::QueryContinueDrag` 。
