---
description: 了解详细信息： CPaneDialog 类
title: CPaneDialog 类
ms.date: 11/04/2016
f1_keywords:
- CPaneDialog
...
|[CPaneDialog::HandleInitDialog](#handleinitdialog)|处理 [WM_INITDIALOG](/windows/win32/dlgbox/wm-initdialog) 消息。  (重定义 `CBasePane::HandleInitDialog` 。 ) |
...
`CPaneDialog`按两个步骤构造对象。 首先，在代码中构造对象。 然后调用 [CPaneDialog：： Create](#create)。 必须指定有效的资源模板名称或模板 ID，并向父窗口传递指针。 否则，创建过程将失败。 此对话框必须指定 WS_CHILD 和 WS_VISIBLE 样式。 建议还指定 WS_CLIPCHILDREN 和 WS_CLIPSIBLINGS 样式。 有关详细信息，请参阅 [窗口样式](styles-used-by-mfc.md#window-styles)。
...
下面的示例演示如何使用 `Create` 类中的方法 `CPaneDialog` 。 此示例是 [集窗格大小示例](../../overview/visual-cpp-samples.md)的一部分。
...
此方法支持 [COccManager](../../mfc/reference/coccmanager-class.md) 类，该类用于管理 OLE 控件站点和 ActiveX 控件。 _AFX_OCC_DIALOG_INFO 结构是在 afxocc 头文件中定义的。
