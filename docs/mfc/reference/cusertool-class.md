---
description: 了解详细信息： CUserTool 类
title: CUserTool 类
ms.date: 11/04/2016
f1_keywords:
- CUserTool
...
ms.locfileid: "97318502"
---
# <a name="cusertool-class"></a>CUserTool 类

用户工具是运行外部应用程序的菜单项。 "**自定义**" 对话框中的 "**工具**" 选项卡 ( [CMFCToolBarsCustomizeDialog 类](../../mfc/reference/cmfctoolbarscustomizedialog-class.md)) 使用户能够添加用户工具，并为每个用户工具指定名称、命令、参数和初始目录。
...
|[CUserTool：:D rawToolIcon](#drawtoolicon)|在指定的矩形中绘制用户工具图标。|
...
|[CUserTool：：串行化](#serialize)|从存档读取该对象或将该对象写入存档。 （重写 [CObject::Serialize](../../mfc/reference/cobject-class.md#serialize)。）|
...
下面的示例演示如何从对象创建工具 `CUserToolsManager` 、设置 `m_strLabel` 成员变量以及设置用户工具运行的应用程序。 此代码片段是 [Visual Studio 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
加载的图标的句柄 ( HICON) ，如果无法加载默认图标，则为 NULL。
...
## <a name="cusertoolserialize"></a><a name="serialize"></a> CUserTool：：串行化
