---
description: 了解详细信息： CImageList 类
title: CImageList 类
ms.date: 11/04/2016
f1_keywords:
- CImageList
...
|ILC_COLOR16|) DIB 部分使用16位 (32/64k 的颜色。|
...
由 `CWinApp` 空闲时间处理程序自动调用，将 `DeleteTempMap` 删除 `CImageList` 由 [FromHandle](#fromhandle)创建的所有临时对象，但不会销毁任何 `hImageList`) 临时与对象关联的句柄 ( `ImageList` 。
...
通过将给定的图像与当前拖动图像) 的鼠标光标图像 (，来创建新的拖动图像。
