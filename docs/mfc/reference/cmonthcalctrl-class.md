...
*pt*<br/>
对用于标识月历控件位置的 [点](/windows/win32/api/windef/ns-windef-point) 结构的引用。
...
调用时 `Create` ，将初始化公共控件。 `Create`您调用的版本确定如何调整它的大小：
...
日历标识符表示特定于区域的日历，如公历 (本地化的) 、日语或回历。 应用程序可以使用具有各种语言支持函数的日历标识符。
...
在 MFC 的实现中 `GetCurSel` ，可以指定使用情况 `COleDateTime` 、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
如果成功，此成员函数将返回非零值并 `lpRect` 接收适用的边界信息。 如果不成功，则成员函数返回0。
...
此成员函数实现 Win32 消息 [MCM_GETMONTHRANGE](/windows/win32/Controls/mcm-getmonthrange)的行为，如 Windows SDK 中所述。 在 MFC 的实现中 `GetMonthRange` ，可以指定 `COleDateTime` 用法、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
一个 DWORD 值，可以为零， (不会设置任何限制) 或指定限制信息的以下值的组合。
...
此成员函数实现 Win32 消息 [MCM_GETRANGE](/windows/win32/Controls/mcm-getrange)的行为，如 Windows SDK 中所述。 在 MFC 的实现中 `GetRange` ，可以指定使用情况 `COleDateTime` 、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
在 MFC 的实现中 `GetSelRange` ，可以指定 `COleDateTime` 用法、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
此成员函数实现 Win32 消息 [MCM_GETTODAY](/windows/win32/Controls/mcm-gettoday)的行为，如 Windows SDK 中所述。 在 MFC 的实现中 `GetToday` ，可以指定使用情况 `COleDateTime` 、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
DWORD 值。 等于结构的 **uHit** 成员 `MCHITTESTINFO` 。
...
下面的代码示例设置月历控件以显示日语高层时代日历。 `SetCalID`仅当计算机上已安装该日历时，此方法才会成功。
...
此成员函数实现 Win32 消息 [MCM_SETCURSEL](/windows/win32/Controls/mcm-setcursel)的行为，如 Windows SDK 中所述。 在 MFC 的实现中 `SetCurSel` ，可以指定使用情况 `COleDateTime` 、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
*pMaxRange*<br/>
指向 `COleDateTime` 对象、 `CTime` 对象或结构的指针，该对象 `SYSTEMTIME` 包含范围最高端的日期。
...
此成员函数实现 Win32 消息 [MCM_SETRANGE](/windows/win32/Controls/mcm-setrange)的行为，如 Windows SDK 中所述。 在 MFC 的实现中 `SetRange` ，可以指定 `COleDateTime` 用法、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
此成员函数实现 Win32 消息 [MCM_SETSELRANGE](/windows/win32/Controls/mcm-setselrange)的行为，如 Windows SDK 中所述。 在 MFC 的实现中 `SetSelRange` ，可以指定 `COleDateTime` 用法、 `CTime` 用法或 `SYSTEMTIME` 结构用法。
...
指向 [RECT](/windows/win32/api/windef/ns-windef-rect) 结构的指针，该结构定义一个其大小小于或等于 *lpRect* 参数所定义的矩形的矩形。
