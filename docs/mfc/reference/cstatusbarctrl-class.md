...
|[CStatusBarCtrl：： GetIcon](#geticon)|在当前状态栏控件中检索部件 (也称为窗格) 的图标。|
...
|[CStatusBarCtrl：： SetSimple](#setsimple)|指定状态栏控件是显示简单文本还是显示通过先前对的调用设置的所有控件部件 `SetParts` 。|
...
有关使用的详细信息 `CStatusBarCtrl` ，请参阅 [控件](../../mfc/controls-mfc.md) 和 [使用 CStatusBarCtrl](../../mfc/using-cstatusbarctrl.md)。
...
可以通过 `CStatusBarCtrl` 两个步骤构造。 首先，调用构造函数，然后调用，它将 `Create` 创建状态栏控件，并将其附加到 `CStatusBarCtrl` 对象。
...
创建一个 (子窗口) 的控件，并将其与 `CStatusBarCtrl` 对象关联。
...
默认情况下，此成员函数不执行任何操作。 重写此成员函数以实现所有者描述对象的绘制 `CStatusBarCtrl` 。
...
下面的代码示例定义了一个 `m_statusBar` 用于访问当前状态栏控件的变量。 此变量将在下一个示例中使用。
