---
description: 了解详细信息： CStringArray 类
...
的成员函数 `CStringArray` 类似于类 [CObArray](../../mfc/reference/cobarray-class.md)的成员函数。 由于此相似性，因此你可以使用 `CObArray` 参考文档获取成员函数细节。 无论你在何处看到 `CObject` 作为返回值的指针，都应将 [cstring](../../atl-mfc-shared/using-cstring.md) 对象替换 (不是 [cstring](../../atl-mfc-shared/using-cstring.md) 指针) 。 无论你在何处看到作为函数参数的 `CObject` 指针，都请替换 `LPCTSTR`。
...
`CStringArray` 合并 IMPLEMENT_SERIAL 宏，以支持其元素的序列化和转储。 如果将 `CString` 对象的数组存储到存档中（使用重载插入运算符或 `Serialize` 成员函数），则将依次序列化每个元素。
