---
description: 了解详细信息： CMFCTabCtrl 类
...
|[CMFCTabCtrl：： EnableActiveTabCloseButton](#enableactivetabclosebutton)|显示或隐藏活动选项卡上 ( **X**) 的 "关闭" 按钮。|
...
|[CMFCTabCtrl：： IsActiveTabCloseButton](#isactivetabclosebutton)|指示 "关闭" 按钮 ( **X**) 显示在 "活动" 选项卡上还是选项卡区域的右上角。|
...
有关如何创建选项卡式窗口作为停靠控件条的示例，请参阅 [CTabbedPane 类](../../mfc/reference/ctabbedpane-class.md)。 若要 `CMFCTabCtrl` 用作非停靠控件，请创建一个 `CMFCTabCtrl` 对象，然后调用 [CMFCTabCtrl：： create](#create)。
...
下面的示例演示如何使用类中的各种方法 `CMFCTabCtrl` 配置 `CMFCTabCtrl` 对象。 该示例说明如何添加选项卡，在 "活动" 选项卡上显示 "关闭" 按钮，启用可编辑的选项卡标签，并显示一个选项卡式窗口标签的弹出菜单。 此示例是 [状态收集示例](../../overview/visual-cpp-samples.md)的一部分。
...
下面的示例演示如何使用 `Create` 类中的方法 `CMFCTabCtrl` 。 此示例是 [状态收集示例](../../overview/visual-cpp-samples.md)的一部分。
...
指示 "关闭" 按钮 ( **X**) 显示在 "活动" 选项卡上还是显示在选项卡区域的右上角。
...
*dwKeyState*<br/>
中包含修改键的状态。 此参数是以下值的按位组合 (或) ： MK_CONTROL、MK_SHIFT、MK_ALT、MK_LBUTTON、MK_MBUTTON 和 MK_RBUTTON。 有关详细信息，请参阅 [关于鼠标输入](/windows/win32/inputdev/about-mouse-input)的 **消息参数** 部分。
...
*dwKeyState*<br/>
中修改键的状态，它是 MK_CONTROL、MK_SHIFT、MK_ALT、MK_LBUTTON、MK_MBUTTON 和 MK_RBUTTON 的按位组合 (或) 。 有关详细信息，请参阅 [关于鼠标输入](/windows/win32/inputdev/about-mouse-input)中的 "消息参数"。
...
*bFlat*<br/>
中若要在选项卡区域周围绘制平面 (2D) 框架，则为 TRUE;若要绘制三维 (3D) 帧，则为 FALSE。 默认值为 TRUE。
