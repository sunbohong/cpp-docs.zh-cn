---
description: 了解详细信息： CIPAddressCtrl 类
...
与 "编辑" 控件类似的 "IP 地址" 控件允许以 Internet 协议 (IP) 格式输入和操作数字地址。

此控件 (，因此 `CIPAddressCtrl` 类) 仅适用于在 Microsoft Internet Explorer 4.0 和更高版本下运行的程序。 它们还将在未来版本的 Windows 和 Windows NT 下可用。
...
调用此函数可创建 (子窗口) 的控件，并将其与 `CIPAddressCtrl` 对象关联。
