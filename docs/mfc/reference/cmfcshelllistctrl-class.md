---
description: 了解详细信息： CMFCShellListCtrl 类
...
|“属性”|描述|
|----------|-----------------|
|[CMFCShellListCtrl：:D isplayFolder](#displayfolder)|显示所提供文件夹中包含的项的列表。|
|[CMFCShellListCtrl：:D isplayParentFolder](#displayparentfolder)|显示作为当前显示文件夹的父文件夹的文件夹中包含的项的列表。|
...
`CMFCShellListCtrl`类通过使程序能够列出 Windows shell 项来扩展[CMFCListCtrl 类](../../mfc/reference/cmfclistctrl-class.md)的功能。 所使用的显示格式类似于 "资源管理器" 窗口的列表视图。

[CMFCShellTreeCtrl](../../mfc/reference/cmfcshelltreectrl-class.md)对象可以与对象相关联 `CMFCShellListCtrl` ，以创建完整的资源管理器窗口。 然后，在中选择一项 `CMFCShellTreeCtrl` 将导致 `CMFCShellListCtrl` 对象列出选定项的内容。

## <a name="example"></a>示例

下面的示例演示如何创建类的对象 `CMFCShellListCtrl` 以及如何显示当前显示文件夹的父文件夹。 此代码片段是 [资源管理器示例](../../overview/visual-cpp-samples.md)的一部分。
...
如果中未选择任何文件夹，则此方法将失败 `CMFCShellListCtrl` 。
...
一个 [SHCONTF](/windows/win32/api/shobjidl_core/ne-shobjidl_core-_shcontf) 值，该值包含中列出的项的类型 `CMFCShellListCtrl` 。

### <a name="remarks"></a>备注

若要设置中列出的项的类型 `CMFCShellListCtrl` ，请调用 [CMFCShellListCtrl：： SetItemTypes](#setitemtypes)。
...
对象中的每一项 `CMFCShellListCtrl` 都有一个或多个列中的文本。 当框架调用此方法时，它将指定所需的列。 如果手动调用此函数，则还必须指定所需的列。
...
默认情况下，框架在对象中创建四个列 `CMFCShellListCtrl` 。 这些列的名称为 " **名称**"、" **大小**"、" **类型**" 和 " **已修改**"。 您可以重写此方法以自定义列数和列名称。
...
调用此方法可刷新由对象显示的项的列表 `CMFCShellListCtrl` 。
...
*nTypes*<br/>
中对象支持的项类型的列表 `CMFCShellListCtrl` 。
