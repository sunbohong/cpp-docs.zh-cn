...
文档数据的应用程序特定视图的基类。 视图显示数据并接受用户输入以编辑或选择数据。  (es) 派生视图类 `CView` 。
...
包含 Windows 列表控件的视图 (参阅 [CListCtrl](../mfc/reference/clistctrl-class.md)) 。 列表控件以类似于文件资源管理器右窗格的方式显示图标和字符串。

[CTreeView](../mfc/reference/ctreeview-class.md)<br/>
包含 Windows 树控件的视图 (参阅 [CTreeCtrl](../mfc/reference/ctreectrl-class.md)) 。 树形控件以与文件资源管理器左窗格类似的方式显示在层次结构中排列的图标和字符串。
