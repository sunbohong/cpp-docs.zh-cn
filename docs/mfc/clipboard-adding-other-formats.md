...
若要将更多格式添加到剪贴板上放置的格式，必须重写 `OnGetClipboardData` 从或 (派生的类中的函数， `COleClientItem` `COleServerItem` 具体取决于要复制的数据是否为本机) 。 在此函数中，应使用以下过程。
