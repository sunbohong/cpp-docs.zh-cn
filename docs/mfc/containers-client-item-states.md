---
description: 了解详细信息：容器： Client-Item 状态
title: 容器：客户端项状态
ms.date: 11/04/2016
helpviewer_keywords:
...
在第二步中，通过调用 `COleClientItem::CreateFromFile` 或其他 `CreateFrom` *xxxx* 函数执行，将完全创建项。 文件或某些其他源（例如剪贴板) ）中的 OLE 数据 (与派生的对象相关联 `COleClientItem` 。 现在，该项处于已加载状态。
...
[容器： Client-Item 通知](containers-client-item-notifications.md)<br/>
