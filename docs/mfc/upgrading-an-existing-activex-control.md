---
description: 了解更多相关信息：升级现有 ActiveX 控件
...
你还可以添加优化，如 [ActiveX 控件：优化](../mfc/mfc-activex-controls-optimization.md)中所述。 名字对象可用于以异步方式下载属性和大型 Blob，如 [Internet 上的 ActiveX 控件](../mfc/activex-controls-on-the-internet.md)中所述。

## <a name="packaging-code-for-downloading"></a><a name="_core_packaging_code_for_downloading"></a> 打包要下载的代码

有关此主题的详细信息，请参阅 [打包 ActiveX 控件](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa751974%28v%3dvs.85%29)。
...
.inf 文件将控制 .ocx 文件及其支持文件的安装。 不建议使用此方法，因为不能对 .inf 文件进行签名 (请参阅代码签名) 上指针的 [签名代码](#_core_signing_code) 。
...
### <a name="the-object-tag"></a>\<OBJECT>标记
...
请注意，在这里， `#Version` 使用 CAB 文件指定的信息适用于由标记的 *CLASSID* 参数指定的控件 `<OBJECT>` 。

根据指定的版本，您可以强制下载控件。 有关 `OBJECT` 包含 *CODEBASE* 参数的标记的完整规范，请参阅 W3C 参考。

## <a name="marking-a-control-safe-for-scripting-and-initializing"></a><a name="_core_marking_a_control_safe_for_scripting_and_initializing"></a> 将控件标记为安全，以便进行脚本编写和初始化
...
添加类似于以下内容的条目 (用控件的类 ID 替换 `{06889605-B8D0-101A-91F1-00608CEAD5B3}`) ，以将你的密钥与以下类 id 相关联：
...
1. 编辑 HTML 页。 在 HTML 页中，在 \<OBJECT> 任何其他标记之前，在许可证管理器对象之前插入标记 \<OBJECT> 。 许可证管理器是与 Internet Explorer 一起安装的 ActiveX 控件。 其类 ID 如下所示。 将许可证管理器对象的 LPKPath 属性设置为 LPK 文件的路径和名称。 每个 HTML 页面只能有一个 LPK 文件。
...
1. 在 \<OBJECT> 许可证管理器标记后，为许可的控件插入标记。
...
1. \<EMBED>如果使用 NCompass ActiveX 插件，请插入 LPK 文件的属性。

   如果你的控件可以在其他启用了活动的浏览器（例如，使用 NCompass ActiveX 插件的情况下）上查看，则必须添加语法，如下 \<EMBED> 所示。
...
有关控制授权的详细信息，请参阅 [Activex 控件：许可 Activex 控件](../mfc/mfc-activex-controls-licensing-an-activex-control.md)。
...
容器确定调色板并使其可用作环境属性， **DISPID_AMBIENT_PALETTE**。 容器（例如，Internet Explorer）选择页面上的所有 ActiveX 控件使用的调色板来确定其自己的调色板。 这将防止显示闪烁并提供一致的外观。
...
## <a name="internet-explorer-browser-safety-levels-and-control-behavior"></a><a name="_core_internet_explorer_browser_safety_levels_and_control_behavior"></a> Internet Explorer 浏览器安全级别和控件行为
