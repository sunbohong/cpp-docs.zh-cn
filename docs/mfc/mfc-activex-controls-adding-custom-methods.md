---
description: 了解更多： MFC ActiveX 控件：添加自定义方法
...
1. 使用 **参数类型** 和 **参数名称** 控件添加名为 *xCoord* 的参数 *OLE_XPOS_PIXELS*)  (类型。

1. 使用 **参数类型** 和 **参数名称** 控件添加名为 *yCoord* 的参数 *OLE_YPOS_PIXELS*)  (类型。
...
此外，位于实现 ( 中的以下行。控件类的 CPP) 文件，将添加到控件的调度映射：
...
最后，添加方法向导将存根函数添加 `CSampleCtrl::PtInCircle` 到控件的实现 ( 的底部。CPP) 文件。 `PtInCircle`要使正常工作，必须按如下所述修改它：
