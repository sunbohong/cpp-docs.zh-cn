...
这些函数签名是使用简单约定派生的。 函数名称始终以 `"On` "。 后面跟有 "WM_" 的 Windows 消息的名称，每个单词的首字母都大写。 参数的顺序为 *wParam* 后接 `LOWORD` (*LParam*) 然后 `HIWORD` (*lParam*) 。 未通过未使用的参数。 由 MFC 类包装的任何句柄都转换为指向相应 MFC 对象的指针。 下面的示例演示如何处理 WM_PAINT 消息并导致 `CMyWnd::OnPaint` 调用函数：
...
此示例) 中已注册的 Windows 消息 ID 变量 (WM_FIND 必须是 *接近* 变量，因为 ON_REGISTERED_MESSAGE 实现方式。
...
- 控件通知代码 (高 *lParam*) ，如 BN_CLICKED，与消息映射项中指定的通知代码匹配。

-  (*wParam*) 控件 id 与消息映射项中指定的控件 id 相匹配。
