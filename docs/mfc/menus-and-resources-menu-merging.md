...
ms.locfileid: "97227970"
---
# <a name="menus-and-resources-menu-merging"></a>菜单和资源：菜单合并

本文详细介绍了 OLE 文档应用程序正确处理可视编辑和就地激活所需的步骤。 就地激活为容器和服务器 (组件) 应用程序带来了挑战。 用户仍在容器文档) 的上下文中 (保留在同一框架窗口中，但实际上是在服务器)  (运行其他应用程序。 这需要在容器和服务器应用程序的资源之间进行协调。
...
1. 文件菜单与打开文件时所用的相同。  (通常不会在下一项之前放置其他菜单项。 ) 

1. 两个连续的分隔符。

1. 与打开文件时使用的窗口菜单相同 (仅当 MDI 应用程序中的容器应用程序) 时使用。 某些应用程序可能具有属于此组的其他菜单，如选项菜单，该菜单在就地激活嵌入项时仍保留在菜单上。
...
服务器应用程序应该创建一个新的工具栏，并将其位图存储在单独的文件中。 应用程序向导生成的应用程序将此位图存储在名为 ITOOLBAR.BMP 的文件中。 就地激活服务器的项时，新工具栏将替换容器应用程序的工具栏，并应包含与普通工具栏相同的项，但会删除表示文件和窗口菜单上的项的图标。
