...
- **对齐** 未附加、左或右的一个。 控制数字调整按钮的位置。 向左和向右定位 "合作者" 窗口旁的数值调节钮。 将减少合作者窗口的宽度以容纳数值调节钮。 与 UDS_ALIGNLEFT 和 UDS_ALIGNRIGHT 样式关联。
