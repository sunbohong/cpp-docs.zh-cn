---
description: 了解更多相关信息： TN054：使用 MFC DAO 类时直接调用 DAO
...
使用 MFC DAO 数据库类时，可能会出现需要直接使用 DAO 的情况。 通常情况下，不会出现这种情况，但 MFC 提供了一些 helper 机制，以便在结合使用 MFC 类和直接 DAO 调用时，使直接 DAO 调用更简单。 直接对 MFC 托管 DAO 对象的方法进行 DAO 调用时，只需编写几行代码。 如果需要创建并使用 *不* 由 MFC 管理的 DAO 对象，则必须通过对对象进行实际调用来完成一些工作 `Release` 。 本技术说明说明了当你可能想要直接调用 DAO 时，MFC 帮助器可以执行哪些操作来帮助你，以及如何使用 DAO OLE 接口。 最后，此注释提供一些示例函数，演示如何直接为 DAO 安全功能调用 DAO。

## <a name="when-to-make-direct-dao-calls"></a>何时进行直接 DAO 调用

当需要刷新集合或实现 MFC 未包装的功能时，进行直接 DAO 调用的最常见情况。 MFC 未公开的最重要功能是安全的。 如果要实现安全功能，则需要使用 DAO 用户 () 并将 () 对象直接分组。 除了安全性，MFC 还不支持其他几个 DAO 功能。 其中包括记录集克隆和数据库复制功能，以及一些到 DAO 的后期补充。

## <a name="a-brief-overview-of-dao-and-mfcs-implementation"></a>DAO 和 MFC 实现的简要概述

通过处理许多详细信息，DAO 包装的 DAO 使 DAO 的使用更加简单，因此您无需担心这些问题。 这包括 OLE 的初始化、创建和管理 DAO 对象 (尤其是集合对象) 、错误检查，并提供强类型、更简单的接口， (不) **变量** 或 `BSTR` 参数。 您可以直接进行 DAO 调用，并且仍可以利用这些功能。 所有代码都必须为 `Release` 直接 DAO 调用创建的任何对象调用，而 *不* 会修改 MFC 可能在内部依赖的任何接口指针。 例如，  `CDaoRecordset` 除非你了解 *所有* 内部后果，否则请不要修改打开对象的 m_pDAORecordset 成员。 不过，您可以使用 *m_pDAORecordset* 接口直接调用 DAO 来获取字段集合。 在这种情况下，不会修改 *m_pDAORecordset* 成员。 使用完对象后，只需 `Release` 在 Fields 集合对象上调用。
...
对于 DAO 接口中的许多方法，您需要操作一个 `BSTR` 对象 (在 OLE 自动化) 中使用的长度为长度的字符串。 `BSTR`对象通常封装在 **变量** 数据类型中。 MFC 类 `COleVariant` 本身继承自 **变量** 数据类型。 根据您是为 ANSI 或 Unicode 生成项目，DAO 接口将返回 ANSI 或 Unicode `BSTR` 。 V_BSTR 和 V_BSTRT 两个宏对于确保 DAO 接口获取预期类型的是很有用的 `BSTR` 。

V_BSTR 将提取的 *bstrVal* 成员 `COleVariant` 。 当你需要将的内容传递 `COleVariant` 到 DAO 接口的方法时，通常使用此宏。 下面的代码片段显示了使用 V_BSTR 宏的 DAO DAOUser 接口的两种方法的声明和实际用法：
...
请注意，在 `VT_BSTRT` 上面的构造函数中指定的参数 `COleVariant` 可确保 `BSTR` `COleVariant` 如果生成应用程序的 ansi 版本和 unicode `BSTR` 版本的应用程序，则会在中存在 ansi。 这正是 DAO 所期望的。

另一个宏 V_BSTRT 会根据 `COleVariant` (ansi 或 unicode) 生成的类型提取 Ansi 或 unicode bstrVal 成员。 下面的代码演示如何将中的 `BSTR` 值提取 `COleVariant` 到中 `CString` ：
...
DAOVIEW 示例演示了 V_BSTRT 宏以及用于打开存储在中的其他类型的其他方法 `COleVariant` 。 具体而言，此转换在方法中执行 `CCrack::strVARIANT` 。 如果可能，此方法会将的值转换 `COleVariant` 为的实例 `CString` 。
