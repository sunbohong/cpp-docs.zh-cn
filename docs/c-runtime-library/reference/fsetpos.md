...
如果成功， **fsetpos** 将返回0。 如果失败，该函数将返回一个非零值，并将 **errno** 设置为以下清单常量之一 (在 errno 中定义。H) ： **ebadf (**，这意味着文件不可访问，或者 *流* 指向的对象不是有效的文件结构;或 **EINVAL**，这意味着传递的 *流* 或 *pos* 的值无效。 如果传入了无效参数，这些函数则会调用无效参数处理程序，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。
