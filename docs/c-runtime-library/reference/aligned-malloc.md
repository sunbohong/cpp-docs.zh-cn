---
description: 了解详细信息： _aligned_malloc
...
使用 [_aligned_free](aligned-free.md) 释放由 **_aligned_malloc** 和获取的内存 `_aligned_offset_malloc` 。 不要使用 `free` ，这不会正确回收对齐的内存，并可能导致难以诊断 bug。
