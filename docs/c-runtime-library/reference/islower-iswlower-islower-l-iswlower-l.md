---
description: 了解详细信息： islower、iswlower、_islower_l、_iswlower_l
...
如果 *c* 是小写字符的特定表示形式，则每个例程将返回非零值。 如果 *c* 是 (a-z) 的小写字符，则 **islower** 将返回一个非零值。 如果 *c* 是对应于小写字母的宽字符，或者如果 *c* 是实现定义的宽字符集（其中， **iswcntrl**、 **iswdigit**、 **iswpunct** 或 **iswspace** 均不为零）中的一个，则 **iswlower** 将返回一个非零值。 如果 *c* 不满足测试条件，则这些例程都将返回0。
