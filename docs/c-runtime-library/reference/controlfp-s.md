---
description: 了解详细信息： _controlfp_s
title: _controlfp_s
ms.date: 4/2/2020
api_name:
- _controlfp_s
...
如果 *掩码* 为非零值，则将设置一个新的控制字值：对于设置 (的任何位（等于) 在 *掩码* 中），将使用 *new* 中的相应位来更新控制字。 换句话说， *fpcntrl* = ( # B3 *fpcntrl* & ~*mask*) &#124; (*newControl*  &  *掩码*) # A7，其中 *fpcntrl* 是浮点控制字。 在这种情况下，在更改完成后， *currentControl* 设置为值;它不是旧的控制字位值。
...
在 ARM 平台上， **_controlfp_s** 函数适用于 FPSCR 寄存器。 在 x64 体系结构上，只会影响存储在 MXCSR 寄存器中的 SSE2 控制字。 在 Intel (x86) 平台上， **_controlfp_s** 会影响 X87 和 SSE2 的控制字（如果存在）。 由于上一次调用 [__control87_2](control87-controlfp-control87-2.md)（例如) ），这两个控制词可能会相互不一致 (;如果两个控制字之间存在不一致， **_controlfp_s** 将在 *currentControl* 中设置 **EM_AMBIGUOUS** 标志。 这是一条警告，指示所返回的控制字可能没有准确地表示两个浮点控制字的状态。
...
使用 [/clr (公共语言运行时编译) ](../../build/reference/clr-common-language-runtime-compilation.md) 进行编译时，将忽略此函数，因为公共语言运行时 (clr) 仅支持默认的浮点精度。
