...
*Compare* 函数是一个指向用户提供的例程的指针，它比较两个数组元素，并返回指定其关系的值。 *Compare* 函数还将指向上下文的指针作为第一个参数。 **_lsearch_s** 调用在搜索过程中 *比较* 一次或多次，将指针传递给每个调用上的两个数组元素。 *compare* 必须比较元素，然后返回非零值 (表示元素不同) 或 0 (表示元素完全相同) 。
