---
description: 了解详细信息： ungetc、ungetwc
...
如果两次调用之间没有读取或文件定位操作 **，则结果** 是不可预知的。 调用 **fscanf** 后，对 **ungetc** 的调用可能会失败，除非执行了另一个读取 (操作，如 **getc**) 。 这是因为， **fscanf** 本身调用了 **ungetc**。
