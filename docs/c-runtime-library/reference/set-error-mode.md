...
修改 **__error_mode** 来确定非默认位置，其中，C 运行时为可能终止程序的错误编写错误消息。
...
如果传入所列出的值之外的值，则将调用无效参数处理程序，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。 如果允许执行继续， **_set_error_mode** 将 **Errno** 设置为 **EINVAL** ，并返回-1。
