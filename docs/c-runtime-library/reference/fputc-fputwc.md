---
description: 了解详细信息： fputc、fputwc
title: fputc、fputwc
ms.date: 4/2/2020
api_name:
- fputc
...
其中每个函数将单个字符 *c* 写入到文件位置指示器所指示的位置上的文件 (如果定义了) ，并根据需要提升指示器。 对于 **fputc** 和 **fputwc**，该文件与 *stream* 关联。 如果文件不支持定位请求，或在追加模式中打开，则该字符将被追加到流的末尾。
