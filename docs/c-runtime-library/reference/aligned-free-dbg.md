---
description: 了解详细信息： _aligned_free_dbg
...
**_Aligned_free_dbg** 函数是 [_aligned_free](aligned-free.md)函数的调试版本。 未定义 [_DEBUG](../../c-runtime-library/debug.md) 时，对 **_aligned_free_dbg** 的每次调用都会减少到对的调用 `_aligned_free` 。 `_aligned_free`和都 **_aligned_free_dbg** 在基堆中释放内存块，但 **_aligned_free_dbg** 可容纳调试功能：能够在堆的链接列表中保留已释放的块，以模拟内存不足的情况。
