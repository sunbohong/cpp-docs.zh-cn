...
当指定 **"r +"**、 **"w +"** 或 **"a +"** 访问类型时，将允许读取和写入 (文件被称为 "更新" ) 。 但是，在读取与写入之间切换时，必须有干预性的 [fsetpos](fsetpos.md)、[fseek](fseek-fseeki64.md) 或 [rewind](rewind.md) 操作。 如果需要，可以为 [fsetpos](fsetpos.md) 或 [fseek](fseek-fseeki64.md) 操作指定当前位置。 除了以上值之外， *模式* 字符串中还可能会包含以下字符之一以指定新行的转换模式。
...
在文本 (翻译的) 模式下，回车换行符 (CR-LF) 组合转换为单行换行符 (LF) 输入的字符;换行符在输出时转换为 CR-LF 组合。 CTRL+Z 也将在输入时解释为文件尾字符。 在打开以进行读取或使用 **"a +"** 进行读取和读取的文件中，运行时库将检查文件末尾的 CTRL + Z 并将其删除（如果可能）。 这样做的原因是，使用 [fseek](fseek-fseeki64.md) 和 [ftell](ftell-ftelli64.md) 在文件中移动可能导致 [fseek](fseek-fseeki64.md) 在文件结尾附近出现错误。 **T** 选项是一个 Microsoft 扩展，不应在需要 ANSI 可移植性时使用。
