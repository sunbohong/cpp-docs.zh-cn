...
**_Getw** 函数从与流关联的文件中读取类型的下一个二进制值 **`int`** ，并递增关联的文件指针 (如果有一个) 指向下一个未读字符。  **_getw** 不假定流中项的任何特殊对齐方式。 由于与系统之间的类型大小 **`int`** 和字节顺序不同，因此可能会在 _getw 中发生移植问题 **`int`** 。
