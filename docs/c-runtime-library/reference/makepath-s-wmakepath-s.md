---
description: 了解详细信息： _makepath_s、_wmakepath_s
...
*dir*<br/>
包含目录路径，但不包括驱动器指示符或实际文件名。 尾随斜杠是可选的，并且可以 \\ 在单个 *dir* 参数中使用正斜杠 (/) 或反斜杠 () 或两者。 如果未指定尾随斜杠（\ 或 \\），将自动插入。 如果 *dir* 为 **NULL** 或指向一个空字符串，则在复合 *路径* 字符串中不会插入目录路径。
...
|*路径*|*sizeInWords*  / *sizeInBytes*|返回|*路径* 的内容|
|------------|------------------------------------|------------|------------------------|
|**NULL**|any|**EINVAL**|未修改|
|any|<= 0|**EINVAL**|未修改|
