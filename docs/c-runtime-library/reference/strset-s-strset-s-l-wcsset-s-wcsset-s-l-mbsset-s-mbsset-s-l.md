---
description: 了解详细信息： _strset_s、_strset_s_l、_wcsset_s、_wcsset_s_l、_mbsset_s、_mbsset_s_l
...
**_Strset_s** 函数将 *str* 的所有字符都设置为 *c* (转换为 **`char`**) （终止 null 字符除外）。 **_wcsset_s** 和 **_mbsset_s** 是 **_strset_s** 的宽字符和多字节字符版本。 参数和返回值的数据类型会相应地变化。 否则这些函数具有相同行为。
