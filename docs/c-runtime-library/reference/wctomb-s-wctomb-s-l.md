---
description: 了解详细信息： wctomb_s、_wctomb_s_l
...
如果 **wctomb_s** 将宽字符转换为多字节字符，则它会将字节数 (该值永远不会大于宽字符中 **MB_CUR_MAX**) ，并将其置于 *PRetValue* 所指向的整数中。 如果 *wchar* 是 "\ 0" )  (的宽字符 null 字符， **wctomb_s** 将 *pRetValue* 填充为1。 如果目标指针 *mbchar* 为 **NULL**， **wctomb_s** 会将0放入 *pRetValue*。 如果当前区域设置中不能进行转换， **wctomb_s** 将 *pRetValue* 中的-1。
