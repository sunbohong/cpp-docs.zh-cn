---
description: 了解详细信息：退出、_Exit、_exit
...
**Exit**、 **_Exit** 和 **_exit** 函数终止调用进程。 **Exit** 函数调用线程本地对象的析构函数，然后调用-后进先出 (后进先出) order- **atexit** 注册的函数和 **_onexit**，然后在终止进程之前刷新所有文件缓冲区。 **_Exit** 和 **_exit** 函数终止进程，而无需销毁线程本地对象或处理 **atexit** 或 **_onexit** 函数，并且无需刷新流缓冲区。
