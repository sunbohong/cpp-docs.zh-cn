---
description: 了解详细信息： strnlen、strnlen_s、wcsnlen、wcsnlen_s、_mbsnlen、_mbsnlen_l、_mbstrnlen、_mbstrnlen_l
...
这些函数将返回字符串中的字符数，不包括以 null 结尾的字符。 如果在字符串的第一个 *numberOfElements* 字节内没有空终止符 (或 **wcsnlen**) 的宽字符，则返回 *numberOfElements* ，以指示错误条件;以 null 结尾的字符串的长度严格小于 *numberOfElements*。
...
> [!NOTE]
> **strnlen** 不是 **strlen** 的替代; **strnlen** 仅用于计算已知大小的缓冲区中传入的不受信任数据的大小，例如网络数据包。 **strnlen** 计算长度，但如果字符串未终止，则不会遍历缓冲区的末尾。 对于其他情况，请使用 **strlen**。  (同样适用于 **wcsnlen**、 **_mbsnlen** 和 **_mbstrnlen**。 ) 
