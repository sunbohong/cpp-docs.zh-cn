---
description: 了解详细信息： _aligned_malloc_dbg
...
**_aligned_malloc_dbg** 分配内存块，其空间比请求的 *大小* 稍多。 其他空间将由调试堆管理器用于链接调试内存块，以及提供具有调试标头信息的应用程序和覆盖缓冲区。 分配该块后，使用值 0xCD 填充该块的用户部分，使用值 0xFD 填充每个覆盖缓冲区。
 ...
 `errno` `ENOMEM` 如果内存分配失败，或者如果所需的内存量 (包含之前提到的开销) 超过，_aligned_malloc_dbg 将设置为 `_HEAP_MAXREQ` 。 有关此代码及其他错误代码的信息，请参阅 [errno、_doserrno、_sys_errlist 和 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。 此外， **_aligned_malloc_dbg** 验证其参数。 如果 *对齐* 不是2的幂或 *大小* 为零，则此函数将调用无效参数处理程序，如 [参数验证](../../c-runtime-library/parameter-validation.md)中所述。 如果允许执行继续，则此函数将返回 NULL，并将设置 `errno` 为 `EINVAL` 。
