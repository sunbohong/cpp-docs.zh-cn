---
description: 了解详细信息： isascii、__isascii、iswascii
...
如果 **c** 是 ASCII 字符的特定表示形式，则每个例程将返回非零值。 如果 **c** 是一个介于 0X00-0x7f) 范围内的 ASCII 字符 (， **__isascii** 将返回一个非零值。 如果 **c** 是 ASCII 字符的宽字符表示形式，则 **iswascii** 将返回一个非零值。 如果 **c** 不满足测试条件，则这些例程都将返回0。
...
|例程所返回的值|必需的标头|
|-------------|---------------------|
|**isascii**、 **__isascii**|Ansi-c \<ctype.h><br /><br /> C + +： \<cctype> 或 \<ctype.h>|
