...
**_Ecvt** 函数将浮点数转换为字符串。 *值* 参数是要转换的浮点数。 此函数以字符串的形式存储 *值* 的 *计数* 位数，并追加 null 字符 ( "\ 0" ) 。 如果 *值* 中的数字位数超过 *计数*，则会舍入低序位。 如果数字少于 *计数* ，则用零填充字符串。
