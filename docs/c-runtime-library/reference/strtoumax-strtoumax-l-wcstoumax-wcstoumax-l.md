---
description: 了解详细信息： strtoumax、_strtoumax_l、wcstoumax、_wcstoumax_l
...
**strtoumax** 返回转换后的值（如果有）或 **UINTMAX_MAX** 溢出。 如果无法执行任何转换，则 **strtoumax** 将返回0。 **wcstoumax** 将类似值返回到 **strtoumax**。 对于这两个函数，如果出现溢出或下溢，则 **errno** 设置为 **ERANGE** 。
...
**strtoumax** 停止读取其无法识别为数字一部分的第一个字符的字符串 *strSource* 。 这可能是终止 null 字符，也可能是大于或等于 *基准* 的第一个数字字符。 区域设置的 **LC_NUMERIC** 类别设置确定 *strSource* 中的基数字符的识别。 有关详细信息，请参阅 [setlocale、_wsetlocale](setlocale-wsetlocale.md)。 **strtoumax** 和 **wcstoumax** 使用当前区域设置; **_strtoumax_l** 和 **_wcstoumax_l** 相同，只不过它们改用传入的区域设置。 有关详细信息，请参阅 [Locale](../../c-runtime-library/locale.md)。
