---
description: 了解详细信息： putchar、putwchar
...
**Putc** 例程将单个字符 *c* 写入当前位置的输出 *流* 中。 任何整数都可以传递到 **putc**，但仅写入较低的8位。 **Putchar** 例程与完全相同 `putc( c, stdout )` 。 对于每个例程，如果发生读取错误，则会设置流的错误指示器。 **putc** 和 **putchar** 与 **fputc** 和 **_fputchar** 分别相似，但同时作为函数和宏实现 (请参阅在 [函数和宏) 之间进行选择](../../c-runtime-library/recommendations-for-choosing-between-functions-and-macros.md) 。 **putwc** 和 **putwchar** 分别是 **putc** 和 **putchar** 的宽字符版本。
