---
description: 了解详细信息： strcoll、wcscoll、_mbscoll、_strcoll_l、_wcscoll_l _mbscoll_l
...
其中每个函数都会返回错误 **_NLSCMPERROR** 。 若要使用 **_NLSCMPERROR**，请包含任意一个字符串。H 或 MBSTRING.H。 如果 *string1* 或 *string2* 为 **NULL** ，或者包含排序序列域外部的宽字符代码，则 **wcscoll** 可能会失败。 出现错误时， **wcscoll** 可能会将 **Errno** 设置为 **EINVAL**。 若要检查对 **wcscoll** 的调用是否有错误，请将 **errno** 设置为0，然后在调用 **wcscoll** 后检查 **errno** 。
