...
|[_ismbcalpha、_ismbcalpha_l](../c-runtime-library/reference/ismbcalnum-functions.md)|字母顺序|当且仅当出现以下情况时返回非零值：`c` 是 ASCII 英文字母的单字节表示形式：查看 `_ismbcupper` 和 `_ismbclower` 的示例；或是片假名字母的单字节表示形式：0xA6<=`c`<=0xDF。|
...
|[_ismbcgraph、_ismbcgraph_l](../c-runtime-library/reference/ismbcgraph-functions.md)|Graphic|当且仅当 `c` 是除空格 () 之外的任何 ASCII 或片假名可打印字符的单字节表示形式时返回非零值。 请参阅 `_ismbcdigit`、`_ismbcalpha` 和 `_ismbcpunct` 的示例。|
...
|[_ismbcspace、_ismbcspace_l](../c-runtime-library/reference/ismbcgraph-functions.md)|空格|当且仅当 `c` 是空白字符 `c`=0x20 或 0x09<=`c`<=0x0D 时返回非零值。|
