...
`<tgmath.h>` 包含可简化对要调用的正确数学函数的选择的宏。 宏将检查传递的类型，然后调用正确的函数。 例如， `sqrt` 宏绑定 `sqrt(9.9f)` 到 `sqrtf()` ，但它绑定 `sqrt(9.9)` 到 `sqrt()` 。 如果泛型参数至少有一个宏参数是复杂的，则宏将绑定到复杂函数;否则，它会调用实际函数。
...
Windows SDK 10.0.20201.0 或更高版本。 这是你可以从 [Windows 预览体验](https://www.microsoft.com/software-download/windowsinsiderpreviewSDK)下载下载的内部版本。 有关安装和使用此 SDK 的说明，请参阅 [C11 和 C17：入门](https://devblogs.microsoft.com/cppblog/c11-and-c17-standard-support-arriving-in-msvc/#c11-and-c17-getting-started) 。

## <a name="see-also"></a>请参阅

[C Run-Time 库参考](c-run-time-library-reference.md)
