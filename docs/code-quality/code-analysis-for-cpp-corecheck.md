...
此部分列出 C++ Core Guidelines 检查器警告。 有关代码分析的信息，请参阅[ `/analyze` (代码分析) ](../build/reference/analyze-code-analysis.md)和[快速入门： c/c + + 代码分析](../code-quality/quick-start-code-analysis-for-c-cpp.md)。
...
请避免使用 malloc ( # A1 和 free ( # A3，更喜欢 new with delete 的 nothrow 版本。 请参阅 [C++ Core Guidelines](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md#Rr-mallocfree)。
...
避免具有自定义构造和析构的未命名对象。 请参阅 [ES：不 (尝试) 声明不带名称的局部变量](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md)。
