---
description: 了解详细信息： C26485 NO_ARRAY_TO_POINTER_DECAY
title: C26485
ms.date: 04/29/2020
ms.topic: conceptual
f1_keywords:
...
此示例将在对的调用中出现两个警告，导致数组与指针衰减 `memcpy` 。
...
若要解决此问题，请避免调用采用指针参数，但不管理边界信息。 此类函数的使用通常容易出错。 首选 c + + 标准库调用 C 运行库函数。 请考虑 `gsl::span` `std::vector` 在自己的函数中使用或。 显式强制转换为 decayed 指针类型会阻止警告，但它不会阻止错误代码。
