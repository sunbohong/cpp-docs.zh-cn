---
description: 了解详细信息：使用加速器和 accelerator_view 对象
...
此外，运行时还 `access_type` `access_type_auto` 为默认快捷键指定了的。 这意味着，如果支持共享内存，并且已知 (带宽和延迟) 的性能特征与专用 (非共享) 内存相同，则它将使用共享内存。
...
共享内存是可以由 CPU 和加速器访问的内存。 使用共享内存可消除或大大降低在 CPU 和加速器之间复制数据的开销。 尽管内存是共享的，但它不能同时由 CPU 和加速器进行访问，这样做会导致未定义的行为。 [](reference/accelerator-class.md#supports_cpu_shared_memory) **`true`** 如果快捷键支持共享内存，则 supports_cpu_shared_memory 返回快捷键属性，而 [default_cpu_access_type](reference/accelerator-class.md#default_cpu_access_type)属性将获取分配的内存的默认 [access_type](reference/concurrency-namespace-enums-amp.md#access_type) ， `accelerator` 例如与关联的 **数组** `accelerator` ，或中 `array_view` 访问的对象 `accelerator` 。

默认情况下，C++ AMP 运行时自动选择每个的最佳默认值 `access_type` `accelerator` ，但是在从 cpu 读取、从 cpu 写入或同时从 cpu 写入时，共享内存的性能特征 (带宽和延迟) 可能比专用的 (非共享) 加速器内存更差。 如果共享内存执行以及用于从 CPU 进行读取和写入的专用内存，则运行时默认为 `access_type_read_write` ; 否则，运行时将选择一个更保守的默认值 `access_type` ，并允许应用程序在其计算内核的内存访问模式受益于其他时进行重写 `access_type` 。
...
- [加速器：:d Irect3d_warp 数据成员](reference/accelerator-class.md#direct3d_warp)：此加速器提供一个回退解决方案，用于在使用 C++ AMP) SSE 的流式处理 SIMD (扩展的多核 cpu 上执行代码。
