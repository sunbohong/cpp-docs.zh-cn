...
ms.locfileid: "97142748"
---
# <a name="texture-class"></a>texture 类

纹理是在范围域中的上的数据聚合 `accelerator_view` 。 它是变量集合，每个元素对应于一个范围域中的一个元素。 每个变量都保留与 c + + 基元类型对应的值 ( **`unsigned int`** 、 **`int`** 、 **`float`** 、 **`double`**) 、标量类型 ( `norm` 或 `unorm`) 或短矢量类型。
