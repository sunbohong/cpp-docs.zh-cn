...
|[运算符% =](#operator_mod_eq)|计算对象中每个元素的取模 (余数) `extent` 。|
...
对于前缀运算符， `extent` 对象 (**`*this`**) 。 对于后缀运算符，则为新的 `extent` 对象。
