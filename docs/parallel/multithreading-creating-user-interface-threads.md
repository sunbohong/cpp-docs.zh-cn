---
description: 了解详细信息：多线程处理：创建 MFC User-Interface 线程
title: 多线程处理：创建 MFC User-Interface 线程
ms.date: 08/27/2018
f1_keywords:
- CREATE_SUSPENDED
- SECURITY_ATTRIBUTES
...
ms.locfileid: "97149963"
---
# <a name="multithreading-creating-mfc-user-interface-threads"></a>多线程处理：创建 MFC User-Interface 线程

用户界面线程通常用于处理用户输入，并独立于执行应用程序其他部分的线程来响应用户事件。 派生类中提供 (主应用程序线程) 已为 `CWinApp` 你创建和启动。 本主题介绍创建其他用户界面线程所需的步骤。
...
### <a name="functions-to-override-when-creating-a-user-interface-thread"></a>创建 User-Interface 线程时要重写的函数
...
|[PreTranslateMessage](../mfc/reference/cwinthread-class.md#pretranslatemessage)|在将消息调度到和之前筛选消息 `TranslateMessage` `DispatchMessage` 。 通常不重写。|
...
- 派生自的类的 [RUNTIME_CLASS](../mfc/reference/run-time-object-model-services.md#runtime_class) `CWinThread` 。

-  (可选) 所需的优先级。 默认值为正常优先级。 有关可用优先级的详细信息，请参阅 Windows SDK 中的 [参见 setthreadpriority](/windows/win32/api/processthreadsapi/nf-processthreadsapi-setthreadpriority) 。
