...
本文档中定义的指令、库函数和环境变量允许您创建和管理并行程序，同时允许可移植性。 指令使用单个程序将 C 和 c + + 顺序编程模型扩展为多个数据 (SPMD) 构造、工作共享构造和同步构造。 它们还支持数据的共享和私有化。 支持 OpenMP C 和 c + + API 的编译器包含一个命令行选项，该选项用于激活并允许解释所有 OpenMP 编译器指令的编译器。
...
- 构造
  ...
  构造是一个语句。 它包含指令和结构化块。 某些指令不是构造的组成部分。  (参见 [附录 C](c-openmp-c-and-cpp-grammar.md)) 中的 *openmp 指令*。
...
- serialize
  ...
  - 仅由一个线程组成的线程团队 (这是该并行构造) 的主线程，

  - 结构化块中语句的执行序列顺序 (相同的顺序，就像块不是并行构造) 的一部分一样，

  - `omp_in_parallel()`除了任何嵌套并行构造) 的影响外， (返回的值都不会产生任何影响。
...
- 结构化块
  ...
  结构化块是一个语句 (单个或复合) ，其中包含单个项和一个退出。 如果有跳转或跳出语句，则该语句是结构化块。  (此规则包括对 `longjmp` (3c) 或使用的调用 `throw` ，但允许对的调用 `exit` ) 。如果其执行始终从开始处开始 `{` ，并在结束时始终结束，则 `}` 复合语句是结构化块。 如果通过将表达式语句、选择语句、迭代语句或 **`try`** 块放入，并将其 `{` `}` 作为结构化块来获取，则该语句是结构化块。 跳转语句、标记语句或声明语句不是结构化的块。
...
OpenMP 使用并行执行的分叉联接模型。 尽管此分叉联接模型可用于解决各种问题，但它是针对大型基于阵列的应用程序定制的。 OpenMP 旨在支持在并行程序 (多个执行线程和完全 OpenMP 支持库) 中正确执行的程序。 它还适用于按顺序程序正确执行 (指令，并) 简单的 OpenMP 存根库。 但是，可以在按顺序执行时开发不能正常运行的程序。 此外，由于数字运算的关联发生变化，不同程度的并行度可能会导致不同的数值结果。 例如，串行加法缩减可能与并行减少模式具有不同的加法关联模式。 这些不同的关联可能会更改浮点加法的结果。
...
如果线程修改了某个共享对象，则该对象不仅会影响其自己的执行环境，还会影响程序中其他线程的执行环境。 从另一个线程的角度来看，修改将保证完成，下一个序列点处的下一个序列点 (仅当该对象被声明为可变时才) 。 否则，修改将保证在第一次修改线程之后完成。 然后，其他线程 (或并发) 会看到一个 `flush` 指令，该指令指定 (隐式或显式) 的对象。 如果 `flush` 其他 OpenMP 指令隐含的指令不能保证正确的副作用顺序，则程序员应负责提供其他显式 `flush` 指令。
...
所有标准 C 和 c + + 库函数和内置函数 (也就是说，编译器具有特定知识) 的函数必须是线程安全的。 并行区域中不同线程的线程安全函数的未同步使用不会产生未定义的行为。 但行为可能与在串行区域中的行为不同。  (随机数字生成函数是一个示例。 ) 
