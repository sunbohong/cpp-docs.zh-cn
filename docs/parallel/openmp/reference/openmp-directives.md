...
|---------|-----------|
|主|指定仅主线程应执行程序的某个部分。|
|[来说](#critical)|指定代码每次只能在一个线程上执行。|
|[关卡](#barrier)|同步团队中的所有线程;所有线程都在关卡处暂停，直到所有线程都执行关卡为止。|
|[原子](#atomic)|指定将以原子方式更新的内存位置。|
|[平](#flush-openmp)|指定所有线程都具有与所有共享对象相同的内存视图。|
|[排序](#ordered-openmp-directives)|指定并行化循环下的代码 `for` 应按照顺序循环执行。|
...
有关详细信息，请参阅 [2.6.4 原子构造](../2-directives.md#264-atomic-construct)。
...
## <a name="barrier"></a><a name="barrier"></a> 关卡
...
有关详细信息，请参阅 [2.6.3 关卡指令](../2-directives.md#263-barrier-directive)。

### <a name="example"></a>示例

有关如何使用的示例 `barrier` ，请参阅 [master](#master)。

## <a name="critical"></a><a name="critical"></a> 来说
...
有关详细信息，请参阅 [2.6.2 临界构造](../2-directives.md#262-critical-construct)。
...
## <a name="flush"></a><a name="flush-openmp"></a> 平
...
有关详细信息，请参阅 [2.6.5 flush 指令](../2-directives.md#265-flush-directive)。
...
## <a name="for"></a><a name="for-openmp"></a> 进行

导致在并行区域内的循环中完成的工作 `for` 被拆分到多个线程中。
...
- [幅度](openmp-clauses.md#reduction)
- [排序](openmp-clauses.md#ordered-openmp-clauses)
- [schedule](openmp-clauses.md#schedule)
- [nowait](openmp-clauses.md#nowait)

如果 `parallel` 还指定了，则 `clauses` 可以是或指令接受的任何子句 `parallel` `for` （除外） `nowait` 。
...
## <a name="master"></a><a name="master"></a> 主从
...
有关详细信息，请参阅 [2.6.1 主构造](../2-directives.md#261-master-construct)。
...
## <a name="ordered"></a><a name="ordered-openmp-directives"></a> 排序

指定并行化循环下的代码 `for` 应按照顺序循环执行。
...
有关详细信息，请参阅 [2.6.6 顺序构造](../2-directives.md#266-ordered-construct)。
...
## <a name="sections"></a><a name="sections-openmp"></a> 个
...
如果 `parallel` 还指定了，则 `clauses` 可以是或指令接受的任何子句 `parallel` `sections` （除外） `nowait` 。

有关详细信息，请参阅 [2.4.2 sections 节构造](../2-directives.md#242-sections-construct)。
...
## <a name="single"></a><a name="single"></a> 单个
...
有关详细信息，请参阅 [2.4.3 单构造](../2-directives.md#243-single-construct)。
...
`threadprivate`指令基于使用[__declspec](../../../cpp/declspec.md)关键字的[线程](../../../cpp/thread.md)属性; 适用于的限制 `__declspec(thread)` `threadprivate` 。 例如， `threadprivate` 变量将存在于进程启动的任何线程中，而不只是作为并行区域生成的线程组的一部分的线程。 请注意此实现的详细信息;你可能会注意到， `threadprivate` 用户定义类型的构造函数被多次调用。
...
当并行区域构成的线程终止时，用户不会进行任何控制。 如果在进程退出时这些线程存在，则不会通知线程有关进程退出的信息，并且不会对任何线程调用析构函数， `threaded_var` 除非在此处退出 (，主线程) 。 因此，代码不应计算正确的 `threadprivate` 变量析构。

有关详细信息，请参阅 [2.7.1 threadprivate 指令](../2-directives.md#271-threadprivate-directive)。

### <a name="example"></a>示例

有关使用的示例 `threadprivate` ，请参阅 [private](openmp-clauses.md#private-openmp)。
