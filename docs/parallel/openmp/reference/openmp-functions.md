...
有关使用的示例，请参阅 [omp_init_lock](#omp-init-lock) `omp_destroy_lock` 。
...
有关使用的示例，请参阅 [omp_init_nest_lock](#omp-init-nest-lock) `omp_destroy_nest_lock` 。
...
有关详细信息，请参阅 [3.1.7 omp_set_dynamic 函数](../3-run-time-library-functions.md#317-omp_set_dynamic-function)。

### <a name="example"></a>示例

有关使用的示例，请参阅 [omp_set_dynamic](#omp-set-dynamic) `omp_get_dynamic` 。
...
有关详细信息，请参阅 [3.1.10 omp_get_nested 函数](../3-run-time-library-functions.md#3110-omp_get_nested-function)。

### <a name="example"></a>示例

有关使用的示例，请参阅 [omp_set_nested](#omp-set-nested) `omp_get_nested` 。
...
有关使用的示例，请参阅 [并行](openmp-directives.md#parallel) `omp_get_thread_num` 。
...
有关使用的示例，请参阅 [omp_get_wtime](#omp-get-wtime) `omp_get_wtick` 。
...
有关详细信息，请参阅 [3.3.1 omp_get_wtime 函数](../3-run-time-library-functions.md#331-omp_get_wtime-function)。
...
使用 [omp_get_dynamic](#omp-get-dynamic) 显示的当前设置 `omp_set_dynamic` 。

的设置 `omp_set_dynamic` 将覆盖 [OMP_DYNAMIC](openmp-environment-variables.md#omp-dynamic) 环境变量的设置。
...
有关使用的示例，请参阅 [omp_init_lock](#omp-init-lock) `omp_set_lock` 。
...
有关使用的示例，请参阅 [omp_init_nest_lock](#omp-init-nest-lock) `omp_set_nest_lock` 。
...
的设置 `omp_set_nested` 将重写环境变量的设置 `OMP_NESTED` 。

启用环境变量可能会中断其他操作程序，因为在嵌套并行区域时，线程数会以指数方式增加。 例如，recurses 6 次，OMP 线程数设置为4的函数需要 4096 (4 到 6) 线程的幂。 除了 i/o 限制的应用程序，如果线程数超过处理器数量，应用程序的性能通常会下降。

使用 [omp_get_nested](#omp-get-nested) 显示的当前设置 `omp_set_nested` 。

有关详细信息，请参阅 [3.1.9 omp_set_nested 函数](../3-run-time-library-functions.md#319-omp_set_nested-function)。
...
有关使用的示例，请参阅 [omp_get_num_threads](#omp-get-num-threads) `omp_set_num_threads` 。
...
有关使用的示例，请参阅 [omp_init_lock](#omp-init-lock) `omp_unset_lock` 。
...
有关使用的示例，请参阅 [omp_init_nest_lock](#omp-init-nest-lock) `omp_unset_nest_lock` 。
