---
description: 了解更多相关信息：自动并行化和自动矢量化
title: 自动并行化和自动矢量化
ms.date: 11/04/2016
...
因为`u`可能是较小的值，则编译器不会自动并行化此循环。 但是，您可能仍希望它并行化，因为您知道 `u` 将始终很大。 若要启用自动并行化，请指定 [#pragma loop (hint_parallel (n) # B3 ](../preprocessor/loop.md)，其中 `n` 是要在其上进行并行化的线程数。 在下面的示例中，编译器将尝试跨 8 个线程并行循环。
...
与所有 [杂注指令](../preprocessor/pragma-directives-and-the-pragma-keyword.md)一样，还支持备用杂注语法 `__pragma(loop(hint_parallel(n)))` 。
...
请注意两个不同/Qpar-report 之间的输出差异 [ (自动并行报告级别) ](../build/reference/qpar-report-auto-parallelizer-reporting-level.md) 选项。 `/Qpar-report:1` 仅为成功进行并行化的循环输出并行化程序消息。 `/Qpar-report:2` 为成功和不成功的循环并行化输出并行化程序消息。
...
默认情况下，自动向量化处于启用状态。 如果要比较矢量化下的代码的性能，可以使用 [ (no_vector) 的 #pragma 循环 ](../preprocessor/loop.md) 来禁用任何给定循环的矢量化。
...
与所有 [杂注指令](../preprocessor/pragma-directives-and-the-pragma-keyword.md)一样，还支持备用杂注语法 `__pragma(loop(no_vector))` 。

与自动并行一样，可以指定 [/Qvec-report (自动向量化报告级别) ](../build/reference/qvec-report-auto-vectorizer-reporting-level.md) 命令行选项，以仅报告成功的向量化循环， `/Qvec-report:1` 或同时报告成功和未成功的向量化循环- `/Qvec-report:2`) 。
...
[/Qpar (自动并行) ](../build/reference/qpar-auto-parallelizer.md)<br/>
[/Qpar-report (自动并行报告级别) ](../build/reference/qpar-report-auto-parallelizer-reporting-level.md)<br/>
