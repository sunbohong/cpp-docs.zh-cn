---
description: 了解详细信息：如何：在数据管道中使用转换器
...
ms.locfileid: "97205611"
---
# <a name="how-to-use-transformer-in-a-data-pipeline"></a>如何：在数据管道中使用转换器

本主题包含一个示例，该示例演示如何在数据管道中使用 [concurrency：：转换器](../../parallel/concrt/reference/transformer-class.md) 类。 有关使用数据管道执行图像处理的更完整示例，请参阅 [演练：创建 Image-Processing 网络](../../parallel/concrt/walkthrough-creating-an-image-processing-network.md)。
...
数据管道中的阶段通常会输出其类型与其输入值不同的值。 在此示例中，第二个阶段采用类型为的值 **`int`** 作为其输入，并生成该值 (**`double`**) 作为其输出的值的平方根。
...
[演练：创建 Image-Processing 网络](../../parallel/concrt/walkthrough-creating-an-image-processing-network.md)
