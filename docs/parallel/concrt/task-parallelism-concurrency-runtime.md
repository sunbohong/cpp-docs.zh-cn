...
- 编写异步代码时，请使用 ([concurrency：： task](../../parallel/concrt/reference/task-class.md) 类) 的任务。 任务类使用 Windows 线程池作为其计划程序中，而不是并发运行时。
...
在异步编程中，一个异步操作在完成时调用另一个操作并将数据传递到其中的情况非常常见。 传统上，这使用回调方法来完成。 在并发运行时中， *延续任务* 提供了相同的功能。 延续任务 (也称为延续) 是一个异步任务，由另一个任务（称为 *前面* 的任务）在前面的任务完成时调用。 使用延续可以：
...
- 提供有关应如何计划延续的提示。  (仅适用于通用 Windows 平台 (UWP) 应用。 有关详细信息，请参阅 [在适用于 UWP 应用的 c + + 中创建异步操作](../../parallel/concrt/creating-asynchronous-operations-in-cpp-for-windows-store-apps.md)。 ) 
...
延续还可以返回另一个任务。 如果没有取消，则此任务会在后续延续之前执行。 此方法称为 *异步解包*。 要在后台执行其他工作，但不想当前任务阻止当前线程时，异步解包会很有用。  (这在 UWP 应用中很常见，在这种情况下，延续可以在 UI 线程) 上运行。 下面的示例演示三个任务。 第一个任务返回在延续任务之前运行的另一个任务。
...
## <a name="value-based-versus-task-based-continuations"></a><a name="value-versus-task"></a> Value-Based 与 Task-Based 延续
...
[并发运行时的任务类 () ](../../parallel/concrt/reference/task-class.md)
