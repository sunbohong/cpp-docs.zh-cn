---
description: 了解详细信息：并发运行时中的一般最佳实践
title: 并发运行时中的常规最佳做法
ms.date: 11/04/2016
helpviewer_keywords:
...
*资源获取是* (RAII 进行初始化) 模式是一种安全地管理给定范围下并发对象的生存期的方法。 在 RAII 模式下，在堆栈上分配数据结构。 此数据结构在创建时初始化或获取资源，并在数据结构销毁时销毁或释放该资源。 RAII 模式可确保在封闭范围退出之前调用析构函数。 当函数包含多个语句时，此模式非常有用 **`return`** 。 此模式还有助于编写异常安全的代码。 当 **`throw`** 语句导致堆栈展开时，将调用 RAII 对象的析构函数; 因此，始终会正确地删除或释放资源。
