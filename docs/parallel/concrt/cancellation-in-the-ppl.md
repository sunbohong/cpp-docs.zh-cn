---
description: 了解有关详细信息，请参阅 PPL 中的取消操作
title: PPL 中的取消操作
ms.date: 11/19/2018
helpviewer_keywords:
...
> [!NOTE]
> 运行时使用异常处理实现取消操作。 请勿在代码中捕捉或处理这些异常。 此外，还建议你在任务的函数体中编写异常安全的代码。 例如，你可以使用 *资源获取* (RAII) 模式进行初始化，以确保在任务正文中引发异常时正确处理资源。 有关使用 RAII 模式清除可取消任务中的资源的完整示例，请参阅 [演练：从 User-Interface 线程中删除工作](../../parallel/concrt/walkthrough-removing-work-from-a-user-interface-thread.md)。
