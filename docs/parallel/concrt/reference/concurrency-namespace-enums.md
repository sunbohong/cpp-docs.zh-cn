...
## <a name="agent_status-enumeration"></a><a name="agent_status"></a> agent_status 枚举
...
## <a name="agents_eventtype-enumeration"></a><a name="agents_eventtype"></a> Agents_EventType 枚举
...
## <a name="concrt_eventtype-enumeration"></a><a name="concrt_eventtype"></a> ConcRT_EventType 枚举
...
## <a name="concrt_traceflags-enumeration"></a><a name="concrt_traceflags"></a> Concrt_TraceFlags 枚举
...
## <a name="criticalregiontype-enumeration"></a><a name="criticalregiontype"></a> CriticalRegionType 枚举
...
## <a name="dynamicprogressfeedbacktype-enumeration"></a><a name="dynamicprogressfeedbacktype"></a> DynamicProgressFeedbackType 枚举
...
## <a name="join_type-enumeration"></a><a name="join_type"></a> join_type 枚举
...
## <a name="message_status-enumeration"></a><a name="message_status"></a> message_status 枚举
...
## <a name="policyelementkey-enumeration"></a><a name="policyelementkey"></a> PolicyElementKey 枚举
...
|`DynamicProgressFeedback`|确定计划程序的资源是根据从计划程序收集的统计信息重新平衡，还是仅基于基础硬件线程的订阅级别。 有关详细信息，请参阅 [DynamicProgressFeedbackType](#dynamicprogressfeedbacktype)。<br /><br /> 有效值：枚举的成员 `DynamicProgressFeedbackType` ， `ProgressFeedbackEnabled` 或 `ProgressFeedbackDisabled`<br /><br /> 默认值： `ProgressFeedbackEnabled`|
|`LocalContextCacheSize`|当 `SchedulingProtocol` 策略密钥设置为值时 `EnhanceScheduleGroupLocality` ，它将指定允许在每个虚拟处理器本地队列中缓存的可运行上下文的最大数目。 此类上下文通常会在导致它们变为可运行的虚拟处理器上的后进先出 (后进先出) 顺序运行。 请注意，在 `SchedulingProtocol` 将密钥设置为值时，此策略项没有任何意义 `EnhanceForwardProgress` 。<br /><br /> 有效值：非负整数<br /><br /> 默认值： `8`|
|`MaxConcurrency`|计划程序所需的最大并发级别。 资源管理器将首先尝试分配此多个虚拟处理器。 特殊值 [MaxExecutionResources](concurrency-namespace-constants1.md#maxexecutionresources) 指示所需的并发级别与计算机上的硬件线程数相同。 如果为指定的值 `MinConcurrency` 大于计算机上的硬件线程数并 `MaxConcurrency` 将指定为 `MaxExecutionResources` ，则 `MaxConcurrency` 将引发的值以匹配为设置的 `MinConcurrency` 。<br /><br /> 有效值：正整数和特殊值 `MaxExecutionResources`<br /><br /> 默认值： `MaxExecutionResources`|
|`MaxPolicyElementKey`|最大策略元素键。 不是有效的元素键。|
|`MinConcurrency`|资源管理器必须向计划程序提供的最低并发级别。 分配给计划程序的虚拟处理器的数目永远不会低于最小值。 特殊值 [MaxExecutionResources](concurrency-namespace-constants1.md#maxexecutionresources) 指示最低并发级别与计算机上的硬件线程数相同。 如果为指定的值 `MaxConcurrency` 小于计算机上的硬件线程数，并 `MinConcurrency` 将指定为，则的 `MaxExecutionResources` 值 `MinConcurrency` 将会降低以与为设置的值匹配 `MaxConcurrency` 。<br /><br /> 有效值：非负整数和特殊值 `MaxExecutionResources` 。 请注意，对于用于并发运行时计划程序构造的计划程序策略，值 `0` 无效。<br /><br /> 默认值： `1`|
...
|`SchedulingProtocol`|描述计划程序将使用的计划算法。 有关详细信息，请参阅 [SchedulingProtocolType](#schedulingprotocoltype)。<br /><br /> 有效值：枚举的成员 `SchedulingProtocolType` ， `EnhanceScheduleGroupLocality` 或 `EnhanceForwardProgress`<br /><br /> 默认值： `EnhanceScheduleGroupLocality`|
...
## <a name="schedulertype-enumeration"></a><a name="schedulertype"></a> SchedulerType 枚举
...
## <a name="schedulingprotocoltype-enumeration"></a><a name="schedulingprotocoltype"></a> SchedulingProtocolType 枚举
...
## <a name="switchingproxystate-enumeration"></a><a name="switchingproxystate"></a> SwitchingProxyState 枚举
...
## <a name="task_group_status-enumeration"></a><a name="task_group_status"></a> task_group_status 枚举
...
## <a name="winrtinitializationtype-enumeration"></a><a name="winrtinitializationtype"></a> WinRTInitializationType 枚举
