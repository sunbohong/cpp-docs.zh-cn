---
description: 了解详细信息： source_block 类
...
如果参数为， [](../../../standard-library/invalid-argument-class.md)则该方法引发 invalid_argument `_PTarget` 异常 `NULL` 。
...
如果参数[](bad-target-class.md) `_PTarget` 不表示调用的目标，则该方法引发 bad_target 异常 `reserve` 。
