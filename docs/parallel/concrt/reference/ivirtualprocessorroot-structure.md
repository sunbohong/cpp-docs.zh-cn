---
description: 了解详细信息： IVirtualProcessorRoot 结构
...
布尔值。 值为 **`true`** 指示线程代理从方法返回， `Deactivate` 以响应对方法的调用 `Activate` 。 值为 **`false`** 指示从方法返回的线程代理，以响应资源管理器中的通知事件。 在用户模式可计划 (UMS) 线程计划程序中，这指示项已显示在计划程序的完成列表中，并且需要计划程序来处理这些项。
