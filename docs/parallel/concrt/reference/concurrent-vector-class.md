---
description: 了解详细信息： concurrent_vector 类
...
|“属性”|描述|
|----------|-----------------|
|[assign](#assign)|已重载。 清除并发向量的元素，并为其分配 `_N` 的副本 `_Item` 或迭代器范围 [，) 指定的值 `_Begin` `_End` 。 此方法不是并发安全方法。|
...
清除并发向量的元素，并为其分配 `_N` 的副本 `_Item` 或迭代器范围 [，) 指定的值 `_Begin` `_End` 。 此方法不是并发安全方法。
...
第五个构造函数指定 ( `_N` 类的默认值元素) 元素的重复 `T` 。

第六个构造函数指定 ( `_N`) 的值元素的重复 `_Item` 。

最后一个构造函数指定迭代器范围 [，) 提供的值 `_Begin` `_End` 。
