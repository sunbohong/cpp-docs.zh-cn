...
ms.locfileid: "97188373"
---
# <a name="task-class-concurrency-runtime"></a>task 类（并发运行时）

并行模式库 (PPL) `task` 类。 `task`对象表示可与其他任务同时异步执行的工作，以及并发运行时中并行算法生成的并行工作。 成功完成后，它将生成类型为 `_ResultType` 的结果。 类型为 `task<void>` 的任务不生成任何结果。 可独立于其他任务等待和取消的任务。 此外，还可以使用延续 () 将其与其他任务组合在一起 `then` ，并 ( `when_all`) 和选项 ( `when_any`) 模式。 当任务对象被分配给新变量时，行为是的 `std::shared_ptr` ; 换言之，这两个对象表示相同的基础任务。
...
> [!IMPORTANT]
> 在通用 Windows 平台 (UWP) 应用程序中，不要在用户界面线程上运行的代码中调用 [concurrency：： task：： wait](#wait) 或 `get` ( `wait` 调用 `get`) 。 否则，运行时会引发 [concurrency：： invalid_operation](invalid-operation-class.md) ，因为这些方法会阻止当前线程，并可能导致应用程序停止响应。 但是，您可以调用 `get` 方法来接收基于任务的延续中的先行任务的结果，因为结果会立即可用。
...
如果该任务已完成或已取消，则该函数将返回 true， (有或没有用户异常) 。
