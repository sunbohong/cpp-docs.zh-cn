...
|[IsCurrentTaskCollectionCanceling](#iscurrenttaskcollectioncanceling)|返回一个指示当前上下文中当前正在执行内联的任务集合是否正在活动取消 (或将很快) 。|
...
并发运行时计划程序 (请参阅 [计划](scheduler-class.md) 程序) 使用执行上下文来执行应用程序在其上排队的工作。 Win32 线程是 Windows 操作系统上的执行上下文的示例。
...
返回一个指示当前上下文中当前正在执行内联的任务集合是否正在活动取消 (或将很快) 。
...
如果将计划程序附加到调用上下文，并且任务组在该上下文上执行内联任务，则指示该任务组是处于活动取消 (中，还是很快) ;否则为值 **`false`** 。
...
此方法可能会引发多个异常。 如果上下文尝试 `Unblock` 自行调用方法，将引发 [context_self_unblock](context-self-unblock-class.md) 异常。 如果对 `Block` 和 `Unblock` 的调用未正确配对 (例如，将对 `Unblock` 当前运行) 的上下文进行两次调用，将引发 [context_unblock_unbalanced](context-unblock-unbalanced-class.md) 异常。
