---
description: 了解详细信息： concurrent_unordered_multiset 类
...
前两个成员函数返回一个迭代器，该迭代器指定已移除的任何元素之外保留的第一个元素; 如果此类元素不存在，则 [结束](#end) ( # A1。 第三个成员函数返回它删除的元素的数目。

### <a name="remarks"></a>备注

第一个成员函数删除所指向的元素 `_Where` 。 第二个成员函数删除范围 [，) 中的元素 `_Begin` `_End` 。

第三个成员函数删除范围 [equal_range](#equal_range) (KVal) 分隔的元素。
