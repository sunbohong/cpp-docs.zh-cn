---
description: 了解详细信息：多线程处理：终止 MFC 中的线程
...
若要获取辅助线程或用户界面线程的退出代码，请调用 [GetExitCodeThread](/windows/win32/api/processthreadsapi/nf-processthreadsapi-getexitcodethread) 函数。 有关此函数的信息，请参阅 Windows SDK。 此函数采用) 的对象的数据成员中存储的线程 (的句柄 `m_hThread` `CWinThread` ，并使用 DWORD 的地址。
...
- 单独存储线程的句柄。 创建线程后，请 `m_hThread` 使用) 将其数据成员复制 (， `::DuplicateHandle` 并通过该变量对其进行访问。 这样一来，当终止发生时，将自动删除该对象，并且仍可以发现该线程终止的原因。 请注意，在复制句柄之前，该线程不会终止。 要执行此操作，最安全的方法是将 CREATE_SUSPENDED 传递给 [AfxBeginThread](../mfc/reference/application-information-and-management.md#afxbeginthread)，存储句柄，然后通过调用 [ResumeThread](../mfc/reference/cwinthread-class.md#resumethread)恢复线程。
