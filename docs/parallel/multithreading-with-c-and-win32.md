---
description: 了解更多：多线程处理 C 和 Win32
...
ms.locfileid: "97149820"
---
# <a name="multithreading-with-c-and-win32"></a>使用 C 和 Win32 进行多线程编程

Microsoft C/c + + 编译器 (MSVC) 为创建多线程应用程序提供支持。 如果你的应用程序需要执行可能导致用户界面无法响应的昂贵操作，请考虑使用多个线程。

使用 MSVC，可以通过多种方式来编程多个线程：可使用 c + +/WinRT 和 Windows 运行时库、Microsoft 基础类 (MFC) 库、c + +/CLI 和 .NET 运行时，或者 C 运行时库和 Win32 API。 本文介绍 C 中的多线程处理。有关示例代码，请参阅 [C 中的示例多线程程序](sample-multithread-c-program.md)。
...
`_beginthread` `_beginthreadex` 如果操作成功，和函数将创建一个新线程并返回线程标识符。 如果线程完成执行，则该线程会自动终止。 或者，它可以通过调用或自行终止 `_endthread` `_endthreadex` 。

> [!NOTE]
> 如果从使用 libcmt.lib 生成的程序调用 C 运行时例程，则必须使用 `_beginthread` 或函数启动线程 `_beginthreadex` 。 不要使用 Win32 函数 `ExitThread` 和 `CreateThread` 。 `SuspendThread`当多个线程被阻止，等待挂起线程完成对 C 运行时数据结构的访问时，使用可能会导致死锁。

### <a name="the-_beginthread-and-_beginthreadex-functions"></a><a name="_core_the__beginthread_function"></a> _Beginthread 和 _beginthreadex 函数
...
`_beginthread` 和与 `_beginthreadex` Win32 API 中的 [CreateThread](/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread) 函数类似，但具有以下差异：

- 它们初始化某些 C 运行时库变量。 仅在线程中使用 C 运行时库时，这一点很重要。

- `CreateThread` 帮助提供对安全属性的控制。 您可以使用此函数启动处于挂起状态的线程。

`_beginthread``_beginthreadex`如果成功，则返回新线程的句柄; 如果出现错误，则返回错误代码。

### <a name="the-_endthread-and-_endthreadex-functions"></a><a name="_core_the__endthread_function"></a> _Endthread 和 _endthreadex 函数

[_Endthread](../c-runtime-library/reference/endthread-endthreadex.md)函数终止由 (创建的线程， `_beginthread` 类似地 `_endthreadex` 终止) 创建的线程 `_beginthreadex` 。 线程完成后会自动终止。 `_endthread` 和 `_endthreadex` 可用于从线程内部进行条件终止。 例如，专用于通信处理的线程可能会在无法获取通信端口的控制时退出。
...
### <a name="sharing-common-resources-between-threads"></a><a name="_core_sharing_common_resources_between_threads"></a> 在线程之间共享公共资源
...
*Mut* ual *ex* clusion) 的 mutex (short，是一种在彼此异步执行的线程或进程之间进行通信的方式。 这种通信可以用于协调多个线程或进程的活动，通常通过锁定和解锁资源来控制对共享资源的访问。 若要解决此 *x*，*y* 坐标更新问题，更新线程将设置一个互斥体，指示在执行更新之前数据结构正在使用。 这会在处理完这两个坐标之后清除互斥体。 在更新显示之前，显示线程必须等待互斥体清楚。 等待 mutex 的这一过程通常称为互斥体，因为该进程已被阻止， *并且在互斥* 体清除之前无法继续。

[示例多线程 c 程序](sample-multithread-c-program.md)中显示的弹跳 c 程序使用一个名为的 mutex `ScreenMutex` 来协调屏幕更新。 每当其中一个显示线程准备好写入屏幕时，它会调用 `WaitForSingleObject` 并将句柄与固定的 `ScreenMutex` 无限，以指示 `WaitForSingleObject` 调用应在互斥体上阻塞，而不会超时。如果 `ScreenMutex` 为 clear，则 wait 函数将设置互斥体，以便其他线程无法干扰显示，并继续执行线程。 否则，在清除互斥体之前，将阻止线程。 当线程完成显示更新时，它通过调用来释放互斥体 `ReleaseMutex` 。

屏幕显示和静态数据只是需要仔细管理的两个资源。 例如，程序可能有多个线程访问同一文件。 由于另一个线程可能已移动文件指针，因此，每个线程都必须在读取或写入之前重置文件指针。 此外，每个线程都必须确保其定位指针的时间和访问文件时的时间不会被抢占。 这些线程应使用信号量来协调对文件的访问，方法是使用和调用括号每个文件访问 `WaitForSingleObject` `ReleaseMutex` 。 下面的代码示例演示了此方法：
...
### <a name="thread-stacks"></a><a name="_core_thread_stacks"></a> 线程堆栈

应用程序的所有默认堆栈空间都分配给第一个执行线程，这称为 "线程 1"。 因此，你必须为程序需要的每个其他线程指定为单独的堆栈分配多少内存。 如果需要，操作系统将为线程分配其他堆栈空间，但必须指定默认值。

调用中的第一个参数 `_beginthread` 是指向函数的指针 `BounceProc` ，该函数执行线程。 第二个参数指定线程的默认堆栈大小。 最后一个参数是传递给的 ID 号 `BounceProc` 。 `BounceProc` 使用 ID 号来设定随机数生成器的种子，并选择线程的颜色属性和显示字符。
...
创建、链接或执行多线程 C 程序时可能会遇到几个问题。 下表描述了一些更常见的问题。  (从 MFC 角度来看类似的讨论，请参见 [多线程处理：编程提示](multithreading-programming-tips.md)。 ) 
...
[用 c + +/WinRT 进行并发和异步操作](/windows/uwp/cpp-and-winrt-apis/concurrency)\
