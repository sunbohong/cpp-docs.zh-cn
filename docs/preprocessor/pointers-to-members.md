---
description: 了解详细信息： pointers_to_members 杂注
...
| **virtual_inheritance** | 最常规的表示形式为虚拟继承（指向成员函数的指针）。 绝不会导致出现错误。 使用时， **virtual_inheritance** 为默认参数 `#pragma pointers_to_members(full_generality)` 。 |

> [!CAUTION]
> 建议仅将 **pointers_to_members** 杂注放置在要影响的源代码文件中，并且在任何 `#include` 指令之后。 这种做法降低了杂注将影响其他文件的风险，并且您将不小心为同一个变量、函数或类名称指定多个定义。
