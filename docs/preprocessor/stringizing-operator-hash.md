---
description: '了解详细信息：字符串化 operator ( # ) '
title: 字符串化运算符 (#)
ms.date: 08/29/2019
f1_keywords:
- '#'
...
> [!NOTE]
> 不再支持对 ANSI C 标准的 Microsoft C（6.0 版和早期版本）扩展，该扩展之前扩展了出现在字符串和字符常量内的扩展的宏形式自变量。 应使用字符串化 () 运算符来重写依赖于此扩展的代码 **#** 。
...
此外，如果参数中包含的字符在字符串中使用时通常需要转义序列（例如，引号 (`"`) 或反斜杠 (`\`) 字符），则必要的转义反斜杠会自动插入到该字符之前。

当与包含转义序列的字符串一起使用时，Microsoft c + + 字符串化运算符的行为不正确。 在这种情况下，编译器将生成 [编译器错误 C2017](../error-messages/compiler-errors-1/compiler-error-c2017.md)。
...
在 `stringer` 预处理过程中将展开宏，从而生成以下代码：
