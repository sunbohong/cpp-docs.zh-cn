...
description: C++ Build Insights SDK TRACING_SESSION_MSVC_EVENT_FLAGS 常量引用。
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
`TRACING_SESSION_MSVC_EVENT_FLAGS` 常量用于描述在跟踪期间要收集的 MSVC 事件。 使用它们来初始化 [TRACING_SESSION_OPTIONS](tracing-session-options-struct.md) 结构的 `MsvcEventFlags` 字段。
