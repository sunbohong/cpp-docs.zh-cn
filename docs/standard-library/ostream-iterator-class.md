---
description: 了解详细信息： ostream_iterator 类
...
必须满足的输出迭代器的要求 `ostream_iterator` 仅要求表达式 \* *ii*  =  为有效，而不是 **`operator`** 自己的或 `operator=` 本身。 此实现中的成员运算符返回 **\* this**。
...
运算符将 *val* 插入与对象关联的输出流，后跟在 [ostream_iterator 构造函数](#ostream_iterator) 中指定的分隔符 (如果任何) ，然后返回对的引用 `ostream_iterator` 。
...
该类型是[](../standard-library/basic-ostream-class.md) <  `CharType` `Traits` iostream 层次结构的一个 stream 类的> basic_ostream 同义词，它定义可用于写入的对象。
