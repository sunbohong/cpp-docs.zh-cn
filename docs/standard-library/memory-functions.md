...
函数将创建对象，该对象 `shared_ptr<T>` 是一个指针，指向 `T(args...)` 由分配分配并构建。
...
模板函数返回一个指针，该指针指向属于对象 sp 的 *删除器* 类型的 `shared_ptr` 删除器。 如果 *sp* 没有删除器，或者其删除器不是 *删除器* 类型，则该函数返回0。
...
第一个重载用于单个对象。 为数组调用第二个重载。 第三个重载防止在类型参数 (make_unique) 中指定数组大小 \<T[N]> ; 当前标准不支持此构造。 当使用 `make_unique` 将 `unique_ptr` 创建到数组时，必须分别初始化数组元素。 可能更好的选择是使用 [std：： vector](vector-class.md)，而不是使用此重载。
...
如果 sp 为空对象，则模板函数将返回空 `shared_ptr` 对象 `shared_ptr` ; 否则，它将返回 `shared_ptr<T>` 拥有 *sp* 拥有的资源的对象。 表达式 `static_cast<T*>(sp.get())` 必须有效。
