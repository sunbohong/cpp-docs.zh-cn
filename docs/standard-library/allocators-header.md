...
C + + 标准库中基于节点的容器 (std：： list、std：： set、std：：多重集、std：： map 和 std：：多重映射) 将它们的元素存储在单个节点中。 针对特定容器类型的所有节点都具有相同大小，因此无需内存管理器常规用途的灵活性。 因为已在编译时知道每个内存块的大小，所以内存管理器可以更简单、更快。

如果用于不是基于节点的容器 (例如 c + + 标准库容器 std：： vector std：:d eque 和 std：： basic_string) ，分配器模板将正常工作，但不可能为默认分配器提供任何性能改善。
