...
|[准备](../standard-library/iterator-functions.md#begin)|检索一个指向指定容器中第一个元素的迭代器。|
...
|[下一步](../standard-library/iterator-functions.md#next)|迭代指定的次数并返回新的迭代器位置。|
...
|[大小](../standard-library/iterator-functions.md#size)||
...
|[运算符<](../standard-library/iterator-operators.md#op_lt)|测试运算符左侧的迭代器对象是否小于右侧的迭代器对象。|
|[操作员\<=](../standard-library/iterator-operators.md#op_gt_eq)|测试运算符左侧的迭代器对象是否小于或等于右侧的迭代器对象。|
|[运算符>](../standard-library/iterator-operators.md#op_gt)|测试运算符左侧的迭代器对象是否大于右侧的迭代器对象。|
|[运算符>=](../standard-library/iterator-operators.md#op_gt_eq)|测试运算符左侧的迭代器对象是否大于或等于右侧的迭代器对象。|
|[operator +](../standard-library/iterator-operators.md#op_add)|将偏移量添加到迭代器，并返回在新偏移位置处发现插入元素的新 `reverse_iterator`。|
|[操作员](../standard-library/iterator-operators.md#operator-)|从另一个迭代器中减去一个迭代器并返回差值。|
...
|[forward_iterator_tag](../standard-library/forward-iterator-tag-struct.md)|为表示向前迭代器的函数提供返回类型的类 `iterator_category` 。|
|[front_insert_iterator](../standard-library/front-insert-iterator-class.md)|类模板描述输出迭代器对象。 它将元素插入类型为的容器 `Container` ，该容器通过它存储的受保护对象（称为容器）进行访问 `pointer` 。|
|[input_iterator_tag](../standard-library/input-iterator-tag-struct.md)|为表示输入迭代器的函数提供返回类型的类 `iterator_category` 。|
...
|[ostreambuf_iterator 类](../standard-library/ostreambuf-iterator-class.md)|类模板描述输出迭代器对象。 它将类的元素插入到 `Elem` 输出流缓冲区中，并通过它所存储的对象访问该缓冲区，并将类型指针转换为 `basic_streambuf` \<**Elem**, **Tr**> 。|
|[output_iterator_tag](../standard-library/output-iterator-tag-struct.md)|一个类，该类提供 `iterator_category` 表示输出迭代器的函数的返回类型。|
|[random_access_iterator_tag](../standard-library/random-access-iterator-tag-struct.md)|一个类，该类提供 `iterator_category` 表示随机访问迭代器的函数的返回类型。|
