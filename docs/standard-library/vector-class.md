...
[Vector \<bool> 引用类](../standard-library/vector-bool-class.md#reference_class)是一个嵌套类，其对象可提供对矢量对象中 (单个位) 的元素的引用 \<bool> 。
...
所有构造函数都存储分配器对象 (*分配* 器) 并初始化向量。

前两个构造函数指定一个空初始矢量。 第二个构造函数显式指定分配器类型 (*分配* 器要使用) 。

第三个构造函数 *指定 (类* 的默认值的元素) 元素的重复次数 `Type` 。

第四个和第五个构造函数指定 (*计数*) 值 *值*元素的重复。
