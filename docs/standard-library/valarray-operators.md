---
description: 了解详细信息： &lt; valarray &gt; 运算符
title: '&lt;valarray&gt; 运算符'
ms.date: 03/27/2019
f1_keywords:
...
按位异 `OR` ( **XOR**) 具有以下语义：给定位 *b* 1 和 *b* 2， *b* 1 **XOR** *b* 2 是 **`true`** 只有一个位为 true; 如果两个位都为 true，则为 **`false`** true。
...
按位 OR 具有与逻辑 `OR` 相同的事实表，但是按位 OR 适用于单个位级别上的数据类型。 给定位 *b* 1 和 *b* 2，  `OR`  **`true`** 如果至少有一个位为 true 或两个位都为 false，则 b 1 b 2 为 **`false`** 。 逻辑 `OR` [运算符&#124;&#124;](../standard-library/valarray-operators.md#op_lor)适用于元素级别，将所有非零值计算为 **`true`** ，结果为布尔值的 valarray。 相反，按位 OR `operator|` 可产生 0 或 1 以外的值的 valarray，具体取决于按位运算的结果。
