---
description: 了解详细信息： move_iterator 类
...
第二个 (后置递增) 运算符生成的副本 **`*this`** ，其计算结果为 `++*this` 。 然后返回该副本。
...
第二个 (后置减量) 运算符生成的副本 **`*this`** ，其计算结果为 `--*this` 。 然后返回该副本。
...
[ (c + + 移动构造函数和移动赋值运算符) ](../cpp/move-constructors-and-move-assignment-operators-cpp.md)\
