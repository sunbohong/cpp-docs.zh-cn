...
|[owner_before](#owner_before)|**`true`** 如果在 `weak_ptr` (或小于提供的指针) ，则返回。|
...
**`true`** 如果在 `weak_ptr` (或小于提供的指针) ，则返回。
...
*以外*\
由参数共享/弱指针控制的类型。 除非 _其他 \*_ 构造函数与兼容，否则这些构造函数不参与重载决策 `element_type*` 。
