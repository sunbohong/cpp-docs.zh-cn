...
要使用的正则表达式语法是通过使用其中一个枚举值指定的 `std::regex_constants::syntax_option_type` 。 以下正则表达式语法在 std：： regex_constants 中定义：
...
- `icase`：匹配时忽略大小写。
- `nosubs`：忽略标记的匹配项 (即括号) 中的表达式;不存储任何替换。
- `optimize`：更快地进行匹配，但代价是更好的构造时间。
- `collate`：使用区分区域设置的排序规则序列 (例如，"[a-z]" ) 格式范围。
...
- “[`expr`]”形式的 *括号表达式*，可匹配目标序列中已包含在表达式 `expr` 定义集内的字符或排序规则元素；或采用“[^`expr`]”形式，可匹配目标序列中未包含在表达式 `expr` 定义集内的字符或排序规则元素。
  ...
  - 格式为 "" 的 *字符范围* `ch1` - `ch2` 。 将闭值域 [`ch1`, `ch2`] 所表示的字符添加到 `expr` 定义的集合。
...
在 `ECMAScript` 、 `basic` 和中 `grep` ，元素也可以是 "" 形式的 *反向引用* \\ `dd` ，其中 `dd` 表示与目标序列中的字符序列匹配的十进制值 N，与第 n 个 *捕获组* 所匹配的字符序列相同。 例如，“(a)\1”可匹配目标序列“aa”，因为第一个（而且是唯一一个）捕获组可匹配初始序列“a”，然后 \1 可匹配最终序列“a”。
...
- *格式* 为 " \\ \\ "、"\a"、"\b"、"\f"、"\n"、"\r"、"\t" 或 "\v" 的文件格式转义符。 它们分别匹配目标序列中的反斜杠、警报符、退格符、换页符、换行符、回车符、水平制表符和垂直制表符。

- "" 形式的 *八进制转义序列* \\ `ooo` 。 可匹配目标序列中由一个、两个或三个八进制数字 `ooo` 的值表示的字符。
...
在中 `ECMAScript` ，所有形式的重复计数都可以后跟字符 "？"，这会指定 *非贪婪重复*。
...
控件转义序列包含一个反斜杠，后跟字母“c”，然后再跟“a”到“z”（或“A”到“Z”）之间的一个字母。 它可匹配由该字母命名的 ASCII 控制字符。 例如，"\ci" 匹配目标序列 "\x09"，因为 \<ctrl-i> 其值为0x09。
...
否定断言可匹配除其内容外的任意项。 它不占用目标序列中的任何字符。 例如，" (！ aa) # B2 a \*) " 匹配目标序列 "a"，并将捕获组1与子序列 "a" 相关联。 它不匹配目标序列“aa”或目标序列“aaa”。
...
非捕获组用于将其内容标记为正则表达式语法中的一个单元，但不会对目标文本设置标签。 例如，" () # A2？： b) \* (c) " 匹配目标文本 "abbc"，并将捕获组1与子序列 "a" 相关联，并将捕获组2与子序列 "c" 相关联。

### <a name="non-greedy-repetition"></a>非贪婪重复

非贪婪重复将占用与模式匹配的目标序列的最短子序列。 贪婪重复将占用最长子序列。 例如，" (a +) # B2 a \* b) " 匹配目标序列 "aaab"。 当使用非贪婪重复时，它会将捕获组 1 与目标序列开头的子序列“a”相关联，将捕获组 2 与目标序列末尾的子序列“aab”相关联。 当使用贪婪匹配时，它会将捕获组 1 与子序列“aaa”相关联，将捕获组 2 与子序列“b”相关联。
...
- " \* " 在所有情况下均具有特殊含义，除非它是正则表达式中的第一个字符，或在正则表达式中的第一个 "^" 之后的第一个字符，或者为捕获组的第一个字符或在捕获组中第一个 "^" 之后的第一个字符。
...
- " (= aa) # B2 a \*) " 匹配目标序列 "aaaa"，并将捕获组1与子序列 "aaaa" 相关联。

- " (aa) # B2 a \*) " 匹配目标序列 "aaaa"，并将捕获组1与目标序列开头的子序列 "aa" 相关联，并将捕获组2与目标序列末尾的子序列 "aa" 相关联。
...
|"$$"||"$"|
||" \\&"|"&"|
|"$ \` " (美元符号后跟后引号) | |与正则表达式匹配的子序列之前的字符序列 (`[match.prefix().first, match.prefix().second)`) |
|“$'”（美元符号后跟正撇号）||与正则表达式相匹配的子序列之后的字符序列 (`[match.suffix().first, match.suffix().second)`)|
|“$n”|“\n”|在位置与捕获组相匹配的字符序列 `n` ，其中 `n` 是一个介于0到9之间的数字 (`[match[n].first, match[n].second)`) |
||"\\\n"|“\n”|
|“$nn”||在位置与捕获组相匹配的字符序列 `nn` ，其中 `nn` 是一个介于10和 99 (之间的数字 `[match[nn].first, match[nn].second)`) |
