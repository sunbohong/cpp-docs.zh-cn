...
`forward` 不转发其参数。 而是在其参数最初为右值或右值引用时，通过有条件地将参数强制转换为右值引用，`forward` 使得编译器能够在得知转发的参数的原始类型后执行重载决策。 转发函数的自变量的外观类型可能不同于其原始类型，例如，当右值用作函数的参数并绑定到参数名称时;名称使其成为左值，其中实际存在的任何值都是右值，并 `forward` 还原参数的右值。
...
对于没有索引参数的重载，要返回的元素由类型参数推导。 `get<T>(Tuple)`如果 *pr* 包含大于或等于类型 T 的元素，则调用将生成编译器错误。
...
- 否则，返回的类型 `T` 为 `decay<T>::type`。 如果不支持 [衰减类](../standard-library/decay-class.md) ，则返回的类型与 `T` 输入类型相同 `T` 。

返回的类型 `U` 以类似的方式通过输入类型 `U` 来确定。

的一个优点 `make_pair` 是，存储的对象类型由编译器自动确定，无需显式指定。 使用时，请不要使用显式模板参数（如） `make_pair<int, int>(1, 2)` `make_pair` ，因为它是详细的，并添加了复杂的右值引用问题，可能会导致编译失败。 就此示例来说，正确的语法应该是 `make_pair(1, 2)`
...
*与我们联系*\
要强制转换的自变量。 尽管 *Arg* 类型显示为右值引用，但 `move` 也接受左值参数，因为左值引用可以绑定到右值引用。
...
的一个优点 `swap` 是，存储的对象类型由编译器自动确定，无需显式指定。 使用时，请不要使用显式模板参数（如） `swap<int, int>(1, 2)` `swap` ，因为它是详细的，并添加了复杂的右值引用问题，可能会导致编译失败。
...
通过填充范围将值转换为字符串 `[first, last)` ，其中需要为 `[first, last)` 有效范围。
