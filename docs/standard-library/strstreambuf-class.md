---
description: 了解详细信息： strstreambuf 类
...
*计*\
确定 *getptr* 所指向的缓冲区的长度。 如果 *getptr* 不是参数 (第一个构造函数) ，则为缓冲区提供建议的分配大小。
...
的行为与第一个类似，不同之处在于 *getptr* 指定用于保存受控序列的数组对象。  (因此，它不能为 null 指针。 ) 确定数组中元素 *N* 的数目，如下所示：
