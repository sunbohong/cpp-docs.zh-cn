...
第三个函数返回一个 `duration` 对象，该对象表示 *左右* 两个时间间隔。
...
第一个函数返回一个 `duration` 对象，该对象的时间间隔等于 *左右* 间隔之和。 

第二个和第三个函数返回一个 `time_point` 对象，该对象表示从时间点开始被替换的时间点。 
