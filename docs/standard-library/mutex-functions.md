---
description: 了解详细信息： &lt; mutex &gt; 函数和变量
title: '&lt;mutex&gt; 函数和变量'
ms.date: 11/04/2016
f1_keywords:
...
如果 *标记* 无效，则该函数将引发一个错误代码为的 [system_error](../standard-library/system-error-class.md) `invalid_argument` 。 否则，模板函数将使用其 *标志* 参数，以确保它 `F(A...)` 只调用一次，而不考虑调用模板函数的次数。 如果通过引发异常退出 `F(A...)`，则该调用未成功。
