---
description: 了解详细信息： basic_filebuf 类
...
*Tr*\
文件缓冲区的基本元素的特征 (通常 `char_traits<Char_T>`) 。
...
> [!NOTE]
> 类型为的对象 `basic_filebuf` 是使用 __\* char__ 类型的内部缓冲区创建的，而不考虑 `char_type` 由类型参数指定的 *Char_T*。 这意味着，在将包含字符)  (包含字符的 Unicode 字符串 **`wchar_t`** 转换为 ANSI 字符串)  (，并将 **`char`** 其写入内部缓冲区。 若要在缓冲区中存储 Unicode 字符串，请创建类型的新缓冲区， **`wchar_t`** 并使用方法对其进行设置 [`basic_streambuf::pubsetbuf`](../standard-library/basic-streambuf-class.md#pubsetbuf) `()` 。 若要查看演示此行为的示例，请参阅下面的内容。
...
下面的示例假定当前目录中有两个文件： *basic_filebuf_close.txt* (内容为 "测试" ) ， *iotest.txt* (内容为 "ssss" ) 。
...
- `ios_base::in` 成为 `"r"` (打开要读取) 的现有文件。
- [ios_base：： out](../standard-library/ios-base-class.md#fmtflags) 或 `ios_base::out | ios_base::trunc` 变成 `"w"` (截断现有文件或创建以写入) 。
- `ios_base::out | app` 成为 `"a"` (打开现有文件以追加所有写入) 。
- `ios_base::in | ios_base::out` 成为 `"r+"` (打开用于读取和写入) 的现有文件。
- `ios_base::in | ios_base::out | ios_base::trunc` 变为 `"w+"` (截断现有文件或创建以供读取和写入) 。
- `ios_base::in | ios_base::out | ios_base::app``"a+"` (打开现有文件以进行读取，并将所有写入追加) 。
...
- 它可以在输出缓冲区中转换任何挂起的输出，然后在 `ch` 需要时使用文件转换方面 `fac` 调用 `fac.out` 。 Char 类型的每个生成元素 `ch` 都写入由文件指针指定的关联流 ， `fp` 就像连续调用窗体一样 `fputc(ch, fp)` 。 如果任何转换或写入失败，该函数不会成功。
...
对于打开供读取和写入的文件，输入流和输出流串联放置。 若要在插入和提取之间切换，必须调用 [`pubseekoff`](../standard-library/basic-streambuf-class.md#pubseekoff) 或 [`pubseekpos`](../standard-library/basic-streambuf-class.md#pubseekpos) 。 对 `pubseekoff` `seekoff` [文本流](../c-runtime-library/text-and-binary-streams.md)、 [二进制流](../c-runtime-library/text-and-binary-streams.md)和 [宽流](../c-runtime-library/byte-and-wide-streams.md)有各种限制 (和) 的调用。
...
对于打开供读取和写入的文件，输入流和输出流串联放置。 若要在插入和提取之间切换，必须调用 [`pubseekoff`](../standard-library/basic-streambuf-class.md#pubseekoff) 或 [`pubseekpos`](../standard-library/basic-streambuf-class.md#pubseekpos) 。 对 `pubseekoff` `seekoff` 文本流、二进制流和宽流的 (和) 的调用具有不同的限制。
