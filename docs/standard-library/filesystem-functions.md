...
1. 如果 `!pval.has_root_name() && pval.has_root_directory()` 函数返回 pval，则为 `absolute(base).root_name()`  /  。

1. 如果 `!pval.has_root_name() && !pval.has_root_directory()` 函数返回 pval，则为 `absolute(base)`  /  。
...
函数全部形成绝对路径名 `pabs = absolute(pval, base)` (或 `pabs = absolute(pval)` 用于没有基参数) 的重载，然后使用以下步骤序列将其缩小为规范格式：

1. 的每个路径组件都 `X` `is_symlink(X)` **`true`** 将替换为 `read_symlink(X)` 。

1. 每个路径组件 `.` (点是由以前的路径组件建立的当前目录) 被删除。

1. 每对路径组件 `X` / `..` (点-点是由以前的路径组件建立的父目录) 被删除。
...
函数可以从 *到的控件* 中将一个或多个文件复制或 *链接到 ""，* 并将其 *从* `copy_options::none` ""  只包含以下项之一：
...
函数首先确定的 `f` *from* 和 for 的 file_status 值 `t` ： 
...
如果 `!exists(f) || equivalent(f, t) || is_other(f) || is_other(t) || is_directory(f)&& is_regular_file(t)` 为，则它们将报告错误 (并且不执行任何其他) 。
...
所有函数都可能会将文件 *从中**的 "* 从"*复制到 "在"* `copy_options::none`  只 *应包含*、或中的 `skip_existing` 一个 `overwrite_existing` `update_existing` 。
...
否则，如果为 `!exists(to) || opts & copy_options::overwrite_existing || opts & copy_options::update_existing&& last_write_time(to) < last_write_time(from) || !(opts & (copy_options::skip_existing | copy_options::overwrite_existing | copy_options:update_existing))` ，则尝试将文件的内容和属性 *从* 复制到文件。  如果复制尝试失败，则报告错误。
...
如果 `exists(pval) && is_regular_file(pval)` 和文件大小可以确定，则函数返回 pval 所选文件的大小（以字节为单位）。 否则，它们会报告错误并返回 `uintmax_t(-1)` 。
