...
由于 `reset` 首先将存储新的指针指针，然后删除原始存储的指针，因此，如果指针与 `reset` 原始存储指针相同，则可以立即删除它。
...
前两个构造函数用于构造不管理任何资源的对象。 第三个构造函数在中存储 *ptr* `stored_ptr` 。 第四个构造函数在 `stored_ptr` 和 `deleter` 中存储 ptr `stored_deleter` 。
