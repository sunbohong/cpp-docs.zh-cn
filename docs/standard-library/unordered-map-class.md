...
description: C + + 标准库容器类的 API 参考 `unordered_map` ，用于控制不同长度的元素序列。
ms.date: 9/9/2020
f1_keywords:
...
|[包含](#contains)<sup>c + + 20</sup>|检查中是否存在具有指定键的元素 `unordered_map` 。|
...
检查中是否存在 `unordered_map` 具有指定键的元素。
在 c + + 20 中引入。
...
*ValTy*\
指定参数类型的模板参数，unordered_map 可以使用该参数类型构造 [value_type](../standard-library/map-class.md#value_type)的元素，并将 "完美转发" 的 *Val* 作为参数。
