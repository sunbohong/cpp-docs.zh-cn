---
description: 了解详细信息： CContainedWindowT 类
...
> [!NOTE]
> [CContainedWindow](ccontainedwindowt-class.md) 是的专用化 `CContainedWindowT` 。 如果要更改基类或特征，请 `CContainedWindowT` 直接使用。
...
|[CContainedWindowT：:D efWindowProc](#defwindowproc)|提供默认消息处理。|
...
|[CContainedWindowT：： WindowProc](#windowproc)| (静态) 处理发送到包含的窗口的消息。|
...
`CContainedWindowT` 实现一个包含在另一个对象中的窗口。 `CContainedWindowT`的窗口过程在包含对象中使用消息映射将消息定向到适当的处理程序。 构造对象时 `CContainedWindowT` ，可以指定应使用的消息映射。

`CContainedWindowT` 允许通过 superclassing 现有窗口类来创建新窗口。 `Create`方法首先注册一个基于现有类但使用的窗口类 `CContainedWindowT::WindowProc` 。 `Create` 然后，基于此新窗口类创建窗口。 的每个实例 `CContainedWindowT` 都可以是不同的窗口类的超类。
...
> [!NOTE]
> 对于任何给定的 `CContainedWindowT` 对象，请调用 `Create` 或 `SubclassWindow` 。 不应对同一对象调用这两种方法。
...
*dwMsgMapID*<br/>
中标识将处理所包含窗口的消息的消息映射。 默认值为0，指定用 [BEGIN_MSG_MAP](message-map-macros-atl.md#begin_msg_map)声明的默认消息映射。 若要使用 [ALT_MSG_MAP (msgMapID) ](message-map-macros-atl.md#alt_msg_map)声明的备用消息映射，请传递 `msgMapID` 。
...
- 如果要在以后提供参数，则使用不带参数的构造函数。 以后调用时，必须提供窗口类名称、消息映射对象和消息映射 ID `Create` 。
...
*dwExStyle*<br/>
中扩展的窗口样式。 默认值为0，表示没有扩展样式。 有关可能值的列表，请参阅 Windows SDK 中的 [CreateWindowEx](/windows/win32/api/winuser/nf-winuser-createwindowexw) 。
...
使用 [BEGIN_MSG_MAP](message-map-macros-atl.md#begin_msg_map)声明的默认消息映射始终由零标识。 使用 [ALT_MSG_MAP (msgMapID) ](message-map-macros-atl.md#alt_msg_map)声明的备用消息映射由标识 `msgMapID` 。
...
如果包含的窗口为子类，则 `m_pfnSuperWindowProc` 指向窗口类的原始窗口过程。
...
[DefWindowProc](#defwindowproc)方法将消息信息发送到保存在中的窗口过程 `m_pfnSuperWindowProc` 。

## <a name="ccontainedwindowtm_pobject"></a><a name="m_pobject"></a> CContainedWindowT：： m_pObject

指向包含对象的对象 `CContainedWindowT` 。
...
*dwMsgMapID*<br/>
中消息映射标识符。 若要使用 [BEGIN_MSG_MAP](message-map-macros-atl.md#begin_msg_map)声明的默认消息映射，请传递零。 若要使用 [ALT_MSG_MAP (msgMapID) ](message-map-macros-atl.md#alt_msg_map)声明的备用消息映射，请传递 `msgMapID` 。
...
从对象分离子类窗口 `CContainedWindowT` ，并还原原始窗口过程，保存在 [m_pfnSuperWindowProc](#m_pfnsuperwindowproc)中。
...
*bForce*<br/>
中如果设置为 TRUE，则强制还原原始窗口过程，即使此对象的窗口过程 `CContainedWindowT` 当前不处于活动状态也是如此。 如果 *bForce* 设置为 FALSE，并且此对象的窗口过程 `CContainedWindowT` 当前不处于活动状态，则不会还原原始窗口过程。

### <a name="return-value"></a>返回值

之前子类的窗口的句柄。 如果 *bForce* 设置为 FALSE，并且此对象的窗口过程 `CContainedWindowT` 当前不处于活动状态，则返回 NULL。
...
`WindowProc` 将消息定向到 [m_dwMsgMapID](#m_dwmsgmapid)标识的消息映射。 如有必要， `WindowProc` 调用 [DefWindowProc](#defwindowproc) 进行其他消息处理。
