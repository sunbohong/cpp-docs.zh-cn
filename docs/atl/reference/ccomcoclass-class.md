---
description: 了解详细信息： CComCoClass 类
title: CComCoClass 类
ms.date: 11/04/2016
f1_keywords:
- CComCoClass
...
|“属性”|描述|
|----------|-----------------|
|[CComCoClass：： CreateInstance](#createinstance)| (静态) 创建类的实例并查询接口。|
|[CComCoClass：： Error](#error)| (静态) 会向客户端返回丰富的错误信息。|
|[CComCoClass：： GetObjectCLSID](#getobjectclsid)| (Static) 返回对象的类标识符。|
|[CComCoClass：： GetObjectDescription](#getobjectdescription)| (Static) Override 返回对象的描述。|
...
实现必需 COM 对象的 ATL 类 (即，用作 [CComCoClass](../../atl/reference/ccomcoclass-class.md)) 的第一个模板参数的类必须与调用代码位于同一项目中。 COM 对象的创建由为此 ATL 类注册的类工厂来执行。
...
在下面的示例中， `CDocument` 是一个由向导生成的、派生自的 ATL 类 `CComCoClass` ，该类实现 `IDocument` 接口。 类在对象映射中使用 OBJECT_ENTRY_NON_CREATEABLE_EX_AUTO 宏注册，因此客户端无法使用 [CoCreateInstance](/windows/win32/api/combaseapi/nf-combaseapi-cocreateinstance)创建文档的实例。 `CApplication` 是一个组件类，它提供了一个方法，用于创建该文档类的实例的一个 COM 接口。 下面的代码演示了如何轻松地使用从基类继承的成员创建 document 类的实例 `CreateInstance` `CComCoClass` 。
...
*lpszDesc*<br/>
中描述错误的字符串。 的 Unicode 版本 `Error` 指定 *lpszDesc* 的类型为 LPCOLESTR; ANSI 版本指定 LPCSTR 的类型。

*iid*<br/>
中定义错误的接口的 IID 或 GUID_NULL (默认值) 如果错误是由操作系统定义的，则为。
...
如果 *hRes* 参数为非零值，则 `Error` 返回 *hRes* 的值。 如果 *hRes* 为零，则返回的前四个版本 `Error` DISP_E_EXCEPTION。 最后两个版本返回宏的结果 **MAKE_HRESULT ( 1，FACILITY_ITF，** *nID* **)**。
...
`GetObjectDescription` 由调用 `IComponentRegistrar::GetComponents` 。 `IComponentRegistrar` 是一个自动化接口，可用于注册和取消注册 DLL 中的单个组件。 使用 ATL 项目向导创建组件注册器对象时，向导将自动实现 `IComponentRegistrar` 接口。 `IComponentRegistrar` 通常由 Microsoft 事务服务器使用。
