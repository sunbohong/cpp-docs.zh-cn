---
description: 了解详细信息： CComCurrency 类
...
`CComCurrency` 对象以两个组件的形式提供对小数点两侧的数字的访问权限：一个整数组件，它存储小数点左侧的值；一个小数组件，它存储小数点右侧的值。 小数部分作为介于 -9999 (CY_MIN_FRACTION) 和 +9999 (CY_MAX_FRACTION) 之间的整数值存储在内部。 方法 [CComCurrency：： GetFraction](#getfraction) 返回一个按系数 10000 (CY_SCALE) 缩放的值。
...
如果发生错误，如溢出，缺少空异常规范的构造函数 (**引发 ( # B2**) 调用 `AtlThrow` ，并使用 HRESULT 描述错误。
...
小数部分是介于-9999 (CY_MIN_FRACTION) 和 + 9999 (CY_MAX_FRACTION) 之间的4位整数值。 `GetFraction` 返回按 10000 (CY_SCALE) 缩放的该值。 CY_MIN_FRACTION、CY_MAX_FRACTION 和 CY_SCALE 的值是在 atlcur.h 中定义的。
...
如果对象相等，则返回 TRUE (也就是说， `m_currency` 两个对象中的数据成员（整数和小数）都具有相同的值) ，否则为 FALSE。
...
*nFraction*<br/>
要分配给数据成员的小数部分的值 `m_currency` 。 小数部分的符号必须与整数组件相同，并且值必须在 9999 (范围内 CY_MIN_FRACTION) 到 + 9999 (CY_MAX_FRACTION) 。
