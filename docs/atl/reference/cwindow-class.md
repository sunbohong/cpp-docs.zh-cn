...
|[CWindow：： GetDlgCtrlID](#getdlgctrlid)|检索仅) 子窗口的窗口标识符 (。|
...
将位图句柄参数 (HBITMAP) 1 传递到 Win32 函数。
...
将位图句柄参数 (HBITMAP) 0 传递到 Win32 函数。
...
> [!NOTE]
> `CWindow::CWindow` 不创建窗口。 类 [CWindowImpl](../../atl/reference/cwindowimpl-class.md)、 [CContainedWindow](../../atl/reference/ccontainedwindowt-class.md)和 [CDialogImpl](../../atl/reference/cdialogimpl-class.md) (派生自 `CWindow`) 提供一种方法来创建窗口或对话框，然后将其分配给 `CWindow::m_hWnd` 。 你还可以使用 [CreateWindow](/windows/win32/api/winuser/nf-winuser-createwindoww) Win32 函数。
...
*bBigIcon*<br/>
中如果为 TRUE (则) 方法返回大图标。 否则，它将返回小图标。
...
要添加或删除的样式可以使用按位 "或" ( &#124; ) "运算符组合在一起。 有关可用窗口样式的信息，请参阅 Windows SDKfor 中的 [CreateWindow](/windows/win32/api/winuser/nf-winuser-createwindoww) 函数。
...
要添加或删除的样式可以使用按位 "或" ( &#124; ) "运算符组合在一起。 有关可用扩展样式的信息，请参阅 Windows SDKfor 中的 [CreateWindowEx](/windows/win32/api/winuser/nf-winuser-createwindowexw) 函数。
