---
description: 了解详细信息： CBindStatusCallback 类
...
|“属性”|描述|
|----------|-----------------|
|[CBindStatusCallback：:D o) ](#download)|用于启动下载过程、创建 `CBindStatusCallback` 对象和调用的静态方法 `StartAsyncDownload` 。|
...
|[CBindStatusCallback::StartAsyncDownload](#startasyncdownload)|初始化可用字节数和读取到零的字节数，从 URL 创建推送类型流对象，并在 `OnDataAvailable` 每次数据可用时调用。|
...
|[CBindStatusCallback：： m_spBinding](#m_spbinding)|指向 `IBinding` 当前绑定操作的接口的指针。|
...
通常， `CBindStatusCallback` 对象与特定的绑定操作相关联。 例如，在 [异步](../../overview/visual-cpp-samples.md) 示例中，在设置 URL 属性时，它将 `CBindStatusCallback` 在对的调用中创建对象 `Download` ：

[!code-cpp[NVC_ATL_Windowing#86](../../atl/codesnippet/cpp/cbindstatuscallback-class_1.h)]

当应用程序包含数据时，异步名字对象使用回调函数 `OnData` 调用它。 异步名字对象由系统提供。
...
构造函数还会将 [m_pT](#m_pt) 和 [M_PFUNC](#m_pfunc) 初始化为 NULL。
...
创建一个 `CBindStatusCallback` 对象，并调用 `StartAsyncDownload` 以从指定的 URL 异步下载数据。
...
*pFunc*<br/>
中指向函数的指针，该函数接收所读取的数据。 函数是对象的类型的类的成员 `T` 。 有关语法和示例，请参阅 [StartAsyncDownload](#startasyncdownload) 。
...
每次有数据可用时，都会通过发送到对象 `OnDataAvailable` 。 `OnDataAvailable` 读取数据并调用 *pFunc* 所指向的函数 (例如，存储数据或将数据打印到屏幕) 。
...
`OnDataAvailable`当数据尚不可用时，BINDF_ASYNCSTORAGE 返回 E_PENDING，而不是在数据可用之前进行阻止。
...
*pbindinfo*<br/>
[in，out]指向结构的指针，该 `BINDINFO` 结构提供有关对象如何进行绑定的详细信息。
...
每次由 `OnDataAvailable` 实际读取的字节数调用时递增。 在中初始化为零 `StartAsyncDownload` 。

## <a name="cbindstatuscallbackm_pfunc"></a><a name="m_pfunc"></a> CBindStatusCallback：： m_pFunc

指向的函数在 `m_pFunc` `OnDataAvailable` 读取可用数据后由调用 (例如，用于存储数据或将数据打印到屏幕) 。
...
指向的函数 `m_pFunc` 是对象类的成员，并且具有以下语法：
...
指向 [IBindCtx](/windows/win32/api/objidl/nn-objidl-ibindctx) 接口的指针，该接口提供对 (存储特定名字对象绑定操作相关信息) 对象的绑定上下文的访问。
...
已在中初始化 `OnStartBinding` 并在中发布 `OnStopBinding` 。
...
`OnDataAvailable` `STGMEDIUM` 当 BCSF 标志 BCSF_FIRSTDATANOTIFICATION 并在 BCSF_LASTDATANOTIFICATION BCSF 标志时发布时，从结构中进行初始化。
...
*dwSize*<br/>
中自绑定开始以来可用数据) 的累计量 (（以字节为单位）。 可以为零，表示数据量不相关或没有特定的可用量。

*pformatetc*<br/>
中指向 [FORMATETC](/windows/win32/com/the-formatetc-structure) 结构的指针，该结构包含可用数据的格式。 如果没有格式，则可以 CF_NULL。
...
释放 `IBinding` 数据成员 [m_spBinding](#m_spbinding)的指针。
...
*pFunc*<br/>
中指向函数的指针，该函数接收所读取的数据。 函数是对象的类型的类的成员 `T` 。 有关语法和示例，请参阅 **备注** 。
...
在以下示例中 (从 [异步](../../overview/visual-cpp-samples.md) 示例) 中获取，该函数将 `OnData` 接收的数据写入文本框。
