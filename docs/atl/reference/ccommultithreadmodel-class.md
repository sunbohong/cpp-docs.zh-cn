---
description: 了解详细信息： CComMultiThreadModel 类
...
|“属性”|描述|
|----------|-----------------|
|[CComMultiThreadModel：:D ecrement](#decrement)| (静态) 以线程安全的方式递减指定变量的值。|
|[CComMultiThreadModel：：递增](#increment)| (静态) 以线程安全的方式递增指定变量的值。|

## <a name="remarks"></a>备注

通常，可以使用 `CComMultiThreadModel` 以下两个 **`typedef`** 名称之一： [CComObjectThreadModel] (的 atl-typedef # CComObjectThreadModel 或 [CComGlobalsThreadModel] (的 atl-typedef # CComGlobalsThreadModel。 每个引用的类都 **`typedef`** 依赖于所使用的线程模型，如下表所示：
