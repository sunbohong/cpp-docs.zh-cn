...
- ATL_URL_ESCAPE 将 (% XX) 的所有转义序列转换为相应的字符。

- ATL_URL_NO_ENCODE 不会将不安全字符转换为转义序列。

- ATL_URL_NO_META 不会删除元序列 (如 "." 和 "..."从 URL ) 。
...
*pbOut*<br/>
调用方分配的缓冲区，用于接收 HTML 颜色值的文本。 缓冲区的空间必须至少包含8个字符，包括 null 结束符) 的空格。

*nBuffer*<br/>
缓冲区的大小（以字节为单位） (包含 null 终止符) 的空间。
