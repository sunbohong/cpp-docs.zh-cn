---
description: 了解详细信息： _ATL_FUNC_INFO 结构
...
`cc`<br/>
调用约定。 将此结构与 [IDispEventSimpleImpl](../../atl/reference/idispeventsimpleimpl-class.md) 类结合使用时，必须 CC_STDCALL 此成员。 `CC_CDECL` 为结构的字段 Windows CE 中唯一支持的选项 `CALLCONV` `_ATL_FUNC_INFO` 。 不支持任何其他值，因此其行为未定义。
