...
- **数据源**
  ...
  - **连接** 选项卡
      ...
      此选项卡的内容具体视你选择哪个提供程序而定。 虽然有许多类型的提供程序，但本部分介绍了两种最常见的连接： SQL 和 ODBC 数据。 其他类型数据就是在下面所述字段的基础上进行类似变体。

      对于 SQL 数据：

      1. **选择或输入服务器名称：** 单击下拉列表菜单，显示网络中所有已注册的数据服务器，并选择一个服务器。

      1. **输入用于登录到服务器的信息：** 输入用户名和密码以登录到数据服务器。
      ...
      1. **选择服务器上的数据库：** 单击下拉列表菜单以显示数据服务器上的所有已注册数据库，并选择一个数据库。
      ...
      1. **指定数据源：** 您可以使用数据源名称或连接字符串。

         **使用数据源名称：** 此下拉列表显示在计算机中注册的数据源。 可以使用 ODBC 数据源管理器提前创建数据源

         \- 或 -

         **使用连接字符串：** 请输入已获得的连接字符串，或单击 " **生成** " 按钮;此时将显示 " **选择数据源** " 对话框。 选择文件或计算机数据源，并单击“确定”****。
  ...
  - **高级** 选项卡
      ...
      **网络设置：** 指定 **模拟级别** (在模拟客户端时允许服务器使用的模拟级别;直接对应于 RPC 模拟级别) 和 **保护级别** (在客户端和服务器之间发送的数据的保护级别;直接对应于) 的 RPC 保护级别。

      **其他：** 在 " **连接超时**" 中，指定在发生超时之前允许的空闲时间（秒）。 在“访问权限”**** 中，指定数据连接的访问权限。
...
- **特性化**
  ...
  - 如果你选择“命令”**** 作为使用者“类型”****，向导使用 `db_source` 和 `db_command` 特性，并使用 `db_column` 来创建列映射。 例如，它创建以下映射：
    ...
    ```
     ...
     有关详细信息，请参阅 [属性的基本机制](../../windows/attributes/cpp-attributes-com-net.md#basic-mechanics-of-attributes) 。
