---
description: 了解详细信息： IDispEventImpl 类
...
*plibid*<br/>
一个指针，指向用于定义 *pdiid* 指向的调度接口的类型库。 如果 **&GUID_NULL**，则将从获得事件的对象中加载类型库。
...
*tihclass*<br/>
用于管理 *T* 的类型信息的类。默认值为类型的类 `CComTypeInfoHolder` ; 但是，可以通过提供除之外的类型的类来重写此模板参数 `CComTypeInfoHolder` 。
...
|[IDispEventImpl：： Idispatch.getidsofnames](#getidsofnames)|将一个成员和一组可选的参数名称映射到一组相应的整数 Dispid。|
...
`IDispEventImpl` 提供实现事件调度接口的方法，无需为该接口上的每个方法/事件提供实现代码。 `IDispEventImpl` 提供方法的实现 `IDispatch` 。 你只需为你感兴趣处理的事件提供实现。

`IDispEventImpl` 与类中的事件接收器映射结合使用，以将事件路由到适当的处理程序函数。 使用此类：

为要处理的每个对象上的每个事件，将 [SINK_ENTRY](composite-control-macros.md#sink_entry) 或 [SINK_ENTRY_EX](composite-control-macros.md#sink_entry_ex) 宏添加到事件接收器映射。 使用 `IDispEventImpl` 作为复合控件的基类时，可以调用 [AtlAdviseSinkMap](connection-point-global-functions.md#atladvisesinkmap) 来建立和中断与事件接收器映射中所有项的事件源的连接。 在其他情况下，或者为了更好地控制，请调用 [DispEventAdvise](idispeventsimpleimpl-class.md#dispeventadvise) 以建立源对象与基类之间的连接。 调用 [DispEventUnadvise](idispeventsimpleimpl-class.md#dispeventunadvise) 断开连接。

必须为 `IDispEventImpl` 需要处理事件的每个对象使用值为 *nID*) 的唯一值才能派生 (。 您可以通过 unadvising 对一个源对象重复使用基类，然后针对不同的源对象进行通知，但是单个对象可以同时处理的最大源对象数受基类的数目限制 `IDispEventImpl` 。

`IDispEventImpl` 提供与 [IDispEventSimpleImpl](../../atl/reference/idispeventsimpleimpl-class.md)相同的功能，不同之处在于从类型库获取有关接口的类型信息，而不是将其作为指向 [_ATL_FUNC_INFO](../../atl/reference/atl-func-info-structure.md) 结构的指针提供。 `IDispEventSimpleImpl`如果你没有用于描述事件接口的类型库，或者想要避免使用类型库关联的开销，请使用。

> [!NOTE]
> `IDispEventImpl` 和 `IDispEventSimpleImpl` 提供自己的实现 `IUnknown::QueryInterface` 来使每个 `IDispEventImpl` 和 `IDispEventSimpleImpl` 基类充当单独的 com 标识，同时仍然允许直接访问主 com 对象中的类成员。
