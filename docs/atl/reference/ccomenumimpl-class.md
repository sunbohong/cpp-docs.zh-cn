---
description: 了解详细信息： CComEnumImpl 类
...
但是，如果确实需要提供自定义枚举器 (例如，其中一个公开接口以及枚举器接口) ，则可以从此类派生。 在这种情况下，可能需要重写 [CComEnumImpl：： Clone](#clone) 方法才能提供自己的实现。
...
如果将指向数组中的项的指针传递到另一个对象 (并且不要求枚举器将数据复制) ，则可以使用 PUnk 参数来确保只要枚举器需要，就可以使用参数来确保该对象及其包含的数组可用。 枚举器仅保存对对象的 COM 引用，以使其保持活动状态。 当枚举器被销毁时，将自动释放 COM 引用。
...
此方法通过创建类型为的对象 `CComEnum` ，并使用当前对象所使用的相同数组和迭代器对其进行初始化，并返回新创建的对象上的接口，来提供 Clone 方法的实现。
...
请注意，克隆的枚举器从不 (生成其自己的副本，也不会) 原始枚举器使用的数据的所有权。 必要时，克隆的枚举器将使用 COM 引用) 使原始枚举器保持活动 (，以确保数据在需要时可供使用。
