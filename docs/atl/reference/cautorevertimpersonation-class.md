---
description: 了解详细信息： CAutoRevertImpersonation 类
...
|[CAutoRevertImpersonation：:D etach](#detach)|取消自动模拟重新确定。|
...
仅当使用 NULL 指针创建了 [CAutoRevertImpersonation](../../atl/reference/cautorevertimpersonation-class.md) 对象 `CAccessToken` ，或以前调用了 [分离](#detach) 时，才应使用此方法。 对于简单的情况，无需使用此方法。
...
访问令牌的实际模拟应该与创建对象之前，分别从和中进行 `CAutoRevertImpersonation` 。 当对象超出范围时，此模拟将自动还原 `CAutoRevertImpersonation` 。
...
调用 **分离** 可防止 `CAutoRevertImpersonation` 对象恢复当前对与此对象关联的 [CAccessToken](../../atl/reference/caccesstoken-class.md) 对象有效的任何模拟。 `CAutoRevertImpersonation`然后，可以使用 Attach 对相同或另一个对象进行不影响或重新关联的销毁 `CAccessToken` 。 [](#attach)
...
如果调用此方法的目的包括对象模拟的重新确定 `CAccessToken` ，则应改用 [分离](#detach) 方法。
