...
*piid*<br/>
指向的 IID 的指针 `InterfaceName` 。

*plibid*<br/>
一个指针，指向包含的定义的类型库的 LIBID `InterfaceName` 。
...
|[get_BorderWidth](#get_borderwidth)|调用此方法可获取控件边框) 宽度 (（以像素为单位）。|
...
|[get_DrawWidth](#get_drawwidth)|调用此方法可获取控件的绘图方法所使用) 的绘图宽度 (（以像素为单位）。|
...
|[get_MouseIcon](#get_mouseicon)|调用此方法可获取图形 (图标、位图或图元文件) 在鼠标位于控件上时要显示的图像属性。|
|[get_MousePointer](#get_mousepointer)|调用此方法可获取鼠标位于控件上时显示的鼠标指针的类型，例如，箭头、交叉或沙漏。|
|[get_Picture](#get_picture)|调用此方法可获取一个指针，该指针指向要显示的图形 (图标、位图或图元文件) 。|
...
|[put_DrawWidth](#put_drawwidth)|调用此方法以设置控件的绘图方法所使用) 宽度 (（以像素为单位）。|
...
|[put_MouseIcon](#put_mouseicon)|调用此方法可设置图形 (图标、位图或图元文件) 在鼠标位于控件上时要显示的图像属性。|
|[put_MousePointer](#put_mousepointer)|调用此方法可设置鼠标位于控件上时显示的鼠标指针的类型，例如，箭头、交叉或沙漏。|
|[put_Picture](#put_picture)|调用此方法以设置要显示的图形 (图标、位图或图元文件) 的图像属性。|
...
为了向后兼容， `CStockPropImpl` 还 `get_Window` 公开 `put_Window` 了和方法， `get_HWND` 分别调用和 `put_HWND` 。 的默认实现将 `put_HWND` 返回 E_FAIL 因为 HWND 应为只读属性。
...
调用此方法可获取控件的绘图方法所使用) 的绘图宽度 (（以像素为单位）。
...
调用此方法可获取图形 (图标、位图或图元文件) 在鼠标位于控件上时要显示的图像属性。
...
调用此方法可获取一个指针，该指针指向要显示的图形 (图标、位图或图元文件) 。
...
调用此方法可设置图形 (图标、位图或图元文件) 在鼠标位于控件上时要显示的图像属性。
...
调用此方法以设置要显示的图形 (图标、位图或图元文件) 的图像属性。
...
调用此方法可设置图形 (图标、位图或图元文件) 在鼠标位于控件上时要显示的图像的属性，具有引用计数。
