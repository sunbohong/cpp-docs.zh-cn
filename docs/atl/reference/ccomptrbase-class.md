---
description: 了解详细信息： CComPtrBase 类
title: CComPtrBase 类
ms.date: 11/04/2016
f1_keywords:
- CComPtrBase
...
|[CComPtrBase::IsEqualObject](#isequalobject)|调用此方法可检查指定的是否 `IUnknown` 指向与对象关联的对象 `CComPtrBase` 。|
|[CComPtrBase：： QueryInterface](#queryinterface)|调用此方法以返回指向指定接口的指针。|
|[CComPtrBase：： Release](#release)|调用此方法可释放接口。|
|[CComPtrBase：： SetSite](#setsite)|调用此方法可将对象的站点设置 `CComPtrBase` 为 `IUnknown` 父对象的。|
...
此类为使用 COM 内存管理例程的其他智能指针（例如 [CComQIPtr](../../atl/reference/ccomqiptr-class.md) 和 [CComPtr](../../atl/reference/ccomptr-class.md)）提供基础。 派生类添加自己的构造函数和运算符，但依赖于提供的方法 `CComPtrBase` 。
...
`Attach`在现有的 [CComPtrBase：:p](#p)成员变量上调用 [CComPtrBase：： Release](#release) ，然后将 *p2* 分配到 `CComPtrBase::p` 。 当 `CComPtrBase` 对象取得指针的所有权时，它将 `Release` 在指针上自动调用，如果对象的引用计数为0，则将删除该指针和任何分配的数据。
...
*pUnkOuter*<br/>
如果为 NULL，则指示对象未作为聚合的一部分创建。 如果非 NULL，则为指向聚合对象的接口的指针， `IUnknown` (控制 `IUnknown`) 。
...
调用此方法可检查指定的是否 `IUnknown` 指向与对象关联的对象 `CComPtrBase` 。
...
返回 CComPtrBase 的值 [：:p](#p);也就是说，指向对象引用的对象的指针 `CComPtrBase` 。
...
使用此运算符可调用对象所指向的类中的方法 `CComPtrBase` 。 在调试版本中，如果 `CComPtrBase` 数据成员指向 NULL，将发生断言失败。
...
调用此方法可将对象的站点设置 `CComPtrBase` 为 `IUnknown` 父对象的。
