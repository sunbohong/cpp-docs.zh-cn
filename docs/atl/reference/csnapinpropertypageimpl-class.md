---
description: 了解详细信息： CSnapInPropertyPageImpl 类
...
|[CSnapInPropertyPageImpl：： Create](#create)|初始化新创建的 `CSnapInPropertyPageImpl` 对象。|
...
|名称|描述|
|----------|-----------------|
|[CSnapInPropertyPageImpl：： m_psp](#m_psp)|`PROPSHEETPAGE`对象使用的 Windows 结构 `CSnapInPropertyPageImpl` 。|
...
`PROPSHEETPAGE`包含新创建的属性表的属性的结构的句柄。
...
属性表跟踪哪些页是 "脏的"，即您已调用的属性页 `SetModified( TRUE )` 。 如果调用某个页面，将始终启用 " **立即应用** " 按钮 `SetModified( TRUE )` 。 当你为某个页面调用时，" **立即应用** " 按钮将被禁用 `SetModified( FALSE )` ，但前提是所有其他页面均未 "更新"。
