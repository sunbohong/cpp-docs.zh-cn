...
`CAdapt` 是用于包装类（将 address-of 运算符 (`operator &`) 重新定义为返回对象地址以外的内容）的简单模板。 这样的类的示例包括 ATL 的 `CComBSTR`、`CComPtr` 和 `CComQIPtr` 类，以及编译器 COM 支持类 `_com_ptr_t`。 这两个类都将重定义该地址的运算符，以返回其一个数据成员的地址 (BSTR 的情况 `CComBSTR` ，并在其他类) 的情况下返回一个接口指针。

`CAdapt`的主要作用是隐藏类 *T* 定义的地址运算符，但仍保留改编类的特征。 `CAdapt`fulfils 此角色的方法是：保存一个 [](#m_t)公共成员，m_T *T*，并定义转换运算符、比较运算符和复制构造函数，以允许将的专用化 `CAdapt` 视为类型为 *T* 的对象。
