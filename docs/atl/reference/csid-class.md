...
[](#loadaccount) `CSid` 在调用 `SidNameUse` 以返回其状态之前，调用 CSid：： LoadAccount 以更新对象。 `SidNameUse` 不 (通过调用或) 来更改对象的状态 `LookupAccountName` `LookupAccountSid` ，但只返回当前状态。
