...
|“属性”|描述|
|----------|-----------------|
|[CRegKey：： Attach](#attach)|调用此方法， `CRegKey` 通过将 [m_hKey](#m_hkey) 成员句柄设置为来将 HKEY 附加到对象 `hKey` 。|
|[CRegKey：： Close](#close)|调用此方法可释放 [m_hKey](#m_hkey) 成员句柄并将其设置为 NULL。|
|[CRegKey：： Create](#create)|调用此方法以创建指定的密钥（如果它不作为的子项存在） `hKeyParent` 。|
|[CRegKey：:D eleteSubKey](#deletesubkey)|调用此方法以从注册表中移除指定的键。|
|[CRegKey：:D eleteValue](#deletevalue)|调用此方法可从 [m_hKey](#m_hkey)中移除值字段。|
|[CRegKey：:D etach](#detach)|调用此方法可从对象分离 [m_hKey](#m_hkey) 成员句柄 `CRegKey` 并将 `m_hKey` 其设置为 NULL。|
...
|[CRegKey：： Open](#open)|调用此方法以打开指定的密钥并将 [m_hKey](#m_hkey) 设置为此密钥的句柄。|
...
|[CRegKey::QueryValue](#queryvalue)|调用此方法可检索 [m_hKey](#m_hkey)的指定值字段的数据。 不再支持此方法的早期版本，并将其标记为 ATL_DEPRECATED。|
...
|[CRegKey：： SetValue](#setvalue)|调用此方法可将数据存储在 [m_hKey](#m_hkey)的指定值字段中。 不再支持此方法的早期版本，并将其标记为 ATL_DEPRECATED。|

### <a name="public-operators"></a>公共运算符

|名称|描述|
|----------|-----------------|
|[CRegKey：： operator HKEY](#operator_hkey)|将 `CRegKey` 对象转换为 HKEY。|
...
|名称|描述|
|----------|-----------------|
|[CRegKey：： m_hKey](#m_hkey)|包含与对象关联的注册表项的句柄 `CRegKey` 。|
|[CRegKey：： m_pTM](#m_ptm)|指向对象的指针 `CAtlTransactionManager`|

## <a name="remarks"></a>备注

`CRegKey` 提供用于创建和删除系统注册表中的键和值的方法。 注册表包含系统组件的特定于安装的一组定义，如软件版本号、已安装硬件的逻辑到物理映射以及 COM 对象。

`CRegKey` 为给定计算机的系统注册表提供编程接口。 例如，若要打开特定的注册表项，请调用 `CRegKey::Open` 。 若要检索或修改数据值，请 `CRegKey::QueryValue` 分别调用或 `CRegKey::SetValue` 。 若要关闭某个键，请调用 `CRegKey::Close` 。

关闭某个键时，会将其注册表数据写入 (刷新) 硬盘。 该过程需要持续数秒。 如果你的应用程序必须将注册表数据显式写入硬盘，则可以调用 [RegFlushKey](/windows/win32/api/winreg/nf-winreg-regflushkey) Win32 函数。 但是， `RegFlushKey` 使用多个系统资源，只应在绝对必要时进行调用。
...
调用此方法， `CRegKey` 通过将 [m_hKey](#m_hkey) 成员句柄设置为 *HKEY*，将 HKEY 附加到对象。
...
如果成功，将返回 ERROR_SUCCESS;否则，将返回错误值。
...
*samDesired*<br/>
密钥的安全访问权限。 默认值为 KEY_READ &#124; KEY_WRITE。 有关可能的值和说明的列表，请参阅 `RegCreateKeyEx` 。

*lpSecAttr*<br/>
指向 [SECURITY_ATTRIBUTES](/previous-versions/windows/desktop/legacy/aa379560\(v=vs.85\)) 结构的指针，该结构指示键的句柄是否可以由子进程继承。 默认情况下，此参数为 NULL (表示不能) 继承句柄。

*lpdwDisposition*<br/>
弄如果为非 NULL，则检索 REG_CREATED_NEW_KEY (如果该注册表项不存在并且是) 创建的，则为; 如果该注册表项存在并已打开，则为 REG_OPENED_EXISTING_KEY () 。

### <a name="return-value"></a>返回值

如果成功，将返回 ERROR_SUCCESS 并打开该密钥。 如果该方法失败，则返回值为 WINERROR.H 中定义的非零错误代码。
...
如果成功，将返回 ERROR_SUCCESS。 如果该方法失败，则返回值为 WINERROR.H 中定义的非零错误代码。
...
调用此方法可从对象分离 [m_hKey](#m_hkey) 成员句柄 `CRegKey` 并将 `m_hKey` 其设置为 NULL。
...
若要枚举子项，请调用 `CRegKey::EnumKey` 索引为零的。 递增索引值并重复此操作，直到该方法返回 ERROR_NO_MORE_ITEMS。 有关详细信息，请参阅 Windows SDK 中的 [RegEnumKeyEx](/windows/win32/api/winreg/nf-winreg-regenumkeyexw) 。
...
*si*<br/>
[SECURITY_INFORMATION](/windows/win32/SecAuthZ/security-information)值，该值指示请求的安全信息。
...
如果该方法成功，则返回值为 ERROR_SUCCESS。 如果该方法失败，则返回值是在 WINERROR.H 中定义的非零错误代码。
...
如果成功，将返回 ERROR_SUCCESS;否则，在 WINERROR.H 中定义的非零错误值。

### <a name="remarks"></a>备注

如果 *lpszKeyName* 参数为 NULL 或指向一个空字符串，则 `Open` 会打开由 *hKeyParent* 标识的密钥的新句柄，但不会关闭任何以前打开的句柄。
...
此运算符从当前的对象中分离 *键* ，并改为将其分配给 `CRegKey` 对象。
...
如果该方法成功，则返回 ERROR_SUCCESS。 如果该方法无法读取值，则它将返回 WINERROR.H 中定义的非零错误代码。 如果引用的数据不属于 REG_BINARY 类型，则返回 ERROR_INVALID_DATA。
...
如果该方法成功，则返回 ERROR_SUCCESS。 如果该方法无法读取值，则它将返回 WINERROR.H 中定义的非零错误代码。 如果引用的数据不属于 REG_DWORD 类型，则返回 ERROR_INVALID_DATA。
...
如果该方法成功，则返回 ERROR_SUCCESS。 如果该方法无法读取值，则它将返回 WINERROR.H 中定义的非零错误代码。 如果引用的数据不是有效的 GUID，则返回 ERROR_INVALID_DATA。
...
如果该方法成功，则返回 ERROR_SUCCESS。 如果该方法无法读取值，则它将返回 WINERROR.H 中定义的非零错误代码。 如果引用的数据不属于 REG_MULTI_SZ 类型，则返回 ERROR_INVALID_DATA。
...
如果该方法成功，则返回 ERROR_SUCCESS。 如果该方法无法读取值，则它将返回 WINERROR.H 中定义的非零错误代码。 如果引用的数据不属于 REG_QWORD 类型，则返回 ERROR_INVALID_DATA。
...
如果该方法成功，则返回 ERROR_SUCCESS。 如果该方法无法读取值，则它将返回 WINERROR.H 中定义的非零错误代码。 如果引用的数据不属于 REG_SZ 类型，则返回 ERROR_INVALID_DATA。 如果该方法返回 ERROR_MORE_DATA，则 *pnChars* 等于零，而不是所需的缓冲区大小（以字节为单位）。
...
调用此方法可检索 [m_hKey](#m_hkey)的指定值字段的数据。 不再支持此方法的早期版本，并将其标记为 ATL_DEPRECATED。
...
如果成功，将返回 ERROR_SUCCESS;否则为 WINERROR.H 中定义的非零错误代码。

### <a name="remarks"></a>备注

的两个原始版本 `QueryValue` 已不再受支持，并被标记为 ATL_DEPRECATED。 如果使用这些窗体，编译器将发出警告。
...
|值|含义|
|-----------|-------------|
|DACL_SECURITY_INFORMATION| (DACL) 设置密钥的自由访问控制列表。 密钥必须具有 WRITE_DAC 访问权限，或者调用进程必须是对象的所有者。|
...
|SACL_SECURITY_INFORMATION| (SACL) 设置密钥的系统访问控制列表。 密钥必须具有 ACCESS_SYSTEM_SECURITY 访问权限。 获取此访问权限的正确方法是在调用方的当前访问令牌中启用 SE_SECURITY_NAME [权限](/windows/win32/secauthz/privileges) ，打开 ACCESS_SYSTEM_SECURITY 访问的句柄，然后禁用该权限。|
...
*dwType*<br/>
要写入到注册表中的字符串的类型： REG_SZ (默认) 或 REG_EXPAND_SZ () 。
...
调用此方法可将数据存储在 [m_hKey](#m_hkey)的指定值字段中。 不再支持此方法的早期版本，并将其标记为 ATL_DEPRECATED。
...
*nBytes*<br/>
指定由 *pValue* 参数指向的信息的大小（以字节为单位）。 如果数据的类型 REG_SZ、REG_EXPAND_SZ 或 REG_MULTI_SZ，则 *nBytes* 必须包含终止 null 字符的大小。
...
*bMulti*<br/>
如果为 false，则指示字符串的类型为 REG_SZ。 如果为 true，则指示字符串是 REG_MULTI_SZ 类型的多字符串。
