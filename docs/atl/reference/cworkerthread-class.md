---
description: 了解详细信息： CWorkerThread 类
...
若要使两个或多个 `CWorkerThread` 对象使用同一个工作线程，请在不传递任何参数的情况下初始化其中一个对象，然后将指向该对象的指针传递给其他对象的 `Initialize` 方法。 使用指针初始化的对象应在用于初始化它们的对象之前关闭。
...
请注意，对 `Shutdown` 使用指向另一对象的指针进行初始化的对象的调用 `CWorkerThread` 不起作用，并且始终返回 S_OK。
