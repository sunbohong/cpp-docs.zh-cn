...
|[CComBSTR：： AppendBytes](#appendbytes)|向添加指定的字节数 `m_str` 。|
|[CComBSTR：： ArrayToBSTR](#arraytobstr)|从 safearray 中每个元素的第一个字符创建一个 BSTR，并将其附加到 `CComBSTR` 对象。|
|[CComBSTR：： AssignBSTR](#assignbstr)|向分配一个 BSTR `m_str` 。|
|[CComBSTR：： Attach](#attach)|将 BSTR 附加到 `CComBSTR` 对象。|
|[CComBSTR：： BSTRToArray](#bstrtoarray)|创建从零开始的一维 safearray，其中数组的每个元素都是对象中的一个字符 `CComBSTR` 。|
|[CComBSTR：： ByteLength](#bytelength)|返回的长度（ `m_str` 以字节为单位）。|
|[CComBSTR：： Copy](#copy)|返回的副本 `m_str` 。|
|[CComBSTR：： CopyTo](#copyto)|`m_str`通过 **[out]** 参数返回的副本|
|[CComBSTR：:D etach](#detach)|`m_str`与对象分离 `CComBSTR` 。|
...
|名称|描述|
|----------|-----------------|
|[CComBSTR：： operator BSTR](#operator_bstr)|将对象强制转换 `CComBSTR` 为 BSTR。|
...
|[CComBSTR：： operator =](#operator_eq)|向赋值 `m_str` 。|
...
|名称|描述|
|----------|-----------------|
|[CComBSTR：： m_str](#m_str)|包含与对象关联的 BSTR `CComBSTR` 。|
...
> [!NOTE]
> `CComBSTR`类提供多个 (构造函数、赋值运算符和比较运算符) 的成员，这些成员采用 ANSI 或 Unicode 字符串作为参数。 这些函数的 ANSI 版本不如它们的 Unicode 对应，因为通常在内部创建临时 Unicode 字符串。 为提高效率，请尽可能使用 Unicode 版本。

> [!NOTE]
> 由于在 Visual Studio .NET 中实现了改进的查找行为，因此 `bstr = L"String2" + bstr;` ，在以前的版本中可能编译的代码（如）应改为实现 `bstr = CStringW(L"String2") + bstr` 。
...
释放在对象中保存的任何现有字符串 `CComBSTR` ，然后从 safearray 中每个元素的第一个字符创建一个 BSTR，并将其附加到 `CComBSTR` 对象。
...
`CComBSTR`通过将 [m_str](#m_str)成员设置为 *src*，将 BSTR 附加到对象。
...
创建从零开始的一维 safearray，其中数组的每个元素都是对象中的一个字符 `CComBSTR` 。
...
*nSize*<br/>
中要从 *sz* 复制的字符数或的初始大小（以字符为字符） `CComBSTR` 。
...
复制构造函数将设置 `m_str` 为 *SRC* 的 BSTR 成员的副本。 `REFGUID`构造函数使用将 GUID 转换为字符串 `StringFromGUID2` ，并存储结果。
...
调用此方法后， *pvarDest* 指向的变量将为类型 VT_BSTR。
...
将 [m_str](#m_str) 与 `CComBSTR` 对象分离，并将设置 `m_str` 为 NULL。
...
允许您将 `CComBSTR` 对象传递给具有 **[IN] BSTR** 参数的函数。
...
如果要比较的项与对象不相等，则返回 TRUE `CComBSTR` ; 否则，返回 FALSE。
...
返回 [m_str](#m_str) 成员中存储的 BSTR 的地址。
...
`CComBstr operator &` 具有与之关联的特殊断言，以帮助标识内存泄漏。 该程序将在 `m_str` 初始化成员时断言。 创建此断言是为了识别程序员使用将 `& operator` 新值分配给 `m_str` 成员而不释放第一次分配的情况 `m_str` 。 如果 `m_str` 等于 NULL，则程序假定尚未分配 m_str。 在这种情况下，程序将不会进行断言。
...
如果要比较的项小于对象，则返回 TRUE `CComBSTR` ; 否则，返回 FALSE。
...
如果要比较的项等于对象，则返回 TRUE `CComBSTR` ; 否则返回 FALSE。
...
如果要比较的项大于对象，则返回 TRUE `CComBSTR` ; 否则返回 FALSE。
...
将 [m_str](#m_str) 成员设置为指定流中包含的 BSTR。
...
`CharLowerBuff`有关如何执行转换的详细信息，请参阅。
...
`CharUpperBuff`有关如何执行转换的详细信息，请参阅。
