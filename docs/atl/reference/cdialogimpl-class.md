...
`CDialogImpl` 派生自 `CDialogImplBaseT` ，而后者又派生自 `CWindowImplRoot` 。
...
**Rect&** *rect* [in] 用于指定对话框大小和位置的 [矩形](/windows/win32/api/windef/ns-windef-rect) 结构。
