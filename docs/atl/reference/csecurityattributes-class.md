---
description: 了解详细信息： CSecurityAttributes 类
...
|“属性”|描述|
|----------|-----------------|
|[CSecurityAttributes：： Set](#set)|调用此方法可设置对象的特性 `CSecurityAttributes` 。|

## <a name="remarks"></a>备注

`SECURITY_ATTRIBUTES`结构包含用于创建对象的[安全描述符](/windows/win32/api/winnt/ns-winnt-security_descriptor)，并指定通过指定此结构检索的句柄是否可继承。
...
调用此方法可设置对象的特性 `CSecurityAttributes` 。
