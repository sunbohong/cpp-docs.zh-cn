...
> [!NOTE]
> `HRESULT_FROM_WIN32(GetLastError())`使用函数[AtlHresultFromLastError](debugging-and-error-reporting-global-functions.md#atlhresultfromlasterror)，而不是使用。
...
> [!CAUTION]
> 不要 `AtlReportError` 在 c + + catch 处理程序中使用。 这些函数的一些替代在内部使用 ATL 字符串转换宏，这反过来又在 `_alloca` 内部使用函数。 `AtlReportError`在 c + + catch 处理程序中使用可能导致 c + + catch 处理程序中出现异常。
...
对于 ATL 项目，可以提供此函数的实现，以便在发生故障时 ATL 使用该函数。 为此，请用与相同的签名定义你自己的函数， `AtlThrow` 并 #define `AtlThrow` 成为函数的名称。 必须在包含 atlexcept 之前完成此操作 (这意味着必须在包含任何 ATL 标头之前完成此操作，因为 atlbase.h 包含 atlexcept) 。 将函数特性 `__declspec(noreturn)` 为可避免出现虚假的 SAL 警告。
...
如果 MFC 项目中未定义 _ATL_NO_EXCEPTIONS，则此函数将基于返回的值引发 [CMemoryException](../../mfc/reference/cmemoryexception-class.md) 或 [COleException](../../mfc/reference/coleexception-class.md) `GetLastError` 。
