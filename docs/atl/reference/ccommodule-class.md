---
description: 了解详细信息： CComModule 类
title: CComModule 类
ms.date: 08/19/2019
f1_keywords:
- CComModule
...
`CComModule`实例使用对象映射来维护一组类对象定义。 此对象映射作为结构数组实现 `_ATL_OBJMAP_ENTRY` ，并包含以下内容的信息：
...
除外 `CComModule` ，ATL 还提供 [CComAutoThreadModule](../../atl/reference/ccomautothreadmodule-class.md)，它为 exe 和 Windows 服务实现了单元模型模块。 `CComAutoThreadModule`如果要在多个单元中创建对象，请从派生模块。
...
[Init](#init)方法将设置 `m_hInst` 为传递给或的句柄 `DLLMain` `WinMain` 。
...
[Init](#init)方法将设置 `m_hInstResource` 为传递给或的句柄 `DLLMain` `WinMain` 。 可以显式设置 `m_hInstResource` 为资源的句柄。

[GetResourceInstance](#getresourceinstance)方法返回存储在中的句柄 `m_hInstResource` 。
...
[Init](#init)方法将设置 `m_hInstTypeLib` 为传递给或的句柄 `DLLMain` `WinMain` 。 您可以显式设置 `m_hInstTypeLib` 为类型库的句柄。

[GetTypeLibInstance](#gettypelibinstance)方法返回存储在中的句柄 `m_hInstTypeLib` 。
...
`RegisterServer``DLLRegisterServer`对于 DLL，或 `WinMain` 对于使用命令行选项运行的 EXE，将自动调用 `/RegServer` 。
...
*lpszIndex*<br/>
中格式的字符串 `"\\N"` ，其中 `N` 是 TYPELIB 资源的整数索引。
...
`UnregisterServer``DLLUnregisterServer`对于 DLL，或 `WinMain` 对于使用命令行选项运行的 EXE，将自动调用 `/UnregServer` 。
...
通过指定 [DECLARE_REGISTRY](registry-macros.md#declare_registry) 宏， `UpdateRegistryClass` 在处理对象映射时将自动调用。
...
> [!NOTE]
> 若要在运行时替换替换值，请不要指定 DECLARE_REGISTRY_RESOURCE 或 DECLARE_REGISTRY_RESOURCEID 宏。 而是创建结构的数组 `_ATL_REGMAP_ENTRIES` ，其中每个条目都包含一个与值配对的变量占位符，以便在运行时替换占位符。 然后调用 `UpdateRegistryFromResourceD` ，传递 *pMapEntries* 参数的数组。 这会将结构中的所有替换值添加 `_ATL_REGMAP_ENTRIES` 到注册机构的替换地图。

> [!NOTE]
> 若要以静态方式链接到 (注册器) 的 ATL 注册表组件，请参阅 [UpdateRegistryFromResourceS](#updateregistryfromresources)。
...
类似于 [UpdateRegistryFromResourceD](#updateregistryfromresourced) ，但会 `UpdateRegistryFromResourceS` 创建指向 ATL 注册表组件 (注册器) 的静态链接。

`UpdateRegistryFromResourceS`在处理对象映射时将自动调用，前提是在 `#define _ATL_STATIC_REGISTRY` Visual Studio 2017 和) 更早版本的 *stdafx.h* 中添加到 *pch* (。

> [!NOTE]
> 若要在运行时替换替换值，请不要指定 [DECLARE_REGISTRY_RESOURCE](registry-macros.md#declare_registry_resource) 或 [DECLARE_REGISTRY_RESOURCEID](registry-macros.md#declare_registry_resourceid) 宏。 而是创建结构的数组 `_ATL_REGMAP_ENTRIES` ，其中每个条目都包含一个与值配对的变量占位符，以便在运行时替换占位符。 然后调用 `UpdateRegistryFromResourceS` ，传递 *pMapEntries* 参数的数组。 这会将结构中的所有替换值添加 `_ATL_REGMAP_ENTRIES` 到注册机构的替换地图。
