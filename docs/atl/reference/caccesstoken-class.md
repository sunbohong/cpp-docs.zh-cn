---
description: 了解详细信息： CAccessToken 类
...
|[CAccessToken：： CheckTokenMembership](#checktokenmembership)|调用此方法以确定是否在对象中启用了指定的 SID `CAccessToken` 。|
...
|[CAccessToken：： CreateProcessAsUser](#createprocessasuser)|调用此方法可创建一个新进程，该进程在由对象表示的用户的安全上下文中运行 `CAccessToken` 。|
|[CAccessToken：： CreateRestrictedToken](#createrestrictedtoken)|调用此方法可创建一个新的受限制的 `CAccessToken` 对象。|
|[CAccessToken：:D etach](#detach)|调用此方法可撤消访问令牌的所有权。|
|[CAccessToken：:D isablePrivilege](#disableprivilege)|调用此方法可禁用对象中的权限 `CAccessToken` 。|
|[CAccessToken：:D isablePrivileges](#disableprivileges)|调用此方法可禁用对象中的一个或多个权限 `CAccessToken` 。|
|[CAccessToken：： EnablePrivilege](#enableprivilege)|调用此方法可在对象中启用权限 `CAccessToken` 。|
|[CAccessToken：： EnablePrivileges](#enableprivileges)|调用此方法以启用对象中的一个或多个权限 `CAccessToken` 。|
...
|[CAccessToken：： GetLogonSessionId](#getlogonsessionid)|调用此方法以获取与对象关联的登录会话 ID `CAccessToken` 。|
|[CAccessToken：： GetLogonSid](#getlogonsid)|调用此方法以获取与对象关联的登录 SID `CAccessToken` 。|
|[CAccessToken：： GetOwner](#getowner)|调用此方法以获取与对象关联的所有者 `CAccessToken` 。|
|[CAccessToken：： GetPrimaryGroup](#getprimarygroup)|调用此方法以获取与对象关联的主要组 `CAccessToken` 。|
|[CAccessToken：： GetPrivileges](#getprivileges)|调用此方法以获取与对象关联的特权 `CAccessToken` 。|
|[CAccessToken：： GetProcessToken](#getprocesstoken)|调用此方法可使用给定进程中的访问令牌初始化 `CAccessToken`。|
|[CAccessToken：： GetProfile](#getprofile)|调用此方法可获取指向与对象关联的用户配置文件的句柄 `CAccessToken` 。|
|[CAccessToken：： GetSource](#getsource)|调用此方法以获取对象的源 `CAccessToken` 。|
|[CAccessToken：： GetStatistics](#getstatistics)|调用此方法以获取与对象关联的信息 `CAccessToken` 。|
|[CAccessToken：： GetTerminalServicesSessionId](#getterminalservicessessionid)|调用此方法以获取与对象关联的终端服务会话 ID `CAccessToken` 。|
|[CAccessToken：： GetThreadToken](#getthreadtoken)|调用此方法可 `CAccessToken` 通过给定线程中的标记初始化。|
|[CAccessToken：： GetTokenId](#gettokenid)|调用此方法以获取与对象关联的标记 ID `CAccessToken` 。|
|[CAccessToken：： GetType](#gettype)|调用此方法以获取对象的标记类型 `CAccessToken` 。|
|[CAccessToken：： GetUser](#getuser)|调用此方法可标识与对象关联的用户 `CAccessToken` 。|
|[CAccessToken：： HKeyCurrentUser](#hkeycurrentuser)|调用此方法可获取指向与对象关联的用户配置文件的句柄 `CAccessToken` 。|
...
|[CAccessToken：： IsTokenRestricted](#istokenrestricted)|调用此方法可测试对象是否 `CAccessToken` 包含受限 sid 的列表。|
|[CAccessToken：： Processmodel.loaduserprofile](#loaduserprofile)|调用此方法可加载与对象关联的用户配置文件 `CAccessToken` 。|
|[CAccessToken：： LogonUser](#logonuser)|调用此方法为与给定凭据关联的用户创建登录会话。|
|[CAccessToken：： OpenCOMClientToken](#opencomclienttoken)|从处理来自客户端的调用的 COM 服务器中调用此方法，以通过 `CAccessToken` com 客户端的访问令牌初始化。|
...
|[CAccessToken：:P rivilegeCheck](#privilegecheck)|调用此方法以确定是否在对象中启用了指定的权限集 `CAccessToken` 。|
|[CAccessToken：： Revert](#revert)|调用此方法可停止使用模拟标记的线程。|
|[CAccessToken：： SetDefaultDacl](#setdefaultdacl)|调用此方法可设置对象的默认 DACL `CAccessToken` 。|
|[CAccessToken：： SetOwner](#setowner)|调用此方法可设置对象的所有者 `CAccessToken` 。|
|[CAccessToken：： SetPrimaryGroup](#setprimarygroup)|调用此方法可设置对象的主要组 `CAccessToken` 。|
...
调用此方法以确定是否在对象中启用了指定的 SID `CAccessToken` 。
...
调用此方法可创建一个新进程，该进程在由对象表示的用户的安全上下文中运行 `CAccessToken` 。
...
`CreateProcessAsUser` 使用 `CreateProcessAsUser` Win32 函数来创建一个新进程，该进程在由对象表示的用户的安全上下文中运行 `CAccessToken` 。 有关所需参数的完整讨论，请参阅 [CreateProcessAsUser](/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessasuserw) 函数的说明。

要使此方法成功， `CAccessToken` 对象必须包含 AssignPrimaryToken (，除非它是受限令牌) 和 IncreaseQuota 特权。

## <a name="caccesstokencreaterestrictedtoken"></a><a name="createrestrictedtoken"></a> CAccessToken：： CreateRestrictedToken

调用此方法可创建一个新的受限制的 `CAccessToken` 对象。
...
*pRestrictedToken*<br/>
新的受限制的 `CAccessToken` 对象。

*SidsToDisable*<br/>
`CTokenGroups`指定仅拒绝 sid 的对象。
...
`CreateRestrictedToken` 使用 [CreateRestrictedToken](/windows/win32/api/securitybaseapi/nf-securitybaseapi-createrestrictedtoken) Win32 函数创建新的 `CAccessToken` 对象，并提供限制。

> [!IMPORTANT]
> 使用时 `CreateRestrictedToken` ，请确保以下各项：现有标记有效 (并且用户未输入) 并且 *SidsToDisable* 和 *PrivilegesToDelete* 都是有效 (，并且不是由用户) 输入。 如果该方法返回 FALSE，则拒绝功能。
...
*pszPrivilege*<br/>
指向字符串的指针，该字符串包含要在对象中禁用的特权 `CAccessToken` 。

*pPreviousState*<br/>
指向对象的指针， `CTokenPrivileges` 该对象将包含特权的以前的状态。
...
调用此方法可禁用对象中的一个或多个权限 `CAccessToken` 。
...
*rPrivileges*<br/>
一个指针，指向包含要在对象中禁用的特权的字符串数组 `CAccessToken` 。
...
*pszPrivilege*<br/>
指向字符串的指针，该字符串包含要在对象中启用的特权 `CAccessToken` 。
...
调用此方法以启用对象中的一个或多个权限 `CAccessToken` 。
...
*rPrivileges*<br/>
一个指针，指向包含在对象中启用的特权的字符串数组 `CAccessToken` 。
...
*pDacl*<br/>
指向将接收对象的默认 DACL 的 [CDacl 类](../../atl/reference/cdacl-class.md) 对象的指针 `CAccessToken` 。
...
调用此方法以获取与对象关联的登录会话 ID `CAccessToken` 。
...
调用此方法以获取与对象关联的登录 SID `CAccessToken` 。
...
调用此方法以获取与对象关联的所有者 `CAccessToken` 。
...
调用此方法以获取与对象关联的主要组 `CAccessToken` 。
...
调用此方法可获取指向与对象关联的用户配置文件的句柄 `CAccessToken` 。
...
调用此方法以获取与对象关联的终端服务会话 ID `CAccessToken` 。
...
*bOpenAsSelf*<br/>
指示是否对调用方法的线程的安全上下文 `GetThreadToken` 或针对调用线程的进程的安全上下文进行访问检查。
...
调用此方法以获取与对象关联的标记 ID `CAccessToken` 。
...
在调试版本中，如果没有指向标记的有效指针，则将发生断言错误 `CAccessToken` 。
...
调用此方法可测试对象是否 `CAccessToken` 包含受限 sid 的列表。
...
调用此方法可加载与对象关联的用户配置文件 `CAccessToken` 。
...
在调试版本中，如果不 `CAccessToken` 包含有效的标记，或用户配置文件已存在，则将发生断言错误。
...
从处理来自客户端的调用的 COM 服务器中调用此方法，以通过 `CAccessToken` com 客户端的访问令牌初始化。
...
调用此方法以确定是否在对象中启用了指定的权限集 `CAccessToken` 。
...
*pbResult*<br/>
指向一个值的指针，该方法将设置，以指示是否在对象中启用了任何或所有指定的权限 `CAccessToken` 。
...
`PrivilegeCheck`返回时， `Attributes` 如果启用了相应的权限，则每个[LUID_AND_ATTRIBUTES](/windows/win32/api/winnt/ns-winnt-luid_and_attributes)结构的成员将设置为 SE_PRIVILEGE_USED_FOR_ACCESS。 此方法调用 [PrivilegeCheck](/windows/win32/api/securitybaseapi/nf-securitybaseapi-privilegecheck) Win32 函数。
...
调用此方法可设置对象的默认 DACL `CAccessToken` 。
