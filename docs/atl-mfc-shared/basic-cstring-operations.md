---
description: 了解详细信息：基本 CString 操作
title: 基本 CString 操作
ms.date: 11/04/2016
helpviewer_keywords:
...
您可以 `CString` 使用和方法访问对象中的单个字符 `GetAt` `SetAt` 。 你还可以使用数组元素、下标、运算符 ( [] ) 而不是 `GetAt` 来获取单个字符。  (这类似于通过索引访问数组元素，就像在标准 C 样式字符串中一样。 ) 字符的索引值 `CString` 是从零开始的。
...
至少一个连接运算符的参数 (+ 或 + =) 必须是一个 `CString` 对象，但你可以使用常数字符串 (例如， `"big"`) 或 **`char`** (例如，使用 "x" ) 作为另一个参数。
...
`CStringT`类模板定义关系运算符， ( # A0、 \<=, > =、>、= = 和！ =) ，它们可供使用 `CString` 。 您可以 `CStrings` 使用这些运算符比较两个，如下面的示例中所示。
