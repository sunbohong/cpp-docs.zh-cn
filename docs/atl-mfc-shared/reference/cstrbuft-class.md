...
- **`wchar_t`** Unicode 字符串的 () 

- ANSI 和 Unicode 字符串的 TCHAR () 
...
*str*<br/>
与缓冲区关联的字符串对象。 通常，开发人员将使用 `CStrBuf` (TCHAR 变体) 的预定义 typedef， `CStrBufA` (**`char`** 变体) 和 `CStrBufW` (**`wchar_t`** 变体) 。
