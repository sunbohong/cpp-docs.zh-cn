...
|[COleDateTimeSpan：： GetStatus](#getstatus)|获取此对象 (有效性) 的状态 `COleDateTimeSpan` 。|
...
获取此对象 (有效性) 的状态 `COleDateTimeSpan` 。
...
- `COleDateTimeSpan::null` 指示此 `COleDateTimeSpan` 对象为 null，即没有为此对象提供值。  (这是 "无值" 的数据库意义上的 "null"，而不是 c + + NULL。 ) 
...
- `COleDateTimeSpan::null` 指示此 `COleDateTimeSpan` 对象为 null，即没有为此对象提供值。  (这是 "无值" 的数据库意义上的 "null"，而不是 c + + NULL。 ) 
   ...
   > [!CAUTION]
   > 此函数适用于高级编程情况。 此函数不会更改此对象中的数据。 通常用于将状态设置为 **null** 或 **无效**。 请注意，赋值运算符 ([运算符 =](#operator_eq)) ， [SetDateTimeSpan](#setdatetimespan) 根据源值 () 设置对象的状态。
