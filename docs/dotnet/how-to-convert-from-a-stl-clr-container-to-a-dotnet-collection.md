---
description: 了解详细信息：如何：从 STL/CLR 容器转换为 .NET 集合
...
在此示例中，我们将创建一个 STL/CLR `vector` 并向其中添加5个元素。 然后，通过调用函数来创建 .NET 集合 `make_collection` 。 最后，我们将显示新创建的集合的内容。
...
在此示例中，我们将创建一个 STL/CLR `map` 并向其中添加5个元素。 然后，创建一个 .NET <xref:System.Collections.Generic.IDictionary%602> 并 `map` 直接将其分配给它。 最后，我们将显示新创建的集合的内容。
