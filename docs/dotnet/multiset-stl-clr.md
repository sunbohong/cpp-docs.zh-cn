---
description: '了解详细信息：多重集 (STL/CLR) '
...
ms.locfileid: "97255647"
---
# <a name="multiset-stlclr"></a>multiset (STL/CLR)

此模板类描述了一个对象，该对象控制具有双向访问权限的不同长度的元素序列。 使用容器可以将 `multiset` 一系列元素作为 (几乎) 均衡的节点的排序树，每个节点存储一个元素。
...
|[multiset::difference_type (STL/CLR)](#difference_type)|在两个元素之间可能有符号) 距离的 (的类型。|
...
|[multiset::size_type (STL/CLR)](#size_type)|两个元素之间 (非负) 距离的类型。|
...
|[operator！ = (多集)  (STL/CLR) ](#op_neq)|确定对象是否 `multiset` 不等于另一个 `multiset` 对象。|
|[运算符< (多集)  (STL/CLR) ](#op_lt)|确定 `multiset` 对象是否小于另一个 `multiset` 对象。|
|[运算符<= (多集)  (STL/CLR) ](#op_lteq)|确定 `multiset` 对象是否小于或等于另一个 `multiset` 对象。|
|[operator = = (多集)  (STL/CLR) ](#op_eq)|确定 `multiset` 对象是否等于另一个 `multiset` 对象。|
|[运算符> (多集)  (STL/CLR) ](#op_gt)|确定 `multiset` 对象是否大于另一个 `multiset` 对象。|
...
对象通过调用类型为 [多重集：：)  (key_compare ](#key_compare)的存储的委托对象，对其控制的序列进行排序。 构造多重集时，可以指定存储的委托对象;如果指定 "无委托对象"，则默认值为 "比较" `operator<(key_type, key_type)` 。 可以通过调用成员函数[多重集：： key_comp (STL/CLR) ](#key_comp)来访问此存储的对象 `()` 。

此类委托对象必须对类型为类型为： [： key_type (STL/CLR) ](#key_type)的密钥施加严格弱排序。 这意味着，对于任意两个密钥 `X` 和 `Y` ：
...
对于位于 `X` `Y` 受控序列中的任何元素， `key_comp()(Y, X)` 为 false。  (默认委托对象时，键永远不会降低值。 ) 与 [ (STL/CLR) ](../dotnet/set-stl-clr.md)的模板类集不同，模板类的对象 `multiset` 不需要所有元素的键都是唯一的。  (两个或更多的密钥可以具有等效的排序。 ) 

每个元素都作为3om-ey-2vk 和值。 序列以允许查找、插入和移除任意元素的方式表示，这些操作与序列中的元素数的对数成正比 (对数时间) 。 此外，插入元素不会使迭代器失效，移除元素仅会使指向已移除元素的迭代器失效。

多集支持双向迭代器，这意味着，可以在给定指定了受控序列元素的迭代器的情况下单步执行相邻元素。 特殊头节点对应于[多重集：： end (STL/CLR) ](#end)返回的迭代器 `()` 。 可以递减此迭代器以到达受控序列中的最后一个元素（如果存在）。 可以递增多集迭代器来访问头节点，然后将其与相等 `end()` 。 但不能取消引用返回的迭代器 `end()` 。
...
成员函数有效地调用[多重集：： erase (stl/clr) ](#erase) `(` [多重集：： begin (stl/clr) ](#begin) `(),` [多重集：： end (stl/clr) ](#end) `())` 。 用于确保受控序列为空。
...
对于空受控序列，该成员函数返回 true。 它等效于[) 的多重集：： size (](#size) `() == 0` 。 用于测试多重集是否为空。
...
此成员函数返回一对迭代器 `cliext::pair<iterator, iterator>(` [多重集：： lower_bound (stl/clr) ](#lower_bound)多 `(key),` [集：： upper_bound (stl/clr) ](#upper_bound) `(key))` 。 用于确定受控序列中当前与指定键匹配的元素范围。
...
第一个成员函数删除由 *where* 指向的受控序列的元素，并返回一个迭代器，该迭代器指定在删除的元素之外保留的第一个元素; 如果此类元素不存在，则返回 [多重集：： end (STL/CLR)](#end) `()` 。 使用它可以删除单个元素。
...
如果受控序列中的至少一个元素具有与 *键* 等效的排序，则成员函数将返回一个指定这些元素之一的迭代器;否则，它将返回 [多重集：： end (STL/CLR)](#end) `()` 。 用于查找当前位于受控序列中的元素，该元素与指定的键匹配。
...
成员函数确定 `X` 受控序列中对 *key* 具有等效顺序的第一个元素。 如果此类元素不存在，它将返回[多重集：： end (STL/CLR) ](#end) `()` ; 否则返回指定的迭代器 `X` 。 用于查找当前在受控序列中与指定键匹配的一系列元素的开头。
...
用序列 [ `right.begin()` ， `right.end()`) 用默认排序谓词初始化受控序列。 使用此方法可以指定初始受控序列，该序列是由多重集对象 *权限* 控制的序列的副本，具有默认排序谓词。
...
## <a name="multisetreference-stlclr"></a><a name="reference"></a> 多重集：：引用 (STL/CLR) 
...
成员函数将返回受控序列的长度。 用于确定受控序列中当前的元素数。 如果你只关心序列的大小是否为非零，请参阅[多重集：： empty (STL/CLR) ](#empty) `()` 。
...
成员函数确定 `X` 受控序列中对 *key* 具有等效排序的最后一个元素。 如果此类元素不存在，或者如果 `X` 是受控序列中的最后一个元素，则它将返回[多重集：： END (STL/CLR) ](#end) `()` ; 否则，它将返回一个指定第一个元素的迭代器 `X` 。 使用它可以查找受控序列中当前与指定键匹配的元素序列的末尾。
...
## <a name="operator-multiset-stlclr"></a><a name="op_neq"></a> operator！ = (多集)  (STL/CLR) 
...
## <a name="operatorlt-multiset-stlclr"></a><a name="op_lt"></a> 运算符 &lt; (多集)  (STL/CLR) 
...
如果为，则运算符函数返回 true，适用于的最低位置 `i` `!(right[i] < left[i])` `left[i] < right[i]` 。 否则，它会返回，用于 `left->size() < right->size()` 测试在按元素对两个多重集进行比较时，是否向 *左* 排序。
...
## <a name="operatorlt-multiset-stlclr"></a><a name="op_lteq"></a> operator &lt; = (多集)  (STL/CLR) 
...
## <a name="operator-multiset-stlclr"></a><a name="op_eq"></a> operator = = (多集)  (STL/CLR) 
...
## <a name="operatorgt-multiset-stlclr"></a><a name="op_gt"></a> 运算符 &gt; (多集)  (STL/CLR) 
...
## <a name="operatorgt-multiset-stlclr"></a><a name="op_gteq"></a> operator &gt; = (多集)  (STL/CLR) 
