...
使用/identity 选项指定标识字符串，其中包含[ \<assemblyIdentity> 元素](/visualstudio/deployment/assemblyidentity-element-clickonce-application)的特性。 标识字符串以属性的值开头 `name` ，后面是 *属性*  =  *值* 对。 标识字符串中的特性以逗号隔开。
...
指定输出清单文件的名称。 当清单工具仅处理一个清单文件时，此属性是可选项。  (： [文件]; # [资源 ID] ) 
...
与-managedassembly 一起使用。 -category 导致生成类别标记。  ( 类别) 
...
指定包含 RGS 文件中可替换字符串的值的文件。  (替换： [file] ) 

## <a name="advanced-property-page"></a>"高级" 属性页

### <a name="update-file-hashes"></a>更新文件哈希

计算在文件元素中指定的文件的哈希值，并将哈希属性更新为此值。  (hashupdate： [path] ) 
