---
description: '了解详细信息：/Z7、/Zi、/ZI (调试信息格式) '
...
指定调试选项时，编译器将生成用于调试器所使用的函数和变量、类型信息和行位置的符号名称。 此符号调试信息可以包含在由编译器生成的对象文件 (*`.obj`*) 文件中，也可以包含在单独的 PDB 文件中 (*`.pdb`* 可执行文件的文件) 。 以下部分介绍了 "调试信息格式" 选项。
...
对于第三方库的调试版本的分发服务器，不包含 PDB 文件有一个优点。 但是，在库链接阶段和调试期间，任何预编译标头的对象文件都是必需的。 如果对象文件中只有类型信息 (并且没有代码) *`.pch`* ，则还必须使用 [ `/Yl` (为调试库插入 PCH 引用)](yl-inject-pch-reference-for-debug-library.md)选项，该选项在生成库时默认启用。

指定时，不推荐使用已弃用的 [ `/Gm` (启用最小重新生成)](gm-enable-minimal-rebuild.md)选项 **`/Z7`** 。
...
**`/ZI`** 选项强制 [ `/Gy` (启用 Function-Level 链接)](gy-enable-function-level-linking.md) ，并 (诊断中要在编译中使用 [ `/FC` 的源代码文件的完整路径)](fc-full-path-of-source-code-file-in-diagnostics.md)选项。
