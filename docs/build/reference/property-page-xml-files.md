...
默认规则文件夹的路径取决于所用的区域设置和 Visual Studio 的版本。 在 Visual Studio 2015 或更早版本的开发人员命令提示符下，"规则" 文件夹为 *`%ProgramFiles(x86)%\MSBuild\Microsoft.Cpp\v4.0\<version>\<locale>`* 。 `<version>`该值 *`v140`* 在 Visual Studio 2015 中。 `<locale>`是一个 LCID，例如 `1033` 英语。 对于安装的每个版本的 Visual Studio 和每种语言，你将使用不同的路径。 例如，Visual Studio 2015 社区版中文版的默认规则文件夹路径可能是 *`C:\Program Files (x86)\MSBuild\Microsoft.Cpp\v4.0\v140\1033\`* 。

::: moniker-end

::: moniker range="vs-2017"

默认规则文件夹的路径取决于所用的区域设置和 Visual Studio 的版本。 在 Visual Studio 2017 开发人员命令提示中，rules 文件夹为 *`%VSINSTALLDIR%Common7\IDE\VC\VCTargets\<locale>\`* 。 `<locale>`是一个 LCID，例如 `1033` 英语。 在 Visual Studio 2015 或更早版本的开发人员命令提示符下，rules 文件夹为 *`%ProgramFiles(x86)%\MSBuild\Microsoft.Cpp\v4.0\<version>\<locale>\`* ，其中的 `<version>` 值 *`v140`* 在 visual Studio 2015 中。 对于安装的每个版本的 Visual Studio 和每种语言，你将使用不同的路径。 例如，Visual Studio 2017 社区版中文版的默认规则文件夹路径可能是 *`C:\Program Files (x86)\Microsoft Visual Studio\2017\Enterprise\Common7\IDE\VC\VCTargets\1033\`* 。

::: moniker-end

::: moniker range=">=vs-2019"

默认规则文件夹的路径取决于所用的区域设置和 Visual Studio 的版本。 在 Visual Studio 2019 或更高版本的开发人员命令提示中，rules 文件夹为 *`%VSINSTALLDIR%MSBuild\Microsoft\VC\<version>\<locale>\`* ，其中的 `<version>` 值 *`v160`* 在 Visual Studio 2019 中。 `<locale>`是一个 LCID，例如 `1033` 英语。 在 Visual Studio 2017 中，rules 文件夹为 *`%VSINSTALLDIR%Common7\IDE\VC\VCTargets\<locale>\`* 。 在 Visual Studio 2015 或更早版本的开发人员命令提示符下，"规则" 文件夹为 *`%ProgramFiles(x86)%\MSBuild\Microsoft.Cpp\v4.0\<version>\<locale>\`* 。 对于安装的每个版本的 Visual Studio 和每种语言，你将使用不同的路径。 例如，Visual Studio 2019 社区版中文版的默认规则文件夹路径可能是 *`C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\MSBuild\Microsoft\VC\v160\1033\`* 。
