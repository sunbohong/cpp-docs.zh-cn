---
description: '了解详细信息：/execution-charset (设置执行字符集) '
...
您可以使用 **/execution-charset** 选项来指定执行字符集。 执行字符集是在所有预处理步骤之后向编译阶段输入的程序文本所使用的编码。 此字符集用于编译的代码中的任何字符串或字符文本的内部表示形式。 设置此选项可指定在源文件包含不能在基本执行字符集中表示的字符时要使用的扩展执行字符集。 可以使用 IANA 或 ISO 字符集名称，也可以使用点 (。 ) 后跟3到5位的十进制代码页标识符，用于指定要使用的字符集。 有关支持的代码页标识符和字符集名称的列表，请参阅 [代码页标识符](/windows/win32/Intl/code-page-identifiers)。

默认情况下，Visual Studio 会检测字节顺序标记，以确定源文件是否为编码的 Unicode 格式，例如 UTF-16 或 UTF-8。 如果未找到字节顺序标记，则假定使用当前用户代码页对源文件进行编码，除非已使用 **/source-charset** 选项或 **/utf-8** 选项指定了字符集名称或代码页。 Visual Studio 允许使用多个字符编码中的任意一种来保存 c + + 源代码。 有关源和执行字符集的信息，请参阅语言文档中的 [字符集](../../cpp/character-sets.md) 。
