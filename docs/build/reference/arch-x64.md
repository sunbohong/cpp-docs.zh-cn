...
`__AVX__`当指定了 **/ARCH： AVX**， **/arch： AVX2** 或 **/arch： AVX512** 编译器选项时，将定义预处理器符号。 `__AVX2__`指定 **/ARCH： AVX2** 或 **/arch： AVX512** 编译器选项时，将定义预处理器符号。 `__AVX512F__` `__AVX512CD__` `__AVX512BW__` `__AVX512DQ__` `__AVX512VL__` 当指定 **/arch： AVX512** 编译器选项时，将定义、、和预处理器符号。 有关更多信息，请参见 [Predefined Macros](../../preprocessor/predefined-macros.md)。 版本 12.0.34567.1) Visual Studio 2013 中引入了 **/arch： AVX2** 选项。 对/arch 的有限支持 **： AVX512** 已添加到 visual studio 2017，并在 visual studio 2019 中扩展。
...
1. 在 " **启用增强指令集** " 下拉框中，选择 " **高级矢量扩展" (/arch： AVX)**、 **advanced vector EXTENSION 2 (/Arch： AVX2)** 或 **ADVANCED vector extension 512 (/arch： AVX512)**。
