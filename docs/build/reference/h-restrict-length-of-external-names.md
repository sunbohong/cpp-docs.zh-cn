---
description: '了解有关以下内容的详细信息：/H (限制外部名称的长度) '
title: /H（限制外部名称长度）
ms.date: 09/05/2018
f1_keywords:
- /h
...
默认情况下，external (公有) 名称的长度为2047个字符。 这适用于 C 和 c + + 程序。 使用 **/h** 只能减少允许的最大标识符长度，而不能增加。 **/H** 和 *number* 之间的空格是可选的。
...
长度限制包括任何编译器创建的前导下划线 (**\_**) 或 () 的符号 **\@** 。 这些字符是标识符的一部分，并且会占用很大的位置。

- 编译器会将前导下划线 (**\_**) 添加到由 **`__cdecl`** (默认) 和调用约定修改的名称 **`__stdcall`** 中，并将前导 at 符号 (**\@**) 到调用约定修改的名称 **`__fastcall`** 。
...
使用 **/h** 与 [/Gl (完全程序优化)](gl-whole-program-optimization.md)不兼容。
