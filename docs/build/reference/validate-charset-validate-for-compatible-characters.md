---
description: '了解详细信息：/validate-charset (验证兼容字符) '
...
默认情况下，Visual Studio 会检测字节顺序标记，以确定源文件是否为编码的 Unicode 格式，例如 UTF-16 或 UTF-8。 如果未找到字节顺序标记，则假定使用当前用户代码页对源文件进行编码，除非已使用 **/utf-8** 或 **/source-charset** 选项指定了代码页。 Visual Studio 允许使用多个字符编码中的任意一种来保存 c + + 源代码。 有关源和执行字符集的信息，请参阅语言文档中的 [字符集](../../cpp/character-sets.md) 。 有关支持的代码页标识符和字符集名称的列表，请参阅 [代码页标识符](/windows/win32/Intl/code-page-identifiers)。
