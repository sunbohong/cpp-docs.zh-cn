---
description: '了解有关以下内容的详细信息：/Yc (创建预编译头文件) '
title: /Yc（创建预编译标头文件）
ms.date: 11/04/2016
f1_keywords:
...
ms.locfileid: "97243641"
---
# <a name="yc-create-precompiled-header-file"></a>/Yc（创建预编译标头文件）

指示编译器创建一个预编译标头 ( .pch) 文件，该文件表示某一时刻的编译状态。
...
预编译代码保存在一个文件中，该文件的名称是通过使用 **/yc** 选项指定的文件的基名称和一个 .pch 扩展名创建的。 你还可以使用 [/fp (名称。Pch File) ](fp-name-dot-pch-file.md) 选项来指定预编译头文件的名称。

如果使用 __/yc__*filename*，则编译器会将所有代码编译为并包含指定的文件，以便以后用于 [/Yu (使用预编译头文件)](yu-use-precompiled-header-file.md) 选项。
...
使用命令编译此代码时 `CL /YcMYAPP.H PROG.CPP` ，编译器会在名为 "MYAPP" 的预编译头文件中保存 afxwin.h、RESOURCE .h 和 MYAPP 的所有预处理。
