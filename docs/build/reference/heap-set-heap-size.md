...
可选 `commit` 参数指定要一次分配的物理内存量。 已提交的虚拟内存会导致在页面文件中保留空间。 `commit`当应用程序需要更多堆空间时，较高的值可节省时间，但会增加内存需求和可能的启动时间。

 `commit` 以十进制或 C 语言表示法指定保留值和值。
