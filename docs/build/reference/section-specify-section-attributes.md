---
description: '了解详细信息：/SECTION (指定节特性) '
title: /SECTION（指定节特性）
ms.date: 12/29/2017
f1_keywords:
- /section
...
可移植可执行文件 (PE) 文件中的 *部分* 是包含代码或数据的已命名连续内存块。 有些部分包含程序声明并直接使用的代码或数据，而其他数据节则由链接器和库管理器为你创建 ( # A0) 并包含对操作系统至关重要的信息。 有关详细信息，请参阅 [PE 格式](/windows/win32/Debug/pe-format)。
...
若要对某个属性求反，请在其字符前面加上 (！ ) 的感叹号。 此表显示了属性字符的含义：
...
**ALIGN =**_number_ 参数可为特定节指定对齐值。 _Number_ 参数以字节为单位，必须是2的幂。 有关详细信息，请参阅 [/align](align-section-alignment.md) 。
