---
description: '了解有关以下内容的详细信息：/EXPORT (导出函数) '
title: /EXPORT（导出函数）
ms.date: 09/05/2018
f1_keywords:
...
LINK 使用标识符的修饰形式。 编译器在创建 .obj 文件时修饰标识符。 如果 *entryname* 在其未修饰形式中指定给链接器 (因为它显示在源代码) 中，则 LINK 尝试匹配该名称。 如果找不到唯一匹配项，则 LINK 发出错误消息。 需要将 [DUMPBIN](dumpbin-reference.md) 工具指定给链接器时，可使用工具获取该标识符的 [修饰名称](decorated-names.md) 格式。

> [!NOTE]
> 不要指定已声明或的 C 标识符修饰形式 **`__cdecl`** **`__stdcall`** 。

如果需要导出未修饰的函数名称，并且具有不同的导出（具体取决于生成配置） (例如，在32或64版本) 中，可以对每个配置使用不同的 .DEF 文件。 .DEF 文件中不允许 (预处理器条件指令。 ) 作为替代方法，可以在 `#pragma comment` 函数声明之前使用指令，其中 `PlainFuncName` 是未修饰的名称， `_PlainFuncName@4` 是函数的修饰名：
