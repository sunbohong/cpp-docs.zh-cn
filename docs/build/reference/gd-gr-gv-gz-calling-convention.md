---
description: '详细了解：/Gd、/Gr、/Gv、/Gz (调用约定) '
...
对于 C， **`__cdecl`** 命名约定使用前面带有下划线 ( `_` ) ; 不执行大小写转换。 除非声明为 `extern "C"`，否则 C++ 函数使用其他名称修饰方案。 有关详细信息，请参阅[修饰名](decorated-names.md)。

## <a name="__fastcall-specifics"></a>__fastcall 详细信息

**`__fastcall`** 函数的某些参数传递到 x86 处理器、ECX 和 EDX) 的寄存器 (中，其余部分将从右到左推送到堆栈上。 所调用的例程在返回之前从堆栈中弹出这些参数。 通常情况下，/Gr 可减少执行时间。

> [!NOTE]
> **`__fastcall`** 对使用内联程序集语言编写的任何函数使用调用约定时，请务必小心。 使用寄存器可能与使用编译器产生冲突。

对于 C， **`__fastcall`** 命名约定使用前面带有 @ 符号 (**\@**) 后跟函数参数大小（以字节为单位）的函数名。 不执行任何大小写转换。 编译器将此模板用于命名约定：
...
对于 C， **`__stdcall`** 命名约定使用前面带有下划线 () 的函数名 **\_** ，后跟一个 at 符号 (**\@**) 和函数的参数的大小（以字节为单位）。 不执行任何大小写转换。 编译器将此模板用于命名约定：

`_functionname@number`

## <a name="__vectorcall-specifics"></a>__vectorcall 详细信息

**`__vectorcall`** 函数的整数参数按值传递，最多使用两个 (x86) 上的两个 (或 x64) 整数寄存器上的四个，以及最多六个用于浮点和向量值的 XMM 寄存器，其余部分将从右到左传递到堆栈。 被调用的函数在返回之前清除堆栈。 向量和浮点返回值在 XMM0 中返回。
