---
description: '了解详细信息：/MANIFEST (创建并行程序集清单) '
...
从 Visual Studio 2008 开始，可执行文件的清单文件包含一个指定 (UAC) 信息的用户帐户控制的部分。 如果指定/MANIFEST，但不指定 [/MANIFESTUAC](manifestuac-embeds-uac-information-in-manifest.md) 或 [/DLL](dll-build-a-dll.md)，则会将 Uac 级别设置为 *asInvoker* 的默认 UAC 片段插入清单中。 有关 UAC 级别的详细信息，请参阅 [/MANIFESTUAC (将 UAC 信息嵌入清单) ](manifestuac-embeds-uac-information-in-manifest.md)。
