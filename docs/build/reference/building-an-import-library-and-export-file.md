...
这些方法与在链接导出程序时指定导出的方法相同。 程序可以使用多种方法。 你可以在 LIB 命令的命令文件中指定 LIB 命令 (（例如多个 *objfiles* 或/export) 规格），就像在 LINK 命令中一样。
...
从程序中导出函数，以允许其他程序调用函数。 还可以使用 **data** 关键字) 导出数据 (。 通常，导出是在 DLL 中定义的。
