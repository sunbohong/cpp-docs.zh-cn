---
description: '了解有关以下内容的详细信息：/Zc： inline (删除未引用的 COMDAT) '
...
指定 **/zc： inline** 后，编译器不会为未引用的 COMDAT 函数或数据发出符号信息。 或者，对于仅具有内部链接的数据或函数。 此优化简化了链接器在发布版本中进行的某些工作，或指定了 [/opt： REF](opt-optimizations.md) 链接器选项。 此编译器优化可显著减少 .obj 文件大小并提高链接器的速度。 禁用 ([/od](od-disable-debug.md)) 的优化时，编译器选项未启用。 或者，指定 [/gl (完全程序优化) ](gl-whole-program-optimization.md)。

默认情况下，此选项在命令行生成 (**/zc：** 内置) 中处于关闭状态。 [/Permissive-](permissive-standards-conformance.md)选项不启用 **/zc： inline**。 在 MSBuild 项目中，选项由 **配置属性**  >  **C/c + +**  >  **语言**"  >  **删除未引用的代码和数据**" 属性设置，默认情况下，该属性设置为 **"是"** 。
