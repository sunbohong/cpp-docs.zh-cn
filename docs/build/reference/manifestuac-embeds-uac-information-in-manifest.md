---
description: '了解详细信息：/MANIFESTUAC (将 UAC 信息嵌入清单) '
...
*`level`*<br/>
**`level=`** 后跟 **`'asInvoker'`** 、 **`'highestAvailable'`** 或 **`'requireAdministrator'`** 。 默认为 `'asInvoker'`。 有关详细信息，请参阅[备注](#remarks)部分。

*`uiAccess`*<br/>
**`uiAccess='true'`** 如果希望应用程序绕过用户界面保护级别并将输入放到桌面上的更高权限窗口上，则为;否则为 **`uiAccess='false'`** 。 默认为 `uiAccess='false'`。 仅将此参数设置为 **`uiAccess='true'`** 适用于用户界面辅助功能的应用程序。
