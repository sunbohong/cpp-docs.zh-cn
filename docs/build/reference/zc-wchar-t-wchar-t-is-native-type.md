---
description: '了解有关以下方面的详细信息：/Zc： wchar_t (wchar_t 是本机类型) '
...
如果 **/zc： wchar_t** 为 on， **`wchar_t`** 则为编译为 c + + 的代码中的内置整型类型的关键字。 如果 **/zc：** 指定了具有减号) wchar_t (，或者在编译为 C 的代码中， **`wchar_t`** 则不是内置类型。 相反， **`wchar_t`** **`typedef`** **`unsigned short`** 在规范标头 stddef.h 中将定义为 for。  (Microsoft 实现在 stddef.h 和其他标准标头包含的另一个标头中定义它。 ) 
...
如果你的代码使用编译器 COM 全局函数，因为 **/zc： wchar_t** 现在默认处于启用状态，因此，我们建议你将对 comsupp.lib 的显式引用更改 (从 [注释杂注](../../preprocessor/comment-c-cpp.md) 或命令行上) 到 comsuppw.lib 或 comsuppwd.lib。  (如果必须使用 **/zc： wchar_t** 进行编译，请使用 comsupp.lib。 ) 如果包括 comdef.h 头文件，则会为你指定正确的库。 有关编译器 COM 支持的详细信息，请参阅 [编译器 Com 支持](../../cpp/compiler-com-support.md)。
