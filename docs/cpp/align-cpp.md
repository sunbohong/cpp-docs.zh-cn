...
如果没有 `__declspec(align(#))` ，则编译器通常会根据目标处理器和数据的大小、32位处理器上最多4个字节的边界以及64位处理器上的8字节边界来对齐自然边界上的数据。 类或结构中的数据在类或结构中的水平对齐方式最小，且当前的打包设置 (`#pragma pack` 或 `/Zp` 编译器选项) 。
...
现在， `aType` 和 `bType` 的大小 (8 个字节) 但类型的变量 `bType` 为32字节。
