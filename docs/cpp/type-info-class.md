---
description: 了解详细信息： type_info 类
title: type_info 类
ms.date: 11/04/2016
f1_keywords:
- type_info
...
`type_info::name`成员函数将返回一个 `const char*` 以 null 结尾的字符串，该字符串表示该类型的用户可读名称。 将缓存所指向的内存，应该从不直接释放它。

`type_info::raw_name`成员函数将返回一个 `const char*` 以 null 结尾的字符串，该字符串表示对象类型的修饰名。 该名称实际上以其修饰的形式存储以节省空间。 因此，此函数的速度要快于这一点， `type_info::name` 因为它不需要取消修饰名称。 函数返回的字符串 `type_info::raw_name` 在比较运算中非常有用，但不可读。 如果需要用户可读的字符串，请改用 `type_info::name` 函数。

仅当指定了 [/GR (启用 Run-Time 类型信息) ](../build/reference/gr-enable-run-time-type-information.md) 编译器选项时，才为多态类生成类型信息。
