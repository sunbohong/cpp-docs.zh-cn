...
|名称修饰约定|下划线字符 (_) 是名称的前缀，但 \_ 导出使用 C 链接 _cdecl 函数时除外。|
|大小写转换约定|不执行任何大小写转换。|

> [!NOTE]
> 有关相关信息，请参阅 [修饰名](../build/reference/decorated-names.md)。

将 **`__cdecl`** 修饰符放置在变量或函数名称之前。 由于 C 命名和调用约定为默认值，因此，仅 **`__cdecl`** 当你已指定 `/Gv` (vectorcall) 、 `/Gz` (stdcall) 或 `/Gr` (fastcall) 编译器选项时，才必须在 x86 代码中使用。 [/Gd](../build/reference/gd-gr-gv-gz-calling-convention.md)编译器选项强制执行 **`__cdecl`** 调用约定。
...
为了与早期版本兼容， **cdecl** 和 **_cdecl** 是同义词， **`__cdecl`** 除非指定了编译器选项 [/za " \( 禁用语言扩展")](../build/reference/za-ze-disable-language-extensions.md) 。
