...
下标表达式从左至右关联。 首先计算最左侧的下标表达式 expression1 [expression2]     。 通过添加 expression1  和 expression2  得到的地址构成一个指针表达式；然后 expression3  将添加到此指针表达式，从而构成一个新的指针表达式，依此类推，直到添加最后一个下标表达式。 在 <strong>\*</strong> 计算最后一个下标表达式后 () 的间接寻址运算符（除非最终指针值用于处理数组类型）。
...
最后一行中的负下标可能产生运行时错误，因为它指向 **`int`** 内存中低于数组的源的地址256位置。 将指针 `midArray` 初始化为的中间 `intArray` ; 因此可能 (但) 在其上使用正数组和负数组索引。 数组下标错误不会产生编译时错误，但它们会产生不可预知的结果。

下标运算符是可交换的。 因此，只要下标运算符未重载，表达式 *数组*[*index*] 和 *index*[*array*] 就是等效的 (参阅) [重载运算符](../cpp/operator-overloading.md) 。 第一种形式是最常见的编码做法，但它们都有效。
