---
description: '了解详细信息：异常规范 (throw、noexcept)  (c + +) '
title: '异常规范 (throw)  (c + +) '
ms.date: 01/18/2018
helpviewer_keywords:
...
ms.locfileid: "97164791"
---
# <a name="exception-specifications-throw-noexcept-c"></a>异常规范 (throw)  (c + +) 
...
如果在应用程序中使用异常处理，则调用堆栈中必须有一个函数，该函数在退出标记为、或的函数的外部范围之前处理引发的异常 **`noexcept`** `noexcept(true)` `throw()` 。 如果在引发异常的函数和处理异常的函数之间调用的任何函数指定为 **`noexcept`** ， `noexcept(true)` (或 `throw()` 在 **/std： c + + 17** 模式) 中，则当 noexcept 函数传播异常时，程序将终止。
