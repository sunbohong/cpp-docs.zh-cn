...
除非异常是整数或浮点溢出，否则此处理程序会将控制权传递给其他处理程序。 如果异常是整数或浮点溢出，则处理程序将调用一个函数（`ResetVars` 只是一个示例，不是 API 函数）以重置某些全局变量。 在此示例中为空的 *语句块 2* 将永远无法执行，因为 `Eval_Exception` 从不返回 EXCEPTION_EXECUTE_HANDLER (1) 。
