---
description: '了解有关以下内容的详细信息：内置类型 (c + +) '
title: '内置类型 (c + +) '
ms.date: 07/22/2020
f1_keywords:
- __int128_cpp
...
特定于 **Microsoft** 的整数类型包括特定宽度 **`__int8`** 、、 **`__int16`** **`__int32`** 和 **`__int64`** 类型。 这些类型可以使用 **`signed`** 和 **`unsigned`** 修饰符。 `__int8` 数据类型与 `char` 类型同义，`__int16` 与 `short` 类型同义，`__int32` 与 `int` 类型同义，`__int64` 与 `long long` 类型同义。
