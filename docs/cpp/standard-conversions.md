...
如果浮动类型的对象位于由该类型表示的范围内，则它也可以转换为不太精确的类型。  (参阅浮动类型范围的 [浮动限制](../cpp/floating-limits.md) ) 。如果原始值不能准确地表示，则可以将其转换为下一个较高的值或下一个较小的可表示值。 如果不存在这样的值，则结果是不确定的。 请看下面的示例：
...
在 [包含二元运算符的表达式](../cpp/expressions-with-binary-operators.md) 中 (讨论了许多二元运算符) 导致操作数的转换，并以相同的方式生成结果。 这些运算符导致的转换称为 *常用算术转换*。 按照下表中所示，完成具有不同本机类型的操作数的算术转换。 Typedef 类型的行为方式基于其基础本机类型。
...
前面的示例中的第二个语句显示添加了 **`float`** 和整型类型： `fVal` 和 `ulVal` 。 `ulVal`变量将转换为 **`float`** 表)  (第三个条件中的类型。 添加的结果将转换为 **`double`** 表中的第二个条件 (类型) 并分配给 `dVal` 。
...
指针 `pA` 的类型为 `A *`，它可解释为“指向类型 `A` 的对象的指针”。  (的成员 `bObject` （如 `BComponent` 和) ）对于 `BMemberFunc` 类型是唯一的 `B` ，因此无法通过进行访问 `pA` 。 `pA` 指针只允许访问类 `A` 中定义的对象的那些特性（成员函数和数据）。
