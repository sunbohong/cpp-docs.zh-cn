---
description: 了解有关详细信息，请参阅如何：创建和使用 CComPtr 和 CComQIPtr 实例
...
## <a name="example-ccomptr"></a>示例： CComPtr
...
`CComPtr` 及其亲属是 ATL 的一部分，在中定义 \<atlcomcli.h> 。 `_com_ptr_t` 在中声明 \<comip.h> 。 当为类型库生成包装器类时，编译器将创建 `_com_ptr_t` 的专用化。

## <a name="example-ccomqipt"></a>示例： CComQIPt
...
## <a name="example-idispatch"></a>示例： IDispatch
