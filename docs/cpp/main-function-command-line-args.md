...
如果您的程序不采用命令行参数，则可以禁止使用命令行处理例程来节省少量空间。 若要禁止使用，请 *`noarg.obj`* `main` `wmain` 在 **`/link`** 编译器选项或命令行中包含和) 的文件 (**`LINK`** 。

同样，如果您从不通过参数访问环境表 *`envp`* ，则可以取消内部环境处理例程。 若要禁止使用，请 *`noenv.obj`* `main` `wmain` 在 **`/link`** 编译器选项或命令行中包含和) 的文件 (**`LINK`** 。

程序可能会调用 `spawn` `exec` C 运行库中的或系列的例程。 如果是这样，则不应取消环境处理例程，因为它用于将环境从父进程传递到子进程。
