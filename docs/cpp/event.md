...
COM 事件是接口。 事件源接口中的方法的参数应在参数 (*中* ，但这并不是严格强制的) ，因为 *out* 参数在多播时不起作用。 如果使用 *out* 参数，将发出1级警告。
...
托管事件是数据成员或方法。 当与事件一起使用时，委托的返回类型必须符合 [公共语言规范](/dotnet/standard/language-independence-and-language-independent-components)。 事件处理程序的返回类型必须与委托的返回类型匹配。 有关委托的详细信息，请参阅 [委托和事件](../dotnet/dotnet-programming-with-cpp-cli-visual-cpp.md)。 如果托管事件是数据成员，则其类型必须是指向委托的指针。
