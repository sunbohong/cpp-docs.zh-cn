...
ms.locfileid: "97113865"
---
# <a name="source-code-organization-c-templates"></a>源代码组织（C++ 模板）

在定义类模板时，必须以这样一种方式组织源代码：在编译器需要成员定义时，可以看到成员定义。 可以选择使用包含模型或显式实例化模型。 在包含模型中，您在使用模板的每个文件中都包含成员定义。 这是最简单的方法，它在可与模板一起使用的具体类型方面提供了最大的灵活性。 缺点是会增加编译时间。 如果项目和/或包含的文件本身很大，那么影响可能很大。 使用显式实例化方法，模板本身会实例化特定类型的具体类或类成员。  此方法可以加快编译时间，但它仅限于模板实现者提前启用的那些类的使用情况。 通常，我们建议使用包含模型，除非由于编译时间问题而存在局限性。
