...
您可以使用 [__declspec](../cpp/declspec.md) 扩展属性 [uuid](../cpp/uuid-cpp.md) 为 COM 对象分配 (GUID) 全局唯一标识符。 关键字 [__uuidof](../cpp/uuidof-operator.md) 可用于提取与 COM 对象关联的 GUID。 另一个 **`__declspec`** 属性（ [property](../cpp/property-cpp.md)）可用于指定 `get` `set` COM 对象的数据成员的和方法。
