---
description: '了解详细信息：抽象 (c + +/CLI 和 c + +/CX) '
...
**`virtual`***返回类型**成员函数标识符* **( # A1 抽象;**

### <a name="remarks"></a>备注

第一个示例语法将类声明为抽象类。 如果指定了或编译器选项，则 *类声明* 组件可为本机 c + + 声明 (**`class` * * * 或) ，或者 (ref class * * 或 ref Struct) **`struct`** 中的 c + + 扩展声明**  `/ZW` `/clr` 。
