...
对于具有两个分部定义的 ref 类，会将 **partial** 关键字应用于定义的第一个匹配项，此操作通常由自动生成的代码执行，因此，人力编码员不会经常使用关键字。 对于此类的所有后续分部定义，请从 class-key 关键字和类标识符中省略 partial 修饰符。 如果编译器遇到先前定义的 ref class 和类标识符，但其中没有 partial 关键字，它就会在内部将 ref class 定义的所有部分都合并到一个定义中。
...
若要详细了解如何使用 partial 关键字，请参阅[分部类 (C++/CX)](../cppcx/partial-classes-c-cx.md)。
