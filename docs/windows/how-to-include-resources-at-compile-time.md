...
- 若要包括资源 (如自定义格式的 RCDATA 资源) 。 RCDATA 资源有特殊要求，即无法将表达式用作字段的值 `nameID` 。

如果你的现有 .rc 文件中包含任何满足这些条件的部分，请将这些部分放在一个或多个单独的 .rc 文件中，并使用 " **资源包括** " 对话框将它们包含在项目中。

## <a name="resource-includes"></a>资源包括

可以在编译时将其他文件中的资源添加到项目中，方法是在 "**资源包括**" 对话框中的 "**编译时指令**" 框中列出这些资源。 使用 " **资源包括** " 对话框可以修改项目环境在项目 .rc 文件中存储所有资源的正常工作方式，以及中的所有 [符号](../windows/symbols-resource-identifiers.md) `Resource.h` 。
...
| **编译时指令** | 允许包含与主资源文件中的资源分开创建和编辑的资源文件、包含编译时指令 (例如，有条件地包含资源) 的指令或包含自定义格式的资源。<br/><br/>还可以使用 " **编译时指令" 框** 来包含标准 MFC 资源文件。 |

> [!NOTE]
> 这些文本框中的条目将显示在由、和标记的 .rc 文件中 `TEXTINCLUDE 1` `TEXTINCLUDE 2` `TEXTINCLUDE 3` 。 有关详细信息，请参阅 [TN035：将多个资源文件和头文件与 Visual C++ 一起使用](../mfc/tn035-using-multiple-resource-files-and-header-files-with-visual-cpp.md)。
