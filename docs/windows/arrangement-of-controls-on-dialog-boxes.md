---
description: '了解有关详细信息，请参阅如何：在 c + + (布局控件) '
title: 如何：在 c + +)  (布局控件 |Microsoft Docs
ms.date: 02/15/2019
f1_keywords:
...
ms.locfileid: "97337833"
---
# <a name="how-to-layout-controls-c"></a>如何：在 c + + (布局控件) 

**对话框编辑器** 提供了自动对齐和调整控件大小的布局工具。 对于大多数任务，您可以使用 [对话框编辑器工具栏](./dialog-editor.md)。 "**格式**" 菜单上还提供了所有 "**对话框编辑器**" 工具栏命令，其中大多数都具有 [快捷键](./dialog-editor.md)。
...
" [对话框编辑器" 工具栏](./dialog-editor.md) 包含控制状态的按钮。

- 若要更改状态，请选择相应的图标，或转到 "菜单 **格式**  >  **指南设置**"。
...
标尺中确定参考线和控件间距的刻度线由对话单位定义 (Dlu) 。 DLU 基于对话框字体的大小，通常为8点 MS Shell Dlg。 水平 DLU 是指用四个对话框字体分隔的平均宽度。 垂直 DLU 是该字体的平均高度除以8。

- 若要更改刻度线的间隔，请转到 "菜单" "**格式** 向导" "  >  **设置**"，然后在 "**网格间距**" 字段中指定以 dlu 为单位的新宽度和高度。
...
- 若要打开或关闭布局网格，请转到 "菜单 **格式**  >  **指南设置**"，然后选择或清除 "**网格**" 按钮。
   ...
   你仍可以使用 "[对话框编辑器" 工具栏](./dialog-editor.md)上的 "**切换网格**" 按钮来控制各个 **对话框编辑器** 窗口中的网格。

- 若要更改布局网格的大小，请转到 "菜单 **格式**  >  **指南**" "设置"，然后在 "值" 中为网格中的单元格键入 dlu 的高度和宽度。 最小高度或宽度为4。
...
- 若要关闭指南，请转到 "菜单 **格式**  >  **指南设置**"。 然后，在 " **布局指南**" 下，选择 " **无**"。
   ...
   > [!TIP]
   > 还可以使用菜单 **格式**  >  **切换参考线** 中的快捷方式。

## <a name="select-controls"></a>选择控件

选择要调整大小、对齐、移动、复制或删除控件的控件，然后完成所需的操作。 在大多数情况下，您需要在 [对话框编辑器工具栏](./dialog-editor.md)上选择多个控件以使用大小调整和对齐工具。

选择控件时，该控件的周围带有阴影边框，其周围有实心 (活动) 或空心 (非活动) 大小调整控点，小方格显示在选择框中。 如果选择了多个控件，则主导控件具有实尺寸控点，并且所有其他选定控件都具有空白尺寸控点。
...
- 若要调整控件大小，请选择控件，并拖动调整大小控点以更改大小。
  ...
  - 拐角处的大小控点同时更改水平和垂直大小。
   ...
   > [!TIP]
   > 可以通过按住 **Shift** 键并使用 **向右** 和 **向下** 键来调整控件一个对话框单位 (DLU) 的大小。

- 若要自动调整控件的大小以适应其中的文本，请参阅菜单 **格式** 或右键单击控件，然后选择 " **大小为内容**"。

- 若要使控件具有相同的大小，请选择要调整大小的控件并选择 "切换到菜单 **格式**  >  **"，** 然后选择 "**高度**" 或 **"****宽度**"。

   根据主导控件的大小调整控件组的大小，该控件是序列中第一个选定的控件。 组中控件的最终大小取决于主导控件的大小。

- 若要调整控件组的大小和参考线，请将控件的一侧对齐 (或将控件) 到参考线，然后将参考线拖动到控件的另一侧 (或控件) 。 现在，你可以移动任一指南来调整控件 (或控件) 的大小。

   如果有多个控件需要，请调整每个控件的大小以与第二个指南对齐。

### <a name="other-controls"></a>其他控件

将组合框添加到对话框中时，可以调整组合框的大小。 您还可以指定下拉列表框的大小。 有关详细信息，请参阅向 [组合框控件添加值](./defining-mnemonics-access-keys.md)。
...
1. 使用较低的调整大小控点来更改下拉列表区域的初始大小。
   ...
   ![MFC 项目中的组合&#45;框大小调整](../mfc/media/vccomboboxsizing.gif "vcComboBoxSizing")
...
- 若要对齐控件，请选择要对齐的控件。 "转向菜单 **格式**  >  " "**对齐**"，然后选择以下对齐方式之一：
...
- 若要均匀隔开控件，请选择要重新排列的控件。 "请按菜单 **格式** 调整  >  **空间**"，然后选择下列间距对齐方式之一：

   |间距|描述|
   |---|---|
   |**交叉**|在选定的最左侧和最右边的控件之间平均间隔控制。|
   |**向下**|在选定的最顶部和最底层控件之间平均间隔控制。|

- 若要使控件居中，请选择要重新排列的控件。   >  **在对话框中**，参阅 "菜单格式中心"，然后选择以下排列方式之一：
...
- 若要对齐 "推送" 按钮，请选择一个或多个 "推送" 按钮。 中转到 "菜单" "**格式**"  >  "**排列" 按钮**，然后选择以下排列方式之一：
