...
1. "前往" 菜单 **查看**  >  **其他窗口**  >  **资源视图** 并右键单击 **菜单** 标题。 选择 " **添加资源**"，然后选择 " **菜单**"。

1. 选择 " **新建项** " 框， (菜单栏上) 包含 " *类型* " 的矩形。
...
1. 在新项框中，键入新菜单命令的名称。
   ...
   > [!TIP]
   > 你可以定义一个允许用户选择菜单命令的助记键（热键）。 在字母前键入与号 (`&`) ，以将其指定为助记键。 用户可以通过键入该字母来选择菜单命令。
