...
对于与单个资源关联的符号，还可以使用 " **属性** " 窗口更改符号值。 您可以使用 " [资源符号" 对话框](./creating-new-symbols.md) 更改当前未分配给资源的符号的值。
...
> [!NOTE]
> 如果不遵循标准符号名称方案 (ID * _ [关键字] ) 下面所述，并且你的符号名称与资源脚本编译器已知的关键字相同，则尝试生成资源脚本文件将导致似乎随机生成错误，这很难诊断。 若要防止出现此情况，请遵循标准命名方案。
...
### <a name="to-change-a-symbol-name-id"></a> (ID 更改符号名称) 
...
> [!NOTE]
> 您可以使用 " [资源符号" 对话框](./creating-new-symbols.md) 更改当前未分配给资源的符号的名称。
...
在 " [资源符号" 对话框](./creating-new-symbols.md)中，您可以编辑或删除尚未分配给资源或对象的现有符号。
...
开发环境首次读取由其他应用程序创建的资源文件时，会将所有包含的头文件都标记为只读。 尽管可以使用 " [资源包括" 对话框](./how-to-include-resources-at-compile-time.md) 添加其他只读符号头文件。
...
1. 在 [资源视图](how-to-create-a-resource-script-file.md#create-resources)中，右键单击 *.rc* 文件，然后选择 " [资源包括](./how-to-include-resources-at-compile-time.md)"。
...
[ (符号的资源标识符) ](symbols-resource-identifiers.md)<br/>
