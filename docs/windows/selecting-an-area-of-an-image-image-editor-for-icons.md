...
> [!NOTE]
> 请参阅 "[图像编辑器" 工具栏](./image-editor-for-icons.md)中的 "**矩形选择**" 和 "**不规则选择**" 工具，或查看与 "**图像编辑器**" 工具栏上每个按钮相关联的工具提示。

您还可以从所选内容创建自定义画笔。 有关详细信息，请参阅 [创建自定义画笔](./using-a-drawing-tool-image-editor-for-icons.md)。
...
有关使用选择进行绘制的详细信息，请参阅 [创建自定义画笔](./using-a-drawing-tool-image-editor-for-icons.md)。
...
![位图上的大小调整控点](../mfc/media/vcimageeditorsizinghandles.gif "vcImageEditorSizingHandles")<br/>
调整大小控点

> [!NOTE]
> 如果您在 "[网格设置" 对话框](./image-editor-for-icons.md)中选择了 "**磁贴网格**" 选项，则调整大小会对齐到下一个平铺网格线。 如果只选择 " **像素网格** " 选项 (默认设置) ，则调整大小会对齐到下一个可用像素。
...
1. 在 " **属性** " 窗口中，更改图像的任意或全部属性。
   ...
   |**SaveCompressed**|指示图像是否为压缩格式。 此属性是只读的。<br/><br/>Visual Studio 不允许以压缩格式保存图像，因此对于在 Visual Studio 中创建的任何图像，此属性将为 **False**。 如果打开 (在 Visual Studio 中的其他程序) 中创建的压缩映像，则此属性将为 **True**。 如果使用 Visual Studio 保存压缩的映像，则会将其解压缩，并且此属性将恢复为 **False**。|
   |Width |以像素为单位设置图像 (的宽度) 。 位图的默认值为48。<br/><br/>裁剪图像或将空白区域添加到现有图像的右侧。|
