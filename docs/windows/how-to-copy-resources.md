---
description: '了解有关详细信息，请参阅如何：在 c + + (管理资源) '
title: '如何：在 c + + (管理资源) '
ms.date: 02/14/2019
f1_keywords:
...
ms.locfileid: "97329379"
---
# <a name="how-to-manage-resources-c"></a>如何：在 c + + (管理资源) 

## <a name="copy-and-edit-resources"></a>复制和编辑资源

你可以在不更改资源的情况下将资源从一个文件复制到另一个文件，或者在复制资源时更改资源的语言或条件。

可以轻松地将资源从现有资源或可执行文件复制到当前资源文件。 若要复制资源，请同时打开包含资源的两个文件，并将项从一个文件拖动到另一个文件，或者在这两个文件之间进行复制和粘贴。 此方法适用于资源脚本 ( .rc) 文件和资源模板 ( .rct) 文件和可执行文件 () 文件。

> [!NOTE]
> Visual C++ 包括可在自己的应用程序中使用的示例资源文件。 有关详细信息，请参阅 [剪贴画：常见资源](https://github.com/Microsoft/VCSamples/tree/master/VC2010Samples/MFC/general)。

不能在项目中的资源文件之间进行拖放、复制、剪切或粘贴 (**资源视图**) ，并在文档窗口中打开独立的 .rc 文件。 你可以在以前版本的产品中执行此操作。 仅在项目外部打开的 .rc 文件之间使用拖放方法。

### <a name="to-copy-resources"></a>复制资源

1. 同时单独打开这两个资源文件。  (参阅 [使用资源脚本文件](how-to-create-a-resource-script-file.md#use-resource-script-files)) 。 例如，打开 " *Source1* " 和 " *Source2*"。
...
1. 在第一个 .rc 文件中，可以是：
   ...
   - 使用复制和粘贴方法
      ...
      1. 右键单击要复制 (的资源，例如 *Source1*) 并选择 " **复制**"。

      1. 右键单击要将资源粘贴到的资源文件 (例如 *Source2*) 并选择 " **粘贴**"。

> [!NOTE]
> 若要避免与现有文件中的符号名或值冲突，Visual C++ 可能会在将传输资源的符号值或符号名称和值复制到新文件时更改这些值。
...
资源的语言和条件显示在 **工作区** 窗口中资源名称后的括号中。 此处名为的资源 `IDD_AboutBox` 使用 `Finnish` 作为其语言，其条件是 `XX33` ：
...
可以导入图形资源（位图、图标、光标和工具栏）、HTML 文件和自定义资源以便在 Visual C++ 中使用。 你可以从 Visual Studio c + + 项目将相同类型的文件导出到可以在开发环境外部使用的单独文件。
...
1. 找到并选择位图 ( 的文件名) ，图标 ( .ico) ，光标 () ，html 文件 ( .htm) 或其他要导入的文件。
...
### <a name="to-export-a-resource-for-use-outside-of-visual-c"></a>导出资源以便在 Visual C++ 之外使用
