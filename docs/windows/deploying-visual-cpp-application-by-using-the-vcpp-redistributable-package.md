---
description: 了解详细信息：演练：使用 Visual C++ 可再发行组件包部署 Visual C++ 应用程序
...
1. 创建自解压安装程序文件：
   ...
   1. 在“包文件”页上，添加以下文件，然后选择“下一步”。
      ...
      - vcredist_x86.exe。 在 Visual Studio 2015 中，此文件位于 *% VCINSTALLDIR \\ \\ 1033%*。 在 Visual Studio 2017 和 Visual Studio 2019 中，此文件位于 *% VCToolsRedistDir%*。 你还可以 [从 Microsoft 下载最新的受支持的再发行文件](https://support.microsoft.com/help/2977003/the-latest-supported-visual-c-downloads)。
...
1. 在另一台没有 Visual C++ 库的计算机上测试自解压安装程序文件：
   ...
   1. 运行 MFC 应用程序。
      ...
      > [!IMPORTANT]
      > 若要确定安装了哪个版本的运行时，安装程序会检查注册表项 \\ HKLM \\ SOFTWARE \\ Microsoft \\ VisualStudio \\ _版本_ \\ VC 运行时 \\ \\ _平台_ \\ 版本。 如果当前安装的版本比安装程序尝试安装的版本高，则安装程序成功返回而不安装较旧版本，并在控制面板中的已安装程序页上留下其他条目。
