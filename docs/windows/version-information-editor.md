...
`FILEFLAGS`在 **版本信息编辑器** 中编辑密钥时，请注意，不能在 .rc 文件的 "**属性**" 窗口中设置 "**调试**"、"**专用生成**" 或 "**特殊生成**" 属性：

- **版本信息编辑器** 将根据 `#ifdef` 生成标志，在资源脚本中设置 Debug 属性 `_DEBUG` 。

- 如果 `Private Build` 键在 **版本信息** 表中设置了 **值**，则键的 "**属性**" 窗口中的相应 "**私有生成**" 属性 `FILEFLAGS` 将为 **True**。 如果 **值** 为空，则属性将为 **False**。 同样，**版本信息** 表中的 **特殊生成** 键将绑定到密钥的 **特殊生成** 属性 `FILEFLAGS` 。

您可以通过选择 **键** 或 **值** 列标题来对字符串块的信息序列进行排序。 这些标题会自动将信息重新排列为所选顺序。

### <a name="to-add-version-information-for-another-language-new-version-info-block"></a>若要添加其他语言的版本信息 (新的版本信息块) 
