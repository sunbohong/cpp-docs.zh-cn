...
下面是快捷键对应表中 **修饰符** 属性的合法条目：
   ...
   |值|说明|
   |-----------|-----------------|
   |无|用户仅按 **键值** 。<br/><br/>此值最有效地用于 ASCII/ANSI 值001到026，它通过使用 ctrl + A 到 **ctrl + Z**) 解释为 ^ A 到 ^ Z (**ctrl + a** 到 ^ z。|
   |**Alt**|用户必须在 **键值** 前按 **Alt** 。|
...
下面是快捷键对应表中的 **键** 属性的合法条目：
   ...
   |在 a-z (大写的单个键盘字符仅) ，前面有一个插入符号 (^) ，例如 ^ C。|此选项在按下 **Ctrl** 键的同时输入该键的 ASCII 值。|
...
**Type** 属性确定与快捷键 **ID** 关联的快捷组合键是否被解释为 ASCII/ANSI 键值或 (VIRTKEY) 组合的虚拟键。
...
> [!NOTE]
> 每个修改值在 " **属性** " 窗口中显示为一个布尔属性。 如果在 " **属性** " 窗口中更改修改值，则快捷键对应表会将新修饰符视为添加到以前存在的任何修饰符。 因此，如果设置了任何修改值，则需要设置所有修改值，以确保每个加速器共享相同的 **修饰符** 设置。
