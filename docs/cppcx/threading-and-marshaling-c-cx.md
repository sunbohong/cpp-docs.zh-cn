---
description: '了解详细信息：线程处理和封送处理 (c + +/CX) '
...
ms.locfileid: "97307595"
---
# <a name="threading-and-marshaling-ccx"></a>线程处理和封送处理 (C++/CX)

在绝大多数情况下，可以从任何线程访问 Windows 运行时类的实例，如标准 c + + 对象。 此类称为“敏捷类”。 但是，Windows 附带的少量 Windows 运行时类是非敏捷类，并且必须比标准 c + + 对象使用的 COM 对象更类似。 您无需是 COM 专家即可使用非敏捷类，但您需要考虑类的线程模型及其封送处理行为。 本文提供了针对您需要使用非敏捷类的实例的极少情况的背景和指南。
...
由于各种原因，一些类不能是敏捷类。 如果从用户界面线程和后台线程访问非敏捷类的实例，则应格外小心以确保运行时正确的行为。 在全局范围内实例化应用程序中的非敏捷运行时类，或将非敏捷类型声明为其本身标记为敏捷的 ref 类中的类成员时，Microsoft c + + 编译器会发出警告。
...
如果这些条件都不适用，则您可以将包含类标记为非敏捷类。 换句话说，您应该直接在非敏捷类中保留非敏捷对象，并通过 agile 类中的 Platform：： Agile 来保存非敏捷对象 \<T> 。
...
在 c + +/CX 中定义 ref 类时，默认情况下它是 agile，也就是说，它具有 `ThreadingModel` = Both 和 `MarshallingType` = agile。  如果使用的是 Windows 运行时 c + + 模板库，则可以通过从（使用）派生来使您的类成为敏捷类 `FtmBase` `FreeThreadedMarshaller` 。  如果创作具有 `ThreadingModel`=Both 或 `ThreadingModel`=MTA 的类，则请确保该类是线程安全的。

可以修改 ref 类的线程处理模型和封送处理行为。 但是，如果所做的更改会使类为非敏捷类，则必须了解与这些更改关联的影响。

下面的示例演示如何将 `MarshalingBehavior` 和特性应用于 `ThreadingModel` Windows 运行时类库中的运行时类。 当应用程序使用 DLL 并使用 `ref new` 关键字激活 `MySTAClass` 类对象时，该对象在单线程单元激活，并且不支持封送处理。
