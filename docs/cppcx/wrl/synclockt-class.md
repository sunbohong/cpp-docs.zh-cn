---
description: 了解详细信息： SyncLockT 类
title: SyncLockT 类
ms.date: 10/03/2018
ms.topic: reference
f1_keywords:
...
第一个构造函数 `SyncLockT` 从另一个 `SyncLockT` 由参数指定的对象初始化当前对象，然后使另一个 `SyncLockT` 对象失效。 第二个构造函数为 **`protected`** ，并将当前 `SyncLockT` 对象初始化为无效状态。
