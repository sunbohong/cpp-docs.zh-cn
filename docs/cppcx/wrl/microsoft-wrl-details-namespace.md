---
description: 了解详细信息： Microsoft：： WRL：:D etails 命名空间
...
|[BoolStruct 结构](boolstruct-structure.md)|定义是否 `ComPtr` 正在管理接口的对象生存期。 `BoolStruct`[BoolType ( # B1](comptr-class.md#operator-microsoft-wrl-details-booltype)运算符在内部使用。|
...
|[交换函数 (WRL) ](swap-function-wrl.md)|交换两个指定参数的值。|
