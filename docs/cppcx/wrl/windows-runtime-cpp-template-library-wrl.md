...
- Windows 运行时 c + + 模板库 (ABI) Windows 运行时应用程序二进制接口添加了少量抽象，使你能够控制基础代码，以便更好地创建或使用 Windows 运行时 Api。
...
在此第9频道视频中，详细了解 Windows 运行时 c + + 模板库如何帮助您编写通用 Windows 平台 (UWP) 应用程序以及如何创作和使用 Windows 运行时组件。
...
[使用 Visual C++ 中 Windows 运行时组件](/windows/uwp/winrt-components/walkthrough-creating-a-basic-windows-runtime-component-in-cpp-and-calling-it-from-javascript-or-csharp)\
