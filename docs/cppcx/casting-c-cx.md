...
在 **`dynamic_cast`** (更具体地强制转换对象时，使用 hat **^**) 转换为派生程度更高的类型，您希望目标对象有时可能是 **`nullptr`** 或转换可能会失败，并且您希望将该条件作为常规代码路径而不是异常来处理。 例如，在 **空白应用 (通用 Windows)** 项目模板中， `OnLaunched` xamp 中的方法使用 **`dynamic_cast`** 测试应用程序窗口是否包含内容。 如果它没有内容，则不是错误；而是一个预期状况。 `Windows::Current::Content` 是 `Windows::UI::XAML::UIElement` ，该转换是一个 `Windows::UI.XAML::Controls::Frame`，是继承层次结构中派生程度更大的类型。
